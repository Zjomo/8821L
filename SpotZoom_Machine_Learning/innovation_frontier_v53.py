"""
SpotZoom innovation frontier v53.

Occlusion-aware memory-tree association gate inspired by:
- OA-SORT: explicit occlusion-status awareness for robust association.
- OC-SORT: observation-centric re-update to reduce occlusion error accumulation.
- SAM2Long: fixed-budget multi-path memory tree for long-horizon robustness.
- BoT-SORT: camera-motion compensation (CMC) under moving-camera scenarios.
- CoTracker3/Norfair: online, lightweight real-time tracking design.
...
class OcclusionTreeAssociationConfig:
    ...
    occlusion_gate_scale: float = 0.74
    occlusion_low_confidence_bonus: float = 0.28
    occlusion_std_ema_decay: float = 0.88
    occlusion_min_texture_std: float = 4.2
    occlusion_reupdate_threshold: float = 0.42
    observation_reupdate_blend: float = 0.66
    observation_reupdate_max_distance_px: float = 36.0
    tree_short_bias: float = 0.04
    tree_switch_similarity_margin: float = 0.03
    long_path_occlusion_threshold: float = 0.45
...
class OcclusionTreeAssociationGate:
    """Single-target gate with occlusion-aware memory-tree + CMC association."""
    ...
    def __init__(self, config: Optional[OcclusionTreeAssociationConfig] = None):
        ...
        self._occlusion_std_ema: Optional[float] = None
        self._last_tree_path: Optional[str] = None
    ...
    def _estimate_occlusion_score(
        ...
        texture_std = 0.0
        if gray is not None:
            patch = _extract_patch(gray, center=center, radius=max(6, int(self.config.patch_radius // 2)))
            if patch is not None and patch.size > 0:
                texture_std = float(np.std(patch))
                if not np.isfinite(texture_std):
                    texture_std = 0.0

        if self._occlusion_std_ema is None:
            self._occlusion_std_ema = max(texture_std, float(self.config.occlusion_min_texture_std))
        else:
            decay = float(np.clip(self.config.occlusion_std_ema_decay, 0.0, 1.0))
            self._occlusion_std_ema = decay * float(self._occlusion_std_ema) + (1.0 - decay) * texture_std
            if not np.isfinite(self._occlusion_std_ema):
                self._occlusion_std_ema = max(texture_std, float(self.config.occlusion_min_texture_std))

        ref_std = max(
            float(self.config.occlusion_min_texture_std),
            float(self._occlusion_std_ema or 0.0),
            1e-6,
        )
        texture_drop = float(np.clip((ref_std - texture_std) / ref_std, 0.0, 1.0))
        low_conf = float(
            np.clip(float(self.config.high_confidence_threshold) - _clip_unit(confidence), 0.0, 1.0)
        )
        occ = texture_drop + float(self.config.occlusion_low_confidence_bonus) * low_conf
        return float(np.clip(occ, 0.0, 1.0))

    def _best_memory_match(
        self,
        bank: List[_MemoryEntry],
        ...
        ranked = sorted(
            bank,
            key=lambda item: (float(item.quality) - age_pen * float(item.age)),
            reverse=True,
        )[:topk]
        ...
        for item in ranked:
            ...
            if score > best_score:
                best_score = score
                best_center = np.asarray(item.center, dtype=np.float64)
        if best_score < -0.5:
            return 0.0, None
        return float(max(0.0, best_score)), best_center
    ...
    def _memory_similarity(
        ...
        *,
        occlusion_score: float,
        motion_consistency: float,
    ) -> Tuple[float, bool, bool, str, bool, Optional[np.ndarray]]:
        short_score, short_center = self._best_memory_match(self._short_memory, embedding)
        long_score, long_center = self._best_memory_match(self._long_memory, embedding)
        ...
        short_bias = float(np.clip(self.config.tree_short_bias, 0.0, 0.2))
        short_adjust = short_score + short_bias * max(0.0, 1.0 - float(np.clip(occlusion_score, 0.0, 1.0)))
        long_adjust = long_score + 0.03 * float(np.clip(occlusion_score, 0.0, 1.0))
        long_adjust += 0.02 * max(0.0, -motion_consistency)
        margin = max(0.0, float(self.config.tree_switch_similarity_margin))
        prefer_long_occ = float(occlusion_score) >= float(self.config.long_path_occlusion_threshold)

        if long_center is not None and (
            prefer_long_occ and long_adjust + margin >= short_adjust
            or long_adjust > short_adjust + margin
        ):
            path = "long"
            used_long = True
            best_score = long_score
            best_center = long_center
        elif short_center is not None:
            path = "short"
            best_score = short_score
            best_center = short_center
        elif long_center is not None:
            path = "long"
            used_long = True
            best_score = long_score
            best_center = long_center

        used_tree_memory = best_center is not None and path in {"short", "long"}
        prev_path = self._last_tree_path
        tree_switched = bool(
        ...
        self._last_tree_path = path

        return (
            float(max(0.0, best_score)),
            bool(used_long),
            bool(used_tree_memory),
            path,
            tree_switched,
            best_center,
        )
    ...
    def update(
        ...
        *,
        ...
        occlusion_score = self._estimate_occlusion_score(
            frame=frame,
            center=(float(obs[0]), float(obs[1])),
            confidence=conf,
        )
        ...
        if self._state is None:
            ...
            return OcclusionTreeAssociationResult(
                ...
                used_tree_memory=False,
                tree_path="none",
                tree_switched=False,
                observation_reupdated=False,
        ...
        gate = float(self.config.base_gate_px) * (
            1.0
            + float(self.config.uncertainty_gate_scale) * unc
            + float(self.config.cmc_gate_scale) * cmc_factor
            + float(self.config.occlusion_gate_scale) * occlusion_score
        )
        ...
        if (
            assoc_dist > gate
            and used_tree_memory
            and memory_center is not None
            and occlusion_score >= float(self.config.occlusion_reupdate_threshold)
        ):
            mem_dist = float(np.linalg.norm(obs - memory_center))
            if mem_dist <= max(0.0, float(self.config.observation_reupdate_max_distance_px)):
                blend = float(
                    np.clip(
                        float(self.config.observation_reupdate_blend) + 0.2 * occlusion_score,
                        0.35,
                        0.95,
                    )
                )
                obs = blend * obs + (1.0 - blend) * memory_center
                residual = obs - pred
                assoc_dist = float(np.linalg.norm(residual))
                motion_cons = self._motion_consistency(residual)
                observation_reupdated = True
        ...
        if rejected:
            ...
            return OcclusionTreeAssociationResult(
                ...
                used_tree_memory=used_tree_memory,
                tree_path=tree_path,
                tree_switched=tree_switched,
                observation_reupdated=observation_reupdated,
                scene_reset=scene_reset,
                memory_size=len(self._short_memory) + len(self._long_memory),

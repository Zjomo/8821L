...
class MeasurementConfig:
    ...
    rule_ab_feature_enable_sam2_fallback: bool = bool(_cfg("rule_ab_feature_enable_sam2_fallback", True))
    rule_ab_bmask_allow_hold_last: bool = bool(_cfg("rule_ab_bmask_allow_hold_last", True))
...
class MeasurementWorkflow:
    ...
    def _repair_rule_ab_angle_if_jump(
        ...
        规则：
            ...
            2) raw_angle 与 last_confirmed_angle 的差 > 2°：
               重新获取当前帧 Bmask，近似多边形拟合，计算所有边角度；
               只接受与 last_confirmed_angle 相差 <= 2° 的候选边作为 repaired_angle。
        ...
        # raw 跳变过大：重新获取当前帧 Bmask，做近似多边形全边候选修复。
        ...
        if not eligible:
            ...
            self.log(
                f"[RuleAB-角度修复] raw_angle={raw_angle_f:.6f}° 与上一帧最终角度 {ref_angle:.6f}° "
                f"差值 {raw_delta_to_last} > {tol:.3f}°；重新取帧多边形候选仍未找到 ±{tol:.3f}° 内的边，本帧角度无效，不关激光、不停止完整测量。"
            )
            return None, info
        ...
        info.update({
            ...
            "final_delta_from_baseline": final_delta,
            "angle_source": "bmask_polygon_edge_repaired_by_last_angle",
            "reason": f"raw_delta_to_last_gt_{tol:g}deg_repaired_by_polygon_edge_within_{tol:g}deg",
        })
        ...
        try:
            ...
            angle_result["angle_source"] = "bmask_polygon_edge_repaired_by_last_angle"
        ...
        self.log(
            f"[RuleAB-角度修复] raw_angle={raw_angle_f:.6f}° 与上一帧最终角度 {ref_angle:.6f}° "
            f"差值 {raw_delta_to_last} > {tol:.3f}°；使用多边形候选 repaired_angle={repaired:.6f}°，"
    ...
    def _install_feature_tracker_on_rule_ab_follower(self, follower: Any, state: Optional[CalibrationState] = None):
        ...
        def _build_templates_from_original_scene() -> Tuple[np.ndarray, Any]:
            ...
            self.log(
                ...
                "后续完整测量优先使用 feature tracker；Bmask 失败时自动 SAM2 fallback / hold_last；"
                f"模板目录={out_dir}"
            )
        ...
        def _feature_capture_and_build_scene():
            ...
            # -------- Bmask 三层稳定流程：feature -> SAM2 fallback -> hold_last --------
            ...
            if ok_b:
                ...
                self.log(f"[Bmask] feature failed: reason={reason_b}, feature_area={int(area_b)}")

            if (not ok_b) and bool(getattr(self.cfg, "rule_ab_feature_enable_sam2_fallback", True)):
                fallback_used = True
                try:
                    # 只在 B feature 失败时调用原始 SAM2/temporal capture 作为 fallback。
                    fb_image_rgb, fb_scene = original_capture()
                    fb_obj = self._get_scene_object_by_label(fb_scene, "b")
                    fb_mask = self._extract_mask_from_scene_object(fb_obj)
                    if fb_mask is not None and np.any(np.asarray(fb_mask).astype(bool)):
                        sam2_fallback_mask = np.asarray(fb_mask).astype(bool)
                        ok_fb, reason_fb, area_fb, center_fb, center_jump_fb = self._is_bmask_valid_for_feature_tracker(
                            sam2_fallback_mask, b_profile, debug.get("b", {}) if isinstance(debug, dict) else {}
                        )
                        if ok_fb:
                            # fallback 取到的图像可能与 feature 当前帧相差很小；为避免整帧切换影响 A/C，
                            # 只取 Bmask，A/C 仍用 feature tracker 当前帧结果。
                            final_b_mask = sam2_fallback_mask.copy()
                            bmask_source = "sam2_fallback"
                            fail_reason = ""
                            area_b, center_b, center_jump_b = area_fb, center_fb, center_jump_fb
                            self.log(f"[Bmask] fallback SAM2 success, area={int(area_fb)}, center_jump={center_jump_fb}")
                        else:
                            fail_reason = f"sam2_fallback_invalid:{reason_fb}"
                            self.log(f"[Bmask] fallback SAM2 failed: reason={reason_fb}, area={int(area_fb)}")
                    else:
                        fail_reason = "sam2_fallback_empty"
                        self.log("[Bmask] fallback SAM2 failed: empty mask")
                except Exception as e:
                    fail_reason = f"sam2_fallback_exception:{e}"
                    self.log(f"[Bmask] fallback SAM2 failed: reason={e}")

            if final_b_mask is None and bool(getattr(self.cfg, "rule_ab_bmask_allow_hold_last", True)):
                last_b = tracker_state.get("last_valid_b_mask")
                hold_count = int(tracker_state.get("bmask_hold_count", 0))
                max_hold = int(getattr(self.cfg, "rule_ab_bmask_hold_last_max_frames", 3))
                if last_b is not None and hold_count < max_hold:
                    final_b_mask = np.asarray(last_b).astype(bool).copy()
                    hold_last_used = True
                    bmask_source = "hold_last"
                    hold_count += 1
                    tracker_state["bmask_hold_count"] = hold_count
                    center_b = self._mask_center_xy(final_b_mask)
                    area_b = float(np.count_nonzero(final_b_mask))
                    fail_reason = f"feature_and_sam2_failed_use_hold_last:{fail_reason}"
                    self.log(f"[Bmask] use hold_last, hold_count={hold_count}, area={int(area_b)}")

            if final_b_mask is not None:
                ...
                tracker_state["bmask_hold_count"] = 0 if bmask_source != "hold_last" else int(tracker_state.get("bmask_hold_count", 0))
                ...
                bmask_source = "failed"
                self.log(f"[Bmask] final failed, skip angle this frame, consecutive_fail={fail_count}, reason={fail_reason}")
                # 给 scene 写入空 Bmask，让角度检测得到 angle_ok=False，而不是读取旧 Bmask。
                if "b" in masks:
                    masks["b"] = np.zeros_like(next(iter(masks.values()))).astype(bool)
                max_fail = int(getattr(self.cfg, "rule_ab_bmask_fail_max_consecutive", 10))
                if fail_count > max_fail:
                    raise RuntimeError(f"Bmask 连续失败超过阈值 {max_fail}：{fail_reason}")
        ...
        self.log("[ABC特征跟踪] 已安装：SAM2 首帧分割 + 多 HSV 颜色/形状模板后续识别 + Bmask SAM2 fallback/hold_last。")
    ...
    def close_all(self):
        ...
        self.log("========== GUI关闭清理完成：激光控制器已关闭，照明光未额外切换；运行态已重置 ==========")
        self.notify_update()
...
class MeasurementWorkflowGUI:
    ...
    def calibrate_rule_ab_b(self):
        ...
        try:
            ...
            state.b_positive_hsv_seed_count = int(len(seeds))
            self._save_calibration_file_raw(state)
            self.log(f"[完整测量标定] RuleAB-B 标定完成：B+={len(pos)}, B-={len(neg)}, hsv_seeds={len(seeds)}")
        except Exception as e:

...
def select_a_b_c_points_interactively(
    image_rgb: np.ndarray,
    window_name: str = "SAM2 init: A/B single point, C multi positive",
    scale: float = 0.85,
    ...
    def redraw() -> np.ndarray:
        ...
        instructions = [
            "SAM2 init",
            f"Current: {current_name} | Left=positive only, N/Enter=next, R=reset, Backspace=previous, ESC=cancel",
            "A/B: only one positive point. C: multiple positive points. No negative points are used.",
        ]
        ...
        for obj_i, name in enumerate(names):
            ...
            if name in ("A", "B"):
                ...
                count_text = f"{prefix}{name}: positive points={len(prompt_sets[name]['pos'])}"
            cv2.putText(
                canvas,
                count_text,
                (20, y_panel + obj_i * 24),
    ...
    logger.info(
        "用户初始化: A(single_pos=%d), B(single_pos=%d), C(pos=%d, fixed static mask after segmentation)",
...
class ActualNanoBoundaryFollower:
    ...
    def initialize_abc_with_first_frame(self) -> None:
        logger.info("开始第一帧 A/B/C 初始化：A/B 单正点；C 多正点；C mask 固定。")

        image_rgb = self.capturer.capture()

        a_prompt, b_prompt, c_prompt = select_a_b_c_points_interactively(
            image_rgb=image_rgb,
            window_name="SAM2 init: A/B single positive, C multi positive fixed",
        ...
        # 保存第一次 C 分割的完整像素位置。后续 A 沿 C 运动只使用这些固定像素。
        self.save_static_c_reference(c_mask)

        # 初始化图保存，便于检查 A/B/C 是否分割正确
        if self.cfg.save_annotated_image:
            init_path = self.image_dir / "sam2_init_AB_single_C_static.png"

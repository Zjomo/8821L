...
class RuntimeConfig:
    ...
    temporal_position_match_margin_px: float = 180.0
    temporal_position_match_min_score: float = 0.25
...
class SAM2ABCSegmenter:
    ...
    def __init__(
        ...
        # 上一帧原始图像。用于模板匹配估计 A/B 的当前位置，避免只用上一帧旧中心。
    ...
    def _estimate_bbox_by_template_match(
        ...
        """
        用上一帧图像块在当前帧中做模板匹配，估计目标当前位置。

        关键目的：
            A/B 的形状不变，但位置会移动。
            不能继续用上一帧旧 center 当作当前帧 SAM2 正点；
            应先估计当前帧的新 center，再用预测 bbox + 预测 center 提示 SAM2。
        """
        if (not self.temporal_position_match_enable) or self.last_image_rgb is None:
            c = last_center if last_center is not None else ((last_bbox[0] + last_bbox[2]) / 2.0, (last_bbox[1] + last_bbox[3]) / 2.0)
            return last_bbox, c, {"enabled": False, "reason": "disabled_or_no_last_image"}
        ...
        if x2i <= x1i + 4 or y2i <= y1i + 4:
            c = last_center if last_center is not None else ((x1 + x2) / 2.0, (y1 + y2) / 2.0)
            return last_bbox, c, {"enabled": True, "fallback": True, "reason": "last_bbox_too_small"}

        template_rgb = self.last_image_rgb[y1i:y2i + 1, x1i:x2i + 1]
        if template_rgb.size <= 0:
            c = last_center if last_center is not None else ((x1 + x2) / 2.0, (y1 + y2) / 2.0)
            return last_bbox, c, {"enabled": True, "fallback": True, "reason": "empty_template"}

        margin = float(self.temporal_position_match_margin_px)
        sx1 = int(max(0, min(w - 1, round(x1 - margin))))
        sy1 = int(max(0, min(h - 1, round(y1 - margin))))
        sx2 = int(max(0, min(w - 1, round(x2 + margin))))
        sy2 = int(max(0, min(h - 1, round(y2 + margin))))
        ...
        if sh < th + 2 or sw < tw + 2:
            c = last_center if last_center is not None else ((x1 + x2) / 2.0, (y1 + y2) / 2.0)
            return last_bbox, c, {"enabled": True, "fallback": True, "reason": "search_window_too_small"}
        ...
        try:
            ...
            c = last_center if last_center is not None else ((x1 + x2) / 2.0, (y1 + y2) / 2.0)
            return last_bbox, c, {"enabled": True, "fallback": True, "reason": f"matchTemplate_failed:{e}"}

        if float(max_val) < float(self.temporal_position_match_min_score):
            c = last_center if last_center is not None else ((x1 + x2) / 2.0, (y1 + y2) / 2.0)
            return last_bbox, c, {
                "enabled": True,
                "fallback": True,

...
from logic.rule_ab import RuntimeConfig as RuleABRuntimeConfig
from logic.rule_ab import ActualNanoBoundaryFollower
...
class MeasurementConfig:
    ...
    rule_ab_stage_velocity: float = int(_cfg("rule_ab_stage_velocity", 10))
    rule_ab_stage_acceleration: float = int(_cfg("rule_ab_stage_acceleration", 10))
    rule_ab_stage_max_voltage: float = int(_cfg("rule_ab_stage_max_voltage", 100))
...
class MeasurementWorkflow:
    ...
    def ensure_rule_ab_follower(self) -> ActualNanoBoundaryFollower:
        ...
        cfg = RuleABRuntimeConfig(
            ...
            stage_default_velocity=float(self.cfg.rule_ab_stage_velocity),
            stage_default_acceleration=float(self.cfg.rule_ab_stage_acceleration),
            stage_default_max_voltage=float(self.cfg.rule_ab_stage_max_voltage),
...
class MeasurementWorkflowGUI:
    ...
    def _build_ui(self):
        ...
        ttk.Checkbutton(rule_frame, text="RuleAB真动Stage34", variable=self.rule_ab_enable_stage_var).grid(row=0, column=0, columnspan=2, padx=4, pady=3, sticky="w")
        ttk.Label(rule_frame, text="AB最大步数").grid(row=1, column=0, padx=4, pady=3, sticky="w")
        ttk.Entry(rule_frame, textvariable=self.rule_ab_max_steps_var, width=10).grid(row=1, column=1, padx=4, pady=3, sticky="ew")
        ttk.Label(rule_frame, text="角度目标min/max").grid(row=1, column=2, padx=4, pady=3, sticky="w")
        ttk.Entry(rule_frame, textvariable=self.rule_ab_delta_min_var, width=8).grid(row=1, column=3, padx=4, pady=3, sticky="ew")
        ttk.Entry(rule_frame, textvariable=self.rule_ab_delta_max_var, width=8).grid(row=2, column=3, padx=4, pady=3, sticky="ew")

        ttk.Checkbutton(rule_frame, text="RuleAC真动Stage12", variable=self.rule_ac_enable_stage_var).grid(row=3, column=0, columnspan=2, padx=4, pady=(8, 3), sticky="w")
        ttk.Label(rule_frame, text="面积阈值px²").grid(row=4, column=0, padx=4, pady=3, sticky="w")
        ttk.Entry(rule_frame, textvariable=self.rule_ac_area_threshold_var, width=10).grid(row=4, column=1, padx=4, pady=3, sticky="ew")
        ttk.Label(rule_frame, text="AC最大循环").grid(row=4, column=2, padx=4, pady=3, sticky="w")
        ttk.Entry(rule_frame, textvariable=self.rule_ac_max_cycles_var, width=10).grid(row=4, column=3, padx=4, pady=3, sticky="ew")

        rule_buttons = ttk.Frame(rule_frame)
        rule_buttons.grid(row=5, column=0, columnspan=4, padx=2, pady=6, sticky="ew")
        rule_buttons.columnconfigure(0, weight=1)
        rule_buttons.columnconfigure(1, weight=1)
        ttk.Button(rule_buttons, text="单独测试A推动B", command=self.test_rule_ab_thread).grid(row=0, column=0, padx=4, pady=3, sticky="ew")
        ttk.Button(rule_buttons, text="单独测试Step9面积", command=self.test_rule_ac_thread).grid(row=0, column=1, padx=4, pady=3, sticky="ew")
        ttk.Label(rule_frame, textvariable=self.rule_module_status_var, wraplength=450).grid(row=6, column=0, columnspan=4, padx=4, pady=(4, 0), sticky="w")
    ...
    def build_config_from_ui(self) -> MeasurementConfig:
        return MeasurementConfig(
            ...
            # 单独 A推动B / 完整流程 RuleAB 默认使用你的 Thorlabs 控制器 97101208 和 3/4 通道。
            # 这里不新增 GUI 输入框，避免界面复杂化；后续如需改通道/速度，直接改这里即可。
    ...
    def test_rule_ab(self):
        """
        单独点击“单独测试A推动B”按钮时，只执行 RuleAB 视觉闭环。

        新逻辑：
            1. 每一次循环都截图并用 SAM2 更新 A/B/C；
            2. 每一次循环都计算：
                - B目标边角度 b_edge_angle_deg；
                - A到B目标边距离 ab_dist；
                - A到C边界距离 c_clearance；
            3. 只有在需要继续推动时才执行 Stage34 动作；
            4. 如果“角度基本不变”并且“A-B距离已经很近”，则立即停止/不再发送运动，
               进入 hold 等待；hold 期间仍然持续截图、分割、检测角度和距离；
            5. 不连接 Rigol、不采集光谱、不读取 xlsx、不刷新光谱图。
        """
        follower = None
        ...
        def _edge_angle_deg(edge) -> Optional[float]:
            try:
                p1, p2 = edge
        ...
        def _stop_stage_safe(reason: str):
            try:
                ...
                if stage is not None and hasattr(stage, "stop_all"):
                    stage.stop_all()
                    self.log(f"[RuleAB-only] 已执行 stage.stop_all()，原因：{reason}")
            except Exception as e:
                self.log(f"[RuleAB-only] stage.stop_all() 失败：{e}")

        def _capture_status_once(mode: str, frame_index: int):
            """
            截图一次、SAM2更新一次、几何计算一次，但不一定运动。
            """
            image_rgb, scene = follower.capture_and_build_scene()
            status = follower.get_boundary_rule_status(scene)
            b_edge_angle = _edge_angle_deg(scene.b_target_edge)
            ab_dist = float(status.get("ab_dist", float("nan")))
        ...
        def _save_and_record(frame_info: Dict[str, Any], action_name: str, direction: str):
            """
            尽量复用 logic.rule_ab.ActualNanoBoundaryFollower 自带的保存函数。
            保存失败不影响控制流程。
            """
            try:
                if bool(getattr(follower.cfg, "save_annotated_image", True)) and hasattr(follower, "save_annotated_image"):
                    follower.save_annotated_image(
                        image_rgb=frame_info["image_rgb"],
                        scene=frame_info["scene"],
                        status=frame_info["status"],
                        action_name=action_name,
                    )
            except Exception as e:
                self.log(f"[RuleAB-only] 保存标注图失败：{e}")
            ...
            try:
                if hasattr(follower, "record_cycle"):
                    follower.record_cycle(
                        scene=frame_info["scene"],
                        status=frame_info["status"],
                        action_id=int(frame_info.get("action_id", 0)),
                        action_name=action_name,
                        direction=direction,
                    )
            except Exception as e:
                self.log(f"[RuleAB-only] 写入 RuleAB CSV 失败：{e}")

        try:
            self.rule_ab_only_stop_requested = False
            self.set_var(self.rule_module_status_var, "规则模块状态：单独A推动B初始化中")
            self.log("========== 单独A推动B：检测角度/距离，必要时停止运动并持续监测 ==========")

            # 只读取 GUI 参数，不调用 self.ensure_workflow()。
            # ensure_workflow() 会创建 MeasurementWorkflow，而 MeasurementWorkflow.__init__ 会创建 Excel 汇总文件。
            ...
            rule_ab_max_steps = int(getattr(cfg_gui, "rule_ab_max_steps", 30))
            rule_ab_loop_interval_s = float(getattr(cfg_gui, "rule_ab_loop_interval_s", 0.15))

            # Stage34 参数。使用 getattr 的原因是兼容旧 MeasurementConfig；
            # 即使旧 dataclass 没有这些字段，也会使用下面的默认值。
            ...
            stage_velocity = int(getattr(cfg_gui, "rule_ab_stage_velocity", 10))
            stage_acceleration = int(getattr(cfg_gui, "rule_ab_stage_acceleration", 10))
            ...
            self.log(
                "[RuleAB-only] 参数："
                f"enable_stage={rule_ab_enable_stage}, "
                f"max_detection_cycles={rule_ab_max_steps}, "
                f"loop_interval_s={rule_ab_loop_interval_s}, "
                f"stage_conn={stage_conn}, "
                f"x_channel={stage_x_channel}, "
                f"y_channel={stage_y_channel}, "
                f"velocity={stage_velocity}, "
                f"acceleration={stage_acceleration}, "
                f"max_voltage={stage_max_voltage}, "
                f"step_x={stage_step_x}, "
                f"step_y={stage_step_y}, "
            ...
            rule_cfg = RuleABRuntimeConfig(
                ...
                # 注意：这些值必须是 int，不能传 200.0 / 100.0。
                stage_default_velocity=stage_velocity,
                stage_default_acceleration=stage_acceleration,
            ...
            # ------------------------------------------------------------
            # 近距离且角度不变时的“停止运动 + 持续检测”参数
            # 这里不额外加 GUI 输入框，先用保守默认值；需要调参时改这里即可。
            # ------------------------------------------------------------
            angle_stable_tol_deg = 0.30        # 连续帧角度变化 <= 0.30°，认为角度没有明显变化
            angle_stable_required = 3          # 连续 3 次满足才触发 hold，避免噪声误判
            ab_close_threshold_px = float(getattr(rule_cfg, "a_b_max_dist", 35.0))  # A-B 已经进入近距离范围
            hold_wait_s = 3.0                  # 停止运动后等待/观察时长
            hold_check_interval_s = max(0.10, rule_ab_loop_interval_s)

            self.log(
                "[RuleAB-only] 停止运动判据："
                f"angle_stable_tol={angle_stable_tol_deg} deg, "
                f"stable_required={angle_stable_required}, "
                f"ab_close_threshold={ab_close_threshold_px} px, "
                f"hold_wait_s={hold_wait_s}, "
                f"hold_check_interval_s={hold_check_interval_s}"
            )

            self.set_var(self.rule_module_status_var, "规则模块状态：请在弹出的图像中点击/确认 A、B、C")
            self.log("[RuleAB-only] 正在初始化 ActualNanoBoundaryFollower")
            follower = ActualNanoBoundaryFollower(rule_cfg)

            self.log("[RuleAB-only] 初始化第一帧 A/B/C")
            follower.initialize_abc_with_first_frame()

            self.set_var(self.rule_module_status_var, "规则模块状态：单独A推动B运行中")
            self.log("[RuleAB-only] 开始执行：检测角度/距离 -> 必要时推动 -> 稳定近距离时停止运动并持续检测")
            ...
            for frame_idx in range(1, rule_ab_max_steps + 1):
                if self.rule_ab_only_stop_requested:
                    self.log("[RuleAB-only] 收到停止请求，退出单独A推动B")
                    last_ok = False
                    break

                detection_cycles = frame_idx

                # --------------------------------------------------------
                # 1. 每轮先截图 + 分割 + 计算角度和距离
                # --------------------------------------------------------
                try:
                    info = _capture_status_once(mode="move_check", frame_index=frame_idx)
                    lost_count = 0
                except Exception as e:
                    lost_count += 1
                    last_ok = False
                    self.log(
                        f"[RuleAB-only] frame={frame_idx} 视觉检测/分割失败：{e}，"
                    ...
                    if lost_count >= int(getattr(rule_cfg, "max_lost_frames", 10)):
                        self.log("[RuleAB-only] 连续视觉失败过多，停止单独A推动B")
                        break

                    time.sleep(hold_check_interval_s)
                    continue

                b_angle = info["b_edge_angle_deg"]
                ab_dist = info["ab_dist"]
                c_clearance = info["c_clearance"]
                action_id = int(info["action_id"])

                d_angle = _angle_diff_180(b_angle, last_angle)

                if d_angle is not None and d_angle <= angle_stable_tol_deg:
                    stable_count += 1
                else:
                    stable_count = 0
                ...
                angle_stable = stable_count >= angle_stable_required
                ab_close = math.isfinite(ab_dist) and ab_dist <= ab_close_threshold_px
                should_hold = angle_stable and ab_close

                self.log(
                    f"[RuleAB-only] frame={frame_idx}/{rule_ab_max_steps}, "
                    f"B_edge_angle={b_angle}, d_angle={d_angle}, stable_count={stable_count}, "
                    f"A-B={ab_dist:.3f}px, A-C={c_clearance:.3f}px, "
                    f"ab_close={ab_close}, should_hold={should_hold}, "
                    f"policy_action={action_id}"
                )

                # --------------------------------------------------------
                # 2. 如果角度不变且 A-B 已经近，停止运动，进入只检测不运动的 hold
                # --------------------------------------------------------
                if should_hold:
                    hold_count += 1
                    _stop_stage_safe("角度无明显变化且A-B距离已很近")
                    self.set_var(
                        self.rule_module_status_var,
                        f"规则模块状态：检测到角度稳定且A-B接近，停止运动并监测 {hold_wait_s:.1f}s"
                    )
                    self.log(
                        f"[RuleAB-only] HOLD#{hold_count}: "
                        f"角度连续 {stable_count} 次无明显变化，且 A-B={ab_dist:.3f}px <= {ab_close_threshold_px:.3f}px；"
                        "本轮不再发送运动命令，进入等待监测。"
                    )

                    # 保存当前帧为 HOLD
                    info["action_id"] = 0
                    _save_and_record(info, action_name="HOLD_START", direction="hold_start")

                    t_hold0 = time.time()
                    hold_frame_idx = 0

                    while time.time() - t_hold0 < hold_wait_s:
                        if self.rule_ab_only_stop_requested:
                            self.log("[RuleAB-only] HOLD期间收到停止请求")
                            last_ok = False
                            break
                        ...
                        try:
                            hold_info = _capture_status_once(
                                mode="hold_monitor",
                            ...
                            lost_count += 1
                            self.log(
                                f"[RuleAB-only] HOLD监测失败：{e}，"
                                f"lost_count={lost_count}/{int(getattr(rule_cfg, 'max_lost_frames', 10))}"
                            )
                            _stop_stage_safe("HOLD期间视觉检测失败")
                            if lost_count >= int(getattr(rule_cfg, "max_lost_frames", 10)):
                                last_ok = False
                                break
                            time.sleep(hold_check_interval_s)
                        ...
                        hold_angle = hold_info["b_edge_angle_deg"]
                        hold_ab_dist = hold_info["ab_dist"]
                        hold_c_clearance = hold_info["c_clearance"]
                        hold_d_angle = _angle_diff_180(hold_angle, last_angle)

                        if hold_d_angle is not None and hold_d_angle > angle_stable_tol_deg:
                            stable_count = 0
                        else:
                            stable_count += 1

                        last_angle = hold_angle
                        hold_info["action_id"] = 0

                        self.log(
                            f"[RuleAB-only] HOLD监测 {hold_frame_idx}: "
                            f"B_edge_angle={hold_angle}, d_angle={hold_d_angle}, "
                            f"A-B={hold_ab_dist:.3f}px, A-C={hold_c_clearance:.3f}px；不运动"
                        )

                        _save_and_record(
                            hold_info,
                            action_name="HOLD_MONITOR",
                            direction="hold_monitor",
                        )

                        time.sleep(hold_check_interval_s)

                    self.set_var(self.rule_module_status_var, "规则模块状态：HOLD监测结束，继续判断")
                    if not last_ok:
                        break

                    # HOLD 后继续下一帧判断；如果仍然满足条件，会再次 HOLD，不会强行推动。
                    continue

                # --------------------------------------------------------
                # 3. 不满足停止条件时，才按照 RuleAB 策略执行一次运动
                # --------------------------------------------------------
                action_name_map = {
                    0: "STAY",
                    1: "UP",
                ...
                _save_and_record(info, action_name=action_name, direction=direction)

                if action_id == 0:
                    self.log("[RuleAB-only] 策略输出 STAY，本帧不运动")
                else:
                    self.log(
                        f"[RuleAB-only] 执行动作：action={action_name}, direction={direction}"
                    )
                    follower.execute_action(action_id)
                    executed_motion_steps += 1
            ...
            self.set_var(
                ...
                (
                    "规则模块状态：单独A推动B完成，"
                    f"detection_cycles={detection_cycles}，"
                    f"motion_steps={executed_motion_steps}，"
                    f"hold_count={hold_count}，ok={last_ok}"
                )
            )
            self.log(
                "[RuleAB-only] 单独A推动B完成："
                f"detection_cycles={detection_cycles}, "
                f"motion_steps={executed_motion_steps}, "
                f"hold_count={hold_count}, ok={last_ok}"
            )
            ...
            self.set_var(self.rule_module_status_var, "规则模块状态：单独A推动B失败")
            self.log(f"[RuleAB] 单独A推动B失败：{e}")
            self.log(traceback.format_exc())

        finally:
            # 退出前兜底停止位移台；只关闭 RuleAB follower，不影响 Rigol/TCP/照明光。
            if follower is not None:
                try:
                    _stop_stage_safe("单独A推动B结束")
                except Exception:
                    pass
                ...
                try:
                    if hasattr(follower, "close"):
                        follower.close()
                    self.log("[RuleAB-only] follower 已关闭")
                except Exception as e:
                    self.log(f"[RuleAB-only] follower 关闭失败：{e}")

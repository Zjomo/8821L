...
from logic.actual_nano_boundary_following_staticC_shapeLockedAB_ch4pause import RuntimeConfig as RuleABRuntimeConfig
from logic.actual_nano_boundary_following_staticC_shapeLockedAB_ch4pause import ActualNanoBoundaryFollower
...
class MeasurementConfig:
    ...
    # CH4（上下方向）每次动作后的强制停顿。
    # 需求：不论 UP / DOWN，执行一次运动后都等待 2 s，再进入下一帧闭环。
...
class MeasurementWorkflowGUI:
    ...
    def test_rule_ab(self):
        ...
        try:
            ...
            self.log("========== 单独A沿C绕行(多点SAM2)：覆盖B时停，只检测B角度；无覆盖时维持A-C距离并沿C运动 ==========")

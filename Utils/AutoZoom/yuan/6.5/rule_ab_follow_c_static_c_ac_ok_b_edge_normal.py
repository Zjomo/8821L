...
class ActualNanoBoundaryFollower:
    ...
    def initialize_abc_with_first_frame(self) -> None:
        logger.info("开始第一帧 A/B/C 初始化：A/B 单正点；C 多正点+负点；C mask 固定。")
        ...
        if self.cfg.save_annotated_image:
            init_path = self.image_dir / "sam2_init_AB_single_C_pos_neg_static.png"
    ...
    def rule_policy(self, scene: SceneGeometry) -> int:
        ...
        新运动规则：
            ...
            4. 如果 A-C 太远：
                  A 靠近 C；
            5. 如果 A-C OK：
                  不再向 target_clearance 做额外法向修正；
                  运动方向改为 B 相对于 A 最近边的垂直方向。

        说明：
            之前 AC_OK 时使用 C 最近边的切向 c_edge_dir。
            如果 C 的局部边缘接近竖直，vector_to_action 会输出 UP/DOWN。
            现在改成使用 B 的目标边 b_target_edge 的垂直方向，
            这样在 B 边角度约 60° 时，垂直方向更接近 LEFT/RIGHT，
            可以避免 AC_OK 后持续向上。

        判据：
            A-C < a_c_min_clearance       -> 远离 C
            A-C > a_c_max_clearance       -> 靠近 C
            a_c_min <= A-C <= a_c_max     -> 沿 B 最近边的垂直方向运动
        """
        ...
        # --------------------------------------------------------
        # 4. A-C OK：沿 B 最近边的垂直方向运动。
        #    不再使用 C 局部切向，也不再向 target_clearance 做小修正。
        # --------------------------------------------------------
        return vector_to_action(b_edge_normal_for_ac_ok())

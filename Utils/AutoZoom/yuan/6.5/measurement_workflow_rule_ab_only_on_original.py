...
class MeasurementWorkflowGUI:
    ...
    def test_rule_ab(self):
        """
        单独点击“单独测试A推动B”按钮时，只执行 A推动B 功能。

        本函数不会调用 ensure_workflow()，因此不会创建 MeasurementWorkflow，
        也不会创建 Excel 汇总、不会连接 Rigol、不会初始化角度检测、
        不会进行 LabVIEW/TCP 光谱采集、不会读取 xlsx 横坐标、不会刷新光谱图。

        只执行：
            1. 读取 GUI 中 RuleAB 相关参数；
            2. 创建 RuleABRuntimeConfig；
            3. 初始化 ActualNanoBoundaryFollower；
            4. 第一帧按提示选择/确认 A、B、C；
            5. 循环执行 follower.run_one_cycle()；
            6. 结束后只关闭 follower。
        """
        follower = None
        ...
        try:
            ...
            self.log("========== 单独A推动B：只运行 RuleAB，不执行角度/光谱/Rigol/xlsx ==========")
            ...
            self.log("[RuleAB-only] 开始执行 A推动B 循环")

            executed_steps = 0
            last_ok = True

            for step_idx in range(1, rule_ab_max_steps + 1):
                executed_steps = step_idx
                self.log(f"[RuleAB-only] step={step_idx}/{rule_ab_max_steps}: run_one_cycle()")

                ok = follower.run_one_cycle()
                last_ok = bool(ok)

                if not ok:
                    self.log("[RuleAB-only] follower.run_one_cycle() 返回 False，停止")
                    break
            ...
            self.set_var(
                ...
                f"规则模块状态：单独A推动B完成，steps={executed_steps}，ok={last_ok}"
            )
            self.log(f"[RuleAB-only] 单独A推动B完成：steps={executed_steps}, ok={last_ok}")
            ...
            # 只关闭 RuleAB follower；不要调用 workflow.close_all()，不要影响 Rigol/TCP/照明光。

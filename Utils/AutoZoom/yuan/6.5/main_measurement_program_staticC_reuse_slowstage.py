...
from logic.actual_nano_boundary_following_staticC_reuse_slowstage import RuntimeConfig as RuleABRuntimeConfig
from logic.actual_nano_boundary_following_staticC_reuse_slowstage import ActualNanoBoundaryFollower
...
class MeasurementWorkflowGUI:
    ...
    def _build_ui(self):
        ...
        ttk.Label(rule_frame, text="完整测量角度目标min/max").grid(row=8, column=0, padx=4, pady=3, sticky="w")
        ttk.Entry(rule_frame, textvariable=self.rule_ab_delta_min_var, width=8).grid(row=8, column=1, padx=4, pady=3, sticky="ew")
        ttk.Entry(rule_frame, textvariable=self.rule_ab_delta_max_var, width=8).grid(row=8, column=2, padx=4, pady=3, sticky="ew")

        ttk.Checkbutton(rule_frame, text="RuleAC真动Stage12", variable=self.rule_ac_enable_stage_var).grid(
            row=9, column=0, columnspan=2, padx=4, pady=(8, 3), sticky="w"
        )
        ttk.Label(rule_frame, text="面积阈值px²").grid(row=10, column=0, padx=4, pady=3, sticky="w")
        ttk.Entry(rule_frame, textvariable=self.rule_ac_area_threshold_var, width=10).grid(row=10, column=1, padx=4, pady=3, sticky="ew")
        ttk.Label(rule_frame, text="AC最大循环").grid(row=10, column=2, padx=4, pady=3, sticky="w")
        ttk.Entry(rule_frame, textvariable=self.rule_ac_max_cycles_var, width=10).grid(row=10, column=3, padx=4, pady=3, sticky="ew")

        rule_buttons = ttk.Frame(rule_frame)
        rule_buttons.grid(row=11, column=0, columnspan=4, padx=2, pady=6, sticky="ew")
        for i in range(3):
        ...
        ttk.Label(rule_frame, textvariable=self.rule_module_status_var, wraplength=450).grid(
            row=12, column=0, columnspan=4, padx=4, pady=(4, 0), sticky="w"
        )
    ...
    def test_rule_ab(self):
        ...
        try:
            ...
            self.log(
                ...
                f"static_c_map_name={static_c_map_name}, load_C={load_static_c_map_if_exists}, force_reselect_C={force_reselect_c_each_run}, "

...
#from logic.rule_ac import RuleACConfig, RuleACOverlapController
...
class MeasurementConfig:
    ...
    rule_ab_max_steps: int = int(_cfg("rule_ab_max_steps", 30))
    rule_ab_angle_delta_min_deg: float = float(_cfg("rule_ab_angle_delta_min_deg", 1.0))
    rule_ab_angle_delta_max_deg: float = float(_cfg("rule_ab_angle_delta_max_deg", 2.0))
...
class MeasurementWorkflowGUI:
    ...
    def test_rule_ab(self):
        """
        单独点击“单独测试A推动B”按钮时，只执行 RuleAB 的 A推动B 功能。

        本函数不执行：
            1. Rigol 连接 / 配置 / CH1 ON / CH1 OFF；
            2. 角度检测 baseline；
            3. 每步后的角度检测；
            4. LabVIEW 光谱采集；
            5. xlsx 横坐标读取；
            6. 光谱图刷新；
            7. Summary CSV / Excel 保存。

        它只做：
            1. 从 GUI 读取 RuleAB 参数；
            2. 初始化 ActualNanoBoundaryFollower；
            3. 第一帧点击 A/B/C；
            4. 连续执行 rule_ab.run_one_cycle()。
        """
        follower: Optional[ActualNanoBoundaryFollower] = None

        try:
            # 只读取 GUI 参数，不创建 MeasurementWorkflow。
            # 避免触发 MeasurementWorkflow.__init__() 中的 Excel 汇总文件创建，
            # 也避免后续 refresh_result_labels() 触发 xlsx 横坐标读取。
            cfg_gui = self.build_config_from_ui()

            rule_cfg = RuleABRuntimeConfig(
                capture_area=cfg_gui.capture_area,
                output_dir=str(Path(cfg_gui.save_root) / "rule_ab_standalone"),
                sam2_device="cuda",

                # 单独测试 A推动B 时，只按 GUI 的 RuleAB 参数执行。
                enable_stage=bool(cfg_gui.rule_ab_enable_stage),
                max_cycles=int(cfg_gui.rule_ab_max_steps),
                loop_interval_s=float(cfg_gui.rule_ab_loop_interval_s),

                # Thorlabs KinesisPiezoMotor 控制器与通道参数。
                stage_conn=str(cfg_gui.rule_ab_stage_conn),
                stage_x_channel=int(cfg_gui.rule_ab_stage_x_channel),
                stage_y_channel=int(cfg_gui.rule_ab_stage_y_channel),
                stage_default_velocity=int(cfg_gui.rule_ab_stage_velocity),
                stage_default_acceleration=int(cfg_gui.rule_ab_stage_acceleration),
                stage_default_max_voltage=int(cfg_gui.rule_ab_stage_max_voltage),
                stage_step_x=int(cfg_gui.rule_ab_stage_step_x),
                stage_step_y=int(cfg_gui.rule_ab_stage_step_y),
                stage_x_sign=int(cfg_gui.rule_ab_stage_x_sign),
                stage_y_sign=int(cfg_gui.rule_ab_stage_y_sign),
            )

            self.set_var(
                self.rule_module_status_var,
                "规则模块状态：RuleAB A推动B 单独测试中；只执行A推动B，不执行角度/光谱/xlsx/Rigol",
            )
            self.log("========== RuleAB 单独测试：只执行 A推动B ==========")
            self.log("[RuleAB] 本次单独测试不会连接/控制 Rigol")
            self.log("[RuleAB] 本次单独测试不会执行角度检测")
            self.log("[RuleAB] 本次单独测试不会执行 LabVIEW 光谱采集")
            self.log("[RuleAB] 本次单独测试不会读取 xlsx 横坐标，也不会刷新光谱图")
            self.log(
                f"[RuleAB] 参数：enable_stage={rule_cfg.enable_stage}, "
                f"max_cycles={rule_cfg.max_cycles}, "
                f"stage_conn={rule_cfg.stage_conn}, "
                f"x_channel={rule_cfg.stage_x_channel}, "
                f"y_channel={rule_cfg.stage_y_channel}"
            )

            follower = ActualNanoBoundaryFollower(rule_cfg)

            self.log("[RuleAB] 开始第一帧 A/B/C 点击初始化")
            follower.initialize_abc_with_first_frame()

            ok_count = 0
            fail_step = None

            for step_idx in range(1, int(rule_cfg.max_cycles) + 1):
                self.log(f"[RuleAB] step={step_idx}: 执行一次 A推动B run_one_cycle()")
                ok = follower.run_one_cycle()

                if ok:
                    ok_count += 1
                    self.log(f"[RuleAB] step={step_idx}: 完成")
                else:
                    fail_step = step_idx
                    self.log(f"[RuleAB] step={step_idx}: run_one_cycle 返回 False，停止")
                    break

                if float(rule_cfg.loop_interval_s) > 0:
                    time.sleep(float(rule_cfg.loop_interval_s))

            if fail_step is None:
                msg = f"规则模块状态：RuleAB A推动B完成，执行步数={ok_count}/{rule_cfg.max_cycles}"
            else:
                msg = f"规则模块状态：RuleAB A推动B提前停止，成功步数={ok_count}，停止步={fail_step}"

            self.set_var(self.rule_module_status_var, msg)
            self.log(f"[RuleAB] 单独A推动B结束：{msg}")

            # 关键：这里不要调用 self.refresh_result_labels(...)
            # 否则会刷新光谱图，从而触发 xlsx 横坐标读取日志。

        except Exception as e:
            self.set_var(self.rule_module_status_var, "规则模块状态：RuleAB A推动B测试失败")
            ...
            if follower is not None:
                try:
                    follower.close()
                    self.log("[RuleAB] 单独A推动B follower 已关闭")
                except Exception as e:
                    self.log(f"[RuleAB] 单独A推动B follower 关闭失败：{e}")

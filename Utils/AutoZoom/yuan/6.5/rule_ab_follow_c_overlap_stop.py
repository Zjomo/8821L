...
class RuntimeConfig:
    ...
    # A-C 距离控制，单位 px
    # --------------------------------------------------------
    a_c_target_clearance: float = 85.0
    a_c_min_clearance: float = 70.0
...
class SAM2ABCSegmenter:
    ...
    def initialize_with_points(
        ...
        self.c_init_point = c_point

        self.predictor.set_image(image_rgb)

        a_mask = self._predict_mask_by_points(
            positive_point=a_point,
            negative_points=[b_point, c_point],
        )
        b_mask = self._predict_mask_by_points(
            positive_point=b_point,
            negative_points=[a_point, c_point],
        )
        c_mask = self._predict_mask_by_points(
            positive_point=c_point,
            negative_points=[a_point, b_point],
        )
    ...
    def infer_abc(self, image_rgb: np.ndarray) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        try:
            ...
            c_mask = self._predict_mask_by_box_and_point(
                box_xyxy=self.last_c_bbox,
                point=self.last_c_center,
    ...
    def _predict_mask_by_box_and_point(
        ...
        if masks is None or len(masks) == 0:
            raise RuntimeError("SAM2 box+point prompt 没有返回 mask。")
...
def select_a_b_c_points_interactively(
    image_rgb: np.ndarray,
    window_name: str = "Click A then B then C",
    scale: float = 0.85,
    ...
    返回原始图像坐标系下的 a_point, b_point, c_point。
    """
    if scale <= 0:
        scale = 1.0
    ...
    def redraw() -> np.ndarray:
        ...
        cv2.putText(
            canvas,
            "Click A first, then B, then C. Press R to reset, ESC to cancel.",
            (20, 30),
    ...
    a_disp = points_display[0]
    b_disp = points_display[1]
    c_disp = points_display[2]

    a_point = (float(a_disp[0] / scale), float(a_disp[1] / scale))
    b_point = (float(b_disp[0] / scale), float(b_disp[1] / scale))
    c_point = (float(c_disp[0] / scale), float(c_disp[1] / scale))

    logger.info(
        "用户点击初始化: A=(%.1f, %.1f), B=(%.1f, %.1f), C=(%.1f, %.1f)",
        a_point[0],
        a_point[1],
        b_point[0],
        b_point[1],
...
class ActualNanoBoundaryFollower:
    ...
    def initialize_abc_with_first_frame(self) -> None:
        ...
        a_point, b_point, c_point = select_a_b_c_points_interactively(
            image_rgb=image_rgb,
            window_name="SAM2 init: click A then B then C",
    ...
    def save_abc_init_image(
        ...
        cv2.circle(img, (int(a_point[0]), int(a_point[1])), 6, (0, 255, 255), -1)
        cv2.putText(
            img,
            "A",
            (int(a_point[0]) + 8, int(a_point[1]) - 8),
            cv2.FONT_HERSHEY_SIMPLEX,
            0.8,
            (0, 255, 255),
            2,
        ...
        cv2.circle(img, (int(b_point[0]), int(b_point[1])), 6, (0, 128, 255), -1)
        cv2.putText(
            img,
            "B",
            (int(b_point[0]) + 8, int(b_point[1]) - 8),
            cv2.FONT_HERSHEY_SIMPLEX,
            0.8,
            (0, 128, 255),
            2,
        ...
        cv2.circle(img, (int(c_point[0]), int(c_point[1])), 6, (255, 255, 0), -1)
        cv2.putText(
            img,
            "C",
            (int(c_point[0]) + 8, int(c_point[1]) - 8),
            cv2.FONT_HERSHEY_SIMPLEX,
            0.8,
            (255, 255, 0),
            2,
    ...
    def rule_policy(self, scene: SceneGeometry) -> int:
        ...
        # A-C 防碰撞：最高优先级
        if ac.get("collision_like", False) or c_clearance < self.cfg.a_c_min_clearance:
            return vector_to_action(away_from_c)

        # A-C 太远：靠近 C 边界
        if c_clearance > self.cfg.a_c_max_clearance:
            return vector_to_action(toward_c)
        ...
        if clearance_error > 0:
            ...
            desired = vec_add(
                desired,
                vec_mul(toward_c, self.cfg.boundary_clearance_correction_weight),
            )
        elif clearance_error < 0:
            # 当前比目标近，稍微远离 C
            desired = vec_add(
                desired,
                vec_mul(away_from_c, self.cfg.boundary_clearance_correction_weight),
            )
    ...
    def save_annotated_image(
        ...
        draw_center(scene.a.center, (0, 255, 255), "A-YOLO")
        ...
        save_path = self.image_dir / f"cycle_{self.cycle_index:04d}_{action_name}.png"
        cv2.imwrite(str(save_path), img)

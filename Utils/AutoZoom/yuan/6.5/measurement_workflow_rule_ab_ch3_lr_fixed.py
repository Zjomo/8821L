...
class MeasurementConfig:
    ...
    # 单独 A推动B 的近距离停止阈值：A-B 距离小于该值后停止 Stage，只持续检测 B 边角度。
    rule_ab_ab_close_threshold_px: float = float(_cfg("rule_ab_ab_close_threshold_px", 35.0))
...
class MeasurementWorkflowGUI:
    ...
    def _build_ui(self):
        ...
        ttk.Label(rule_frame, text="A-B停止阈值/px").grid(row=1, column=0, padx=4, pady=3, sticky="w")
        ttk.Entry(rule_frame, textvariable=self.rule_ab_ab_close_threshold_var, width=10).grid(
            row=1, column=1, padx=4, pady=3, sticky="ew"
        )
        ttk.Label(rule_frame, text="小于阈值后：Stage不动，只检测B角度").grid(
            row=1, column=2, columnspan=2, padx=4, pady=3, sticky="w"
        )

        ttk.Label(rule_frame, text="CH3 速度/加速度").grid(row=2, column=0, padx=4, pady=3, sticky="w")
        ttk.Entry(rule_frame, textvariable=self.rule_ab_ch3_velocity_var, width=8).grid(row=2, column=1, padx=4, pady=3, sticky="ew")
        ttk.Entry(rule_frame, textvariable=self.rule_ab_ch3_acceleration_var, width=8).grid(row=2, column=2, padx=4, pady=3, sticky="ew")

        ttk.Label(rule_frame, text="CH4 速度/加速度").grid(row=3, column=0, padx=4, pady=3, sticky="w")
        ttk.Entry(rule_frame, textvariable=self.rule_ab_ch4_velocity_var, width=8).grid(row=3, column=1, padx=4, pady=3, sticky="ew")
        ttk.Entry(rule_frame, textvariable=self.rule_ab_ch4_acceleration_var, width=8).grid(row=3, column=2, padx=4, pady=3, sticky="ew")

        ttk.Label(rule_frame, text="完整测量角度目标min/max").grid(row=4, column=0, padx=4, pady=3, sticky="w")
        ttk.Entry(rule_frame, textvariable=self.rule_ab_delta_min_var, width=8).grid(row=4, column=1, padx=4, pady=3, sticky="ew")
        ttk.Entry(rule_frame, textvariable=self.rule_ab_delta_max_var, width=8).grid(row=4, column=2, padx=4, pady=3, sticky="ew")

        ttk.Checkbutton(rule_frame, text="RuleAC真动Stage12", variable=self.rule_ac_enable_stage_var).grid(
            row=5, column=0, columnspan=2, padx=4, pady=(8, 3), sticky="w"
        )
        ttk.Label(rule_frame, text="面积阈值px²").grid(row=6, column=0, padx=4, pady=3, sticky="w")
        ttk.Entry(rule_frame, textvariable=self.rule_ac_area_threshold_var, width=10).grid(row=6, column=1, padx=4, pady=3, sticky="ew")
        ttk.Label(rule_frame, text="AC最大循环").grid(row=6, column=2, padx=4, pady=3, sticky="w")
        ttk.Entry(rule_frame, textvariable=self.rule_ac_max_cycles_var, width=10).grid(row=6, column=3, padx=4, pady=3, sticky="ew")

        rule_buttons = ttk.Frame(rule_frame)
        rule_buttons.grid(row=7, column=0, columnspan=4, padx=2, pady=6, sticky="ew")
        ...
        ttk.Label(rule_frame, textvariable=self.rule_module_status_var, wraplength=450).grid(
            row=8, column=0, columnspan=4, padx=4, pady=(4, 0), sticky="w"
        )
    ...
    def test_rule_ab(self):
        ...
        当前逻辑：
            1. 不使用最大推动次数；
            2. 持续截图、SAM2更新/分割 A/B/C、计算 A-B 距离；
            3. 当 A-B 距离 >= 阈值时，根据 RuleAB 策略推动 A；
            4. 当 A-B 距离 < 阈值时，立即停止 Stage34，之后只持续检测 B 的目标边角度；
            5. CH3 和 CH4 的速度、加速度分别设置；
            6. 不连接 Rigol、不采集光谱、不读取 xlsx、不刷新光谱图。
        """
        follower = None
        ...
        def _configure_stage_ch3_ch4_separately(
            ...
            """
            RuleAB 的 Stage34 初始化后，分别设置 CH3 / CH4 的速度和加速度。
            """
            try:
                stage = getattr(follower, "stage", None)
                if stage is None:
                    self.log("[RuleAB-only] enable_stage=False 或 stage=None，跳过 CH3/CH4 单独速度设置")
                    return
                ...
                if dev is not None and hasattr(dev, "setup_drive"):
                    ...
                    self.log(
                        "[RuleAB-only] 已通过 stage.dev 分别设置 CH3/CH4 速度和加速度"
                    )
                    return

                self.log("[RuleAB-only] 当前 stage 对象没有 setup_channel/setup_drive 接口，无法分别设置 CH3/CH4")
            except Exception as e:
                self.log(f"[RuleAB-only] 分别设置 CH3/CH4 速度和加速度失败：{e}")
                self.log(traceback.format_exc())

        def _capture_status_once(mode: str, frame_index: int):
            """
            截图一次、SAM2更新一次、几何计算一次。
            """
        ...
        try:
            ...
            self.log("========== 单独A推动B：无最大推动次数；近距离后停止Stage，只检测B角度 ==========")
            ...
            ab_close_threshold_px = float(getattr(cfg_gui, "rule_ab_ab_close_threshold_px", 35.0))
            monitor_interval_s = max(0.10, rule_ab_loop_interval_s)
            ...
            self.log(
                ...
                f"A-B停止阈值={ab_close_threshold_px}px, "
                f"stage_conn={stage_conn}, "
                f"x_channel={stage_x_channel}, y_channel={stage_y_channel}, "
                ...
                f"step_x={stage_step_x}, step_y={stage_step_y}, "
                f"x_sign={stage_x_sign}, y_sign={stage_y_sign}, "
                f"output_dir={output_dir}"
            )

            # RuntimeConfig 仍然需要 max_cycles 字段；这里给一个很大的兼容值，但本函数不会用它作为循环上限。
            ...
            # follower 初始化后，再分别设置 3/4 通道速度和加速度。
            ...
            self.set_var(
                ...
                "规则模块状态：A-B未到阈值时持续推动；到阈值后只检测B角度"
            )
            self.log("[RuleAB-only] 开始持续运行。点击“停止测量”可停止。")
            ...
            while not self.rule_ab_only_stop_requested:
                ...
                try:
                    mode = "b_angle_monitor_only" if monitor_only else "move_until_ab_close"
                    info = _capture_status_once(mode=mode, frame_index=frame_idx)
                    lost_count = 0
                except Exception as e:
                    lost_count += 1
                    self.log(
                ...
                # --------------------------------------------------------
                # A-B 距离已经小于阈值：停止 Stage，只检测 B 角度。
                # 一旦进入 monitor_only，本次单独测试不再恢复运动。
                # --------------------------------------------------------
                if monitor_only or ab_close:
                    if not monitor_only:
                        monitor_only = True
                        _stop_stage_safe(f"A-B距离 {ab_dist:.3f}px < 阈值 {ab_close_threshold_px:.3f}px")
                        self.set_var(
                            self.rule_module_status_var,
                            "规则模块状态：A-B已小于阈值，Stage不动，只检测B角度"
                        )
                        self.log(
                            f"[RuleAB-only] A-B={ab_dist:.3f}px < {ab_close_threshold_px:.3f}px，"
                            "停止控制器运动，进入 B 角度持续检测模式。"
                        )
                    ...
                    self.log(
                        f"[RuleAB-only] B角度监测 frame={frame_idx}, "
                        f"B_edge_angle={b_angle}, d_angle={d_angle}, A-B={ab_dist:.3f}px；Stage不动"
                    )

                    _save_and_record(
                        info,
                        action_name="B_ANGLE_MONITOR",
                ...
                # --------------------------------------------------------
                # A-B 距离未小于阈值：继续按照 RuleAB 策略推动 A。
                # --------------------------------------------------------
                ...
                self.log(
                    f"[RuleAB-only] 推动判断 frame={frame_idx}, "
                    f"B_edge_angle={b_angle}, d_angle={d_angle}, "
                    f"A-B={ab_dist:.3f}px >= 阈值 {ab_close_threshold_px:.3f}px, "
                ...
                if action_id == 0:
                    self.log("[RuleAB-only] 策略输出 STAY，本帧不运动；继续检测")
                else:
                    follower.execute_action(action_id)
                    motion_steps += 1
                    self.log(
                        f"[RuleAB-only] 已执行 Stage 动作：{action_name}；motion_steps={motion_steps}"
                    )

                if rule_ab_loop_interval_s > 0:
                    time.sleep(rule_ab_loop_interval_s)

            _stop_stage_safe("单独A推动B停止/结束")
            ...
            self.set_var(
                ...
                (
                    ...
                    f"monitor_frames={monitor_frames}"
                )
            )
            self.log(
                "[RuleAB-only] 单独A推动B结束："
                f"frames={frame_idx}, motion_steps={motion_steps}, monitor_frames={monitor_frames}"
            )
            ...
            if follower is not None:
                try:
                    _stop_stage_safe("单独A推动B finally")
                except Exception:
                    pass

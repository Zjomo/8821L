...
class RuntimeConfig:
    ...
    # 综合评分权重。score/IoU/面积/中心位移/C 独有区域泄漏共同决定 A mask。
    a_temporal_score_weight: float = 0.35
    a_temporal_iou_weight: float = 0.35
    a_temporal_area_weight: float = 0.15
    a_temporal_center_weight: float = 0.10
    a_temporal_c_leak_weight: float = 0.25

    # A 可以移动，但中心突跳越大得分越低；值越小，约束越严格。
    a_temporal_center_sigma_px: float = 80.0

    # A 形状基本不变，因此候选 mask 面积变化超过这个范围时直接强惩罚/拒绝。
    a_temporal_area_ratio_min: float = 0.50
    a_temporal_area_ratio_max: float = 1.80

    # A 不应大面积吸入 C 的独有区域；但允许上一帧 A/C 已经真实接近或接触的区域。
    # C 独有区域定义为 static_C - last_A。
    a_temporal_max_c_unique_leak_ratio: float = 0.35

    # 如果所有候选都很差，是否退回上一帧 A。正式实验建议 True，避免错误 mask 继续传递。
    a_temporal_fallback_to_last_if_bad: bool = True
    a_temporal_min_accept_score: float = 0.15
    ...
    # 综合评分权重。score/IoU/面积/中心位移/C 独有区域泄漏共同决定 B mask。
    b_temporal_score_weight: float = 0.35
    b_temporal_iou_weight: float = 0.35
    b_temporal_area_weight: float = 0.15
    b_temporal_center_weight: float = 0.10
    b_temporal_c_leak_weight: float = 0.25

    # B 可以移动，但中心突跳越大得分越低；值越小，约束越严格。
    b_temporal_center_sigma_px: float = 80.0

    # B 形状基本不变，因此候选 mask 面积变化超过这个范围时直接强惩罚/拒绝。
    b_temporal_area_ratio_min: float = 0.50
    b_temporal_area_ratio_max: float = 1.80

    # B 不能大面积吸入 C 的独有区域；但允许真实 B/C 重合区域。
    # C 独有区域定义为 static_C - last_B。
    b_temporal_max_c_unique_leak_ratio: float = 0.35

    # 如果所有候选都很差，是否退回上一帧 B。正式实验建议 True，避免错误 mask 继续传递。
    b_temporal_fallback_to_last_if_bad: bool = True
    b_temporal_min_accept_score: float = 0.15
...
def c_unique_leak_ratio(
    ...
    """
    候选 B mask 泄漏到 C 独有区域的比例。

    因为 B/C 可以真实重合，所以不能惩罚所有 B∩C。
    只惩罚候选 B 吸入 static_C - last_B 的区域。
    """
...
class SAM2ABCSegmenter:
    ...
    def _score_a_candidate_mask(
        ...
        """
        对 SAM2 返回的候选 A mask 进行时序稳定评分。

        A 的物理形状基本不变，但位置会变化；因此评分鼓励：
            1. SAM2 score 高；
            2. 与上一帧 A 有一定 IoU；
            3. 面积接近上一帧 A；
            4. 中心不要突跳；
            5. 不要大量泄漏到 C 独有区域 static_C - last_A。

        说明：A 可能靠近 C，但 A 的 mask 不应突然把 C 的主体吸进去。
        因此只惩罚 static_C - last_A 的 C 独有区域，不惩罚上一帧 A 已经占据/接触的区域。
        """
        ...
        area_ratio = cand_area / ref_area if ref_area > 1e-8 else 1.0

        raw_score = float(sam2_score)
        score_norm = float(max(0.0, min(1.0, raw_score)))

        iou = mask_iou(cand, ref)
        area_sim = mask_area_similarity(cand, ref)
        center_sim = center_similarity(cand, reference_center, sigma_px=self.a_temporal_center_sigma_px)
        leak_ratio = c_unique_leak_ratio(cand, static_c_mask, ref)

        hard_penalty = 0.0
        reject_reason = ""
        if ref_area > 1e-8 and (area_ratio < self.a_temporal_area_ratio_min or area_ratio > self.a_temporal_area_ratio_max):
            hard_penalty += 0.75
            reject_reason += f"area_ratio_out({area_ratio:.3f});"
        if leak_ratio > self.a_temporal_max_c_unique_leak_ratio:
            hard_penalty += 0.75
            reject_reason += f"c_unique_leak_high({leak_ratio:.3f});"

        total = (
            self.a_temporal_score_weight * score_norm
            + self.a_temporal_iou_weight * iou
            + self.a_temporal_area_weight * area_sim
            + self.a_temporal_center_weight * center_sim
            - self.a_temporal_c_leak_weight * leak_ratio
            - hard_penalty
        )
    ...
    def _score_b_candidate_mask(
        ...
        """
        对 SAM2 返回的候选 B mask 进行时序稳定评分。

        B 的物理形状基本不变，但位置会变化；因此评分鼓励：
            1. SAM2 score 高；
            2. 与上一帧 B 有一定 IoU；
            3. 面积接近上一帧 B；
            4. 中心不要突跳；
            5. 不要大量泄漏到 C 独有区域 static_C - last_B。
        """
        cand = candidate_mask.astype(bool)
        ref = reference_mask.astype(bool) if reference_mask is not None else None
        ...
        raw_score = float(sam2_score)
        # SAM2 score 通常已经在 0~1 附近，这里仍做裁剪，避免异常值影响。
        score_norm = float(max(0.0, min(1.0, raw_score)))

        iou = mask_iou(cand, ref)
        area_sim = mask_area_similarity(cand, ref)
        center_sim = center_similarity(cand, reference_center, sigma_px=self.b_temporal_center_sigma_px)
        leak_ratio = c_unique_leak_ratio(cand, static_c_mask, ref)

        # 面积变化太大，或者候选 B 吸入过多 C 独有区域，强惩罚。
        hard_penalty = 0.0
        reject_reason = ""
        if ref_area > 1e-8 and (area_ratio < self.b_temporal_area_ratio_min or area_ratio > self.b_temporal_area_ratio_max):
            hard_penalty += 0.75
            reject_reason += f"area_ratio_out({area_ratio:.3f});"
        if leak_ratio > self.b_temporal_max_c_unique_leak_ratio:
            hard_penalty += 0.75
            reject_reason += f"c_unique_leak_high({leak_ratio:.3f});"

        total = (
            self.b_temporal_score_weight * score_norm
            + self.b_temporal_iou_weight * iou
            + self.b_temporal_area_weight * area_sim
            + self.b_temporal_center_weight * center_sim
            - self.b_temporal_c_leak_weight * leak_ratio
            - hard_penalty
        )

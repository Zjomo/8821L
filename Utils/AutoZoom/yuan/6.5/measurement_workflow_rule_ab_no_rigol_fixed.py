...
class MeasurementConfig:
    ...
    rule_ab_stage_velocity: float = int(_cfg("rule_ab_stage_velocity", 200))
    rule_ab_stage_acceleration: float = int(_cfg("rule_ab_stage_acceleration", 200))
...
class MeasurementWorkflowGUI:
    ...
    def test_rule_ab(self):
        """
        单独点击“单独测试A推动B”按钮时，只运行 RuleAB 与角度检测。

        注意：这里不连接、不配置、不打开、不关闭 Rigol 信号发生器。
        Rigol 只在完整循环测量或用户手动点击 Rigol 控制按钮时使用。
        """
        try:
            wf = self.ensure_workflow()

            # 单独测试 A推动B 只需要角度检测模块和 RuleAB 模块。
            # 不要在这里连接 Rigol，否则没有接信号发生器或 VISA 资源不可用时，
            # 会导致“单独测试A推动B”无法运行。
            if wf.angle_module is None:
                wf.init_angle_module()

            self.set_var(self.rule_module_status_var, "规则模块状态：RuleAB测试中；未连接/未控制Rigol")
            self.log("[RuleAB] 单独测试A推动B：跳过 Rigol 连接、配置、CH1 ON 和 CH1 OFF")

            baseline_result = wf.detect_angle_once(label="rule_ab_manual_baseline", allow_fail=True)
            baseline_angle = (
                float(baseline_result["angle_deg"])
                if baseline_result.get("ok", False) and baseline_result.get("angle_deg") is not None
                else None
            )

            result = wf.run_rule_ab_until_angle_delta(baseline_angle=baseline_angle)

            self.set_var(
                self.rule_module_status_var,
                f"规则模块状态：RuleAB完成，ok={result.get('ok')}，delta={result.get('final_delta')}；未控制Rigol",
            )
            self.refresh_result_labels(wf)

        except Exception as e:
            self.set_var(self.rule_module_status_var, "规则模块状态：RuleAB测试失败")
            self.log(f"[RuleAB] 单独测试失败：{e}")
            self.log(traceback.format_exc())

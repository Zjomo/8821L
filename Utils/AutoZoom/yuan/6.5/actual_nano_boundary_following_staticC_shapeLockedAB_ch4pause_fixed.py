...
class RuntimeConfig:
    ...
    # CH4 上下方向运动节拍控制
    # --------------------------------------------------------
    # 需求：不论 UP / DOWN，只执行一次 CH4 动作，然后强制停顿 2 s。
    # 这个停顿发生在每次 action=UP 或 action=DOWN 的 Stage 指令之后。
...
class SAM2ABCSegmenter:
    ...
    def infer_abc(self, image_rgb: np.ndarray) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        try:
            # A：后续帧允许 A 移动。默认只用上一帧中心点做弱提示，
            #    旧 mask 的全部像素不再作为硬提示，而是用于候选 mask 形状筛选。
            if self.temporal_use_center_point_only and self.last_a_center is not None:
                a_pos = [self.last_a_center]
            else:
            ...
            # B：后续帧允许 B 移动。默认只用上一帧中心点做弱提示，
            #    旧 mask 的像素形状用于 temporal candidate selection。
            #    B/C 真实重合区域不能作为 B 负点，所以负点仍取 static_C - last_B。
            if self.temporal_use_center_point_only and self.last_b_center is not None:
                b_pos = [self.last_b_center]
            else:
            ...
            if self.a_use_temporal_mask_selection:
                a_mask, a_debug = self._predict_a_mask_by_box_and_points_temporal(
                    ...
                    static_c_mask=self.static_c_mask,
                )
                ...
                self.last_a_temporal_debug = {"enabled": False}
            ...
            if self.b_use_temporal_mask_selection:
                ...
                self.last_b_temporal_debug = {"enabled": False}
    ...
    def _score_temporal_candidate_mask(
        ...
        score_weight: float,
        iou_weight: float,
        shape_iou_weight: float,
        area_weight: float,
        center_weight: float,
        aspect_weight: float,
        bbox_size_weight: float,
        c_leak_weight: float,
        center_sigma_px: float,
        area_ratio_min: float,
        area_ratio_max: float,
        min_shape_iou: float,
        min_aspect_similarity: float,
        min_bbox_size_similarity: float,
        max_c_unique_leak_ratio: float,
    ) -> Dict[str, Any]:
        """
        A/B 通用候选 mask 评分。
        ...
        if leak_ratio > float(max_c_unique_leak_ratio):
            hard_reject = True
...
class ActualNanoBoundaryFollower:
    ...
    def __init__(self, cfg: RuntimeConfig):
        ...
        self.abc_segmenter = SAM2ABCSegmenter(
            ...
            a_temporal_c_leak_weight=cfg.a_temporal_c_leak_weight,
            ...
            a_temporal_max_c_unique_leak_ratio=cfg.a_temporal_max_c_unique_leak_ratio,
            ...
            b_temporal_c_leak_weight=cfg.b_temporal_c_leak_weight,
            ...
            b_temporal_max_c_unique_leak_ratio=cfg.b_temporal_max_c_unique_leak_ratio,
    ...
    def execute_action(self, action_id: int) -> None:
        ...
        # 需求：CH4 不论上/下，每次只执行一次动作，动作结束后强制停顿 2 s。
        ...
        if self.stage is None:
            ...
            if is_ch4_vertical_action and ch4_pause_s > 0:
                logger.info(
                    "[DRY-RUN][Stage34-CH4] action=%s 后模拟停顿 %.3f s",
                    ACTION_NAMES[action_id],
                    ch4_pause_s,
                )
                time.sleep(ch4_pause_s)
            return
        ...
        # 对 CH4 上下方向，强制 wait=True，避免 wait=False 时刚发命令就 stop_all。
        # 水平方向仍使用原 cfg.wait_stage_move。
        wait_flag = bool(self.cfg.wait_stage_move)
        if is_ch4_vertical_action and ch4_pause_s > 0 and ch4_force_wait:
            wait_flag = True
        ...
        if is_ch4_vertical_action and ch4_pause_s > 0:
            if ch4_stop_after:
                try:
                    self.stage.stop_all()
                    logger.info("[Stage34-CH4] action=%s 后已 stop_all。", ACTION_NAMES[action_id])
                except Exception as e:
                    logger.warning("[Stage34-CH4] action=%s 后 stop_all 失败: %s", ACTION_NAMES[action_id], e)

            logger.info(
                "[Stage34-CH4] action=%s 已完成；强制停顿 %.3f s 后再进入下一帧。",
                ACTION_NAMES[action_id],
                ch4_pause_s,
            )
            time.sleep(ch4_pause_s)

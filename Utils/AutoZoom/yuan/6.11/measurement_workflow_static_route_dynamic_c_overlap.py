...
class CalibrationState:
    ...
    def missing_items(self) -> List[str]:
        ...
        # 新版 Step9 使用 B ∩ C_dynamic 的中心做位置对齐，
        # 不再需要颜色 HSV 标定；只需要 Step9 目标点即可。
        return missing
...
class MeasurementWorkflow:
    def __init__(self, cfg: MeasurementConfig, on_log=None, on_update=None):
        ...
        # Step9 动态 C：用于 B ∩ C_dynamic 重合区域中心对齐。
        # 注意：Step7 运动路线仍然使用 full_calibration_static_c 的固定 C 路线，
        # 这里的 C_dynamic 只用于重合区域计算和保存，不参与 Step7 路线控制。
    ...
    def begin_step9_run_session(self) -> Path:
        ...
        meta = {
            ...
            "step9_detection_mode": "bc_dynamic_overlap_center",
    ...
    def run_rule_ab_until_angle_delta(
        ...
        while True:
            ...
            if angle_result.get("ok", False) and angle_result.get("angle_deg") is not None:
                ...
                if angle_result.get("edge_selection_mode") == "nearest_b_edge_to_a_center":
                    # 新逻辑已经明确选中了 A中心最近的 B 边，不再用“最接近上一帧角度”的修复逻辑覆盖。
                    final_angle = raw_angle
                    repaired_info = {
                        "enabled": bool(getattr(self.cfg, "rule_ab_enable_angle_repair", True)),
                        "triggered": False,
                        "used_repaired_angle": False,
                        "reason": "nearest_b_edge_to_a_center_primary_no_repair_override",
                    }
                else:
    ...
    def prepare_step9_color_region_detection(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if center is not None:
                ...
                cv2.putText(canvas, "B&Cdyn center", (cx + 10, cy - 8), cv2.FONT_HERSHEY_SIMPLEX, 0.55, (0, 0, 255), 2, cv2.LINE_AA)
            ts = datetime.now().strftime("%Y%m%d_%H%M%S_%f")[:-3]
    ...
    def _predict_step9_dynamic_c_mask_by_sam2(self, image_rgb: np.ndarray) -> Dict[str, Any]:
        ...
        重要分工：
            ...
            - 这里得到的 C_dynamic 只用于 Step9/Step8 的 B∩C_dynamic 重合区域中心计算。
        """
        import torch
        ...
        # 后续帧优先用上一帧 dynamic C 的中心点 + bbox 更新，避免平台运动后原始 C 正点坐标失效。
        box = None
        ...
        if masks is None or len(masks) == 0:
            raise RuntimeError("SAM2 没有返回 Step9 dynamic C mask。")
        ...
        if int(np.count_nonzero(raw_c)) <= 0:
            raise RuntimeError("Step9 dynamic C 清理后为空。")
        ...
        self.log(
            f"[Step9-BC中心] dynamic C 分割完成：mode={prompt_mode}, "
    ...
    def _capture_bc_overlap_center_once(self) -> Dict[str, Any]:
        """
        Step9 当前帧检测：B ∩ C_dynamic 重合区域中心对齐。

        关键分工：
            1. Step7 的 A 沿 C 运动路线仍使用 full_calibration_static_c 固定路线；
            2. Step9/Step8 的位置对齐使用当前帧重新分割得到的 C_dynamic；
            3. 重合中心 = center(B_current ∩ C_dynamic_geometry)。

        因此新版 Step9 不再需要颜色选择，只需要一次人工选定目标位置。
        """
        ...
        try:
            ...
            try:
                self._postprocess_scene_ab_masks(
                    follower=follower,
                    scene=scene,
                    tag="step9_bc_dynamic_overlap",
                )
            except Exception as e:
                self.log(f"[Step9-BC中心] Step9 A/B mask 后处理失败，将继续尝试读取 B mask：{e}")
        except Exception as e:
            return {
                "ok": False,
                "reason": f"capture_rule_ab_scene_failed:{e}",
                "image_rgb": None,
                "overlap_mask": None,
                "overlap_area_px": 0,
                "b_mask_area_px": "",
        ...
        try:
            b_obj = getattr(scene, "b", None)
            b_mask = self._extract_mask_from_scene_object(b_obj)
            if b_mask is None or int(np.count_nonzero(np.asarray(b_mask).astype(bool))) <= 0:
                raise RuntimeError("当前帧 B mask 为空，不能计算 B∩C_dynamic。")
            b_mask = np.asarray(b_mask).astype(bool)
        except Exception as e:
            return {
                "ok": False,
                "reason": f"b_mask_missing:{e}",
                "image_rgb": image_rgb,
                "overlap_mask": None,
                "b_mask": None,
                "c_mask": None,
        ...
        try:
            ...
            c_raw_mask = np.asarray(dyn.get("raw_c_mask")).astype(bool) if dyn.get("raw_c_mask") is not None else None
        except Exception as e:
            self.log(f"[Step9-BC中心] dynamic C 分割失败，尝试回退上一帧 dynamic C / static C：{e}")
            c_raw_mask = None
            try:
                if self.step9_dynamic_c_last_mask is not None and np.any(self.step9_dynamic_c_last_mask):
                    c_mask = self.step9_dynamic_c_last_mask.copy().astype(bool)
                    dyn = {
                        "ok": True,
                        "reason": f"fallback_last_dynamic_c:{e}",
                        "geometry": self.step9_dynamic_c_last_geometry or {},
                    }
                else:
                    c_mask = self._load_step9_c_mask_from_config(image_shape=image_rgb.shape[:2]).astype(bool)
                    dyn = {"ok": True, "reason": f"fallback_static_c:{e}", "geometry": {"source": "fallback_static_c"}}
            except Exception as ee:
                return {
                    "ok": False,
                    "reason": f"dynamic_c_failed:{e}; fallback_failed:{ee}",
                    "image_rgb": image_rgb,
                    "overlap_mask": None,
                    "b_mask": b_mask,
                    "c_mask": None,
        ...
        if c_mask.shape[:2] != b_mask.shape[:2]:
            h, w = b_mask.shape[:2]
            c_mask = cv2.resize(c_mask.astype(np.uint8), (w, h), interpolation=cv2.INTER_NEAREST).astype(bool)
            if c_raw_mask is not None:
                c_raw_mask = cv2.resize(c_raw_mask.astype(np.uint8), (w, h), interpolation=cv2.INTER_NEAREST).astype(bool)

        overlap = np.logical_and(b_mask, c_mask)
        center = self._mask_center_xy(overlap)
        overlap_area = int(np.count_nonzero(overlap))
        if center is None or overlap_area <= 0:
            return {
                "ok": False,
                "reason": "empty_b_and_dynamic_c_overlap",
                "image_rgb": image_rgb,
                "b_mask": b_mask,
                "c_mask": c_mask,
                "c_raw_mask": c_raw_mask,
                "overlap_mask": overlap,
                "overlap_area_px": overlap_area,
                "b_mask_area_px": int(np.count_nonzero(b_mask)),
                "c_mask_area_px": int(np.count_nonzero(c_mask)),
                "dynamic_c": self._json_safe(dyn),
            }

        return {
            "ok": True,
            "reason": "ok_b_intersect_dynamic_c",
            "image_rgb": image_rgb,
            "b_mask": b_mask,
            "c_mask": c_mask,
            "c_raw_mask": c_raw_mask,
    ...
    def _save_step9_bc_overlay(
        ...
        """
        保存 Step9 B∩C_dynamic 重合区域中心对齐的检查图和检测结果。
        ...
        try:
            ...
            if c_raw_mask is not None:
                p_cr = mask_dir / f"{stem}_c_dynamic_raw_sam2_mask.png"
                cv2.imwrite(str(p_cr), (np.asarray(c_raw_mask).astype(np.uint8) * 255))
                saved["c_raw_mask_path"] = str(p_cr)

            if c_mask is not None:
                p_c = mask_dir / f"{stem}_c_dynamic_geometry_mask.png"
                cv2.imwrite(str(p_c), (np.asarray(c_mask).astype(np.uint8) * 255))
                saved["c_mask_path"] = str(p_c)

            if overlap_mask is not None:
                p_o = mask_dir / f"{stem}_b_intersect_c_dynamic_mask.png"
            ...
            if overlap_mask is not None:
                ...
                overlay = canvas.copy()
                overlay[om] = (0, 255, 255)    # B∩C_dynamic: yellow
...
class MeasurementWorkflowGUI:
    ...
    def _build_ui(self):
        ...
        ttk.Button(button_grid, text="4 标定Step9目标", command=self.calibrate_step9_target_color_thread).grid(row=4, column=0, padx=4, pady=4, sticky="ew")
    ...
    def calibrate_step9_target_color(self):
        ...
        try:
            ...
            # 新版 Step9 使用 B ∩ C_dynamic 的中心做位置对齐，
            # 不再需要颜色选择/HSV 标定；只保存本次新目标点。

            # 重新同步 GUI，确保本次新目标点进入 cfg。
            wf = self.sync_config_from_ui_to_workflow()
            wf.cfg.rule_ac_target_x_px = target_x
            wf.cfg.rule_ac_target_y_px = target_y
            state = self._current_or_new_calibration_state()
            ...
            self.log(
                ...
                "新版 Step9 使用 B∩C_dynamic 中心对齐，不再需要颜色选择。"
            )
        except Exception as e:
    ...
    def test_rule_ac(self):
        """
        Step9 单独测试：B∩C_dynamic 重合区域中心对齐。

        不再使用颜色检测；只需要提前选定目标位置。
        B 来自当前 RuleAB/SAM2 分割，C_dynamic 每帧用完整标定 C 正点/负点重新分割并生成外接矩形/四边形。
        """
        try:
            self.set_var(self.rule_module_status_var, "规则模块状态：Step9 B∩C_dynamic中心对齐测试中")
            self.log("========== Step9 单独测试：B∩C_dynamic中心对齐 ==========")
            ...
            self.log(
                ...
                "detection=B∩C_dynamic, no_color_selection=True, "
            ...
            # 新版不再选择颜色，直接用 B∩C_dynamic 的中心做对齐。
            self.log("[Step9-BC中心] 不再选择颜色；使用当前帧 B∩C_dynamic 的中心做位置对齐。")

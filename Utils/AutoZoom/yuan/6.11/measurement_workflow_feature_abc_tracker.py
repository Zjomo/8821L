...
class MeasurementWorkflow:
    ...
    def detect_ab_nearest_b_edge_angle_once(
        ...
        try:
            ...
            saved = self._save_ab_nearest_edge_angle_debug(
                ...
                label=label,
                extra_info={"ab_guard": ab_guard_info, "ab_seg_retry": self._json_safe(seg_retry_info)},
            )
            if selected is None:
    ...
    def _install_feature_tracker_on_rule_ab_follower(self, follower: Any, state: Optional[CalibrationState] = None):
        ...
        def _build_templates_from_original_scene() -> Tuple[np.ndarray, Any]:
            image_rgb, scene = original_capture()
            profiles: Dict[str, Dict[str, Any]] = {}
            masks: Dict[str, np.ndarray] = {}
            for lab in ("a", "b", "c"):
                ...
                masks[lab] = np.asarray(mask).astype(bool)
                profiles[lab] = self._extract_feature_profile_from_mask(image_rgb, masks[lab], lab)
            out_dir = Path(tracker_state["out_dir"])
            saved = self._save_feature_templates_debug(image_rgb, profiles, masks, out_dir)
            tracker_state["profiles"] = profiles
            tracker_state["base_scene"] = copy.deepcopy(scene)
            ...
            try:
                ...
                pass
            self.log(
                "[ABC特征跟踪] 已用首帧 SAM2 建立 A/B/C 模板；后续完整测量不再逐帧调用 SAM2，"
                f"模板目录={out_dir}"
            )
        ...
        def _feature_capture_and_build_scene():
            ...
            if scene is None:
                # 极端兜底：如果 deepcopy 失败，就重新走一次原始 SAM2。正常不应进入。
                self.log("[ABC特征跟踪] base_scene 缺失，临时回退一次原始 SAM2。")
                image_rgb2, scene2 = original_capture()
                tracker_state["base_scene"] = copy.deepcopy(scene2)
                return image_rgb2, scene2

            for lab, mask in masks.items():
                obj = self._get_scene_object_by_label(scene, lab)
                self._set_mask_to_scene_object(obj, mask)
                # 同步常见缓存字段，避免外部模块仍读取 last_a_mask/current_b_mask。

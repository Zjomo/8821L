...
class MeasurementWorkflow:
    ...
    def _repair_rule_ab_angle_if_jump(
        ...
        Step7 关闭激光判断：
            检测结果 raw_delta 或修复结果 repaired_delta 只要 >= min_delta，立即关闭激光。
            也就是：角度变化一旦达到 3.5° 就立刻关激光；
            max_delta=6° 只作为“理想上限/过冲提示”，不再作为关闭激光的硬上限。
        """
        info: Dict[str, Any] = {
    ...
    def run_rule_ab_until_angle_delta(
        ...
        while True:
            ...
            #   2) 未修复时看 raw_delta，修复时看 repaired_delta / final_delta；
            #   3) 检测结果或者修复结果只要 >= min_delta，就立即关闭激光；
            #   4) max_delta 只作为“是否超过理想上限6°”的记录和日志提示，不再作为关闭硬窗口。
            close_min_delta = float(min_delta)
            close_target_max_delta = float(max_delta)
            raw_close_hit = raw_delta is not None and float(raw_delta) >= close_min_delta
            repaired_close_hit = repaired_delta is not None and float(repaired_delta) >= close_min_delta
            close_hit = bool(raw_close_hit or repaired_close_hit)

            close_delta_for_reason = None
            close_source = "none"
            if repaired_close_hit and repaired_delta is not None:
                close_delta_for_reason = float(repaired_delta)
                close_source = "repaired"
            elif raw_close_hit and raw_delta is not None:
            ...
            try:
                ...
                record["close_rule"] = "raw_or_repaired_delta_ge_min"
                record["close_hit_raw"] = bool(raw_close_hit)
                record["close_hit_repaired"] = bool(repaired_close_hit)
            ...
            self.log(
                ...
                f"close_rule=delta>={close_min_delta}, ideal_max={close_target_max_delta}, "
            ...
            if close_hit:
                ...
                if raw_close_hit and repaired_close_hit:
                    stop_reason = f"raw_or_repaired_delta_ge_min_close_laser_{close_band}"
                elif raw_close_hit:
                    stop_reason = f"raw_delta_ge_min_close_laser_{close_band}"
                else:
                    stop_reason = f"repaired_delta_ge_min_close_laser_{close_band}"

                if close_delta_for_reason is not None and close_delta_for_reason > close_target_max_delta:
                    self.log(
                        f"[RuleAB] 角度变化已达到关闭阈值但超过理想上限："
                    ...
                    self.log(
                        f"[RuleAB] 角度变化达到关闭阈值："
                        f"source={close_source}, raw_delta={raw_delta}, repaired_delta={repaired_delta}, "
                ...
                try:
                    if bool(self.context.get("laser_on", False)):
                        self.laser_off()
                        self.log("[RuleAB] 角度变化达到关闭阈值后已立即执行 laser_off()")
                    else:
            ...
            if final_delta is not None:
                ...
                self.log(
                    f"[RuleAB] 当前角度变化 {final_delta:.3f}° < 关闭阈值 {close_min_delta:.3f}°，"
                    "继续下一步推动/检测。"
                )

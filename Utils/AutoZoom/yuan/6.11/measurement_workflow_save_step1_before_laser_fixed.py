...
class MeasurementWorkflow:
    ...
    def _repair_rule_ab_angle_if_jump(
        ...
        # 最新逻辑：
        #   1) raw_angle 相对上一帧确定角度 raw_jump > 6°，需要修复；
        #   2) raw_angle 相对 Step1 baseline 的 raw_delta >= 6°，也必须先修复，
        #      用于区分“真实过冲”和“选错边”。
        trigger_by_jump = bool(raw_jump is not None and float(raw_jump) > max_jump)
        trigger_by_delta_ge_max = bool(raw_delta is not None and float(raw_delta) >= max_jump)
        if not (trigger_by_jump or trigger_by_delta_ge_max):
            info["reason"] = "raw_jump_and_raw_delta_within_no_repair_use_raw_angle"
            self.log(
                f"[RuleAB-角度修复] raw_angle={raw_angle:.6f}，"
                f"raw_jump={raw_jump:.3f}° <= {max_jump:.3f}°，"
                f"raw_delta={raw_delta} < {max_jump:.3f}°，检测角度可信，不执行修复。"
            )
            try:
                angle_result["angle_repair"] = self._json_safe(info)
                angle_result["angle_deg_raw_before_repair"] = float(raw_angle)
        ...
        info["triggered"] = True
        if trigger_by_delta_ge_max and trigger_by_jump:
            info["trigger_reason"] = "raw_delta_from_baseline_ge_max_and_raw_jump_from_previous_above_max"
        elif trigger_by_delta_ge_max:
            info["trigger_reason"] = "raw_delta_from_baseline_ge_max"
        else:
            info["trigger_reason"] = "raw_jump_from_previous_above_max"
        self.log(
            f"[RuleAB-角度修复] raw_angle={raw_angle:.6f} 触发修复："
            f"raw_delta={raw_delta}, raw_jump={raw_jump:.3f}°，阈值={max_jump:.3f}°；"
            "开始基于当前 B mask 全边候选修复，用于区分真实过冲/选错边。"
        )
    ...
    def run_rule_ab_until_angle_delta(
        ...
        while True:
            ...
            # 关闭激光条件（按最新需求）：
            #   1) Step7 每推动一次后检测 raw_angle；
            #   2) raw_delta = |raw_angle - Step1 baseline|；
            #   3) raw_delta < 3.5°：未达到关闭条件，继续推动；
            #   4) 3.5° <= raw_delta < 6°：检测角度可信，立即关闭激光，保存 raw_angle；
            #   5) raw_delta >= 6°：必须先区分“真实过冲”和“选错边”：
            #        - repaired_delta >= 6° 且 |repaired_delta - raw_delta| <= 2°：
            #              视为真实过冲/检测速度没跟上，关闭激光，保存 repaired_angle；
            #        - repaired_delta < 6° 且 |repaired_delta - raw_delta| >= 6°：
            #              视为选错边，不关闭激光，使用 repaired_angle 更新确定角度并继续推动；
            #        - 其他修复后已经 >=3.5° 的情况：关闭激光，保存 repaired_angle；
            #        - 修复失败或修复后 <3.5°：继续推动/重新取帧分割。
            close_min_delta = float(min_delta)
            close_target_max_delta = float(max_delta)
            repair_delta_tol = float(getattr(self.cfg, "rule_ab_overshoot_repair_delta_tolerance_deg", 2.0))
            ...
            if raw_delta is not None and float(raw_delta) >= close_target_max_delta:
                # raw 已经 >=6°，必须先用当前 B mask 全边修复结果区分“真实过冲”和“选错边”。
                close_source = "raw_delta_ge_6_repair_required"
                ...
                if repair_used:
                    ...
                    if raw_delta is not None and close_delta_for_reason is not None:
                        repaired_vs_raw_delta_diff = abs(float(close_delta_for_reason) - float(raw_delta))

                    if (
                        close_delta_for_reason is not None
                        and float(close_delta_for_reason) >= close_target_max_delta
                        and repaired_vs_raw_delta_diff is not None
                        and float(repaired_vs_raw_delta_diff) <= repair_delta_tol
                    ):
                        # 情况1：修复角度变化与检测角度变化基本一致，而且都 >=6°。
                        # 说明更像是真实过冲/推动过快/检测速度没跟上。
                        close_hit = True
                        close_source = "repaired_confirmed_real_overshoot"
                        close_band = "repaired_delta_ge_6_and_close_to_raw_delta_close"
                        final_angle = float(repaired_info.get("repaired_angle", final_angle))
                        final_delta = float(close_delta_for_reason)

                    elif (
                        close_delta_for_reason is not None
                        and float(close_delta_for_reason) < close_target_max_delta
                        and repaired_vs_raw_delta_diff is not None
                        and float(repaired_vs_raw_delta_diff) >= wrong_edge_jump
                    ):
                        # 情况2：raw 看起来 >=6°，但修复后 <6°，且两者差异很大。
                        # 说明 raw 很可能选错边；不关闭激光，继续推动。
                        close_hit = False
                        wrong_edge_continue = True
                        close_source = "wrong_edge_repaired_delta_below_6_continue"
                        close_band = "wrong_edge_continue"
                        if final_angle is not None:
                            last_valid_step_angle = float(final_angle)

                    elif close_delta_for_reason is not None and float(close_delta_for_reason) >= close_min_delta:
                        # 修复后没有满足“真实过冲”的严格条件，但已经达到 >=3.5°；
                        # 这种情况下仍应关闭激光，避免继续推动造成更大过冲。
                        close_hit = True
                        close_source = "repaired_delta_ge_3p5_close_after_raw_ge6"
                        close_band = "repaired_delta_ge_3p5_close"
                        final_delta = float(close_delta_for_reason)

                    else:
                        close_hit = False
                        close_source = "repaired_delta_below_3p5_continue_after_raw_ge6"
                        close_band = "repaired_delta_below_3p5_continue"
                        if final_angle is not None:
                            last_valid_step_angle = float(final_angle)

                else:
                    # raw_delta >=6 但修复失败/无候选时，不直接用 raw_angle 关激光；
                    # 下一轮重新取帧、重新分割、重新检测。
                    close_delta_for_reason = None
                    close_source = "raw_delta_ge_6_repair_failed_continue"
                    close_hit = False
                    close_band = "repair_failed_continue"

            elif repair_used:
                # raw_delta 未到 6°，但因 raw_jump 等原因触发了修复；
                # 这时使用 repaired_delta 判断是否达到 3.5°。
                if repaired_delta is not None:
                    close_delta_for_reason = float(repaired_delta)
                elif final_delta is not None:
                    close_delta_for_reason = float(final_delta)
                ...
                # raw_delta <6 且未触发修复：直接按 raw_delta >=3.5° 关闭。
                ...
                if close_delta_for_reason is not None:
                    if float(close_delta_for_reason) < close_min_delta:
                        ...
                        close_band = "raw_delta_above_6_close_without_repair_unexpected"
            ...
            try:
                ...
                record["close_rule"] = "raw_delta_lt3p5_continue__3p5_to6_close_raw__raw_delta_ge6_repair_classify_real_overshoot_or_wrong_edge"
                record["repair_delta_tolerance_deg"] = repair_delta_tol
            ...
            self.log(
                ...
                f"close_rule=raw_delta<3.5继续；3.5<=raw_delta<6直接关；raw_delta>=6先修复区分真实过冲/选错边"
            )

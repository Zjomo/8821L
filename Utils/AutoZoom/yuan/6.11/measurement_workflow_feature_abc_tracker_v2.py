...
class MeasurementConfig:
    ...
    rule_ab_feature_update_template_each_frame: bool = bool(_cfg("rule_ab_feature_update_template_each_frame", True))
...
class MeasurementWorkflow:
    ...
    def run_rule_ab_until_angle_delta(
        ...
        while True:
            ...
            if repair_failed:
                ...
                if repair_used:
                    # raw 与 repaired 差异 >2°，说明检测边异常；用修复角度判断是否关激光。
                ...
                if close_delta_for_reason is not None:
                    if float(close_delta_for_reason) < close_min_delta:
                        close_band = (
                            "repaired_delta_below_3p5_continue"
                            if repair_used else
                            "raw_delta_below_3p5_continue"
                        )
                    elif float(close_delta_for_reason) < close_target_max_delta:
                        close_band = (
                            "repaired_delta_ge_3p5_lt_6_close"
                            if repair_used else
                            "raw_delta_ge_3p5_lt_6_close"
                        )
                    else:
                        close_band = (
                            "repaired_delta_ge_6_close"
                            if repair_used else
                            "raw_delta_ge_6_confirmed_close"
                        )
            ...
            try:
                ...
                record["close_rule"] = "every_angle_detection_repair_check__use_raw_if_repaired_vs_raw_within_2deg__else_use_repaired__close_if_final_delta_ge_3p5"
                ...
                record["close_hit_repaired"] = bool(repair_used and close_hit and close_source == "repaired")
            ...
            self.log(
                ...
                f"close_rule=每帧都修复检查；|repaired-raw|<=2°用raw；>2°用repaired；最终delta>=3.5°关激光"
            )
    ...
    def _save_feature_templates_debug(
        ...
        """保存首帧模板、mask 和 overlay，便于检查 SAM2 初始分割是否正确。"""
        paths: Dict[str, str] = {}
        out_dir.mkdir(parents=True, exist_ok=True)
        try:
            ...
            for lab, m in masks.items():
                ...
                cv2.imwrite(str(out_dir / f"{lab}_sam2_initial_mask.png"), mb.astype(np.uint8) * 255)
                ...
                if center:
                    ...
                    cv2.putText(canvas, lab.upper(), (cx + 8, cy - 8), cv2.FONT_HERSHEY_SIMPLEX, 0.7, color_map.get(lab, (255, 255, 0)), 2, cv2.LINE_AA)
            canvas = cv2.addWeighted(overlay, 0.35, canvas, 0.65, 0)
    ...
    def _update_feature_profile_runtime(self, image_rgb: np.ndarray, profile: Dict[str, Any], mask: np.ndarray) -> Dict[str, Any]:
        ...
        if bool(getattr(self.cfg, "rule_ab_feature_update_template_each_frame", True)):
    ...
    def _install_feature_tracker_on_rule_ab_follower(self, follower: Any, state: Optional[CalibrationState] = None):
        ...
        def _build_templates_from_original_scene() -> Tuple[np.ndarray, Any]:
            ...
            for lab in ("a", "b", "c"):
                ...
                sam2_masks[lab] = np.asarray(mask).astype(bool)
                profiles[lab] = self._extract_feature_profile_from_mask(image_rgb, sam2_masks[lab], lab)

            out_dir = Path(tracker_state["out_dir"])
            saved = self._save_feature_templates_debug(image_rgb, profiles, sam2_masks, out_dir)

            # 关键修改：首帧也不直接返回旧 SAM2 scene。
            # 用刚建立的颜色/形状模板在同一张首帧图上再识别一次，得到 feature_tracker_scene，
            # 后续 Step7、角度检测、overlay 保存全部从这个 scene.a 读取 A。
            feature_masks, debug = self._track_abc_by_saved_features(image_rgb, profiles)
            for lab in ("a", "b", "c"):
                if lab not in feature_masks or not np.any(np.asarray(feature_masks[lab]).astype(bool)):
                    feature_masks[lab] = sam2_masks[lab].copy()
                    debug.setdefault(lab, {})["fallback_to_initial_sam2_mask"] = True

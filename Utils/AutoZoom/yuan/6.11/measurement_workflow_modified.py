...
class MeasurementWorkflow:
            ...
            if final_delta is not None:
                if min_delta <= float(final_delta) <= max_delta:
                    ...
                    self.log(
                        ...
                        f"目标范围=[{min_delta:.3f}, {max_delta:.3f}]；立即关闭激光，然后继续完整测量。"
                    )
                    try:
                        if bool(self.context.get("laser_on", False)):
                            self.laser_off()
                            self.log("[RuleAB] 角度达标后已立即执行 laser_off()")
                        else:
                            self.log("[RuleAB] 角度达标时 context 显示 laser_on=False，跳过立即 laser_off()")
                    except Exception as e:
                        self.log(f"[RuleAB] 角度达标后立即关闭激光失败：{e}")
                    break

                if float(final_delta) > max_delta:
                    # 新需求：超过上限也要立即关闭激光，但不要停止完整测量。
                    reached = False
                    allow_continue_measurement = True
                    ...
                    try:
                        if bool(self.context.get("laser_on", False)):
                            self.laser_off()
                            self.log("[RuleAB] 角度超过上限后已立即执行 laser_off()")
                        else:
                            self.log("[RuleAB] 角度超过上限时 context 显示 laser_on=False，跳过立即 laser_off()")
                    except Exception as e:
                        self.log(f"[RuleAB] 角度超过上限后立即关闭激光失败：{e}")
                    break
    ...
    def run_one_cycle(self, cycle_index: int) -> bool:
        ...
        Step 5：照明光 ON
        Step 6：打开激光
        Step 7：A推动B（logic/rule_ab.py），每次运动后立即检测B对边角度；
                一旦修复后的角度差进入阈值或超过上限，就在 Step7 内立即关闭激光。
        Step 8：兜底关闭激光，保证异常情况下也不会长时间开激光
        Step 9：检测颜色区域中心，与提前选定位置对齐；偏离过大则移动1/2通道
        Step 10：保存本轮数据；保存角度使用 Step7 的最终修复角度，而不是 Step1 baseline
        """
        ...
        try:
            ...
            # 注意：这里不再提前保存。
            # 保存必须放在 Step7/Step8/Step9 之后，否则保存线程拿到的 context_snapshot
            # 只有 Step1 baseline 角度，没有 Step7 的最终修复角度。
            if self.stop_requested:
                return False
            ...
            try:
                ...
                self.log("========== Step 8：关闭激光（兜底） ==========")
                try:
                    if bool(self.context.get("laser_on", False)):
                        self.laser_off()
                    else:
                        self.log("[激光开关] Step8 兜底检查：laser_on=False，说明 Step7 内已关闭，不重复反向运动。")
                except Exception as e:
            ...
            self.log("========== Step 10：保存数据（异步线程，使用 Step7 最终修复角度） ==========")
            ...
            self.log(f"[流程] 第 {cycle_index} 轮保存任务已在 Step10 提交，可以进入下一轮")
            self.notify_update()
            return True

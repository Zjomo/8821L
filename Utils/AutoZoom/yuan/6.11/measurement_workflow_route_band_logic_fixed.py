...
# 已按需求停用光谱仪 TCP：不再导入/连接 LabVIEWTCPServer
LabVIEWTCPServer = None
...
class MeasurementConfig:
    ...
    # C mask 形状自适应：如果 C 原始 SAM2 mask 接近圆形，
    # 不再用 approxPolyDP 的随机四边形近似，而是用 cv2.minAreaRect 得到圆的外接矩形/四边形，
    # 然后仍按 min/target/max 三线安全带生成 Step7 路线。
    rule_ab_c_circle_as_rect_enable: bool = bool(_cfg("rule_ab_c_circle_as_rect_enable", True))
    rule_ab_c_circle_circularity_threshold: float = float(_cfg("rule_ab_c_circle_circularity_threshold", 0.78))
    rule_ab_c_circle_min_aspect_ratio: float = float(_cfg("rule_ab_c_circle_min_aspect_ratio", 0.72))
    rule_ab_c_circle_min_enclosing_fill_ratio: float = float(_cfg("rule_ab_c_circle_min_enclosing_fill_ratio", 0.58))

    # A/B mask 运行时保护：优先保护 B 的形状。
    # 如果当前 B 相对参考/上一帧的面积、中心、主轴角度变化过大，则回退到上一帧 B。
    #
    # 重要语义：
    #   1. AB 真实重合区域归属于 B，不允许因为 A_exclude_B 把 B 的重合区域裁掉；
    #   2. 只从“候选 B mask”中删除明显属于 A、且不在上一帧/参考 B 支撑区内的 A-only 误识别区域；
    #   3. 随后再执行 A = A - dilate(B)，让 A 不把 B 包进去，但 B 保持完整。
    rule_ab_enable_ab_mask_guard: bool = bool(_cfg("rule_ab_enable_ab_mask_guard", True))
    rule_ab_b_area_ratio_min: float = float(_cfg("rule_ab_b_area_ratio_min", 0.45))
    rule_ab_b_area_ratio_max: float = float(_cfg("rule_ab_b_area_ratio_max", 2.20))
    rule_ab_b_center_jump_max_px: float = float(_cfg("rule_ab_b_center_jump_max_px", 80.0))
    rule_ab_b_angle_jump_max_deg: float = float(_cfg("rule_ab_b_angle_jump_max_deg", 25.0))

    # B 候选 mask 清理：仅删除 A mask 中、但不属于 B 历史/参考支撑区的 A-only 误识别区域。
    # 真实 A-B 重合区域通常仍落在 B 的历史/参考支撑区内，因此保留给 B。
    rule_ab_b_exclude_a_unique_enable: bool = bool(_cfg("rule_ab_b_exclude_a_unique_enable", True))
    rule_ab_b_exclude_a_support_dilate_px: int = int(_cfg("rule_ab_b_exclude_a_support_dilate_px", 10))
    rule_ab_b_min_area_after_a_unique_exclude_px: int = int(_cfg("rule_ab_b_min_area_after_a_unique_exclude_px", 10))

    rule_ab_a_exclude_b_dilate_px: int = int(_cfg("rule_ab_a_exclude_b_dilate_px", 3))
...
class MeasurementWorkflow:
    ...
    def start_tcp_server(self):
        """
        已按本次需求停用光谱仪 / LabVIEW TCP。

        这里保留函数名是为了兼容 GUI 按钮和旧调用路径，但不再创建
        LabVIEWTCPServer，也不占用 TCP 端口。完整测量中的光谱峰值统一使用
        占位值 1000。
        """
        self.tcp_server = None
        self.context["tcp_started"] = False
        self.context["labview_ready"] = True
        self.context["spectrometer_tcp_disabled"] = True
        self.log("[TCP] 光谱仪/LabVIEW TCP 已按需求停用：不启动 TCP Server，不等待 LabVIEW 连接。")
        self.notify_update()
        return {"ok": True, "disabled": True, "reason": "spectrometer_tcp_disabled"}

    def wait_labview_ready(self):
        """光谱仪 TCP 已停用；READY 直接视为通过。"""
        self.context["tcp_started"] = False
        self.context["labview_ready"] = True
        self.context["spectrometer_tcp_disabled"] = True
        self.log("[TCP] 光谱仪/LabVIEW TCP 已停用：跳过 READY 等待。")
        self.notify_update()
        return {"ok": True, "disabled": True, "reason": "spectrometer_tcp_disabled"}
    ...
    @staticmethod
    def _point_to_segment_distance_info(
        point_xy: Tuple[float, float],
        p1_xy: Tuple[float, float],
        p2_xy: Tuple[float, float],
    ) -> Dict[str, Any]:
        """
        计算点到线段的最短距离，并返回投影点信息。
        用于选择“距离 A 中心最近的 B 边”。
        """
    ...
    def _select_bmask_angle_candidate(
        self,
        candidates: List[Dict[str, Any]],
        previous_angle: Optional[float] = None,
        a_center_xy: Optional[Tuple[float, float]] = None,
    ) -> Optional[Dict[str, Any]]:
        """
        从当前 B mask 的候选边中选择用于完整测量的 B 边角度。

        主规则：
            1. 如果有 A 中心，则选择“距离 A 中心最近”的 B 候选边；
            2. 如果 A 中心不可用，才回退到 previous_angle 跟踪同一方向；
            3. 再回退到完整标定包里的 angle_edge_index；
            4. 最后回退到最长候选边。

        注意：角度修复仍然由上层逻辑完成。若最近 A 中心的边角度
        相对上一帧有效角度跳变 > rule_ab_angle_repair_jump_threshold_deg，
        上层会用所有候选边重新选择最接近上一帧的角度。
        """
        if not candidates:
            return None
        cands = [dict(c) for c in candidates]

        if a_center_xy is not None:
            ...
            for c in cands:
                try:
                    ...
                    if p1 is None or p2 is None:
                        continue
                    info = self._point_to_segment_distance_info(a_center_xy, p1, p2)
                    cc = dict(c)
                    cc["distance_to_a_center_px"] = float(info["distance_px"])
                    cc["nearest_point_to_a_center_xy"] = info["nearest_xy"]
                    cc["nearest_point_to_a_center_t"] = float(info["segment_t"])
                    cc["selection_rule"] = "nearest_to_a_center"
                    scored.append(cc)
                except Exception:
                    continue
            if scored:
                return min(scored, key=lambda c: float(c.get("distance_to_a_center_px", 1e18)))

        if previous_angle is not None:
            try:
                prev = float(previous_angle)
                cc = dict(min(cands, key=lambda c: self.angle_diff_deg(float(c.get("angle_deg", 0.0)), prev)))
                cc["selection_rule"] = "nearest_to_previous_angle_fallback"
                return cc
            except Exception:
                pass
        ...
        try:
            state = self._get_loaded_calibration_state()
            if state is not None:
                edge_index = int(getattr(state, "angle_edge_index", -1))
        except Exception:
            edge_index = -1
        if 0 <= edge_index < len(cands):
            cc = dict(cands[edge_index])
            cc["selection_rule"] = "angle_edge_index_fallback"
            return cc

        try:
            cc = dict(max(cands, key=lambda c: float(c.get("length_px", 0.0))))
            cc["selection_rule"] = "longest_edge_fallback"
            return cc
        except Exception:
            cc = dict(cands[0])
            cc["selection_rule"] = "first_candidate_fallback"
            return cc

    @staticmethod
    def _scene_get_obj(scene: Any, name: str) -> Any:
        try:
            obj = getattr(scene, name, None)
            if obj is not None:
                return obj
        except Exception:
            pass
        try:
            if isinstance(scene, dict):
                return scene.get(name) or scene.get(name.upper())
        except Exception:
            pass
        return None

    def _get_scene_center_xy_by_name(self, scene: Any, name: str) -> Optional[Tuple[float, float]]:
        obj = self._scene_get_obj(scene, name)
    ...
    @staticmethod
    def _nearest_point_on_mask_contour(point_xy: Tuple[float, float], mask: Optional[np.ndarray]) -> Tuple[Optional[Tuple[float, float]], Optional[float]]:
        if mask is None:
            return None, None
        try:
            m = np.asarray(mask).astype(bool)
            if m.size <= 0 or not np.any(m):
                return None, None
            cnts, _ = cv2.findContours((m.astype(np.uint8) * 255), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)
            if not cnts:
                return None, None
            ...
            q = (float(pts[idx, 0]), float(pts[idx, 1]))
            return q, float(math.sqrt(float(d2[idx])))
        except Exception:
            return None, None

    def _get_angle_results_output_dir(self) -> Path:
        """
        RuleAB-B mask 角度检测不再走 ScreenAngleDetector，所以 ScreenAngleDetector
        自己的 angle_results 目录不会自动产图。这里为完整测量建立一个同名
        angle_results 目录，把 Step1/Step7 的 RuleAB 角度标注图同步保存进去。
        """
        base = Path(str(getattr(self.cfg, "angle_output_dir", "outputs/captured_frames") or "outputs/captured_frames"))
        if self.run_session_name:
            run_name = f"run_{self.run_session_name}"
        else:
            run_name = datetime.now().strftime("run_%Y%m%d_%H%M%S")
            self.context.setdefault("rule_ab_angle_results_run_name", run_name)
            run_name = str(self.context.get("rule_ab_angle_results_run_name") or run_name)
        out_dir = base / run_name / "angle_results"
        out_dir.mkdir(parents=True, exist_ok=True)
        return out_dir

    def _mirror_rule_ab_annotated_to_angle_results(
        self,
        annotated_bgr: np.ndarray,
        action_name: str,
        index: int,
        angle_info: Optional[Dict[str, Any]] = None,
        ac_distance_px: Optional[float] = None,
    ) -> str:
        """
        把 RuleAB 自定义 annotated frame 同步保存到 outputs/captured_frames/.../angle_results。

        注意：这里保存的是“RuleAB/SAM2 约束后的 B-mask 角度检测图”，不是
        ScreenAngleDetector 的 YOLO 独立检测图。这样 Step1 / Step7 的角度检测图
        都能在 angle_results 下看到，并且图上的角度与实际用于判断的 angle_info 一致。
        """
        try:
            if annotated_bgr is None:
                return ""
            out_dir = self._get_angle_results_output_dir()
            safe_action = str(action_name or "ruleab_angle").replace("/", "_").replace("\\", "_").replace(" ", "_")
            out_path = out_dir / f"frame_{int(index):06d}_{datetime.now().strftime('%Y%m%d_%H%M%S_%f')}_{safe_action}.png"
            cv2.imwrite(str(out_path), annotated_bgr)

            csv_path = out_dir / "rule_ab_angle_results.csv"
            file_exists = csv_path.exists()
            with csv_path.open("a", encoding="utf-8-sig", newline="") as f:
                writer = csv.writer(f)
                if not file_exists:
                    writer.writerow([
                        "index", "time", "action_name", "angle_result_image_path",
                        "angle_deg", "angle_source", "ac_distance_px",
                        "rule_ab_annotated_frame_path",
                    ])
                writer.writerow([
                    int(index),
                    ...
                    action_name,
                    str(out_path),
                    angle_info.get("angle_deg") if isinstance(angle_info, dict) else "",
                    angle_info.get("source") if isinstance(angle_info, dict) else "",
                    ac_distance_px if ac_distance_px is not None else "",
                    self.context.get("last_rule_ab_annotated_frame_path", ""),
                ])
            self.context["last_angle_results_image_path"] = str(out_path)
            self.context["last_angle_results_csv_path"] = str(csv_path)
            return str(out_path)
        except Exception as e:
            self.log(f"[角度检测] 同步保存 RuleAB 标注图到 angle_results 失败：{e}")
            return ""

    def _save_rule_ab_annotated_frame_custom(
        self,
        ...
        action_name: str = "",
        angle_info: Optional[Dict[str, Any]] = None,
        ab_guard_info: Optional[Dict[str, Any]] = None,
    ) -> str:
        """
        自己保存 RuleAB annotated_frames。

        目的：当前 Step7 自定义三线安全带路线绕过了外部 follower.run_one_cycle()，
        外部模块不一定会再生成 annotated_frames。这里强制保存 A/B/C 分割叠加、
        B 边角度、A-C 距离和 A/B mask guard 信息，恢复旧版 annotated_frames 的可追溯性。
        """
        try:
            ...
            out_base = None
            try:
                follower = self.rule_ab_follower
                fc = getattr(follower, "cfg", None) if follower is not None else None
                od = getattr(fc, "output_dir", None)
                if od:
                    out_base = Path(str(od))
            except Exception:
                out_base = None
            if out_base is None:
                if self.run_session_dir is not None:
                    out_base = Path(self.run_session_dir) / "rule_ab_from_measurement"
                else:
                    out_base = self.output_root / "rule_ab_from_measurement" / datetime.now().strftime("run_%Y%m%d_%H%M%S")
            out_dir = Path(out_base) / "annotated_frames"
            out_dir.mkdir(parents=True, exist_ok=True)

            img = cv2.cvtColor(np.asarray(image_rgb).copy(), cv2.COLOR_RGB2BGR)
            overlay = img.copy()
            a_obj = self._scene_get_obj(scene, "a")
            b_obj = self._scene_get_obj(scene, "b")
            c_obj = self._scene_get_obj(scene, "c")
            ...
            if c_mask is not None:
                overlay[np.asarray(c_mask).astype(bool)] = (255, 255, 0)
            if b_mask is not None:
                overlay[np.asarray(b_mask).astype(bool)] = (0, 128, 255)
            if a_mask is not None:
                overlay[np.asarray(a_mask).astype(bool)] = (0, 255, 255)
            img = cv2.addWeighted(overlay, 0.35, img, 0.65, 0)

            a_center = self._get_scene_center_xy_by_name(scene, "a")
            b_center = self._get_scene_center_xy_by_name(scene, "b")
            c_center = self._get_scene_center_xy_by_name(scene, "c")
            if a_center is not None:
                ax, ay = a_center
                cv2.circle(img, (int(round(ax)), int(round(ay))), 6, (0, 255, 255), -1)
                cv2.putText(img, "A", (int(ax)+8, int(ay)-8), cv2.FONT_HERSHEY_SIMPLEX, 0.65, (0,255,255), 2, cv2.LINE_AA)
            if b_center is not None:
                bx, by = b_center
                cv2.circle(img, (int(round(bx)), int(round(by))), 6, (0, 128, 255), -1)
                cv2.putText(img, "B", (int(bx)+8, int(by)-8), cv2.FONT_HERSHEY_SIMPLEX, 0.65, (0,128,255), 2, cv2.LINE_AA)
            if c_center is not None:
                cx, cy = c_center
                cv2.circle(img, (int(round(cx)), int(round(cy))), 6, (255, 255, 0), -1)
                cv2.putText(img, "C", (int(cx)+8, int(cy)-8), cv2.FONT_HERSHEY_SIMPLEX, 0.65, (255,255,0), 2, cv2.LINE_AA)

            ac_dist = None
            if isinstance(status, dict):
                for key in ("c_clearance", "ac_clearance", "a_c_distance", "ac_dist"):
                    if status.get(key) is not None:
                        try:
                            ac_dist = float(status.get(key))
                            break
                        except Exception:
                            pass
            if ac_dist is None and a_center is not None:
                q, d = self._nearest_point_on_mask_contour(a_center, c_mask)
                ac_dist = d
                if q is not None:
                    cv2.line(img, (int(round(a_center[0])), int(round(a_center[1]))), (int(round(q[0])), int(round(q[1]))), (0, 255, 0), 2)
                    cv2.circle(img, (int(round(q[0])), int(round(q[1]))), 4, (0, 255, 0), -1)

            if angle_info:
                try:
                    p1 = angle_info.get("p1") or angle_info.get("selected_edge", {}).get("p1")
                    p2 = angle_info.get("p2") or angle_info.get("selected_edge", {}).get("p2")
                    if p1 is not None and p2 is not None:
                        cv2.line(img, (int(round(float(p1[0]))), int(round(float(p1[1])))), (int(round(float(p2[0]))), int(round(float(p2[1])))), (0, 0, 255), 3)
                except Exception:
                    pass

            lines = [
                f"RuleAB annotated: {action_name}",
            ]
            if angle_info and angle_info.get("angle_deg") is not None:
                lines.append(f"B edge angle={float(angle_info.get('angle_deg')):.3f} deg, source={angle_info.get('source','bmask')}")
            if ac_dist is not None:
                lines.append(f"A-C distance={float(ac_dist):.2f} px")
            if ab_guard_info:
                lines.append(f"AB guard: {ab_guard_info.get('reason','')}, A_exclude_B={bool(ab_guard_info.get('a_b_excluded', False))}")
            if isinstance(status, dict) and status:
                if status.get("ac_status") is not None:
                    lines.append(f"AC status={status.get('ac_status')}")
                if status.get("b_edge_angle_deg") is not None:
                    lines.append(f"status B angle={status.get('b_edge_angle_deg')}")

            y0 = 24
            for i, line in enumerate(lines[:8]):
                cv2.putText(img, str(line), (12, y0 + i * 24), cv2.FONT_HERSHEY_SIMPLEX, 0.62, (255, 255, 255), 3, cv2.LINE_AA)
                cv2.putText(img, str(line), (12, y0 + i * 24), cv2.FONT_HERSHEY_SIMPLEX, 0.62, (0, 0, 0), 1, cv2.LINE_AA)

            idx = int(self.context.get("rule_ab_custom_annotated_index", 0) or 0) + 1
            self.context["rule_ab_custom_annotated_index"] = idx
            out_path = out_dir / f"frame_{idx:06d}_{datetime.now().strftime('%Y%m%d_%H%M%S_%f')}_{action_name or 'ruleab'}.png"
            cv2.imwrite(str(out_path), img)
            self.context["last_rule_ab_annotated_frame_path"] = str(out_path)

            # 同步保存到 outputs/captured_frames/.../angle_results。
            # 因为当前 Step1/Step7 的角度来自 RuleAB/SAM2 B-mask，
            # 不再调用独立 ScreenAngleDetector，所以必须在这里主动保存。
            mirrored_angle_path = self._mirror_rule_ab_annotated_to_angle_results(
                annotated_bgr=img,
                action_name=action_name,
                index=idx,
                angle_info=angle_info,
                ac_distance_px=ac_dist,
            )
            if mirrored_angle_path:
                self.context["last_rule_ab_angle_results_image_path"] = mirrored_angle_path

            # 同步保存 Step1/Step7 每次角度检测和 A-C 距离的数值索引，
            # 便于从 annotated_frames 图片反查对应数据。
            try:
                csv_path = out_dir.parent / "angle_ac_measurements.csv"
                file_exists = csv_path.exists()
                with csv_path.open("a", encoding="utf-8-sig", newline="") as f:
                    writer = csv.writer(f)
                    if not file_exists:
                        writer.writerow([
                            "index", "time", "action_name", "annotated_frame_path",
                            "angle_deg", "angle_source", "ac_distance_px",
                            "a_center_x", "a_center_y", "b_center_x", "b_center_y", "c_center_x", "c_center_y",
                            "a_mask_area_px", "b_mask_area_px", "c_mask_area_px",
                        ])
                    def _area(m):
                        try:
                            return int(np.count_nonzero(np.asarray(m).astype(bool))) if m is not None else ""
                        except Exception:
                            return ""
                    writer.writerow([
                        idx,
                        ...
                        angle_info.get("angle_deg") if isinstance(angle_info, dict) else "",
                        angle_info.get("source") if isinstance(angle_info, dict) else "",
                        ac_dist if ac_dist is not None else "",
                        a_center[0] if a_center is not None else "",
                        a_center[1] if a_center is not None else "",
                        b_center[0] if b_center is not None else "",
                        b_center[1] if b_center is not None else "",
                        c_center[0] if c_center is not None else "",
                        c_center[1] if c_center is not None else "",
                        _area(a_mask),
                        _area(b_mask),
                        _area(c_mask),
                    ])
                self.context["last_rule_ab_angle_ac_csv_path"] = str(csv_path)
            except Exception as e:
                self.log(f"[RuleAB] 保存 angle_ac_measurements.csv 失败：{e}")

            return str(out_path)
        except Exception as e:
            try:
                self.log(f"[RuleAB] 保存 annotated_frames 自定义标注图失败：{e}")
            except Exception:
                pass
            return ""

    def measure_rule_ab_bmask_angle_once(
        self,
        label: str,
        ...
        """
        用 RuleAB 的 A/B/C 场景计算一次 B 边角度。

        关键点：
            - 不再依赖独立 ScreenAngleDetector 的背景边检测；
            - 先执行 A/B mask guard：优先保护 B，再从 A 中排除 B；
            - 用受保护后的 B mask 计算候选边角度；
            - annotated_frames 中保存 A/B/C 分割、B角度、A-C距离。
        """
        try:
            if follower is None:
                follower = self.ensure_rule_ab_follower()
            if follower is None:
                raise RuntimeError("RuleAB follower 不可用")

            image_rgb = None
            scene = None
            if refresh_scene and hasattr(follower, "capture_and_build_scene"):
                ...
                for attr in ("last_scene", "current_scene", "scene", "last_valid_scene", "latest_scene", "_last_scene"):
                    scene = getattr(follower, attr, None)
                    if scene is not None:
                        break
                if scene is None and hasattr(follower, "capture_and_build_scene"):
                    image_rgb, scene = follower.capture_and_build_scene()

            if scene is None:
                raise RuntimeError("没有获得 RuleAB scene")

            ab_guard_info = self._postprocess_scene_ab_masks(follower=follower, scene=scene, tag=f"angle_{label}")

            b_mask = self._extract_mask_from_scene_object(self._scene_get_obj(scene, "b"))
            candidates = self._bmask_edge_angle_candidates_from_mask(b_mask)
            a_center_for_edge = self._get_scene_center_xy_by_name(scene, "a")
            selected = self._select_bmask_angle_candidate(
                candidates,
                previous_angle=previous_angle,
                a_center_xy=a_center_for_edge,
            )
            if selected is None:
                raise RuntimeError("当前受约束 B mask 没有可用候选边")
            angle = float(selected.get("angle_deg"))
            ...
            angle_info = dict(selected)
            angle_info.update({
                "angle_deg": angle,
                "source": "rule_ab_guarded_bmask_nearest_to_a_center" if a_center_for_edge is not None else "rule_ab_guarded_bmask",
                "label": label,
                "candidate_count": len(candidates),
                "candidate_angles": [float(c.get("angle_deg", 0.0)) for c in candidates],
                "a_center_xy_for_edge_selection": [float(a_center_for_edge[0]), float(a_center_for_edge[1])] if a_center_for_edge is not None else None,
            })

            # 同步计算并保存 A-C 距离，保证 Step1 / Step7 每次角度检测都有可追溯数据。
            ac_distance_px = None
            ac_nearest_point_xy = None
            try:
                a_center_for_ac = self._get_scene_center_xy_by_name(scene, "a")
                c_mask_for_ac = self._extract_mask_from_scene_object(self._scene_get_obj(scene, "c"))
                if isinstance(status, dict):
                    for key in ("c_clearance", "ac_clearance", "a_c_distance", "ac_dist"):
                        if status.get(key) is not None:
                            ac_distance_px = float(status.get(key))
                            break
                if ac_distance_px is None and a_center_for_ac is not None:
                    q_ac, d_ac = self._nearest_point_on_mask_contour(a_center_for_ac, c_mask_for_ac)
                    ac_distance_px = d_ac
                    if q_ac is not None:
                        ac_nearest_point_xy = [float(q_ac[0]), float(q_ac[1])]
            except Exception as e:
                self.log(f"[RuleAB角度-{label}] A-C距离计算失败：{e}")

            angle_info["ac_distance_px"] = ac_distance_px
            angle_info["ac_nearest_point_xy"] = ac_nearest_point_xy

            annotated_path = ""
            if save_annotated and image_rgb is not None:
                annotated_path = self._save_rule_ab_annotated_frame_custom(
                    image_rgb=image_rgb,
                    scene=scene,
                    status=status,
                    action_name=f"ANGLE_{label}",
                    angle_info=angle_info,
                    ab_guard_info=ab_guard_info,
                )
            ...
            return {
                ...
                "label": label,
                "angle_deg": angle,
                "source": "rule_ab_guarded_bmask",
                "selected_edge": self._json_safe(selected),
                "candidate_count": len(candidates),
                "candidate_angles": [float(c.get("angle_deg", 0.0)) for c in candidates],
                "ab_mask_guard": self._json_safe(ab_guard_info),
                "annotated_frame_path": annotated_path,
                "ac_distance_px": ac_distance_px,
                "ac_nearest_point_xy": ac_nearest_point_xy,
                "rule_ab_status": self._json_safe(status),
            }
        except Exception as e:
            self.log(f"[RuleAB角度-{label}] 使用 A/B 约束后的 B mask 计算角度失败：{e}")
            if allow_fail:
                return {"ok": False, "label": label, "angle_deg": None, "reason": str(e), "source": "rule_ab_guarded_bmask_failed"}
            raise
    ...
    def run_rule_ab_until_angle_delta(
        ...
        initial_angle_result: Optional[Dict[str, Any]] = None,
    ) -> Dict[str, Any]:
        ...
        # Step1 已经用 A/B mask guard 得到了基准角度、B mask 约束结果和 annotated frame。
        # 这里把 Step1 的结果作为 Step7 的第 0 帧实时输入：
        #   - baseline_angle 继续用于后续 delta 计算；
        #   - initial_angle_result 作为 Step7 进入推动前的当前帧角度结果；
        #   - 如果第 0 帧已经满足阈值，则不执行任何推动，直接退出 Step7，随后 Step8 关闭激光。
        pending_angle_result: Optional[Dict[str, Any]] = initial_angle_result if isinstance(initial_angle_result, dict) else None
        ...
        while True:
            ...
            # ------------------------------------------------------------
            # 先检查“当前帧”的 B 角度，再决定是否推动。
            # 第一次循环直接使用 Step1 传进来的 initial_angle_result；
            # 后续循环重新取当前帧，并使用同一套 A/B mask guard + B mask 候选边角度。
            # 这样 annotated_frames 中看到已经达到阈值的那一帧，会在执行下一次推动前立即触发退出。
            # ------------------------------------------------------------
            if pending_angle_result is not None:
                angle_result = pending_angle_result
                pending_angle_result = None
                angle_source_for_record = "step1_guarded_frame_passed_to_step7"
                self.log("[RuleAB] Step7 第0帧使用 Step1 已保存的 A/B约束B-mask角度结果作为当前帧实时停止判断输入。")
            else:
                angle_result = self.measure_rule_ab_bmask_angle_once(
                    label=f"rule_ab_edge_step_{step_idx}_before_move",
                    follower=follower,
                ...
                angle_source_for_record = "pre_move_guarded_bmask_frame"
            ...
            if angle_result.get("ok", False) and angle_result.get("angle_deg") is not None:
                ...
                final_angle = None
                final_delta = None
                repaired_info = {"enabled": bool(getattr(self.cfg, "rule_ab_enable_angle_repair", True)), "reason": "angle_detect_failed_before_move"}

            # 当前帧先判断阈值；达到阈值时，不再执行本轮推动动作。
            pre_move_stop = False
            if final_delta is not None:
                if min_delta <= float(final_delta) <= max_delta:
                    reached = True
                    allow_continue_measurement = True
                    stop_reason = "angle_delta_in_target_range_before_move"
                    pre_move_stop = True
                    self.log(
                        f"[RuleAB] 当前帧角度变化已进入目标范围：{final_delta:.3f}°，"
                        f"目标范围=[{min_delta:.3f}, {max_delta:.3f}]；不再执行本轮推动，立即退出 Step7 并进入 Step8 关闭激光。"
                    )
                elif float(final_delta) > max_delta:
                    reached = False
                    allow_continue_measurement = True
                    stop_reason = "angle_delta_above_max_before_move_close_laser_continue"
                    pre_move_stop = True
                    self.log(
                        f"[RuleAB] 当前帧角度变化 {final_delta:.3f}° 已超过上限 {max_delta:.3f}°；"
                        "不再执行本轮推动，立即退出 Step7 并进入 Step8 关闭激光，但不停止完整测量。"
                    )

            use_route = bool(getattr(self.cfg, "rule_ab_use_c_edge_route", True))
            route_info: Dict[str, Any] = {"route_enabled": use_route, "not_moved_reason": "pre_move_angle_check" if pre_move_stop else ""}
            ok = True
            ...
            if pre_move_stop:
                record = {
                    ...
                    "rule_ab_ok": True,
                    "moved": False,
                    "angle_check_timing": angle_source_for_record,
            ...
            if use_route:
                ...
                self.log(
                    f"[RuleAB] step={step_idx}: 当前帧未到阈值，执行一次 C边沿路线运动；"
                ...
                ok, route_info = self._run_one_c_edge_route_cycle(
                    follower,
                    step_idx,
                    route_points,
                    baseline_angle=baseline_angle,
                    min_delta_deg=min_delta,
                    max_delta_deg=max_delta,
                    previous_angle=last_valid_step_angle,
                )
                if bool(route_info.get("pre_move_stop", False)):
                    ri_angle = route_info.get("route_internal_angle_result") or {}
                    if isinstance(ri_angle, dict):
                        try:
                            final_angle = float(ri_angle.get("angle_deg"))
                            final_delta = float(ri_angle.get("delta_from_baseline")) if ri_angle.get("delta_from_baseline") is not None else final_delta
                            last_valid_step_angle = float(final_angle)
                        except Exception:
                            pass
                    reason0 = str(route_info.get("pre_move_stop_reason", "route_internal_pre_move_stop"))
                    if "above_max" in reason0:
                        reached = False
                        allow_continue_measurement = True
                        stop_reason = "angle_delta_above_max_route_internal_before_move_close_laser_continue"
                    else:
                        reached = True
                        allow_continue_measurement = True
                        stop_reason = "angle_delta_in_target_range_route_internal_before_move"
                    record = {
                        "step": step_idx,
                        "rule_ab_ok": ok,
                        "moved": False,
                        "angle_check_timing": "route_internal_before_execute_action",
                        "angle_deg_raw": raw_angle,
                        "angle_deg": final_angle,
                        "angle_delta_from_baseline": final_delta,
                        "angle_repair": self._json_safe(repaired_info),
                ...
                # ------------------------------------------------------------
                # 新增：C 路线运动执行完以后，立即再检测一次角度。
                # 关闭激光的 Step7 判据必须使用 post-move 角度：
                #   delta = angle_diff(post_move_final_angle, Step1 baseline_angle)
                # 只有 post-move 失败时，才保留 pre-move 角度作为记录/兜底；
                # 不再只依赖 pre_move_guarded_bmask_frame 判断是否关闭激光。
                # ------------------------------------------------------------
                if ok:
                    pre_move_angle_result_for_record = self._json_safe(angle_result)
                    pre_move_repair_for_record = self._json_safe(repaired_info)
                    pre_move_raw_angle_for_record = raw_angle
                    pre_move_final_angle_for_record = final_angle
                    pre_move_delta_for_record = final_delta

                    post_angle_result = self.measure_rule_ab_bmask_angle_once(
                        label=f"rule_ab_edge_step_{step_idx}_after_move",
                        follower=follower,
                        previous_angle=last_valid_step_angle,
                        refresh_scene=True,
                        allow_fail=True,
                    ...
                    post_raw_angle = None
                    post_final_angle = None
                    post_final_delta = None
                    post_repaired_info: Dict[str, Any] = {
                        "enabled": bool(getattr(self.cfg, "rule_ab_enable_angle_repair", True)),
                        "reason": "post_move_angle_not_checked",
                    }

                    if post_angle_result.get("ok", False) and post_angle_result.get("angle_deg") is not None:
                        post_raw_angle = float(post_angle_result["angle_deg"])
                        post_final_angle, post_repaired_info = self._repair_rule_ab_angle_if_jump(
                            raw_angle=post_raw_angle,
                            previous_angle=last_valid_step_angle,
                            baseline_angle=baseline_angle,
                            follower=follower,
                            angle_result=post_angle_result,
                        )
                        if post_final_angle is None:
                            post_final_angle = post_raw_angle
                        if baseline_angle is not None:
                            post_final_delta = self.angle_diff_deg(float(post_final_angle), float(baseline_angle))
                        last_valid_step_angle = float(post_final_angle)

                        raw_angle = post_raw_angle
                        final_angle = post_final_angle
                        final_delta = post_final_delta
                        repaired_info = post_repaired_info
                        angle_result = post_angle_result
                        angle_source_for_record = "post_move_guarded_bmask_frame"

                        self.log(
                            f"[RuleAB] step={step_idx}: 路线运动后立即角度检测完成，"
                            f"post_raw_angle={post_raw_angle}, post_final_angle={post_final_angle}, "
                            f"post_delta_from_baseline={post_final_delta}, target=[{min_delta}, {max_delta}], "
                            f"repair_used={bool(post_repaired_info.get('used_repaired_angle', False))}"
                        )
                    else:
                        post_repaired_info = {
                            "enabled": bool(getattr(self.cfg, "rule_ab_enable_angle_repair", True)),
                            "reason": "post_move_angle_detect_failed",
                        }
                        self.log(
                            f"[RuleAB] step={step_idx}: 路线运动后立即角度检测失败，"
                            "本步暂用 pre-move 角度记录，下一轮会继续检测。"
                        )

                    if isinstance(route_info, dict):
                        route_info["post_move_angle_checked"] = True
                        route_info["pre_move_angle_result_for_outer_record"] = pre_move_angle_result_for_record
                        route_info["pre_move_angle_repair_for_outer_record"] = pre_move_repair_for_record
                        route_info["pre_move_angle_deg_raw_for_outer_record"] = pre_move_raw_angle_for_record
                        route_info["pre_move_angle_deg_for_outer_record"] = pre_move_final_angle_for_record
                        route_info["pre_move_delta_from_baseline_for_outer_record"] = pre_move_delta_for_record
                        route_info["post_move_angle_result"] = self._json_safe(post_angle_result)
                        route_info["post_move_angle_repair"] = self._json_safe(post_repaired_info)
                        route_info["post_move_angle_deg_raw"] = post_raw_angle
                        route_info["post_move_angle_deg"] = post_final_angle
                        route_info["post_move_delta_from_baseline"] = post_final_delta

                if not ok:
                    self.log(f"[RuleAB-Route] 路线运动失败：{route_info}")
            else:
                self.log(
                    f"[RuleAB] step={step_idx}: 当前帧未到阈值，执行一次 rule_ab.run_one_cycle()；"
            ...
            self.log(
                f"[RuleAB] step={step_idx}, timing={angle_source_for_record}, raw_angle={raw_angle}, final_angle={final_angle}, "
        ...
        result = {
            ...
            "used_step1_frame_as_step7_initial_input": isinstance(initial_angle_result, dict),
            "records": records,
        }
    ...
    def _build_and_save_quad_edge_route(self, c_path: Path) -> List[Tuple[float, float]]:
        """从 static_c_quad_points.json 或 static_c_mask.png 四边形兜底生成 Step7 三线安全带路线。

        新增圆形 C mask 处理：
            如果 static_c_sam2_mask.png 或 static_c_mask.png 被判定为接近圆形，
            则强制使用 cv2.minAreaRect(contour) 得到圆的外接矩形/四边形，
            再按三线安全带生成路线。这样圆形 C 不会被 approxPolyDP 拟合成不稳定四边形。
        """
        c_path = Path(c_path)
        quad_path = c_path / "static_c_quad_points.json"

        # 圆形 C 优先用原始 SAM2 mask 检测；没有原始 mask 时再用 static_c_mask.png。
        # 即使旧目录里已经有 static_c_quad_points.json，只要检测到原始 C 是圆形，也覆盖为外接矩形。
        try:
            circle_mask_path = c_path / "static_c_sam2_mask.png"
            if not circle_mask_path.exists():
                circle_mask_path = c_path / "static_c_mask.png"
            if circle_mask_path.exists():
                m_circle = cv2.imread(str(circle_mask_path), cv2.IMREAD_GRAYSCALE)
                if m_circle is not None and int(np.count_nonzero(m_circle > 0)) > 0:
                    circle_info = MeasurementWorkflowGUI._detect_c_mask_circle_like(
                        m_circle > 0,
                        circularity_threshold=float(getattr(self.cfg, "rule_ab_c_circle_circularity_threshold", 0.78)),
                        min_aspect_ratio=float(getattr(self.cfg, "rule_ab_c_circle_min_aspect_ratio", 0.72)),
                        min_enclosing_fill_ratio=float(getattr(self.cfg, "rule_ab_c_circle_min_enclosing_fill_ratio", 0.58)),
                    )
                    if bool(getattr(self.cfg, "rule_ab_c_circle_as_rect_enable", True)) and bool(circle_info.get("is_circle_like", False)):
                        quad, _quad_mask, method = MeasurementWorkflowGUI._fit_quadrilateral_from_mask(m_circle > 0)
                        with quad_path.open("w", encoding="utf-8") as f:
                            json.dump({
                                "method": method,
                                "shape_type": "circle_external_rect",
                                "shape_detection": circle_info,
                                "source_mask": circle_mask_path.name,
                                "points_xy": [[float(x), float(y)] for x, y in quad.tolist()],
                            }, f, ensure_ascii=False, indent=2)
        except Exception as e:
            self.log(f"[RuleAB-Route] 圆形 C 检测/外接矩形生成失败，将继续使用普通四边形路线：{e}")
        ...
        try:
            ...
            route_shape_type = str(data.get("shape_type", "quadrilateral") or "quadrilateral")
            route_source_name = "circle_external_rect_quad_three_line_safety_band" if route_shape_type == "circle_external_rect" or "circle_like" in str(data.get("method", "")) else "quad_points_three_line_safety_band"
            ...
            with (c_path / "static_c_edge_route.json").open("w", encoding="utf-8") as f:
                json.dump({
                    ...
                    "note": "Step7 三线安全带：min/max 是边界线，target 是中间方向参考线；A 在 min-max 之间时只沿 target 切向运动，越界才法向修正。",
                }, f, ensure_ascii=False, indent=2)
            ...
            self.log(f"[RuleAB-Route] 从 C 四边形重建三线安全带路线失败：{e}")
            return []
    ...
    def _run_one_c_edge_route_cycle(
        ...
        # 关键修复：安全带外不允许因为“到 safe_target_xy 的误差小于 tol”而 STAY。
        #
        # 正确语义：
        #   1) min <= route_dist <= max：在安全带内，只沿 target 线局部切向方向运动；
        #   2) route_dist < min：太靠近 C，必须执行 SAFE_ 法向远离修正；
        #   3) route_dist > max：离 C 太远，必须执行 SAFE_ 法向靠近修正。
        #
        # 旧逻辑在 route_dist 仍小于 min 或大于 max 时，如果 safe_target_xy 与 A 中心
        # 的 dx/dy 小于 tol，会得到 SAFE_STAY，导致角度未达到 [min,max] 也不再推动。
        # 这里把“安全带外”的 STAY 强制改成法向修正动作。
        if use_safety_correction and action_id == 0:
            try:
                ux, uy = safety_info.get("away_unit_xy", [0.0, -1.0])
                ux = float(ux)
                uy = float(uy)
                ...
                safety_mode = str(safety_info.get("mode", ""))
                if safety_mode == "too_close_to_route_move_out":
                    # 太靠近 C：沿 away_unit 远离 C。
                    force_dx, force_dy = ux, uy
                    forced_reason = "force_safe_move_out_when_outside_min_distance"
                elif safety_mode == "too_far_from_route_move_in":
                    # 离 C 太远：沿 -away_unit 靠近 C。
                    force_dx, force_dy = -ux, -uy
                    forced_reason = "force_safe_move_in_when_outside_max_distance"
                else:
                    force_dx, force_dy = ux, uy
                    forced_reason = "force_safe_move_when_safety_correction_required"

                action_id = self._route_action_from_error(force_dx, force_dy, 0.0)
                virtual_safe_step_px = max(float(getattr(self.cfg, "rule_ab_c_edge_route_spacing_px", 12.0)), tol * 2.0, 10.0)
                cur_target = (
                    float(a_center[0]) + float(force_dx) * float(virtual_safe_step_px),
                    float(a_center[1]) + float(force_dy) * float(virtual_safe_step_px),
                )
                dx = float(cur_target[0] - a_center[0])
                dy = float(cur_target[1] - a_center[1])
                safety_info["forced_safe_action_when_outside_band"] = True
                safety_info["forced_safe_action_reason"] = forced_reason
                safety_info["forced_safe_direction_xy"] = [float(force_dx), float(force_dy)]
                safety_info["forced_safe_virtual_step_px"] = float(virtual_safe_step_px)
            except Exception as e:
                safety_info["forced_safe_action_when_outside_band"] = False
                safety_info["forced_safe_action_error"] = str(e)
        ...
        # 第二道保险：_run_one_c_edge_route_cycle 内部重新捕获了一帧。
        # 这帧就是 route annotated_frames/overlay 使用的同一帧。
        # 如果这帧已经达到角度阈值，则绝不执行 execute_action，直接返回给外层关闭激光。
        route_frame_angle_info: Dict[str, Any] = {}
        try:
            b_mask_now = self._extract_mask_from_scene_object(self._scene_get_obj(scene, "b"))
            cand_now = self._bmask_edge_angle_candidates_from_mask(b_mask_now)
            sel_now = self._select_bmask_angle_candidate(cand_now, previous_angle=previous_angle, a_center_xy=a_center)
            if sel_now is not None:
                ang_now = float(sel_now.get("angle_deg"))
                delta_now = self.angle_diff_deg(ang_now, float(baseline_angle)) if baseline_angle is not None else None
                route_frame_angle_info = dict(sel_now)
                route_frame_angle_info.update({
                    "ok": True,
                    "angle_deg": ang_now,
                    "delta_from_baseline": delta_now,
                    "source": "route_internal_pre_move_guarded_bmask_frame",
                    "candidate_count": len(cand_now),
                    "candidate_angles": [float(c.get("angle_deg", 0.0)) for c in cand_now],
                })
                if delta_now is not None and min_delta_deg is not None and max_delta_deg is not None:
                    if float(min_delta_deg) <= float(delta_now) <= float(max_delta_deg):
                        self.log(
                            f"[RuleAB-Route] 内部当前帧角度变化已进入目标范围：{delta_now:.3f}°，"
                            f"目标范围=[{float(min_delta_deg):.3f}, {float(max_delta_deg):.3f}]；本帧不执行推动，返回外层关闭激光。"
                        )
                        try:
                            self._save_rule_ab_annotated_frame_custom(
                                image_rgb=image_rgb,
                                scene=scene,
                                status=status,
                                action_name=f"ROUTE_PRESTOP_{action_name}",
                                angle_info=route_frame_angle_info,
                                ab_guard_info=ab_guard_info,
                            )
                        except Exception as e:
                            self.log(f"[RuleAB-Route] 预停止帧标注保存失败：{e}")
                        return True, {
                            "route_enabled": True,
                            "step": int(step_idx),
                            "route_index": int(idx),
                            ...
                            "pre_move_stop": True,
                            "pre_move_stop_reason": "angle_delta_in_target_range_route_internal_before_execute_action",
                            "route_internal_angle_result": self._json_safe(route_frame_angle_info),
                            ...
                            "move_info": {"skipped": True, "reason": "angle_threshold_before_execute_action"},
                        }
                    if float(delta_now) > float(max_delta_deg):
                        self.log(
                            f"[RuleAB-Route] 内部当前帧角度变化 {delta_now:.3f}° 已超过上限 {float(max_delta_deg):.3f}°；"
                            "本帧不执行推动，返回外层关闭激光并继续完整测量。"
                        )
                        try:
                            self._save_rule_ab_annotated_frame_custom(
                                image_rgb=image_rgb,
                            ...
                            self.log(f"[RuleAB-Route] 超上限预停止帧标注保存失败：{e}")
                        return True, {
                            "route_enabled": True,
                            "step": int(step_idx),
                            "route_index": int(idx),
                            ...
                            "target_mode": target_mode,
                            "pre_move_stop": True,
                            "pre_move_stop_reason": "angle_delta_above_max_route_internal_before_execute_action",
                            "route_internal_angle_result": self._json_safe(route_frame_angle_info),
                            "route_safety": self._json_safe(safety_info),
                            "route_distance_px": safety_info.get("route_distance_px"),
                            "route_min_distance_px": safety_info.get("min_distance_px"),
                            ...
                            "ab_mask_guard": self._json_safe(ab_guard_info),
                            "status": self._json_safe(status),
                            "move_info": {"skipped": True, "reason": "angle_above_max_before_execute_action"},
                        }
        except Exception as e:
            route_frame_angle_info = {"ok": False, "reason": str(e), "source": "route_internal_pre_move_angle_failed"}
        ...
        # 无论外部 follower.save_annotated_image 是否可用，都强制写一份 annotated_frames，
        # 保证 measurement_output/rule_ab_from_measurement/run_xxx/annotated_frames 中持续显示 A/B/C、B角度、A-C距离。
        try:
            # 图像上写的角度必须使用前面 route_frame_angle_info 中已经用于
            # 阈值判断的同一个候选边，不能重新 previous_angle=None 再选一次，
            # 否则 annotated_frames 的角度会和保存/判断角度不一致。
            angle_info_for_overlay = dict(route_frame_angle_info) if isinstance(route_frame_angle_info, dict) else {}
            if not angle_info_for_overlay.get("ok", False) or angle_info_for_overlay.get("angle_deg") is None:
                b_mask_for_overlay = self._extract_mask_from_scene_object(self._scene_get_obj(scene, "b"))
                cand_for_overlay = self._bmask_edge_angle_candidates_from_mask(b_mask_for_overlay)
                sel_for_overlay = self._select_bmask_angle_candidate(cand_for_overlay, previous_angle=previous_angle, a_center_xy=a_center)
                angle_info_for_overlay = dict(sel_for_overlay) if sel_for_overlay is not None else {}
                if sel_for_overlay is not None:
                    angle_info_for_overlay.update({
                        "ok": True,
                        "angle_deg": float(sel_for_overlay.get("angle_deg")),
                        "source": "rule_ab_guarded_bmask_route_frame_fallback",
                        "candidate_count": len(cand_for_overlay),
                        "candidate_angles": [float(c.get("angle_deg", 0.0)) for c in cand_for_overlay],
                    })
            self._save_rule_ab_annotated_frame_custom(
                image_rgb=image_rgb,
                scene=scene,
                status=status,
                action_name=f"ROUTE_{action_name}",
                angle_info=angle_info_for_overlay,
                ab_guard_info=ab_guard_info,
            )
        except Exception as e:
            self.log(f"[RuleAB-Route] 自定义 annotated_frames 保存失败：{e}")
    ...
    def _postprocess_scene_ab_masks(self, follower: Any, scene: Any, tag: str = "") -> Dict[str, Any]:
        """
        对当前 scene 的 A/B mask 做运行时保护。

        本版的核心语义：
            1. B 优先保全：如果当前 B 相对参考/上一帧的面积、中心、主轴角度异常，则回退上一帧 B；
            2. 只从候选 B mask 中删除“明显属于 A 的 A-only 误识别区域”：
               - AB 真实重合区域仍然属于 B；
               - 判断依据不是简单 B = B - A，而是：
                 remove_from_B = A_mask & B_candidate & outside(B_reference_or_last_support)
               - 也就是说，只有落在 A 上、同时又不在上一帧/参考 B 支撑区内的那部分，才认为是 A 被误识别进了 B；
            3. B 清理完成后，再执行 A = A - dilate(B)，避免 A mask 把 B 包进去；
            4. 修正结果写回 scene.a / scene.b，后续 A 中心、路线控制和角度检测都使用修正后的 mask。
        """
        info: Dict[str, Any] = {
            "enabled": bool(getattr(self.cfg, "rule_ab_enable_ab_mask_guard", True)),
            "tag": tag,
            "b_guard_used_previous": False,
            "b_a_unique_excluded": False,
            "a_b_excluded": False,
            "reason": "not_checked",
        }
        if not info["enabled"]:
        ...
        ref_feat = guard.get("b_ref_feat")
        last_feat = guard.get("b_last_feat")
        last_mask = guard.get("b_last_mask")
        ref_mask = guard.get("b_ref_mask")

        # ----------------------------------------------------
        # 1) B 形状保护：当前 B 异常时回退上一帧 B。
        # ----------------------------------------------------
        if ref_feat is None:
            ref_feat = self._json_safe(b_feat)
            guard["b_ref_feat"] = ref_feat
            guard["b_last_feat"] = ref_feat
            guard["b_ref_mask"] = np.asarray(b_mask).astype(bool).copy()
            guard["b_last_mask"] = np.asarray(b_mask).astype(bool).copy()
            info.update({"reason": "b_reference_initialized", "b_current_feat": self._json_safe(b_feat)})
        else:
            area = float(b_feat.get("area", 0.0))
            ref_area = max(1.0, float(ref_feat.get("area", area)))
            area_ratio = area / ref_area
            cx, cy = b_feat.get("center", (0.0, 0.0))
            if last_feat is not None and last_feat.get("center") is not None:
                lcx, lcy = last_feat.get("center")
            else:
                lcx, lcy = ref_feat.get("center", (cx, cy))
            center_jump = math.hypot(float(cx) - float(lcx), float(cy) - float(lcy))
            angle_ref = float(last_feat.get("angle_deg", b_feat.get("angle_deg", 0.0))) if last_feat else float(ref_feat.get("angle_deg", 0.0))
            angle_jump = self.angle_diff_deg(float(b_feat.get("angle_deg", 0.0)), angle_ref)

            min_ratio = float(getattr(self.cfg, "rule_ab_b_area_ratio_min", 0.45))
            max_ratio = float(getattr(self.cfg, "rule_ab_b_area_ratio_max", 2.20))
            max_center_jump = float(getattr(self.cfg, "rule_ab_b_center_jump_max_px", 80.0))
            max_angle_jump = float(getattr(self.cfg, "rule_ab_b_angle_jump_max_deg", 25.0))
            bad_reasons = []
            if area_ratio < min_ratio or area_ratio > max_ratio:
                bad_reasons.append(f"area_ratio={area_ratio:.3f} not in [{min_ratio:.3f},{max_ratio:.3f}]")
            if center_jump > max_center_jump:
                bad_reasons.append(f"center_jump={center_jump:.2f}>{max_center_jump:.2f}")
            if angle_jump > max_angle_jump:
                bad_reasons.append(f"angle_jump={angle_jump:.2f}>{max_angle_jump:.2f}")

            if bad_reasons and last_mask is not None:
                b_mask = np.asarray(last_mask).astype(bool).copy()
                b_feat = self._mask_area_center_angle(b_mask) or b_feat
                self._set_mask_to_scene_object(b_obj, b_mask)
                info["b_guard_used_previous"] = True
                info["reason"] = "b_replaced_by_previous_due_to_" + ";".join(bad_reasons)
                self.log(f"[RuleAB-ABMaskGuard] {tag} B异常，已回退上一帧B：" + "; ".join(bad_reasons))
            else:
                info["reason"] = "b_accepted" if not bad_reasons else "b_bad_but_no_previous_available"

            info.update({
                "b_area_ratio": float(area_ratio),
                "b_center_jump_px": float(center_jump),
                "b_angle_jump_deg": float(angle_jump),
                "b_current_feat": self._json_safe(b_feat),
                "b_ref_feat": self._json_safe(ref_feat),
            })

        # ----------------------------------------------------
        # 2) B 候选 mask 清理：只删除 A-only 误识别区域，保留真实 AB 重合区给 B。
        #
        # 不能写成 b_clean = b_bool & (~a_bool)，因为这会把真实 AB 重合区域也从 B 里裁掉。
        # 这里使用“B历史/参考支撑区”区分：
        #   - 在 A 上、也在 B 上、并且仍处于上一帧/参考 B 附近：认为是真实 AB 重合，保留给 B；
        #   - 在 A 上、也被候选 B 识别到、但不在上一帧/参考 B 支撑区内：认为是 A-only 污染，才从 B 中删除。
        # ----------------------------------------------------
        if bool(getattr(self.cfg, "rule_ab_b_exclude_a_unique_enable", True)) and a_mask is not None and b_mask is not None:
            ...
            if last_mask is not None:
                try:
                    support_mask = np.asarray(last_mask).astype(bool)
                    support_source = "last_b_mask"
                except Exception:
                    support_mask = None
            if support_mask is None and ref_mask is not None:
                try:
                    support_mask = np.asarray(ref_mask).astype(bool)
                    support_source = "ref_b_mask"
                except Exception:
                    support_mask = None

            # 第一帧还没有历史支撑时，不做 B = B - A，避免误裁真实重合区。
            if support_mask is not None and support_mask.shape == b_bool.shape:
                support_dilate_px = max(0, int(getattr(self.cfg, "rule_ab_b_exclude_a_support_dilate_px", 10)))
                if support_dilate_px > 0:
                    k = 2 * support_dilate_px + 1
                    kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (k, k))
                    support_mask_d = cv2.dilate(support_mask.astype(np.uint8), kernel, iterations=1).astype(bool)
                else:
                    support_mask_d = support_mask

                ab_overlap_candidate = a_bool & b_bool
                a_unique_wrong_in_b = ab_overlap_candidate & (~support_mask_d)
                b_clean = b_bool & (~a_unique_wrong_in_b)

                area_before_b = int(np.count_nonzero(b_bool))
                area_after_b = int(np.count_nonzero(b_clean))
                removed_px = int(np.count_nonzero(a_unique_wrong_in_b))
                kept_ab_px = int(np.count_nonzero(ab_overlap_candidate & support_mask_d))
                min_b_area = int(getattr(self.cfg, "rule_ab_b_min_area_after_a_unique_exclude_px", 10))

                info.update({
                    "b_a_unique_support_source": support_source,
                    "b_a_unique_support_dilate_px": int(support_dilate_px),
                    "b_area_before_a_unique_exclude_px": area_before_b,
                    "b_area_after_a_unique_exclude_px": area_after_b,
                    "b_a_unique_removed_px": removed_px,
                    "b_ab_overlap_preserved_px": kept_ab_px,
                })

                if removed_px > 0 and area_after_b >= max(1, min_b_area):
                    # 轻度形态学修复，避免删掉 A-only 小污染后留下毛刺/孔洞。
                    try:
                        kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (3, 3))
                        b_clean_u8 = (b_clean.astype(np.uint8) * 255)
                        b_clean_u8 = cv2.morphologyEx(b_clean_u8, cv2.MORPH_CLOSE, kernel, iterations=1)
                        b_clean = b_clean_u8 > 0
                    except Exception:
                        pass
                    ...
                    b_feat = self._mask_area_center_angle(b_mask) or b_feat
                    self._set_mask_to_scene_object(b_obj, b_mask)
                    info["b_a_unique_excluded"] = True
                    self.log(
                        f"[RuleAB-ABMaskGuard] {tag} 已从候选B中删除 A-only 误识别区域："
                        f"removed={removed_px}px；保留真实AB重合={kept_ab_px}px；support={support_source}"
                    )
                elif removed_px > 0:
                    info["b_a_unique_exclude_reason"] = f"b_clean_area_too_small<{min_b_area}"
            else:
                info["b_a_unique_exclude_reason"] = "no_valid_b_support_mask_do_not_cut_B_by_A"

        # B 最终版本写入历史缓存。注意：这里必须在 B 的 A-only 清理之后更新，
        # 这样下一帧的 support_mask 才是已经保留真实 AB 重合、并删除 A-only 污染后的 B。
        if b_mask is not None:
            final_b_feat = self._mask_area_center_angle(b_mask)
            if final_b_feat is not None:
                guard["b_last_mask"] = np.asarray(b_mask).astype(bool).copy()
                guard["b_last_feat"] = self._json_safe(final_b_feat)
                if guard.get("b_ref_mask") is None:
                    guard["b_ref_mask"] = np.asarray(b_mask).astype(bool).copy()
                if guard.get("b_ref_feat") is None:
                    guard["b_ref_feat"] = self._json_safe(final_b_feat)
                info["b_final_feat"] = self._json_safe(final_b_feat)

        # ----------------------------------------------------
        # 3) A 排除 B：B 已经保全；从 A 中扣掉 B，避免 A mask 包含 B。
        #    这一步不会改变 B。真实 AB 重合区已经保留在 B 中。
        # ----------------------------------------------------
        if a_mask is not None and b_mask is not None:
            a_bool = np.asarray(a_mask).astype(bool)
            b_bool = np.asarray(b_mask).astype(bool)
            dilate_px = max(0, int(getattr(self.cfg, "rule_ab_a_exclude_b_dilate_px", 3)))
            if dilate_px > 0:
                k = 2 * dilate_px + 1
                kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (k, k))
    ...
    def request_labview_spectrum(self, cycle_index: int) -> Dict[str, Any]:
        """
        已按本次需求停用完整测量中的光谱仪 TCP 采集。

        不再向 LabVIEW 发送 MEASURE，不再读取/解析/保存真实光谱数组。
        为保证后续保存、Excel 汇总、Δw 判断和 GUI 绘图不报空值错误，
        本轮峰值统一写入占位值 1000。
        """
        self.log("========== Step 4：光谱仪 TCP 采集已停用，使用峰值占位值 1000 ==========")

        result = {
            "ok": True,
            "disabled": True,
            "reason": "spectrometer_tcp_disabled",
        ...
        self.context["raw_original_peak"] = 1000.0
        self.context["raw_filtered_peak"] = 1000.0
        self.context["raw_median_peak"] = 1000.0
        ...
        self.context["raw_peak"] = 1000.0
        self.context["fit_peak"] = 1000.0
        self.context["fit_values"] = []
        self.context["fit_params"] = {"disabled": True, "placeholder_peak": 1000.0}
        self.context["spectrometer_tcp_disabled"] = True

        self.log("[光谱] 已跳过 LabVIEW/TCP 采集；raw_original_peak/raw_peak/fit_peak 均保存为 1000。")
        self.notify_update()
        return result
    ...
    def _make_save_context_snapshot(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "raw_original_peak": 1000.0 if ctx.get("spectrometer_tcp_disabled") else ctx.get("raw_original_peak"),
            "raw_filtered_peak": 1000.0 if ctx.get("spectrometer_tcp_disabled") else ctx.get("raw_filtered_peak"),
            "raw_median_peak": 1000.0 if ctx.get("spectrometer_tcp_disabled") else ctx.get("raw_median_peak"),
            "raw_peak": 1000.0 if ctx.get("spectrometer_tcp_disabled") else ctx.get("raw_peak"),
            "fit_peak": 1000.0 if ctx.get("spectrometer_tcp_disabled") else ctx.get("fit_peak"),
            "fit_params": self._json_safe(ctx.get("fit_params")),
            "spectrometer_tcp_disabled": bool(ctx.get("spectrometer_tcp_disabled", False)),
    ...
    def save_cycle_result(
        ...
        with self.save_io_lock:
            ...
            # 光谱仪 TCP 已按需求停用：不再保存真实光谱 CSV。
            # 这里只在仍然手动传入了光谱数组时才兼容写 CSV；完整测量默认数组为空，峰值在汇总表中保存为 1000。
    ...
    def run_one_cycle(self, cycle_index: int) -> bool:
        ...
        Step 4：光谱仪 TCP 采集已停用，峰值占位 1000
        Step 5：照明光 ON
        Step 5.5：保存本轮角度与光谱数据（异步线程，提前到激光打开前）
        Step 6：打开激光
        ...
        try:
            ...
            self.log("========== Step 1：用 A/B 约束后的 RuleAB-B mask 检测 current_angle ==========")
            # Step1 基准角度必须来自受 A/B mask guard 约束后的 B mask，
            # 并写入 rule_ab_from_measurement/.../annotated_frames，避免独立角度检测把背景边当成 B 边。
            try:
                follower_for_step1 = self.ensure_rule_ab_follower()
                self.apply_runtime_rule_ab_params_to_follower(follower_for_step1, reason="step1_angle_guarded_bmask")
                angle_result = self.measure_rule_ab_bmask_angle_once(
                    label="current_angle",
                    follower=follower_for_step1,
                    previous_angle=None,
                    refresh_scene=True,
                    allow_fail=True,
                    save_annotated=True,
                ...
                self.log(f"[Step1] A/B约束B-mask角度检测失败，回退独立角度检测：{e}")
            ...
            if isinstance(angle_result, dict):
                self.log(
                    "[Step1角度保存] "
                    f"rule_ab_annotated={angle_result.get('annotated_frame_path', '')}; "
                    f"angle_results={self.context.get('last_rule_ab_angle_results_image_path', '')}; "
                    f"angle={current_angle}"
                )
            # 把 Step1 的 A/B约束B-mask角度结果完整保留给 Step7。
            # Step7 会把它作为第0帧实时输入，先判断是否已经达到阈值，再决定是否推动。
            self.context["step1_guarded_angle_result_for_step7"] = copy.deepcopy(angle_result)
            ...
            if wait_after_off > 0:
                ...
                self.log("[照明光] OFF 后稳定等待：0 s，立即进入占位光谱峰值保存")

            if self.stop_requested:
                return False

            self.log("========== Step 4：光谱仪 TCP 采集已停用，峰值占位 1000 ==========")
            ...
            self.log("========== Step 5.5：保存数据（异步线程，提前到 Step6 打开激光前） ==========")
            ...
            self.log(f"[Step 5.5] 异步保存任务提交耗时：{time.time() - t_save_submit0:.3f} s")
            ...
            try:
                rule_ab_result = self.run_rule_ab_until_angle_delta(
                    ...
                    min_delta_deg=None,
                    max_delta_deg=None,
                    initial_angle_result=copy.deepcopy(self.context.get("step1_guarded_angle_result_for_step7")),
                )
            finally:
            ...
            self.log("========== Step 10：保存数据已在 Step5.5 提交，本步骤不重复保存 ==========")

            self.log(f"[流程] 第 {cycle_index} 轮保存任务已在 Step5.5 提交，可以进入下一轮")
            self.notify_update()
            return True
    ...
    def run(self):
        ...
        # 光谱仪/LabVIEW TCP 已按需求停用：完整测量不再要求 TCP Server 和 READY。
        self.context["spectrometer_tcp_disabled"] = True
        self.context["labview_ready"] = True
...
class MeasurementWorkflowGUI:
    ...
    def _build_ui(self):
        ...
        ttk.Label(rule_frame, text="AB覆盖阈值px²").grid(row=2, column=0, padx=4, pady=3, sticky="w")
        ttk.Entry(rule_frame, textvariable=self.rule_ab_ab_overlap_threshold_var, width=8).grid(
            row=2, column=1, padx=4, pady=3, sticky="ew"
        )

        # C 路径绕行方向：底层仍然使用原有 rule_ab_follow_c_direction 参数，
        # 这里只把原来的手动数字输入框改成单选按钮，避免把 1/-1 填反。
        # 约定：
        #   1  = 屏幕逆时针
        #   -1 = 屏幕顺时针
        follow_dir_frame = ttk.Frame(rule_frame)
        follow_dir_frame.grid(row=2, column=2, columnspan=2, padx=4, pady=3, sticky="w")

        ttk.Label(follow_dir_frame, text="C路径方向").pack(side=tk.LEFT, padx=(0, 8))
        ttk.Radiobutton(
            follow_dir_frame,
            text="逆时针",
            variable=self.rule_ab_follow_c_direction_var,
            value=1
        ...
        ttk.Label(follow_dir_frame, text="AB覆盖不再停；持续沿C绕行").pack(side=tk.LEFT, padx=(4, 0))
    ...
    @staticmethod
    def _detect_c_mask_circle_like(
        mask_bool: np.ndarray,
        circularity_threshold: float = 0.78,
        min_aspect_ratio: float = 0.72,
        min_enclosing_fill_ratio: float = 0.58,
    ) -> Dict[str, Any]:
        """
        判断 C mask 是否接近圆形/近圆形。

        判据同时使用：
            circularity = 4*pi*area/perimeter^2，越接近 1 越圆；
            minAreaRect 的短长轴比，避免细长椭圆误判；
            contour_area / minEnclosingCircle_area，避免不规则形状误判。

        返回 dict，便于写入 static_c_quad_points.json / static_c_meta.json 追溯。
        """
        info: Dict[str, Any] = {
            "ok": False,
        ...
        try:
            ...
            aspect = min(rw, rh) / max(rw, rh) if max(rw, rh) > 1e-6 else 0.0
            ...
            is_circle = (
                circularity >= float(circularity_threshold)
                and aspect >= float(min_aspect_ratio)
                and fill >= float(min_enclosing_fill_ratio)
            )
            info.update({
                "ok": True,
                "is_circle_like": bool(is_circle),
                "circularity": float(circularity),
                "aspect_ratio": float(aspect),
                "enclosing_fill_ratio": float(fill),
                "area_px": float(area),
                "perimeter_px": float(peri),
                "min_area_rect_size_wh": [float(rw), float(rh)],
                "min_enclosing_circle": [float(_cx), float(_cy), float(radius)],
                "thresholds": {
                    "circularity_threshold": float(circularity_threshold),
                    "min_aspect_ratio": float(min_aspect_ratio),
                    "min_enclosing_fill_ratio": float(min_enclosing_fill_ratio),
                },
                "reason": "circle_like" if is_circle else "not_circle_like",
            })
            return info
        except Exception as e:
    ...
    def _fit_quadrilateral_from_mask(mask_bool: np.ndarray) -> Tuple[np.ndarray, np.ndarray, str]:
        ...
        # 圆形/近圆形 C：直接用最小外接旋转矩形作为外接四边形路线。
        # 不使用 approxPolyDP，否则圆形轮廓可能被不同 epsilon 近似成随机四点，
        # 路径方向和角点都会不稳定。
        circle_info = MeasurementWorkflowGUI._detect_c_mask_circle_like(c_bool)
        if bool(circle_info.get("is_circle_like", False)):
            rect = cv2.minAreaRect(contour)
            quad_points = cv2.boxPoints(rect).astype(np.float32)
            method = (
                "circle_like_minAreaRect_external_quad"
                f"_circularity_{float(circle_info.get('circularity', 0.0)):.3f}"
                f"_aspect_{float(circle_info.get('aspect_ratio', 0.0)):.3f}"
                f"_fill_{float(circle_info.get('enclosing_fill_ratio', 0.0)):.3f}"
            )

        # 非圆形才从较小 epsilon 到较大 epsilon 搜索 4 个点的近似多边形。
    ...
    def _save_static_c_map_result(
        ...
        circle_info = self._detect_c_mask_circle_like(
            c_sam2_bool,
            circularity_threshold=float(_cfg("rule_ab_c_circle_circularity_threshold", 0.78)),
            min_aspect_ratio=float(_cfg("rule_ab_c_circle_min_aspect_ratio", 0.72)),
            min_enclosing_fill_ratio=float(_cfg("rule_ab_c_circle_min_enclosing_fill_ratio", 0.58)),
        )
        quad_points, c_quad_bool, quad_method = self._fit_quadrilateral_from_mask(c_sam2_bool)
        c_shape_type = "circle_external_rect" if bool(circle_info.get("is_circle_like", False)) or "circle_like" in str(quad_method) else "quadrilateral"
        ...
        if route_source in ("quad", "quadrilateral"):
            base_route_points = self._generate_c_edge_route_points_from_quad(
                quad_points, spacing_px=route_spacing_px, follow_direction=route_follow_direction, safe_clearance_px=0.0, image_shape=image_rgb.shape[:2]
            )
            min_route_points = self._generate_c_edge_route_points_from_quad(
                quad_points, spacing_px=route_spacing_px, follow_direction=route_follow_direction, safe_clearance_px=route_min_clearance_px, image_shape=image_rgb.shape[:2]
            )
            route_points = self._generate_c_edge_route_points_from_quad(
                quad_points, spacing_px=route_spacing_px, follow_direction=route_follow_direction, safe_clearance_px=route_safe_clearance_px, image_shape=image_rgb.shape[:2]
            )
            max_route_points = self._generate_c_edge_route_points_from_quad(
                quad_points, spacing_px=route_spacing_px, follow_direction=route_follow_direction, safe_clearance_px=route_max_clearance_px, image_shape=image_rgb.shape[:2]
            )
            outer_edge_points: List[Tuple[float, float]] = []
            route_payload_source = "circle_external_rect_quad_three_line_safety_band" if c_shape_type == "circle_external_rect" else "static_c_quad_points_three_line_safety_band"
        else:
        ...
        # overlay：黄色为 SAM2 原始 mask，绿色线为四边形/圆外接矩形。
        ...
        # quad_overlay：只显示最终会被加载使用的四边形 mask；若 C 为圆形，则该四边形是圆的外接矩形。
        ...
        meta = {
            ...
            "saved_static_c_mask_type": c_shape_type,
        ...
        self.log(
            "[RuleAB-C] C 已生成路径几何并保存："
            f"shape={c_shape_type}，"
            f"SAM2面积={int(c_sam2_bool.sum())} px，"
            f"几何mask面积={int(c_quad_bool.sum())} px，"
            f"method={quad_method}；已生成A路线点数={len(route_points)}；"
            "后续加载使用 static_c_mask.png=四边形/圆外接矩形mask，Step7 使用 static_c_edge_route.json（三线安全带）"
        )
    ...
    def get_rule_ab_follow_c_direction_from_ui(self) -> int:
        """
        从 GUI 读取 C 路径绕行方向。
        ...
        只做数值归一化，不改变现有路径生成和运动逻辑：
            非负数统一按 1 处理；
            负数统一按 -1 处理。
        """
        try:
            raw = int(self.rule_ab_follow_c_direction_var.get())
        except Exception:
            raw = 1
        ...
        try:
            self.rule_ab_follow_c_direction_var.set(direction)
        except Exception:
            pass
    ...
    def build_config_from_ui(self) -> MeasurementConfig:
        return MeasurementConfig(
            ...
            rule_ab_follow_c_direction=self.get_rule_ab_follow_c_direction_from_ui(),
    ...
    def calibrate_step9_target_color(self):
        ...
        try:
            ...
            self.set_var_blocking(self.rule_ac_color_h_var, int(hsv[0]))
            self.set_var_blocking(self.rule_ac_color_s_var, int(hsv[1]))
            self.set_var_blocking(self.rule_ac_color_v_var, int(hsv[2]))

            # 重新同步 GUI，确保本次新目标点和 HSV 都进入 cfg。
            wf = self.sync_config_from_ui_to_workflow()
            wf.cfg.rule_ac_target_x_px = target_x
            wf.cfg.rule_ac_target_y_px = target_y
            state = self._current_or_new_calibration_state()
    ...
    def run_workflow(self):
        ...
        try:
            ...
            # 2. 光谱仪/LabVIEW TCP 已按需求停用：不再检查 TCP Server / READY。
            wf.context["spectrometer_tcp_disabled"] = True
            wf.context["labview_ready"] = True
            self.log("[流程] 已跳过 TCP Server / LabVIEW READY 检查；本次完整测量峰值占位保存为 1000。")

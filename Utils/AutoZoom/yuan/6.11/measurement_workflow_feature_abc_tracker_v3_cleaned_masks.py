...
class MeasurementWorkflow:
        ...
        while True:
            ...
            if use_route:
                ...
                self.log(
                    ...
                    f"每帧都执行角度修复检查，|repaired-raw|>{getattr(self.cfg, 'rule_ab_angle_repair_raw_tolerance_deg', 2.0)}° 时使用修复角度"
                )
                try:
                ...
                if not ok:
                    ...
                    records.append({
                        ...
                        "angle_repair": {"reason": "skip_angle_after_route_ab_seg_fail"},
                ...
                try:
                    ...
                    self.log(f"[RuleAB] run_one_cycle/A-B分割异常：{e}；不停止完整测量，重新取下一帧继续。")
                    records.append({
                        "step": step_idx,
                        "rule_ab_ok": False,
                        "angle_deg_raw": None,
                        "angle_deg": None,
                        "angle_delta_from_baseline": None,
                        "angle_repair": {"reason": "skip_angle_after_run_one_cycle_exception"},
                        "angle_result": {"ok": False, "reason": "skip_angle_after_run_one_cycle_exception"},
                        "c_edge_route": self._json_safe(route_info),
                        "retry_next_frame": True,
                    })
                    stop_reason = "run_one_cycle_exception_retry_next_frame"
                    time.sleep(float(self.cfg.rule_ab_loop_interval_s))
                    continue
            ...
            # 关闭激光条件（按最新需求）：
            #   1) Step7 每推动一次后都检测 raw_angle；
            #   2) 每一次检测都执行 B mask 全边候选修复/确认；
            #   3) 若 |repaired_angle - raw_angle| <= 2°：检测角度可信，使用 raw_delta 判断；
            #   4) 若 |repaired_angle - raw_angle| > 2°：检测边异常，使用 repaired_delta 判断；
            #   5) 最终角度变化 >= 3.5° 即满足关闭激光条件；否则继续推动/重新取帧。
            close_min_delta = float(min_delta)
            close_target_max_delta = float(max_delta)
            repair_delta_tol = float(getattr(self.cfg, "rule_ab_angle_repair_raw_tolerance_deg", 2.0))
            wrong_edge_jump = float(getattr(self.cfg, "rule_ab_wrong_edge_delta_jump_deg", close_target_max_delta))
            ...
            repair_failed = bool(repaired_info.get("repair_failed", False)) if isinstance(repaired_info, dict) else False
            ...
            repaired_vs_raw_angle_diff = repaired_info.get("repaired_vs_raw_diff_deg") if isinstance(repaired_info, dict) else None

            close_delta_for_reason = None
            close_source = "none"
            close_hit = False
            close_band = "not_reached"
            repaired_vs_raw_delta_diff = None
            wrong_edge_continue = False

            if repair_failed:
                # 每帧都需要修复/确认；如果本帧无法得到修复候选，则不允许 raw_angle 直接触发关闭。
                close_delta_for_reason = None
                ...
                if repair_used:
                    # 保守关闭规则：
                    # raw 与 repaired 差异 >2° 时，只能说明“边选择存在冲突”。
                    # 不能因为 repaired_delta 落入 [3.5, 6) 就直接关闭激光。
                    raw_for_close = float(raw_delta) if raw_delta is not None else None
                    repaired_for_close = float(repaired_delta) if repaired_delta is not None else (float(final_delta) if final_delta is not None else None)

                    if raw_for_close is None:
                        close_delta_for_reason = repaired_for_close
                        close_source = "repair_conflict_no_raw_delta_no_close"
                        close_hit = False
                        close_band = "repair_conflict_missing_raw_delta_continue"
                        wrong_edge_continue = True
                    elif raw_for_close < close_min_delta:
                        close_delta_for_reason = raw_for_close
                        close_source = "raw_below_threshold_repair_conflict_no_close"
                        close_hit = False
                        close_band = "raw_delta_below_3p5_repair_conflict_continue"
                        wrong_edge_continue = True
                    elif repaired_for_close is None:
                        close_delta_for_reason = raw_for_close
                        close_source = "repair_conflict_missing_repaired_delta_no_close"
                        close_hit = False
                        close_band = "repair_conflict_missing_repaired_delta_continue"
                        wrong_edge_continue = True
                    else:
                        delta_diff_for_close = abs(raw_for_close - repaired_for_close)
                        agree_tol = float(getattr(self.cfg, "rule_ab_overshoot_repair_delta_tolerance_deg", 2.0))
                        if repaired_for_close >= close_min_delta and delta_diff_for_close <= agree_tol:
                            # 只有 raw_delta 自己达到阈值，并且 repaired_delta 支持同一趋势，才允许关闭激光。
                            close_delta_for_reason = raw_for_close
                            close_source = "raw_and_repaired_agree"
                            close_hit = True
                            if raw_for_close < close_target_max_delta:
                                close_band = "raw_repaired_agree_ge_3p5_lt_6_close"
                            else:
                                close_band = "raw_repaired_agree_ge_6_close"
                        else:
                            close_delta_for_reason = raw_for_close
                            close_source = "raw_repaired_conflict_no_close"
                            close_hit = False
                            close_band = "raw_repaired_delta_conflict_continue"
                            wrong_edge_continue = True
                else:
                    # raw 与 repaired 差异 <=2°，说明检测角度可信；用 raw/final 判断是否关激光。
                    if raw_delta is not None:
                        close_delta_for_reason = float(raw_delta)
                    elif final_delta is not None:
                        close_delta_for_reason = float(final_delta)
                    close_source = "raw_confirmed_by_repair"

                    close_hit = bool(
                        close_delta_for_reason is not None
                        and float(close_delta_for_reason) >= close_min_delta
                    )
                    if close_delta_for_reason is not None:
                        if float(close_delta_for_reason) < close_min_delta:
                            ...
                            close_band = "raw_delta_ge_3p5_lt_6_close"
                        else:
                            close_band = "raw_delta_ge_6_confirmed_close"

            try:
                if raw_delta is not None and repaired_delta is not None:
                    repaired_vs_raw_delta_diff = abs(float(repaired_delta) - float(raw_delta))
            except Exception:
                repaired_vs_raw_delta_diff = None
            ...
            try:
                ...
                record["close_rule"] = "conservative_repair_check__close_only_when_raw_delta_ge_3p5_and_repaired_agrees__raw_below_threshold_never_closed_by_repaired"
                record["repair_raw_tolerance_deg"] = repair_delta_tol
                record["wrong_edge_delta_jump_deg"] = wrong_edge_jump
                record["repaired_vs_raw_angle_diff"] = repaired_vs_raw_angle_diff
                record["repaired_vs_raw_delta_diff"] = repaired_vs_raw_delta_diff
                record["wrong_edge_continue"] = bool(wrong_edge_continue)
                record["repair_triggered"] = bool(repair_triggered)
                record["repair_used"] = bool(repair_used)
                record["repair_failed"] = bool(repair_failed)
                record["raw_delta_ignored_for_close"] = False
                ...
                record["close_hit_repaired"] = bool(repair_used and close_hit and close_source == "raw_and_repaired_agree")
            ...
            self.log(
                ...
                f"repaired_vs_raw_delta_diff={repaired_vs_raw_delta_diff}, wrong_edge_continue={wrong_edge_continue}, "
                f"close_rule=保守修复检查；只有 raw_delta>=3.5° 且 repaired_delta 支持同一趋势时才关激光；raw_delta<3.5° 时禁止仅凭 repaired_delta 关激光"
            )

            if repair_triggered:
                self.log(
                    f"[RuleAB] 本帧 raw_angle 与 repaired_angle 差异={repaired_vs_raw_angle_diff}° "
                    f"> {repair_delta_tol:.3f}°，判定检测边可能选错；"
                    "关闭判断采用保守规则：raw_delta 未达到阈值时，不允许仅凭 repaired_delta 关闭激光。"
                )
            ...
            if repair_failed:
                stop_reason = "every_frame_repair_failed_continue_no_raw_close"
                self.log(
                    "[RuleAB] 本帧每帧修复检查失败，没有得到可用 B 边候选；"
                    "不使用 raw_angle 关闭激光，继续下一步推动/检测。"
                )
            elif repair_used:
                stop_reason = "repaired_delta_below_3p5_continue"
                self.log(
                    f"[RuleAB] 本帧检测角度与修复角度差异超过阈值，已使用修复角度；close_band={close_band}, "
            ...
            if not ok:
                # 最新需求：RuleAB/路线运动偶发失败不能直接停止完整测量。
                # 本轮已经重新取帧做过一次角度检测；若没有达到关闭条件，
                # 下一次 while 会再次取一帧、重新分割 A/B/C、重新计算路线/动作并继续推动。
                stop_reason = "rule_ab_returned_false_retry_next_frame"
                self.log(
                    "[RuleAB] 本步 RuleAB/路线运动返回 False，但不停止完整测量；"
                    "将重新取一帧、重新分割 A/B/C，并继续 Step7 闭环。"
                )
                try:
                    # 外部 follower 有些实现会在失败后保留坏状态；尽量清理轻量缓存，
                    # 不重新标定，不清空完整测量的 A/B/C prompt。
                    for attr in ("last_scene", "current_scene", "scene"):
                        if hasattr(follower, attr):
                            setattr(follower, attr, None)
                except Exception:
                    pass
    ...
    def run_one_cycle(self, cycle_index: int) -> bool:
        ...
        try:
            ...
            # 新逻辑：完整测量的 Step1 baseline 也必须使用 feature_tracker_scene.a。
            # 不再先运行旧 ScreenAngleDetector，避免生成/保存一张使用旧 A 来源的角度图。
            angle_result = {"ok": False, "angle_deg": None, "reason": "ab_feature_angle_not_run"}
            ...
            try:
                ...
                self._ensure_rule_ab_feature_tracker_installed(follower_for_angle, calibration_state if 'calibration_state' in locals() else self._get_loaded_calibration_state())
                ab_angle_result = self.detect_ab_nearest_b_edge_angle_once(
                    follower=follower_for_angle,
                    label="current_angle_ab_nearest_feature_tracker",
                    allow_fail=True,
                )
                if ab_angle_result.get("ok", False) and ab_angle_result.get("angle_deg") is not None:
                    angle_result = ab_angle_result
                    self.log(
                        f"[角度检测-current_angle] 已使用 feature_tracker_scene.a + 当前 B mask 最近边角度作为本轮 baseline："
                        f"{ab_angle_result.get('angle_deg')}°"
                    )
                else:
                    self.log(
                        f"[角度检测-current_angle] feature_tracker A/B 最近边角度失败，才临时回退旧 ScreenAngleDetector；"
                        f"reason={ab_angle_result.get('reason')}"
                    )
                    angle_result = self.detect_angle_once(label="current_angle_screen_fallback_after_feature_fail", allow_fail=True)
            except Exception as e:
                self.log(f"[角度检测-current_angle] feature_tracker A/B 最近边 baseline 计算异常，才临时回退旧 ScreenAngleDetector：{e}")
                angle_result = self.detect_angle_once(label="current_angle_screen_fallback_after_feature_exception", allow_fail=True)

            if angle_result.get("ok", False) and angle_result.get("angle_deg") is not None:
                current_angle_raw = float(angle_result["angle_deg"])

                # Step1 baseline 也必须做角度修复/确认。
                # 若 repaired_angle 与 raw_angle 相差 <=1.5°，认为 Step1 检测角度正确；
                # 若相差 >1.5° 且存在上一轮角度参考，则用 repaired_angle 替换 baseline。
                try:
                    if follower_for_angle is None:
                        follower_for_angle = self.ensure_rule_ab_follower()
                        self.apply_runtime_rule_ab_params_to_follower(follower_for_angle, reason="step1_baseline_repair")
                    current_angle, step1_repair_info = self._repair_step1_baseline_angle(
                        raw_angle=current_angle_raw,
                        follower=follower_for_angle,
                        angle_result=angle_result,
                    )
                    angle_result["step1_angle_repair"] = self._json_safe(step1_repair_info)
                except Exception as e:
                    current_angle = current_angle_raw
                    self.log(f"[Step1-角度修复] baseline 修复异常，使用 raw_angle={current_angle_raw:.6f}°：{e}")
            else:
                current_angle = None
            ...
            # 当前保存角度固定使用 Step1 的最终 baseline 角度：
            #   - 如果 Step1 修复后被采用，则 current_angle 已经是修复角度；
            #   - 否则 current_angle 是 Step1 检测角度。
            # Step7 的最终角度只用于关闭激光判断，不再覆盖 save_angle_deg。

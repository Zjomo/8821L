...
class MeasurementWorkflow:
    ...
    def _repair_rule_ab_angle_if_jump(
        ...
        当前版本的核心原则：
            只有 raw_angle 相对上一帧有效角度跳变超过阈值时才修复；
            未触发修复时使用检测角度 raw_angle，触发修复时使用 repaired_angle。

        修复方式：
            当跳变超过阈值时，读取当前 RuleAB/SAM2 的 B mask，计算 B mask 轮廓所有候选边角度；
            选择最接近上一帧有效角度 previous_angle 的候选边，作为 repaired_angle。

        Step7 关闭激光判断：
            必须先完成角度修复判断。
            如果 raw_angle 相对上一帧跳变超过阈值并且修复成功，则关闭激光只看 repaired_delta/final_delta；
            raw_delta 此时只作为“触发修复”的信号，不允许再触发关闭激光。
            如果没有触发修复，则关闭激光看 raw_delta/final_delta。
            也就是：修复优先，最终用于保存和关激光的是同一个 final_angle。
        """
        info: Dict[str, Any] = {
        ...
        if previous_angle is None:
            # 没有上一帧时无法判断“同一条边”，只能使用 raw_angle。
            info["reason"] = "previous_angle_none_use_raw_angle"
            return raw_angle, info

        try:
            jump = self.angle_diff_deg(float(raw_angle), float(previous_angle))
        except Exception:
            info["reason"] = "jump_calc_failed_use_raw_angle"
            return raw_angle, info

        info["jump_from_previous_deg"] = float(jump)
        threshold = float(info["jump_threshold_deg"])
        info["triggered"] = bool(jump > threshold)

        # 按最新需求：只有 raw_angle 相对上一帧有效角度跳变超过阈值时才修复。
        # 未超过阈值时，Step7 使用检测角度 raw_angle 作为最终角度，保存结果也保存 raw_angle。
        if jump <= threshold:
            info["reason"] = "jump_within_threshold_use_raw_angle"
            self.log(
                f"[RuleAB-角度修复] raw_angle={raw_angle:.6f} 相对上一帧={previous_angle:.6f} "
                f"跳变 {jump:.3f}° <= {threshold:.3f}°，不执行修复，使用检测角度。"
            )
            try:
        ...
        self.log(
            f"[RuleAB-角度修复] raw_angle={raw_angle:.6f} 相对上一帧={previous_angle:.6f} "
            f"跳变 {jump:.3f}° > {threshold:.3f}°，执行 B mask 全边候选修复。"
        )
        ...
        if not candidates:
            ...
            self.log("[RuleAB-角度修复] 未能从当前 B mask 得到候选边角度，只能继续使用 raw_angle。")
            return raw_angle, info
        ...
        info.update({
            ...
            "reason": "jump_over_threshold_repaired_by_nearest_bmask_edge_to_previous_angle",
        })
        ...
        while True:
            ...
            if angle_result.get("ok", False) and angle_result.get("angle_deg") is not None:
                ...
                try:
                    ...
                    angle_result["used_repaired_angle"] = bool(repaired_info.get("used_repaired_angle", False))
                except Exception:
                    pass
            ...
            # 关闭激光条件（修复优先，按最新需求）：
            #   1) 角度修复只在 jump > threshold 时执行；
            #   2) 如果触发并成功使用修复角度，则 raw_delta 只作为异常检测/触发修复依据，
            #      不能再用于关闭激光；此时只看 repaired_delta / final_delta；
            #   3) 如果未使用修复角度，则使用检测角度 raw_delta / final_delta；
            #   4) 使用最终有效角度变化 close_delta >= min_delta 时立即关闭激光；
            #   5) max_delta 只作为“是否超过理想上限6°”的记录和日志提示，不再作为关闭硬窗口。
            ...
            if repair_used:
                # 关键修复：raw_angle 跳变超过阈值后，必须等待并采用修复结果判断是否关激光。
                # raw_delta 此时可能很大，但它只说明“原始边选错/跳变”，不能触发关激光。
                ...
                # 触发了修复但没有得到可信 repaired_angle。为了避免 raw_angle 异常误关激光，
                # 本帧不允许用 raw_delta 关闭激光，只记录并继续下一帧。
                close_delta_for_reason = None
                close_source = "repair_failed_no_close"
            else:
                # 未触发修复：检测角度本身可信，raw_delta/final_delta 可以用于关闭激光。
            ...
            if close_delta_for_reason is not None:
                close_band = "target_3p5_to_6" if close_delta_for_reason <= close_target_max_delta else "overshoot_above_6"
            ...
            try:
                ...
                record["close_rule"] = "repair_first_final_delta_ge_min"
                ...
                record["close_hit_raw"] = bool((not repair_triggered) and raw_delta is not None and float(raw_delta) >= close_min_delta)
                record["close_hit_repaired"] = bool(repair_used and close_delta_for_reason is not None and float(close_delta_for_reason) >= close_min_delta)
            ...
            self.log(
                ...
                f"repair_triggered={repair_triggered}, repair_used={repair_used}, "
                f"close_source={close_source}, close_delta_used={close_delta_for_reason}, "
                f"close_rule=repair_first_delta>={close_min_delta}, ideal_max={close_target_max_delta}"
            )

            if repair_triggered and raw_delta is not None:
                self.log(
                    f"[RuleAB] 本帧 raw_angle 相对上一帧跳变已触发修复；"
                    f"raw_delta={raw_delta:.3f}° 只作为异常检测值，不参与关闭激光判断。"
                )

            if close_hit:
                reached = True
                allow_continue_measurement = True
                stop_reason = f"{close_source}_delta_ge_min_close_laser_{close_band}"

                if close_delta_for_reason is not None and close_delta_for_reason > close_target_max_delta:
                    self.log(
                        f"[RuleAB] 最终有效角度变化已达到关闭阈值但超过理想上限："
                        f"source={close_source}, delta={close_delta_for_reason:.3f}° > {close_target_max_delta:.3f}°。"
                        "立即关闭激光；后续需要减小单步运动/等待来减少过冲。"
                    )
                else:
                    self.log(
                        f"[RuleAB] 最终有效角度变化达到关闭阈值："
                        f"source={close_source}, close_delta={close_delta_for_reason}, "
                        f"raw_delta={raw_delta}, repaired_delta={repaired_delta}, "
                        f"关闭阈值={close_min_delta:.3f}°，理想上限={close_target_max_delta:.3f}°；立即关闭激光。"
                    )

                try:
                    if bool(self.context.get("laser_on", False)):
                        self.laser_off()
                        self.log("[RuleAB] 最终有效角度变化达到关闭阈值后已立即执行 laser_off()")
                    else:
                        self.log("[RuleAB] 角度关闭条件满足时 context 显示 laser_on=False，跳过 laser_off()")
                except Exception as e:
                    self.log(f"[RuleAB] 角度达到关闭阈值后关闭激光失败：{e}")
                break

            if repair_failed:
                stop_reason = "angle_repair_failed_continue_no_raw_close"
                self.log(
                    "[RuleAB] 本帧已触发角度修复但修复失败；为避免 raw_angle 异常误关激光，"
                    "本帧不关闭激光，继续下一步推动/检测。"
                )
            elif close_delta_for_reason is not None:
                stop_reason = "angle_delta_not_enough_continue"
                self.log(
                    f"[RuleAB] 当前最终有效角度变化 {close_delta_for_reason:.3f}° < 关闭阈值 {close_min_delta:.3f}°，"
                    "继续下一步推动/检测。"
                )
    ...
    def _postprocess_scene_ab_masks(self, follower: Any, scene: Any, tag: str = "") -> Dict[str, Any]:
        ...
        if ref_feat is None:
            ...
            min_ratio = float(getattr(self.cfg, "rule_ab_b_area_ratio_min", 0.75))
            max_ratio = float(getattr(self.cfg, "rule_ab_b_area_ratio_max", 1.35))
            max_center_jump = float(getattr(self.cfg, "rule_ab_b_center_jump_max_px", 40.0))
            max_angle_jump = float(getattr(self.cfg, "rule_ab_b_angle_jump_max_deg", 12.0))
            bad_reasons = []

...
class MeasurementConfig:
    ...
    # ============================================================
    rule_ab_use_klt_selected_b_edge_angle: bool = bool(_cfg("rule_ab_use_klt_selected_b_edge_angle", True))
    ...
    rule_ab_force_reselect_klt_edge_each_run: bool = bool(_cfg("rule_ab_force_reselect_klt_edge_each_run", True))
...
class MeasurementWorkflow:
    ...
    def _reset_b_edge_klt_state_for_new_run(
        ...
        """
        完整循环测量启动时强制清空 KLT 指定 B 边运行状态。

        只清空 KLT 选边/跟踪状态，不改 KLT 角度计算、角度阈值判断、关闭激光、
        Step7 路线推进、Step9 颜色中心对齐和光谱保存逻辑。
        """
        if state is None:
            state = self._get_loaded_calibration_state()
        ...
        self.context["b_edge_klt_state"] = {
            k: self._json_safe(v)
            for k, v in self.b_edge_klt_state.items()
            if k not in ("prev_gray", "prev_pts")
        }

        self.log(
            "[KLT指定边] 已清空旧 KLT 指定边与跟踪状态："
    ...
    def _select_initial_b_edge_interactively(
        ...
        """首帧 cleaned B mask 上人工点击两个端点，确定需要跨轮次 KLT 跟踪的 B 物理边。"""
        # 旧逻辑：若标定包已有 selected_b_edge_endpoints，则直接复用，不再弹窗。
        # 新需求：完整循环测量每次开始时默认强制重新选择 KLT 边；
        # 因此 force_reselect_klt_edge_this_run=True 时，不允许复用 calibration.json 中的旧边。
        ...
        try:
            ...
            if force_reselect_this_run and endpoints and len(endpoints) >= 2:
                self.log("[KLT指定边] 本次完整测量已要求重新选边，忽略 calibration.json 中保存的旧 selected_b_edge_endpoints。")
        except Exception:
            pass
        ...
        win = "Select B edge endpoints for KLT"
        ...
        while True:
            ...
            for i, (x, y) in enumerate(clicks):
                ...
                cv2.putText(canvas, f"P{i+1}", (int(round(x)) + 8, int(round(y)) - 8), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2, cv2.LINE_AA)
            if len(clicks) == 2:
                cv2.line(canvas, (int(clicks[0][0]), int(clicks[0][1])), (int(clicks[1][0]), int(clicks[1][1])), (0, 0, 255), 2, cv2.LINE_AA)
                cv2.putText(canvas, "ENTER: confirm  R: reset  ESC: cancel", (12, 56), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 255), 2, cv2.LINE_AA)
            ...
            if key == 27:
                ...
                raise RuntimeError("用户取消了 B 指定边 KLT 端点选择。")
        cv2.destroyWindow(win)

        raw_p1, raw_p2 = clicks[0], clicks[1]
        # 将点击结果尽量匹配到当前 B mask 的候选边，避免端点点击偏离轮廓。
        selected_edge = None
        if candidates:
            clicked_angle = self._edge_angle_from_endpoints(raw_p1, raw_p2)
            clicked_len, clicked_mid = self._edge_length_midpoint(raw_p1, raw_p2)
            best_score = 1e18
            for c in candidates:
                try:
                    cp1, cp2 = c["p1"], c["p2"]
                    _, cmid = self._edge_length_midpoint(cp1, cp2)
                    da = self.angle_diff_deg(float(c["angle_deg"]), clicked_angle)
                    dl = abs(math.log(max(float(c.get("length_px", 1.0)), 1.0) / max(clicked_len, 1.0)))
                    dm = math.hypot(cmid[0] - clicked_mid[0], cmid[1] - clicked_mid[1]) / 120.0
                    score = da / 15.0 + dl + dm
                    if score < best_score:
                        best_score = score
                        selected_edge = c
                    ...
                    continue
        if selected_edge is not None:
            p1 = (float(selected_edge["p1"][0]), float(selected_edge["p1"][1]))
            p2 = (float(selected_edge["p2"][0]), float(selected_edge["p2"][1]))
            projection_source = "nearest_b_candidate_edge"
        else:
            p1, p2 = raw_p1, raw_p2
            projection_source = "raw_clicked_endpoints"
        desc = self._build_selected_b_edge_descriptor(mask_bool, p1, p2)
        desc["projection_source"] = projection_source
        desc["raw_clicked_endpoints"] = [[float(raw_p1[0]), float(raw_p1[1])], [float(raw_p2[0]), float(raw_p2[1])]]
        return {"ok": True, "p1": p1, "p2": p2, "descriptor": desc, "source": "interactive"}

    def _persist_selected_b_edge_descriptor(self, descriptor: Dict[str, Any], state: Optional[CalibrationState] = None) -> None:
        """把首帧指定 B 边写入当前 CalibrationState 和 calibration.json。"""
        try:
            endpoints = [descriptor.get("p1"), descriptor.get("p2")]
            if state is None:
                state = self._get_loaded_calibration_state()
            if state is not None:
                state.selected_b_edge_endpoints = [[float(endpoints[0][0]), float(endpoints[0][1])], [float(endpoints[1][0]), float(endpoints[1][1])]]
                state.selected_b_edge_angle = float(descriptor.get("angle_deg"))
                state.selected_b_edge_length = float(descriptor.get("length_px"))
                state.selected_b_edge_midpoint = [float(x) for x in descriptor.get("midpoint_xy", [])]
                state.selected_b_edge_descriptor = self._json_safe(descriptor)
                self.context["calibration_state"] = state.to_dict()
                try:
                    self.save_calibration_state(state)
                except Exception:
                    # 运行中保存失败不影响当前状态；只记录日志。
                    pass
            self.context["selected_b_edge_descriptor"] = self._json_safe(descriptor)
        except Exception as e:
            self.log(f"[KLT指定边] 保存 selected_b_edge_descriptor 失败：{e}")

    def _initialize_b_edge_klt(
        self,
        image_rgb: np.ndarray,
        ...
        pts = self._sample_points_on_edge(p1, p2)
        angle = self._edge_angle_from_endpoints(p1, p2)
        length, mid = self._edge_length_midpoint(p1, p2)
        ...
        self.context["b_edge_klt_state"] = {k: self._json_safe(v) for k, v in self.b_edge_klt_state.items() if k not in ("prev_gray", "prev_pts")}
        return {"ok": True, "angle_deg": float(angle), "endpoints": self.b_edge_klt_state["last_edge_endpoints"], "source": source}
    ...
    def _ensure_initial_b_edge_klt_selected_and_initialized(
        ...
        if not bool(getattr(self.cfg, "rule_ab_use_klt_selected_b_edge_angle", True)):
            return {"ok": False, "reason": "klt_selected_b_edge_disabled"}
        if self.b_edge_klt_state.get("initialized", False):
            # 本次完整测量内已经人工选择并初始化过，则后续 Step1/Step7 继续复用本次新选边；
            # 只有 run_workflow 启动阶段会调用 _reset_b_edge_klt_state_for_new_run() 清空旧状态。
        ...
        p1 = tuple(sel["p1"])
        p2 = tuple(sel["p2"])
        desc = dict(sel.get("descriptor") or {})
        desc.setdefault("p1", [float(p1[0]), float(p1[1])])
        desc.setdefault("p2", [float(p2[0]), float(p2[1])])
        self._persist_selected_b_edge_descriptor(desc, state=state)
        init = self._initialize_b_edge_klt(image_rgb, p1, p2, descriptor=desc, source="initial_selected_b_edge")
        self.context["force_reselect_klt_edge_this_run"] = False
        self.log(f"[KLT指定边] 已初始化：angle={init.get('angle_deg'):.6f}°, endpoints={init.get('endpoints')}, source={sel.get('source')}")
        return init
    ...
    def _save_b_edge_klt_overlay(
        ...
        out_dir = self._b_edge_klt_output_dir()
        ts = datetime.now().strftime("%Y%m%d_%H%M%S_%f")[:-3]
        stem = f"{ts}_{label}_B_KLT_edge"
        paths: Dict[str, str] = {}
        try:
            ...
            for p in result.get("rejected_points", []) or []:
                cv2.circle(canvas, (int(round(p[0])), int(round(p[1]))), 3, (0, 0, 255), -1, cv2.LINE_AA)
            for p in result.get("good_points", []) or []:
                cv2.circle(canvas, (int(round(p[0])), int(round(p[1]))), 4, (0, 255, 255), -1, cv2.LINE_AA)
            endpoints = result.get("endpoints") or result.get("repaired_edge_endpoints")
            if endpoints and len(endpoints) >= 2:
                p1 = tuple(int(round(float(v))) for v in endpoints[0])
                p2 = tuple(int(round(float(v))) for v in endpoints[1])
                cv2.line(canvas, p1, p2, (255, 0, 255), 3, cv2.LINE_AA)
                cv2.circle(canvas, p1, 6, (255, 255, 0), -1, cv2.LINE_AA)
                cv2.circle(canvas, p2, 6, (255, 255, 0), -1, cv2.LINE_AA)
            repaired = result.get("repaired_edge_endpoints")
            if repaired and len(repaired) >= 2:
                q1 = tuple(int(round(float(v))) for v in repaired[0])
                q2 = tuple(int(round(float(v))) for v in repaired[1])
                cv2.line(canvas, q1, q2, (255, 255, 0), 2, cv2.LINE_AA)
            info_lines = [
                f"KLT selected B edge: {label}",
                f"status={result.get('klt_status')}, source={result.get('angle_source')}",
                f"angle={result.get('angle_deg')}, baseline={result.get('baseline_angle')}, delta={result.get('delta')}",
                f"good={result.get('good_count')}, rejected={result.get('rejected_count')}, residual={result.get('fit_residual_px')}",
                f"allow_close={result.get('allow_close')}, relocated={result.get('relocated')}",
            ]
            y = 24
            cv2.rectangle(canvas, (4, 4), (900, 150), (0, 0, 0), -1)
            for line in info_lines:
                cv2.putText(canvas, str(line), (12, y), cv2.FONT_HERSHEY_SIMPLEX, 0.58, (255, 255, 255), 2, cv2.LINE_AA)
                y += 25
            img_path = out_dir / f"{stem}.png"
            cv2.imwrite(str(img_path), canvas)
            paths["overlay_path"] = str(img_path)
            meta_path = out_dir / f"{stem}.json"
            with meta_path.open("w", encoding="utf-8") as f:
                json.dump(self._json_safe(result), f, ensure_ascii=False, indent=2)
            paths["meta_path"] = str(meta_path)
        except Exception as e:
            self.log(f"[KLT指定边] 保存 overlay 失败：{e}")
        return paths

    def _fit_klt_edge_from_points(
        self,
        good_prev: np.ndarray,
        good_next: np.ndarray,
    ) -> Dict[str, Any]:
        min_good = int(getattr(self.cfg, "rule_ab_klt_min_good_points", 6))
        if good_next is None or len(good_next) < min_good:
            return {"ok": False, "reason": f"good_points<{min_good}"}
        prev2 = np.asarray(good_prev, dtype=np.float32).reshape(-1, 2)
        nxt2 = np.asarray(good_next, dtype=np.float32).reshape(-1, 2)
        disp = nxt2 - prev2
        med = np.median(disp, axis=0)
        dev = np.linalg.norm(disp - med.reshape(1, 2), axis=1)
        mad = float(np.median(dev))
        keep = dev <= max(2.5, 3.0 * mad + 1.0)
        if int(np.count_nonzero(keep)) < min_good:
            return {"ok": False, "reason": "too_few_after_displacement_outlier_filter", "raw_good": int(len(nxt2))}
        pts = nxt2[keep].astype(np.float32)
        rejected = nxt2[~keep].astype(np.float32)
        vx, vy, x0, y0 = cv2.fitLine(pts.reshape(-1, 1, 2), cv2.DIST_HUBER, 0, 0.01, 0.01).reshape(-1)
        v = np.asarray([float(vx), float(vy)], dtype=np.float32)
        vn = float(np.linalg.norm(v))
        if vn <= 1e-9:
            return {"ok": False, "reason": "fitline_degenerate"}
        v = v / vn
        p0 = np.asarray([float(x0), float(y0)], dtype=np.float32)
        residuals = np.abs((pts[:, 0] - p0[0]) * v[1] - (pts[:, 1] - p0[1]) * v[0])
        residual = float(np.median(residuals))
        max_res = float(getattr(self.cfg, "rule_ab_klt_max_fit_residual_px", 4.0))
        if residual > max_res:
            return {"ok": False, "reason": f"fit_residual>{max_res}", "fit_residual_px": residual, "good_count": int(len(pts))}
        last_len = None
        try:
            endpoints = self.b_edge_klt_state.get("last_edge_endpoints")
            if endpoints:
                last_len, _ = self._edge_length_midpoint(tuple(endpoints[0]), tuple(endpoints[1]))
        except Exception:
            pass
        if last_len is None or last_len <= 1:
            last_len = float(self.b_edge_klt_state.get("descriptor", {}).get("length_px", 30.0))
        # 用 good points 均值作为中点，使用上一帧边长重构端点，避免遮挡导致端点收缩。
        mid = np.mean(pts, axis=0)
        p1 = mid - v * (float(last_len) * 0.5)
        p2 = mid + v * (float(last_len) * 0.5)
        ...
        return {
            ...
            "good_points": pts.tolist(),
            "rejected_points": rejected.tolist(),
            "good_count": int(len(pts)),
    ...
    def _track_selected_b_edge_klt_on_image(
        ...
        img = np.asarray(image_rgb).astype(np.uint8)
        curr_gray = cv2.cvtColor(cv2.cvtColor(img, cv2.COLOR_RGB2BGR), cv2.COLOR_BGR2GRAY)
        st = self.b_edge_klt_state
        if not st.get("initialized", False) or st.get("prev_gray") is None or st.get("prev_pts") is None:
            if b_mask is not None and allow_relocate:
                reloc = self._relocate_selected_b_edge_from_mask(b_mask, st.get("descriptor"))
                if reloc.get("ok", False):
                    ep = reloc["endpoints"]
                    self._initialize_b_edge_klt(image_rgb, tuple(ep[0]), tuple(ep[1]), descriptor=st.get("descriptor"), source=f"{label}_relocate_before_klt")
                    result.update({
                        "ok": True,
                        "angle_deg": float(reloc["angle_deg"]),
                        "endpoints": ep,
                        "klt_status": "relocated_reinit",
                        "angle_source": "klt_failed_relocated_reinit",
                        "relocated": True,
                        "repaired_edge_endpoints": ep,
                        "reason": "klt_not_initialized_relocated",
                    })
                    ...
                    result.update(paths)
                    return result
            result["reason"] = "klt_not_initialized"
            return result

        prev_gray = st.get("prev_gray")
        prev_pts = np.asarray(st.get("prev_pts"), dtype=np.float32).reshape(-1, 1, 2)
        lk_params = dict(
            winSize=(21, 21),
            maxLevel=3,
        ...
        try:
            ...
            back_pts, status_back, err_back = cv2.calcOpticalFlowPyrLK(curr_gray, prev_gray, next_pts, None, **lk_params)
            status = status.reshape(-1).astype(bool) if status is not None else np.zeros((len(prev_pts),), dtype=bool)
            status_back = status_back.reshape(-1).astype(bool) if status_back is not None else np.zeros((len(prev_pts),), dtype=bool)
            fb = np.linalg.norm(prev_pts.reshape(-1, 2) - back_pts.reshape(-1, 2), axis=1)
            fb_ok = fb <= float(getattr(self.cfg, "rule_ab_klt_max_fb_error_px", 2.0))
            keep = status & status_back & fb_ok
            good_prev = prev_pts.reshape(-1, 2)[keep]
            good_next = next_pts.reshape(-1, 2)[keep]
            rejected = next_pts.reshape(-1, 2)[~keep] if next_pts is not None else np.empty((0, 2), dtype=np.float32)
            fit = self._fit_klt_edge_from_points(good_prev, good_next)
            if not fit.get("ok", False):
                raise RuntimeError(str(fit.get("reason", "fit_failed")))
            angle = float(fit["angle_deg"])
            last_angle = st.get("last_angle")
            if last_angle is not None:
                jump = self.angle_diff_deg(angle, float(last_angle))
                max_jump = float(getattr(self.cfg, "rule_ab_klt_max_angle_jump_deg", 25.0))
                if jump > max_jump:
                    raise RuntimeError(f"angle_jump>{max_jump}: {jump:.3f}")
            endpoints = [fit["p1"], fit["p2"]]
            # KLT 成功：更新跨轮次状态。
            self._initialize_b_edge_klt(image_rgb, tuple(endpoints[0]), tuple(endpoints[1]), descriptor=st.get("descriptor"), source=label)
            # 用当前拟合线上重新均匀采样的点覆盖 initialize 中的端点采样即可。
            ...
            result.update({
                ...
                "delta": delta,
                "klt_status": "success",
                "angle_source": "klt_success",
                "good_points": fit.get("good_points", []),
                "rejected_points": (fit.get("rejected_points", []) + rejected.tolist()),
                "good_count": int(fit.get("good_count", 0)),
                "rejected_count": int(fit.get("rejected_count", 0)) + int(len(rejected)),
                "fit_residual_px": float(fit.get("fit_residual_px", 0.0)),
                "allow_close": True,
                "reason": "ok",
            })
            st["failure_count"] = 0
            st["relocate_stable_count"] = 0
        except Exception as e:
            st["failure_count"] = int(st.get("failure_count", 0) or 0) + 1
            result.update({"klt_status": "failed", "reason": str(e), "failure_count": int(st.get("failure_count", 0))})
            if allow_relocate and b_mask is not None:
                reloc = self._relocate_selected_b_edge_from_mask(b_mask, st.get("descriptor"))
                if reloc.get("ok", False):
                    repaired_angle = float(reloc["angle_deg"])
                    ep = reloc["endpoints"]
                    prev_reloc = st.get("last_relocated_angle")
                    stable_tol = float(getattr(self.cfg, "rule_ab_klt_relocate_stable_tol_deg", 2.0))
                    if prev_reloc is not None and self.angle_diff_deg(float(prev_reloc), repaired_angle) <= stable_tol:
                        st["relocate_stable_count"] = int(st.get("relocate_stable_count", 0) or 0) + 1
                    else:
                        st["relocate_stable_count"] = 1
                    st["last_relocated_angle"] = repaired_angle
                    self._initialize_b_edge_klt(image_rgb, tuple(ep[0]), tuple(ep[1]), descriptor=st.get("descriptor"), source=f"{label}_relocated_reinit")
                    delta = self.angle_diff_deg(repaired_angle, float(baseline_angle)) if baseline_angle is not None else None
                    stable_required = int(getattr(self.cfg, "rule_ab_klt_relocate_stable_required", 2))
                    relocated_confirmed = bool(st.get("relocate_stable_count", 0) >= stable_required)
                    result.update({
                        "ok": True,
                        "angle_deg": repaired_angle,
                        "angle_deg_raw": repaired_angle,
                        "baseline_angle": baseline_angle,
                        "delta": delta,
                        "endpoints": ep,
                        "repaired_edge_endpoints": ep,
                        "klt_status": "failed_relocated_reinit",
                        "angle_source": "relocated_confirmed" if relocated_confirmed else "klt_failed_relocated_reinit",
                        "relocated": True,
                        "relocate_stable_count": int(st.get("relocate_stable_count", 0)),
                        # 默认单帧 repaired_angle 不允许关激光；只有连续稳定确认且显式允许时才参与关闭。
                        "allow_close": bool(allow_close_from_relocation and relocated_confirmed),
                        "reason": f"klt_failed_relocated: {e}",
                    })
                else:
                    result.update({"relocate_result": self._json_safe(reloc), "angle_source": "failed_no_close", "allow_close": False})
            else:
                result.update({"angle_source": "failed_no_close", "allow_close": False})
        paths = self._save_b_edge_klt_overlay(image_rgb, label, result, b_mask=b_mask)
        result.update(paths)
        self.context["b_edge_klt_state"] = {k: self._json_safe(v) for k, v in self.b_edge_klt_state.items() if k not in ("prev_gray", "prev_pts")}
        return result
    ...
    def detect_b_klt_selected_edge_angle_once(
        ...
        """Step1/Step7 默认角度入口：使用跨轮次 KLT 跟踪的指定 B 物理边。"""
        self.log(f"========== KLT指定B边角度检测：{label} ==========")
        try:
            if follower is None:
                raise RuntimeError("RuleAB follower 为空，无法做 KLT 指定边角度检测")
            image_rgb, scene, seg_retry_info = self._capture_and_build_scene_with_ab_retry(
                follower=follower,
                label=f"KLT指定B边角度/{label}",
                allow_fail=False,
            )
            try:
                self._postprocess_scene_ab_masks(follower=follower, scene=scene, tag=f"klt_angle_{label}")
            except Exception:
                pass
            b_mask = self._extract_mask_from_scene_object(getattr(scene, "b", None))
            if b_mask is None or not np.any(np.asarray(b_mask).astype(bool)):
                raise RuntimeError("当前帧 B mask 为空，无法 KLT 跟踪/重定位指定边")
            result = self._track_selected_b_edge_klt_on_image(
                image_rgb=image_rgb,
                b_mask=b_mask,
                label=label,
                baseline_angle=baseline_angle,
                allow_relocate=True,
                allow_close_from_relocation=allow_close_from_relocation,
            )
            result["ab_seg_retry"] = self._json_safe(seg_retry_info)
            result["scene_source"] = "feature_tracker"
            result["edge_selection_mode"] = "initial_selected_b_edge_cross_cycle_klt"
            if result.get("ok", False):
                self.log(
                    f"[KLT指定B边-{label}] angle={float(result.get('angle_deg')):.6f}°, "
                    f"baseline={baseline_angle}, delta={result.get('delta')}, "
                    f"source={result.get('angle_source')}, good={result.get('good_count')}, "
                    f"residual={result.get('fit_residual_px')}, overlay={result.get('overlay_path')}"
                )
            else:
                self.log(f"[KLT指定B边-{label}] 失败：{result.get('reason')}")
            return result
        except Exception as e:
            msg = f"KLT指定B边角度检测失败：{e}"
            self.log(f"[KLT指定B边-{label}] {msg}")
            if not allow_fail:
                raise
            return {"ok": False, "angle_deg": None, "label": label, "angle_source": "failed_no_close", "allow_close": False, "reason": msg, "error": str(e)}
    ...
    def _repair_rule_ab_angle_if_jump(
        ...
        最新逻辑：
            1) Step7 每推动一次后，先得到检测角度 raw_angle；
            2) 每一次角度检测都必须从当前 B mask 的全部候选边中计算 repaired_angle；
            3) repaired_angle 的选择原则：
                  - 有上一帧确定角度 previous_angle 时，选择与 previous_angle 最接近的候选边；
                  - 没有 previous_angle 时，选择与 raw_angle 最接近的候选边；
            4) 计算 repaired_raw_diff = |repaired_angle - raw_angle|；
                  - repaired_raw_diff <= rule_ab_angle_repair_raw_tolerance_deg，默认 ±2°：
                        认为本帧检测角度 raw_angle 正确，最终角度使用 raw_angle；
                  - repaired_raw_diff > 2°：
                        认为检测角度选错边，最终角度使用 repaired_angle；
            5) 后续关闭激光判断只使用“最终角度 final_angle”的角度变化。
        """
        raw_tol = float(getattr(self.cfg, "rule_ab_angle_repair_raw_tolerance_deg", 2.0))
        previous_accept_tol = float(getattr(self.cfg, "rule_ab_angle_repair_accept_tolerance_deg", 1.0))
        max_delta = float(getattr(self.cfg, "rule_ab_angle_delta_max_deg", 6.0))

        info: Dict[str, Any] = {
            "enabled": bool(getattr(self.cfg, "rule_ab_enable_angle_repair", True)),
            "checked_every_frame": True,
            "triggered": False,  # True 表示 raw 与 repaired 差异超过阈值，最终使用 repaired_angle
            "trigger_reason": "none",
            ...
            "baseline_angle": baseline_angle,
            "raw_tolerance_repaired_vs_raw_deg": raw_tol,
            "accept_tolerance_to_previous_deg": previous_accept_tol,
            "legacy_max_delta_for_log_only_deg": max_delta,
            "raw_delta_from_baseline": None,
            "raw_jump_from_previous_deg": None,
            "candidate_count": 0,
            "candidate_angles": [],
            "candidate_diffs_to_reference": [],
            "candidate_diffs_to_raw": [],
            "eligible_candidate_angles_within_raw_tolerance": [],
            "selected_reference_angle": None,
            "selected_reference_name": "",
            "nearest_candidate_angle": None,
            "nearest_candidate_diff_to_reference_deg": None,
            "repaired_angle": None,
        ...
        if not info["enabled"]:
            ...
            try:
                ...
                angle_result["angle_deg_raw_before_repair"] = float(raw_angle) if raw_angle is not None else None
                angle_result["angle_deg"] = float(raw_angle) if raw_angle is not None else None
        ...
        if baseline_angle is not None:
            try:
                raw_delta = self.angle_diff_deg(raw_angle_f, float(baseline_angle))
            except Exception:
                raw_delta = None
        ...
        if previous_angle is not None:
            try:
                raw_jump = self.angle_diff_deg(raw_angle_f, float(previous_angle))
            except Exception:
                raw_jump = None
        info["raw_jump_from_previous_deg"] = raw_jump

        # 每一帧都尝试从当前 B mask 全边候选得到 repaired_angle。
        ...
        if not candidates:
            ...
            self.log(
                "[RuleAB-角度修复] 每帧修复检查失败：当前 B mask 没有候选边角度；"
                "本帧不使用 raw_angle 关闭激光，下一帧重新取帧/分割/检测。"
            )
            try:
                angle_result["angle_repair"] = self._json_safe(info)
        ...
        # 选择修复参考：优先上一帧确定角度；没有上一帧时用 raw_angle 自身。
        if previous_angle is not None:
            ref_angle = float(previous_angle)
            ref_name = "previous_confirmed_angle"
        else:
            ref_angle = raw_angle_f
            ref_name = "raw_angle_no_previous_confirmed_angle"

        info["selected_reference_angle"] = ref_angle
        info["selected_reference_name"] = ref_name
        ...
        if not enriched:
            ...
            self.log(
                "[RuleAB-角度修复] 每帧修复检查失败：B mask 候选边角度无有效数值；"
                "本帧不关闭激光，下一帧重新检测。"
            )
            try:
                angle_result["angle_repair"] = self._json_safe(info)
        ...
        info["candidate_diffs_to_reference"] = [float(x[0]) for x in enriched]
        info["candidate_diffs_to_raw"] = [float(x[1]) for x in enriched]

        best_ref_diff, best_raw_diff, repaired, best = enriched[0]
        repaired = float(repaired)
        repaired_vs_raw_diff = float(best_raw_diff)
        repaired_jump = float(best_ref_diff) if previous_angle is not None else None
        ...
        # 现在的核心判据：修复角度与检测角度是否在 ±2° 内。
        raw_confirmed = bool(repaired_vs_raw_diff <= raw_tol)
        use_repaired = not raw_confirmed
        final_angle = repaired if use_repaired else raw_angle_f
        ...
        info["eligible_candidate_angles_within_raw_tolerance"] = [
            float(x[2]) for x in enriched if float(x[1]) <= raw_tol
        ]
        info.update({
            "triggered": bool(use_repaired),
            "trigger_reason": "repaired_vs_raw_diff_gt_2deg" if use_repaired else "repaired_vs_raw_diff_within_2deg",
            "used_repaired_angle": bool(use_repaired),
            "raw_confirmed_correct": bool(raw_confirmed),
            "repair_failed": False,
            # 保留字段兼容旧保存/日志；当前是否修复不再由 previous ±1° 决定。
            "repair_out_of_tolerance": False,
            "repaired_angle": repaired,
            "repaired_jump_from_previous_deg": repaired_jump,
            "repaired_delta_from_baseline": repaired_delta,
            "repaired_vs_raw_diff_deg": repaired_vs_raw_diff,
            "nearest_candidate_angle": repaired,
            "nearest_candidate_diff_to_reference_deg": float(best_ref_diff),
            "selected_edge": self._json_safe(best),
            "final_angle": float(final_angle),
            "final_delta_from_baseline": final_delta,
            "reason": (
                "raw_confirmed_by_repaired_candidate_within_2deg_use_raw_angle"
                if raw_confirmed
                else "raw_angle_replaced_by_repaired_candidate_diff_gt_2deg"
            ),
        })
        ...
        try:
            ...
            if repair_overlay_path:
                ...
                self.log(f"[RuleAB-角度修复] 已保存 raw/repaired 边叠加图：{repair_overlay_path}")
        except Exception as e:
        ...
        try:
            ...
            angle_result["used_repaired_angle"] = bool(use_repaired)
            angle_result["repair_out_of_tolerance"] = False
            angle_result["repaired_vs_raw_diff_deg"] = repaired_vs_raw_diff
        except Exception:
            pass

        if raw_confirmed:
            self.log(
                f"[RuleAB-角度修复] 每帧修复检查：raw_angle={raw_angle_f:.6f}°，"
                f"repaired_angle={repaired:.6f}°，两者差={repaired_vs_raw_diff:.3f}° <= {raw_tol:.3f}°；"
                "判定检测角度正确，使用 raw_angle。"
            )
        else:
            self.log(
                f"[RuleAB-角度修复] 每帧修复检查：raw_angle={raw_angle_f:.6f}°，"
                f"repaired_angle={repaired:.6f}°，两者差={repaired_vs_raw_diff:.3f}° > {raw_tol:.3f}°；"
                f"判定检测边异常，使用 repaired_angle；repaired_delta={repaired_delta}。"
            )

        return float(final_angle), info
    ...
    def run_rule_ab_until_angle_delta(
        ...
        """
        Step7 主循环：默认使用“初始帧人工指定 B 边 + 跨轮次 KLT 光流点集”作为角度来源。

        原 A 最近边角度/全边候选修复函数仍保留在代码中作为 debug/fallback，
        但本函数默认不再用它们作为关闭激光依据。KLT 失败时只允许用当前 B mask
        重定位指定边并重新初始化 KLT；单帧重定位角度不直接触发关激光。
        """
        self.log("========== RuleAB：A推动B，KLT指定边检测B角度 ==========")
        ...
        klt_fail_limit = int(getattr(self.cfg, "rule_ab_klt_relocate_max_failures", 3))
        ...
        while True:
            ...
            if use_route:
                ...
                self.log(
                    ...
                    f"目标角度范围=[{min_delta}, {max_delta}]；角度来源=KLT指定B边。"
                )
                try:
            ...
            # 每一步运动后，使用同一个跨轮次 KLT 指定边跟踪器计算当前 B 指定物理边角度。
            angle_result = self.detect_b_klt_selected_edge_angle_once(
                follower=follower,
                label=f"rule_ab_klt_edge_step_{step_idx}",
                baseline_angle=baseline_angle,
                allow_fail=True,
            ...
            angle_source = str(angle_result.get("angle_source", "failed_no_close"))
            allow_close_by_angle = bool(angle_result.get("allow_close", False))
            current_angle = float(angle_result["angle_deg"]) if angle_ok else None
            ...
            record = {
                ...
                "klt_status": angle_result.get("klt_status"),
                "klt_good_points": angle_result.get("good_count"),
                "klt_fit_residual_px": angle_result.get("fit_residual_px"),
                "klt_allow_close": allow_close_by_angle,
                "klt_relocated": bool(angle_result.get("relocated", False)),
            ...
            self.log(
                f"[RuleAB-KLT] step={step_idx}, angle={current_angle}, baseline={baseline_angle}, "
                f"delta={final_delta}, source={angle_source}, klt_status={angle_result.get('klt_status')}, "
                f"good={angle_result.get('good_count')}, residual={angle_result.get('fit_residual_px')}, "
                f"allow_close={allow_close_by_angle}"
            )

            if not angle_ok:
                if int(self.b_edge_klt_state.get("failure_count", 0) or 0) >= klt_fail_limit:
                    stop_reason = "selected_b_edge_klt_tracking_or_relocation_failed"
                    self.log(
                        f"[RuleAB-KLT] 指定边 KLT 跟踪/重定位连续失败达到 {klt_fail_limit} 次；"
                        "停止 Step7/完整测量，不关闭激光。"
                    )
                    allow_continue_measurement = False
                    break
                stop_reason = "klt_angle_failed_retry_next_frame_no_close"
                self.log("[RuleAB-KLT] 本帧 KLT 指定边角度无效，不允许关激光；继续下一步推动/重新取帧。")
                time.sleep(float(self.cfg.rule_ab_loop_interval_s))
                continue
            ...
            if close_hit:
                ...
                stop_reason = "klt_success_delta_in_target_range_close"
                self.log(
                    f"[RuleAB-KLT] 指定边角度变化进入目标范围：delta={float(final_delta):.3f}°，"
                    f"目标范围=[{float(min_delta):.3f}, {float(max_delta):.3f})；立即关闭激光。"
                )
                try:
                    if bool(self.context.get("laser_on", False)):
                        self.laser_off()
                        self.log("[RuleAB-KLT] 角度关闭条件满足后已立即执行 laser_off()")
                    else:
                        self.log("[RuleAB-KLT] 角度关闭条件满足时 context 显示 laser_on=False，跳过 laser_off()")
                except Exception as e:
                    self.log(f"[RuleAB-KLT] 角度达到关闭条件后关闭激光失败：{e}")
                break

            if final_delta is not None and float(final_delta) < float(min_delta):
                stop_reason = "klt_delta_below_target_continue"
                self.log(
                    f"[RuleAB-KLT] 当前指定边角度变化 {float(final_delta):.3f}° 未达到关闭阈值 "
                    f"{float(min_delta):.3f}°；继续下一步推动。"
                )
            elif final_delta is not None and float(final_delta) >= float(max_delta):
                # 当前需求是目标区间 [min,max)，超过 max 视为未在目标区间；不误关激光。
                stop_reason = "klt_delta_ge_target_max_no_close_continue"
                self.log(
                    f"[RuleAB-KLT] 当前指定边角度变化 {float(final_delta):.3f}° 已超过目标上限 "
                    f"{float(max_delta):.3f}°；不因 repaired_angle 关激光，继续下一步/等待下一帧。"
                )
            else:
                stop_reason = "klt_relocated_reinit_no_single_frame_close_continue"
                self.log("[RuleAB-KLT] 本帧用于 KLT 重定位/重新初始化，不允许单帧 repaired_angle 关激光；继续下一步。")
        ...
        result = {
            ...
            "target_min": min_delta,
            "target_max": max_delta,
            "angle_source": "klt_selected_b_edge",
            "records": records,
        }
    ...
    def _save_step7_route_overlay_image(
        ...
        try:
            ...
            dir_name = "CCW(+1)" if int(direction) > 0 else "CW(-1)"
            loop_text = "loop" if route_loop else "no-loop"
            osc_text = " oscillation=True" if oscillation_detected else ""

            lines = [
                f"Step7 fixed-index route: step={step_idx}, cur_idx={route_index}/{max(0, len(route_points)-1)}, target_idx={target_idx}, dir={dir_name}, {loop_text}",
                f"mode={target_mode}{osc_text}",
    ...
    def _run_one_c_edge_route_cycle(
        ...
        direction = 1 if int(getattr(self.cfg, "rule_ab_follow_c_direction", 1)) >= 0 else -1
        ...
        route_sig = (n_route, tuple(route_points[0]), tuple(route_points[-1]), int(direction), bool(loop_route))
        ...
        if action_id != 0:
            ...
            stage_action = f"{ch_name}/CH{ch}: move_step={move_step}, dry_run={dry_run}"
        ...
        info = {
            ...
            "direction_name": "visual_ccw" if int(direction) > 0 else "visual_cw",
        ...
        self.log(
            ...
            f"target_idx={target_idx}, direction={direction}, loop={loop_route}, mode={target_mode}, "
    ...
    def run_one_cycle(self, cycle_index: int) -> bool:
        ...
        try:
            ...
            self.log("========== Step 1：KLT指定B边角度检测 current_angle ==========")
            angle_result = {"ok": False, "angle_deg": None, "reason": "klt_selected_b_edge_not_run"}
            follower_for_angle = None
            try:
                follower_for_angle = self.ensure_rule_ab_follower()
                self.apply_runtime_rule_ab_params_to_follower(follower_for_angle, reason="step1_klt_selected_b_edge_angle")
                ...
                angle_result = self.detect_b_klt_selected_edge_angle_once(
                    ...
                    label="current_angle_klt_selected_b_edge",
                    baseline_angle=None,
                    allow_fail=True,
                    # Step1 如果 KLT 失败，可用当前 B mask 重定位并重新初始化 KLT；
                    # 该角度仅作为本轮 baseline，不是关闭激光依据。
                    allow_close_from_relocation=False,
                )
                if angle_result.get("ok", False) and angle_result.get("angle_deg") is not None:
                    self.log(
                        f"[角度检测-current_angle] 已使用跨轮次 KLT 指定 B 边角度作为本轮 baseline："
                        f"{angle_result.get('angle_deg')}°；source={angle_result.get('angle_source')}"
                    )
                else:
                    self.log(
                        f"[角度检测-current_angle] KLT 指定 B 边 baseline 计算失败，"
                        f"不再回退 A 最近边作为主角度来源；reason={angle_result.get('reason')}"
                    )
            except Exception as e:
                self.log(f"[角度检测-current_angle] KLT 指定 B 边 baseline 计算异常：{e}")
            ...
            if angle_result.get("ok", False) and angle_result.get("angle_deg") is not None:
                ...
                current_angle = None
                self.log("[角度检测-current_angle] 本轮 KLT 指定边角度无效，流程停止，避免使用错误边角度进入 Step7。")
...
class MeasurementWorkflowGUI:
    ...
    def run_workflow(self):
        ...
        try:
            ...
            if bool(getattr(wf.cfg, "rule_ab_force_reselect_klt_edge_each_run", True)):
                calibration_state = wf._reset_b_edge_klt_state_for_new_run(
                    calibration_state,
                    reason="run_workflow_before_prepare_runtime",
                )
            else:

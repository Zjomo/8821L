...
class MeasurementConfig:
    ...
    # Step7 新方案：不再由实时 RuleAB 策略决定 A 的运动方向，而是在 C 四边形生成时
    # 沿 C 的边沿预生成 A 的运动路线；Step7 每一帧都让 A 追踪这条路线。
...
class MeasurementWorkflow:
    ...
    def _materialize_full_calibration_c_for_runtime(self, state: CalibrationState) -> CalibrationState:
        ...
        if not (runtime_dir / "static_c_edge_route.json").exists():
            try:
                mask_bool_rt = mask > 0
                contours_rt, _ = cv2.findContours((mask_bool_rt.astype(np.uint8) * 255), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
                if contours_rt:
                    contour_rt = max(contours_rt, key=cv2.contourArea)
                    rect_rt = cv2.minAreaRect(contour_rt)
                    quad_rt = cv2.boxPoints(rect_rt).astype(np.float32)
                    try:
                        quad_rt = MeasurementWorkflow._generate_c_edge_route_points_from_quad.__func__.__globals__.get('MeasurementWorkflowGUI', MeasurementWorkflow)._order_quad_points_clockwise(quad_rt)
                    except Exception:
                        pass
                    route_rt = self._generate_c_edge_route_points_from_quad(
                        quad_rt,
                    ...
                    with (runtime_dir / "static_c_edge_route.json").open("w", encoding="utf-8") as f:
                        json.dump({
                            "created_at": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                            "source": "rebuilt_from_runtime_static_c_mask",
                            "spacing_px": float(getattr(self.cfg, "rule_ab_c_edge_route_spacing_px", 12.0)),
                            "follow_direction": int(getattr(self.cfg, "rule_ab_follow_c_direction", 1)),
                            "route_points_xy": [[float(x), float(y)] for x, y in route_rt],
                        }, f, ensure_ascii=False, indent=2)
                    state.static_c_edge_route_path = str((runtime_dir / "static_c_edge_route.json").resolve())
            except Exception as e:
                self.log(f"[完整测量标定] 重建 C 边沿路线失败：{e}")
    ...
    def _load_or_build_step7_c_edge_route(self, follower: Any) -> List[Tuple[float, float]]:
        ...
        优先级：
            1. 完整测量锁定的 full_calibration_static_c/static_c_edge_route.json；
            2. cfg.rule_ab_static_c_map_dir/static_c_edge_route.json；
            3. 若只有 static_c_quad_points.json，则现场生成路线并写回。
        """
        c_dir = self._get_strict_full_calibration_c_dir() or str(getattr(self.cfg, "rule_ab_static_c_map_dir", "") or "")
        route = self._load_c_edge_route_from_dir(c_dir)
        if route:
            return route
            ...
            try:
                ...
                if quad.shape[0] >= 4:
                    ...
                    with out_path.open("w", encoding="utf-8") as f:
                        json.dump({
                            "created_at": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                            "source": "rebuilt_from_static_c_quad_points",
                            "spacing_px": float(getattr(self.cfg, "rule_ab_c_edge_route_spacing_px", 12.0)),
                            "follow_direction": int(getattr(self.cfg, "rule_ab_follow_c_direction", 1)),
...
class MeasurementWorkflowGUI:
    ...
    def _generate_c_edge_route_points_from_quad(
        ...
        """
        根据 C 的四边形边沿生成 A 的闭合运动路线。

        route_points_xy 使用截图区域内坐标，顺序沿四边形四条边依次采样；
        follow_direction=1 使用顺时针方向，-1 反向。
        """
        pts = np.asarray(quad_points, dtype=np.float32).reshape(4, 2)
        if int(follow_direction) < 0:
            pts = pts[::-1].copy()
    ...
    def _save_static_c_map_result(
        ...
        # 3.5 根据 C 四边形边沿预生成 A 的运动路线。Step7 会直接读取这个文件，
        # 不再每帧临时由 RuleAB 策略决定绕行方向。
        ...
        # 这个文件用于直接检查 Step7 的 A 路线是否贴合 C 的边沿。
        ...
        if route_points:
            ...
            cv2.putText(
                route_overlay,
                f"A motion route on C edge: {len(route_points)} pts, spacing={route_spacing_px:.1f}px",
                (12, 28),
                cv2.FONT_HERSHEY_SIMPLEX,
                0.70,
                (255, 0, 255),
        ...
        self.log(
            ...
            f"method={quad_method}；已生成C边沿路线点数={len(route_points)}；"
            "后续加载使用 static_c_mask.png=四边形mask，Step7 使用 static_c_edge_route.json"
        )

...
class MeasurementWorkflow:
    ...
    def apply_calibration_state(self, state: CalibrationState):
        ...
        说明：
            这里不初始化任何设备，只把完整流程需要的 cfg 参数填好。
            RuleAB/角度模块如果后续支持直接传入正负点，也可以从 context['calibration_state'] 读取。
        """
        ...
        # C map：完整测量中的 RuleAB 优先加载这个 static C。
        if state.static_c_map_dir:
            self.cfg.rule_ab_static_c_map_dir = str(state.static_c_map_dir)
    ...
    def _execute_stage12_diagonal_action(self, action_code: int) -> Dict[str, Any]:
        ...
        说明：
            _step9_direction_from_error() 输出的是“希望检测区域中心在图像中移动的方向”。
            你反馈 1/2 通道实际运动方向与上一版相反，因此这里把 CH1/CH2
            的正负步长整体调换。

        调换后的动作映射：
            +1: 目标方向=右上，实际发送 CH1=-step, CH2=-step
            -1: 目标方向=左下，实际发送 CH1=+step, CH2=+step
            +2: 目标方向=左上，实际发送 CH1=+step, CH2=-step
            -2: 目标方向=右下，实际发送 CH1=-step, CH2=+step
        """
        step = abs(float(getattr(self.cfg, "rule_ac_stage_step_size", 20)))

        # 已按实际反馈调换 1/2 通道运动方向。
        # action_name 仍表示“希望检测区域中心移动的图像方向”。
        mapping = {
            1:  (-step, -step, "右上"),
            -1: (step,  step,  "左下"),
            2:  (step,  -step, "左上"),
...
class MeasurementWorkflowGUI:
    ...
    def _build_ui(self):
        ...
        ttk.Button(button_grid, text="3 标定全局C", command=self.calibrate_global_c_thread).grid(row=3, column=0, padx=4, pady=4, sticky="ew")
        ttk.Button(button_grid, text="4 标定Step9目标/颜色", command=self.calibrate_step9_target_color_thread).grid(row=3, column=1, padx=4, pady=4, sticky="ew")
        ttk.Button(button_grid, text="保存/检查完整标定", command=self.save_and_check_calibration_thread).grid(row=4, column=0, padx=4, pady=4, sticky="ew")
        ttk.Button(button_grid, text="加载完整标定", command=self.load_calibration_to_gui_thread).grid(row=4, column=1, padx=4, pady=4, sticky="ew")
    ...
    def _current_or_new_calibration_state(self) -> CalibrationState:
        ...
        state.static_c_map_dir = str(cfg.rule_ab_static_c_map_dir or state.static_c_map_dir or "")
        if state.static_c_map_dir:
    ...
    def run_workflow(self):
        ...
        try:
            ...
            self.load_calibration_to_gui()
            self.log(f"[流程] 完整测量标定检查通过，标定文件={wf.get_calibration_path()}")

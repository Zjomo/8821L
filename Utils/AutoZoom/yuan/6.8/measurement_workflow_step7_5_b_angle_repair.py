...
class MeasurementConfig:
    ...
    rule_ab_angle_delta_min_deg: float = float(_cfg("rule_ab_angle_delta_min_deg", 5.0))
    ...
    # B 边角度初始化与跳变修复。
    # 目的：当 SAM2 的 B mask 某一帧分割不稳导致目标边角度突然跳变时，
    # 从 B 的所有候选边角度中选择最接近上一帧稳定角度的结果。
    rule_ab_b_angle_repair_enable: bool = bool(_cfg("rule_ab_b_angle_repair_enable", True))
    rule_ab_b_angle_jump_threshold_deg: float = float(_cfg("rule_ab_b_angle_jump_threshold_deg", 10.0))
    rule_ab_b_angle_init_frames: int = int(_cfg("rule_ab_b_angle_init_frames", 5))
    rule_ab_b_angle_init_interval_s: float = float(_cfg("rule_ab_b_angle_init_interval_s", 0.08))
    rule_ab_b_angle_init_max_spread_deg: float = float(_cfg("rule_ab_b_angle_init_max_spread_deg", 5.0))
...
class MeasurementWorkflow:
    def __init__(self, cfg: MeasurementConfig, on_log=None, on_update=None):
        ...
        self.context: Dict[str, Any] = {
            ...
            "last_rule_ab_angle_repair": None,
            "rule_ab_last_b_angle": None,
            "rule_ab_last_repaired_b_angle": None,
    ...
    def laser_on(self):
        ...
        self.laser_move_and_wait(
            axis=int(self.cfg.laser_axis),
            steps=int(self.cfg.laser_on_steps),
            wait=True,
        )
        self.context["laser_on"] = True
        self.log(
            f"[激光开关] ON 完成：axis={self.cfg.laser_axis}, "
            f"steps={self.cfg.laser_on_steps}"
        )
        self.notify_update()
    ...
    def laser_off(self):
        ...
        self.laser_move_and_wait(
            axis=int(self.cfg.laser_axis),
            steps=int(self.cfg.laser_off_steps),
            wait=True,
        )
        self.context["laser_on"] = False
        self.log(
            f"[激光开关] OFF 完成：axis={self.cfg.laser_axis}, "
            f"steps={self.cfg.laser_off_steps}"
        )
        self.notify_update()
    ...
    # --------------------------------------------------------
    # RuleAB B角度修复：用于完整测量 Step 7 和 Step 8 之间
    # --------------------------------------------------------

    @staticmethod
    def _angle_diff_180_for_rule_ab_repair(a: Optional[float], b: Optional[float]) -> Optional[float]:
        """
        0~180° 方向角的最小差值。
        """
    ...
    @staticmethod
    def _line_angle_deg_for_rule_ab_repair(p1: Any, p2: Any) -> float:
        """
        两点连线的方向角，统一到 0~180°。
        """
        x1, y1 = float(p1[0]), float(p1[1])
        x2, y2 = float(p2[0]), float(p2[1])
        return float(math.degrees(math.atan2(y2 - y1, x2 - x1)) % 180.0)

    @staticmethod
    def _dedup_rule_ab_angles(angles: List[float], min_sep_deg: float = 2.0) -> List[float]:
        out: List[float] = []
        for a in angles:
            try:
                aa = float(a) % 180.0
            ...
            duplicated = False
            for b in out:
                d = MeasurementWorkflow._angle_diff_180_for_rule_ab_repair(aa, b)
                if d is not None and d <= float(min_sep_deg):
                    duplicated = True
                    break
            if not duplicated:
                out.append(aa)
    ...
    @staticmethod
    def _get_scene_b_mask_for_rule_ab_repair(scene: Any) -> Optional[np.ndarray]:
        """
        从 RuleAB 的 scene 对象中尽量取出 B mask。
        """
        try:
            b = getattr(scene, "b", None)
            if b is None:
                return None
    ...
    def _candidate_b_edge_angles_for_rule_ab_repair(
        self,
        scene: Any,
        ...
        候选来源：
            1. status['b_edge_angle_deg'] 当前算法给出的角度；
            2. scene.b_target_edge 当前目标边；
            3. B mask 的最小外接旋转矩形 4 条边；
            4. B mask 轮廓 approxPolyDP 后的各条边。
        """
        angles: List[float] = []

        def _add(angle: Any):
            try:
                angles.append(float(angle) % 180.0)
            except Exception:
                pass

        if status is not None:
            _add(status.get("b_edge_angle_deg"))

        try:
            edge = getattr(scene, "b_target_edge", None)
            if edge is not None and len(edge) == 2:
                _add(self._line_angle_deg_for_rule_ab_repair(edge[0], edge[1]))
        except Exception:
            pass

        b_mask = self._get_scene_b_mask_for_rule_ab_repair(scene)
        if b_mask is not None:
            try:
                m = (b_mask.astype(np.uint8) * 255)
                    ...
                    if cv2.contourArea(cnt) >= 5:
                        ...
                        for i in range(4):
                            ...
                            if float(np.linalg.norm(p2 - p1)) >= 3.0:
                                _add(self._line_angle_deg_for_rule_ab_repair(p1, p2))

                        # 轮廓近似后的边，给不规则 B 一个更丰富的候选集
                        peri = cv2.arcLength(cnt, True)
                        for eps_ratio in (0.01, 0.02, 0.04, 0.08):
                ...
                self.log(f"[RuleAB角度修复] 计算 B mask 候选边失败：{e}")

        return self._dedup_rule_ab_angles(angles, min_sep_deg=2.0)

    def repair_rule_ab_angle_between_step7_step8(self, rule_ab_result: Dict[str, Any]) -> Dict[str, Any]:
        """
        完整测量 Step 7 和 Step 8 之间执行的 B 角度修复。

        为什么放在这里：
            Step 7 完成 A 推动 B 后，B 的 SAM2 分割和角度可能因为某一帧 mask 不稳出现跳变；
            Step 8 关闭激光前，额外获取一帧 RuleAB scene，计算 B 的所有候选边角度，
            如果当前 raw angle 与上一稳定角度相差过大，则选取最接近上一稳定角度的候选边。
        """
        if not isinstance(rule_ab_result, dict):
            return rule_ab_result

        enable = bool(getattr(self.cfg, "rule_ab_b_angle_repair_enable", True))
        jump_th = float(getattr(self.cfg, "rule_ab_b_angle_jump_threshold_deg", 10.0))
        ...
        if not enable:
            info["reason"] = "repair_disabled"
            rule_ab_result["angle_repair_between_step7_step8"] = info
            return rule_ab_result

        follower = self.rule_ab_follower
        if follower is None:
            info["reason"] = "rule_ab_follower_is_none"
            rule_ab_result["angle_repair_between_step7_step8"] = info
            return rule_ab_result

        # 参考角度优先使用上一轮修复后的 B 角度；如果没有，则用本轮 Step 7 的 baseline_angle。
        reference_angle = self.context.get("rule_ab_last_repaired_b_angle")
        if reference_angle is None:
            reference_angle = rule_ab_result.get("baseline_angle")
        try:
            reference_angle = None if reference_angle is None else float(reference_angle) % 180.0
        except Exception:
            reference_angle = None
        info["reference_angle"] = reference_angle

        try:
            image_rgb, scene = follower.capture_and_build_scene()
            status = follower.get_boundary_rule_status(scene)
        except Exception as e:
            info["reason"] = f"capture_or_status_failed: {e}"
            rule_ab_result["angle_repair_between_step7_step8"] = info
            self.log(f"[RuleAB角度修复] Step 7.5 获取 B scene/status 失败，跳过修复：{e}")
            return rule_ab_result

        raw_angle = status.get("b_edge_angle_deg")
        try:
            raw_angle = None if raw_angle is None else float(raw_angle) % 180.0
        except Exception:
            raw_angle = None
        info["raw_angle"] = raw_angle

        candidates = self._candidate_b_edge_angles_for_rule_ab_repair(scene, status)
        info["candidate_angles"] = [float(a) for a in candidates]

        selected_angle = raw_angle
        if raw_angle is None:
            info["reason"] = "raw_angle_is_none"
        elif reference_angle is None:
            info["reason"] = "no_reference_use_raw"
        else:
            raw_delta = self._angle_diff_180_for_rule_ab_repair(raw_angle, reference_angle)
            info["raw_delta"] = raw_delta
            if raw_delta is not None and raw_delta <= jump_th:
                info["reason"] = "raw_angle_close_to_reference"
            elif candidates:
                selected_angle = min(
                    candidates,
                    key=lambda a: self._angle_diff_180_for_rule_ab_repair(float(a), float(reference_angle))
                    if self._angle_diff_180_for_rule_ab_repair(float(a), float(reference_angle)) is not None
                    else 999.0,
                )
                selected_delta = self._angle_diff_180_for_rule_ab_repair(selected_angle, reference_angle)
                info["used_repair"] = True
                info["selected_angle"] = float(selected_angle)
                info["selected_delta"] = selected_delta
                info["reason"] = "raw_jump_use_nearest_candidate_to_reference"
            else:
                info["reason"] = "raw_jump_but_no_candidates_use_raw"

        if selected_angle is None:
            selected_angle = raw_angle
        info["selected_angle"] = selected_angle

        old_final_angle = rule_ab_result.get("final_angle")
        rule_ab_result["final_angle_before_repair"] = old_final_angle
        rule_ab_result["final_angle"] = selected_angle
        rule_ab_result["angle_repair_between_step7_step8"] = info

        baseline_angle = rule_ab_result.get("baseline_angle")
        if selected_angle is not None and baseline_angle is not None:
            try:
                rule_ab_result["final_delta_before_repair"] = rule_ab_result.get("final_delta")
                rule_ab_result["final_delta"] = self.angle_diff_deg(float(selected_angle), float(baseline_angle))
            except Exception:
                pass

        if selected_angle is not None:
            self.context["rule_ab_last_repaired_b_angle"] = float(selected_angle)
            self.context["rule_ab_last_b_angle"] = float(selected_angle)

        self.context["last_rule_ab_angle_repair"] = info

        self.log(
            "[RuleAB角度修复] Step 7.5 完成："
            f"raw={raw_angle}, reference={reference_angle}, selected={selected_angle}, "
            f"used_repair={info.get('used_repair')}, reason={info.get('reason')}, "
            f"candidates={[round(float(a), 3) for a in candidates]}"
        )
    ...
    def ensure_rule_ab_follower(self) -> ActualNanoBoundaryFollower:
        ...
        cfg = RuleABRuntimeConfig(
            ...
            # 第一帧 SAM2 分割必须由用户确认；C mask 再拟合成四边形轮廓。
        ...
        for step_idx in range(1, int(self.cfg.rule_ab_max_steps) + 1):
            ...
            if final_delta is not None and final_delta >= min_delta:
                ...
                if final_delta > max_delta:
                    self.log(
                        f"[RuleAB] 角度变化 {final_delta:.3f}° 已超过上限 {max_delta:.3f}°，立即关闭激光"
                    )
                else:
    ...
    def run_one_cycle(self, cycle_index: int) -> bool:
        ...
        Step 7.5：对 Step 7 得到的 B 角度做跳变修复
        Step 8：当对边角度移动 5° 时，关闭激光
        ...
        try:
            ...
            try:
                ...
                self.log("========== Step 7.5：B角度跳变修复 ==========")
                rule_ab_result = self.repair_rule_ab_angle_between_step7_step8(rule_ab_result)
    ...
    def request_stop(self):
        ...
        # 停止测量后，兜底关闭激光；Rigol CH1/CH2 仍保持 OFF。
        try:
...
class MeasurementWorkflowGUI:
    ...
    def _build_ui(self):
        ...
        ttk.Label(rule_frame, text="面积阈值px²").grid(row=11, column=0, padx=4, pady=3, sticky="w")
        ttk.Entry(rule_frame, textvariable=self.rule_ac_area_threshold_var, width=10).grid(row=11, column=1, padx=4, pady=3, sticky="ew")
        ttk.Label(rule_frame, text="AC最大循环").grid(row=11, column=2, padx=4, pady=3, sticky="w")
        ttk.Entry(rule_frame, textvariable=self.rule_ac_max_cycles_var, width=10).grid(row=11, column=3, padx=4, pady=3, sticky="ew")

        rule_buttons = ttk.Frame(rule_frame)
        rule_buttons.grid(row=12, column=0, columnspan=4, padx=2, pady=6, sticky="ew")
        for i in range(4):
        ...
        ttk.Button(rule_buttons, text="单独测试Step9面积", command=self.test_rule_ac_thread).grid(row=0, column=3, padx=4, pady=3, sticky="ew")
    ...
    def _save_static_c_map_result(
        ...
        self.log(
            ...
            f"method={quad_method}；后续加载使用 static_c_mask.png=四边形mask"
        )
    ...
    def test_rule_ab(self):
        ...
        def _line_angle_deg(p1, p2) -> float:
            """
            计算一条边的方向角，范围归一到 0~180°。
            """
            x1, y1 = float(p1[0]), float(p1[1])
            x2, y2 = float(p2[0]), float(p2[1])
        ...
        def _dedup_angles(angles: List[float], min_sep_deg: float = 2.0) -> List[float]:
            out: List[float] = []
            for a in angles:
                try:
                    fa = float(a) % 180.0
                    ...
                    continue
                if all((_angle_diff_180(fa, b) or 999.0) > min_sep_deg for b in out):
                    out.append(fa)
            return out

        def _get_scene_b_mask(scene) -> Optional[np.ndarray]:
            """
            兼容不同 SceneGeometry / FlakeGeometry 实现，提取 B 的 mask。
            """
            try:
                b = getattr(scene, "b", None)
                if b is not None and getattr(b, "mask", None) is not None:
                    return np.asarray(getattr(b, "mask")).astype(bool)
            except Exception:
                pass
            try:
                seg = getattr(follower, "abc_segmenter", None)
                if seg is not None and getattr(seg, "last_b_mask", None) is not None:
                    return np.asarray(getattr(seg, "last_b_mask")).astype(bool)
            except Exception:
                pass
            return None

        def _get_scene_b_contour(scene) -> List[Tuple[float, float]]:
            try:
                b = getattr(scene, "b", None)
                contour = getattr(b, "contour", None) if b is not None else None
                if contour:
        ...
        def _candidate_b_edges(scene, status: Optional[Dict[str, Any]] = None) -> List[Dict[str, Any]]:
            """
            从当前 B 的所有可用几何信息里计算候选边。
            ...
            候选来源：
                1. status['b_edge_angle_deg'] 当前算法给出的角度；
                2. scene.b_target_edge 当前目标边；
                3. scene.b.contour 的多边形边；
                4. B mask 的 minAreaRect 四条边；
                5. B mask 的 approxPolyDP 轮廓边。
            """
            edges: List[Dict[str, Any]] = []

            def _add_edge(angle: Any, p1=None, p2=None, source: str = "unknown"):
                try:
                    fa = float(angle) % 180.0
                except Exception:
                    return
                ...
                if p1 is not None and p2 is not None:
                    try:
                        ...
                        length = float(math.hypot(fp2[0] - fp1[0], fp2[1] - fp1[1]))
                    except Exception:
                        fp1 = None
                        fp2 = None
                        length = 0.0

                # 去重：角度非常接近且来源不是 raw/status 时，不重复加入。
                # 但保留第一条，因为第一条通常来自当前主算法或目标边。
                for e in edges:
                    if (_angle_diff_180(fa, float(e.get("angle", 0.0))) or 999.0) <= 2.0:
                        # 如果新边有端点且旧边没有端点，则用新边补充端点信息。
                        if e.get("p1") is None and fp1 is not None:
                            e["p1"] = fp1
                            e["p2"] = fp2
                            e["length"] = length
                            e["source"] = str(e.get("source", "")) + "+" + source
                        return
            ...
            # 1. 当前算法的角度
            try:
                if status is not None and status.get("b_edge_angle_deg") is not None:
                    _add_edge(status.get("b_edge_angle_deg"), source="status_raw")
            except Exception:
                pass
            ...
            try:
                edge = getattr(scene, "b_target_edge", None)
                if edge is not None and len(edge) == 2:
                    _add_edge(_line_angle_deg(edge[0], edge[1]), edge[0], edge[1], source="scene.b_target_edge")
            except Exception:
                pass

            # 3. scene.b.contour 边
            try:
                contour = _get_scene_b_contour(scene)
                if len(contour) >= 2:
                    for i in range(len(contour)):
                        p1 = contour[i]
                        p2 = contour[(i + 1) % len(contour)]
                        if math.hypot(float(p2[0]) - float(p1[0]), float(p2[1]) - float(p1[1])) >= 3.0:
                            _add_edge(_line_angle_deg(p1, p2), p1, p2, source="scene.b.contour")
            except Exception:
                pass

            # 4/5. B mask 候选边
            b_mask = _get_scene_b_mask(scene)
            if b_mask is not None:
                try:
                    m = (b_mask.astype(np.uint8) * 255)
                    contours, _ = cv2.findContours(m, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
                    ...
                    if contours:
                        ...
                        for i in range(4):
                            ...
                            p2 = box[(i + 1) % 4]
                            if float(np.linalg.norm(p2 - p1)) >= 3.0:
                                _add_edge(_line_angle_deg(p1, p2), p1, p2, source="mask.minAreaRect")
                        ...
                        for eps_ratio in (0.01, 0.02, 0.04, 0.08):
                            ...
                            if len(pts) >= 2:
                                for i in range(len(pts)):
                                    ...
                                    p2 = pts[(i + 1) % len(pts)]
                                    if float(np.linalg.norm(p2 - p1)) >= 3.0:
                                        _add_edge(_line_angle_deg(p1, p2), p1, p2, source=f"mask.approxPolyDP_{eps_ratio}")
                except Exception:
                    pass

            # 优先把有端点、边长较长的候选排前面；status_raw 如果没有端点，一般只作为兜底。
            edges.sort(key=lambda e: (0 if e.get("p1") is not None else 1, -float(e.get("length", 0.0))))
            return edges

        def _candidate_b_edge_angles(scene, status: Optional[Dict[str, Any]] = None) -> List[float]:
            """仅返回候选角度，用于角度修复和兼容旧逻辑。"""
            return _dedup_angles([float(e["angle"]) for e in _candidate_b_edges(scene, status)], min_sep_deg=2.0)

        def _draw_b_edge_candidates_for_manual_select(image_rgb, scene, status: Dict[str, Any], edges: List[Dict[str, Any]], selected_idx: Optional[int] = None) -> np.ndarray:
            """
            生成 B 候选边人工选择预览图。
            按键说明会写在图上：1~9选择边，A自动选择，R刷新，ESC取消。
            """
            ...
            try:
                ...
                if b_mask is not None:
                    overlay = vis.copy()
                    overlay[b_mask.astype(bool)] = (0, 180, 255)
            ...
            colors = [
                ...
                (255, 0, 255), (255, 255, 0), (0, 128, 255), (128, 0, 255), (255, 128, 0),
            ]

            for idx, e in enumerate(edges[:9], start=1):
                p1 = e.get("p1")
                p2 = e.get("p2")
                angle = float(e.get("angle", 0.0))
                color = colors[(idx - 1) % len(colors)]
                thickness = 3 if selected_idx == idx else 2
                ...
                if p1 is not None and p2 is not None:
                    ...
                    cv2.putText(vis, f"{idx}: {angle:.1f}", (mx + 5, my - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.55, color, 2, cv2.LINE_AA)
                else:
                    cv2.putText(vis, f"{idx}: {angle:.1f} deg ({e.get('source')})", (15, 110 + idx * 22), cv2.FONT_HERSHEY_SIMPLEX, 0.55, color, 2, cv2.LINE_AA)

            header_lines = [
                "Manual select B initial edge angle",
                "Press 1-9: choose edge | A: auto | R: refresh | ESC: cancel",
                "The selected edge angle will be used as initial_b_angle / last_b_angle.",
            ]
            y = 25
            for line in header_lines:
                cv2.putText(vis, line, (12, y), cv2.FONT_HERSHEY_SIMPLEX, 0.62, (255, 255, 255), 3, cv2.LINE_AA)
                cv2.putText(vis, line, (12, y), cv2.FONT_HERSHEY_SIMPLEX, 0.62, (0, 0, 0), 1, cv2.LINE_AA)
                y += 25

            return vis

        def _manual_select_b_initial_angle(image_rgb, scene, status: Dict[str, Any], edges: List[Dict[str, Any]]) -> Tuple[Optional[float], Dict[str, Any]]:
            """
            人工确认 B 初始目标边。
            ...
            info = {
                ...
                "selected_index": None,
                "selected_angle": None,
                "candidate_angles": [float(e.get("angle", 0.0)) for e in edges],
                "candidate_sources": [str(e.get("source", "")) for e in edges],
                "reason": "not_selected",
                "request_refresh": False,
            }

            if not edges:
                info["reason"] = "no_edges"
                return None, info

            window_name = "Select B initial edge angle"
            try:
                vis = _draw_b_edge_candidates_for_manual_select(image_rgb, scene, status, edges)
                cv2.namedWindow(window_name, cv2.WINDOW_NORMAL)
                cv2.imshow(window_name, vis)
                cv2.resizeWindow(window_name, min(1400, vis.shape[1]), min(950, vis.shape[0]))

                self.log("[RuleAB-C][B角度初始化] 请在弹出的图像中按数字键 1~9 选择 B 的目标边；按 A 自动选择；按 R 刷新；按 ESC 取消。")

                while True:
                    key = cv2.waitKey(50) & 0xFF
                    if key == 255:
                        if self.rule_ab_only_stop_requested or self.rule_ab_controller_stop_requested:
                            info["reason"] = "stop_requested"
                            return None, info
                        continue

                    if ord('1') <= key <= ord('9'):
                        idx = key - ord('1') + 1
                        if 1 <= idx <= min(9, len(edges)):
                            selected = float(edges[idx - 1].get("angle", 0.0)) % 180.0
                            info["selected_index"] = idx
                            info["selected_angle"] = selected
                            info["reason"] = "manual_number_selected"
                            vis2 = _draw_b_edge_candidates_for_manual_select(image_rgb, scene, status, edges, selected_idx=idx)
                            cv2.imshow(window_name, vis2)
                            cv2.waitKey(250)
                            return selected, info

                    if key in (ord('a'), ord('A')):
                        info["reason"] = "manual_auto_requested"
                        return None, info

                    if key in (ord('r'), ord('R')):
                        info["reason"] = "manual_refresh_requested"
                        info["request_refresh"] = True
                        return None, info

                    if key in (27, ord('q'), ord('Q')):
                        info["reason"] = "manual_cancel_use_auto"
                        return None, info
            except Exception as e:
                info["reason"] = f"manual_window_failed:{e}"
                self.log(f"[RuleAB-C][B角度初始化] 人工选择窗口失败，退回自动选择：{e}")
                return None, info
            finally:
                try:
                    cv2.destroyWindow(window_name)
        ...
        def _select_nearest_angle_to_reference(candidates: List[float], ref_angle: Optional[float]) -> Optional[float]:
            if not candidates:
                return None
            if ref_angle is None:
                return float(candidates[0])
            return float(min(candidates, key=lambda a: (_angle_diff_180(a, ref_angle) or 999.0)))

        def _repair_b_angle(scene, status: Dict[str, Any], last_angle: Optional[float]) -> Tuple[Optional[float], Dict[str, Any]]:
            """
            B 角度跳变修复：
                - 如果 raw_angle 与 last_angle 差异不大，直接用 raw_angle；
                - 如果跳变超过阈值，则计算 B 所有候选边角度，选择最接近 last_angle 的角度。
            """
            raw_angle = status.get("b_edge_angle_deg")
            enable = bool(getattr(self.build_config_from_ui(), "rule_ab_b_angle_repair_enable", True))
            jump_th = float(getattr(self.build_config_from_ui(), "rule_ab_b_angle_jump_threshold_deg", 10.0))
            ...
            try:
                raw_angle = float(raw_angle) % 180.0
            except Exception:
                info["reason"] = "raw_angle_not_float"
                return None, info
            ...
            if last_angle is None:
                info["reason"] = "no_last_angle_use_raw"
                return raw_angle, info

            raw_delta = _angle_diff_180(raw_angle, last_angle)
            info["raw_delta"] = raw_delta

            if raw_delta is not None and raw_delta <= jump_th:
                info["reason"] = "raw_angle_close_to_last"
                return raw_angle, info

            candidates = _candidate_b_edge_angles(scene, status)
            info["candidate_angles"] = [float(a) for a in candidates]
            selected = _select_nearest_angle_to_reference(candidates, last_angle)

            if selected is None:
                info["reason"] = "jump_but_no_candidate_use_raw"
                return raw_angle, info

            selected_delta = _angle_diff_180(selected, last_angle)
            info["used_repair"] = True
            info["selected_angle"] = float(selected)
            info["selected_delta"] = selected_delta
            info["reason"] = "jump_use_nearest_candidate_to_last"
            return float(selected), info

        def _auto_initialize_b_angle_from_multiframe(frame_candidates: List[List[float]], raw_angles: List[float]) -> Optional[float]:
            """
            自动模式：从多帧候选角度中选出跨帧最稳定的一组角度。
            该函数只在用户按 A/ESC 或人工窗口失败时作为兜底。
            """
            all_candidates = [a for cs in frame_candidates for a in cs]
            all_candidates = _dedup_angles(all_candidates, min_sep_deg=1.0)

            if not all_candidates:
                if raw_angles:
                    init_angle = float(np.median(raw_angles)) % 180.0
                    self.log(f"[RuleAB-C][B角度初始化] 无候选边，自动使用 raw 中位数：{init_angle:.3f}°")
                    return init_angle
                self.log("[RuleAB-C][B角度初始化] 没有获得有效 B 角度，后续第一帧使用 raw_angle")
                return None

            scored: List[Tuple[float, float, float]] = []
            for cand in all_candidates:
                diffs = []
                for cs in frame_candidates:
                    if not cs:
                        continue
                    diffs.append(min((_angle_diff_180(cand, x) or 999.0) for x in cs))
                if diffs:
                    scored.append((float(np.median(diffs)), float(np.max(diffs)), float(cand)))

            if not scored:
                init_angle = float(all_candidates[0])
                self.log(f"[RuleAB-C][B角度初始化] 自动评分失败，使用第一个候选角度：{init_angle:.3f}°")
                return init_angle

            scored.sort(key=lambda x: (x[0], x[1]))
            median_spread, max_spread, init_angle = scored[0]
            max_allowed_spread = float(getattr(self.build_config_from_ui(), "rule_ab_b_angle_init_max_spread_deg", 5.0))

            msg = (
                f"[RuleAB-C][B角度初始化] 自动选定初始角度={init_angle:.3f}°，"
                f"median_spread={median_spread:.3f}°, max_spread={max_spread:.3f}°, "
                f"allowed_max_spread={max_allowed_spread:.3f}°"
            )
            if max_spread > max_allowed_spread:
                msg += "；警告：初始角度跨帧波动偏大，建议检查 B 分割质量"
            self.log(msg)
            return float(init_angle)

        def _initialize_b_angle_before_motion(init_frames: int, init_interval_s: float) -> Optional[float]:
            """
            正式运动前的 B 初始目标边角度标定。

            新逻辑：
                1. 先采集 1 帧并显示 B 的所有候选边；
                2. 用户按数字键 1~9 手动选择要跟踪的 B 边；
                3. 被选中的角度作为 initial_b_angle / last_b_angle；
                4. 如果用户按 A / ESC 或窗口失败，则退回旧的自动多帧稳定选择。

            这样后续角度修复不再由程序猜测初始边，而是围绕用户确认的目标边进行修复。
            """
            init_frames = max(1, int(init_frames))
            init_interval_s = max(0.0, float(init_interval_s))
            manual_enable = bool(getattr(self.build_config_from_ui(), "rule_ab_b_angle_manual_select_enable", True))

            self.log(
                f"[RuleAB-C] 开始初始化 B 目标边角度：manual_select={manual_enable}；"
                f"自动兜底采集 {init_frames} 帧，不运动"
            )

            # ----------------------------------------------------
            # 1. 人工选择：先采集当前帧，显示 B 的候选边编号。
            # ----------------------------------------------------
            if manual_enable:
                manual_refresh_max = 3
                for refresh_idx in range(1, manual_refresh_max + 1):
                    if self.rule_ab_only_stop_requested or self.rule_ab_controller_stop_requested:
                        return None
                    try:
                        image_rgb_m, scene_m = follower.capture_and_build_scene()
                        status_m = follower.get_boundary_rule_status(scene_m)
                        edges_m = _candidate_b_edges(scene_m, status_m)

                        raw_m = status_m.get("b_edge_angle_deg")
                        self.log(
                            f"[RuleAB-C][B角度初始化] 人工选择预览 refresh={refresh_idx}/{manual_refresh_max}, "
                            f"raw={raw_m}, candidates={[round(float(e.get('angle', 0.0)), 3) for e in edges_m[:9]]}"
                        )

                        selected, manual_info = _manual_select_b_initial_angle(
                            image_rgb=image_rgb_m,
                            scene=scene_m,
                            status=status_m,
                            edges=edges_m,
                        )
                        ...
                        if selected is not None:
                            self.log(
                                f"[RuleAB-C][B角度初始化] 人工确认 B 初始目标边："
                                f"edge_index={manual_info.get('selected_index')}, "
                                f"angle={float(selected):.3f}°, "
                                f"source={manual_info.get('candidate_sources')}"
                            )
                            return float(selected) % 180.0

                        if manual_info.get("request_refresh", False):
                            self.log("[RuleAB-C][B角度初始化] 用户请求刷新 B 候选边，重新截图/分割")
                            continue

                        self.log(
                            f"[RuleAB-C][B角度初始化] 未人工选择目标边，原因={manual_info.get('reason')}；"
                            "退回自动多帧稳定选择"
                        )
                        break
                    except Exception as e:
                        self.log(f"[RuleAB-C][B角度初始化] 人工选择预览失败：{e}，退回自动选择")
                        self.log(traceback.format_exc())
                        break

            # ----------------------------------------------------
            # 2. 自动兜底：保留上一版多帧稳定选择逻辑。
            # ----------------------------------------------------
            frame_candidates: List[List[float]] = []
            raw_angles: List[float] = []

            for k in range(1, init_frames + 1):
                if self.rule_ab_only_stop_requested or self.rule_ab_controller_stop_requested:
                    return None
                try:
                    image_rgb0, scene0 = follower.capture_and_build_scene()
                    status0 = follower.get_boundary_rule_status(scene0)
                    raw0 = status0.get("b_edge_angle_deg")
                    if raw0 is not None:
                        raw_angles.append(float(raw0) % 180.0)
                    candidates0 = _candidate_b_edge_angles(scene0, status0)
                    if raw0 is not None:
                        candidates0 = _dedup_angles([float(raw0)] + candidates0, min_sep_deg=2.0)
                    frame_candidates.append(candidates0)
                    self.log(
                        f"[RuleAB-C][B角度初始化-自动兜底] init_frame={k}/{init_frames}, "
                        f"raw={raw0}, candidates={[round(float(a), 3) for a in candidates0]}"
                    )
                except Exception as e:
                    self.log(f"[RuleAB-C][B角度初始化-自动兜底] 第 {k} 帧失败：{e}")
                    self.log(traceback.format_exc())
                if init_interval_s > 0:
                    time.sleep(init_interval_s)

            return _auto_initialize_b_angle_from_multiframe(frame_candidates, raw_angles)
        ...
        try:
            ...
            lost_count = 0
            # 正式运动前先确定 B 的目标边初始角度。
            # 后续每帧如果 raw B_angle 相对该角度/上一帧角度跳变过大，
            # 会从 B 的所有候选边中选择最接近上一帧稳定角度的结果。
            init_b_angle_frames = int(getattr(cfg_gui, "rule_ab_b_angle_init_frames", 5))
            init_b_angle_interval_s = float(getattr(cfg_gui, "rule_ab_b_angle_init_interval_s", 0.08))
            last_b_angle: Optional[float] = _initialize_b_angle_before_motion(
                init_frames=init_b_angle_frames,
                init_interval_s=init_b_angle_interval_s,
            )
            ...
            while not self.rule_ab_only_stop_requested:
                ...
                raw_b_angle = status.get("b_edge_angle_deg")
                b_angle, b_angle_repair_info = _repair_b_angle(scene, status, last_b_angle)

                # 把修复后的角度写回 status，后续日志、CSV 和可视化都统一使用修复后的角度。
                status["b_edge_angle_raw_deg"] = raw_b_angle
                status["b_edge_angle_deg"] = b_angle
                status["b_edge_angle_repair"] = b_angle_repair_info

                d_angle = _angle_diff_180(b_angle, last_b_angle)
                if b_angle is not None:
                    last_b_angle = float(b_angle)
                ...
                self.log(
                    ...
                    f"B_edge_angle_raw={raw_b_angle}, B_edge_angle={b_angle}, d_angle={d_angle}, "
                    f"angle_repair={bool(b_angle_repair_info.get('used_repair', False))}, "
                    f"repair_reason={b_angle_repair_info.get('reason')}, "
    ...
    def test_rule_ac(self):
        """
        RuleAC 单独测试：运行过程中支持修改 GUI 输入框。

        修改后的行为：
            1. 每一轮 RuleAC run_one_cycle() 前，重新读取 GUI 当前输入；
            2. 阈值、max_cycles、stage_axis、stage_direction、stage_step_size、enable_stage 等参数下一轮生效；
            3. 如果参数变化导致 workflow.rule_ac_controller 被置空，则自动重新初始化 RuleACController；
            4. 下一次点击 RuleAC 测试也会使用最新 GUI 参数。
        """
        try:
            self.set_var(self.rule_module_status_var, "规则模块状态：RuleAC测试中，可运行中修改参数")
            self.log("========== RuleAC 单独测试：运行中参数可修改，下一轮立即生效 ==========")
            ...
            while True:
                # 每轮开始前同步 GUI -> workflow.cfg。
                wf = self.sync_config_from_ui_to_workflow()
                cfg_live = wf.cfg

                max_cycles_live = int(getattr(cfg_live, "rule_ac_max_cycles", 50))
                if cycle_idx >= max_cycles_live:
                    self.log(
                        f"[RuleAC] 已达到当前 GUI max_cycles={max_cycles_live}，停止单独测试"
                    )
                    break

                # 如果 sync_config_from_ui_to_workflow() 检测到 RuleAC 关键参数变化，
                # 会把 wf.rule_ac_controller = None。这里会自动重建。
                controller = wf.ensure_rule_ac_controller()

                # 对已有 controller 进行运行时参数同步，避免只改 cfg 但 controller 内部仍用旧值。
                if hasattr(controller, "cfg"):
                    try:
                        controller.cfg.area_threshold_px = float(getattr(cfg_live, "rule_ac_area_threshold_px", controller.cfg.area_threshold_px))
                        controller.cfg.enable_stage = bool(getattr(cfg_live, "rule_ac_enable_stage", controller.cfg.enable_stage))
                        controller.cfg.max_cycles = int(getattr(cfg_live, "rule_ac_max_cycles", controller.cfg.max_cycles))
                        controller.cfg.stage_axis = str(getattr(cfg_live, "rule_ac_stage_axis", controller.cfg.stage_axis))
                        controller.cfg.stage_direction = int(getattr(cfg_live, "rule_ac_stage_direction", controller.cfg.stage_direction))
                        controller.cfg.stage_step_size = int(getattr(cfg_live, "rule_ac_stage_step_size", controller.cfg.stage_step_size))
                        controller.cfg.area_threshold_px = float(getattr(cfg_live, "rule_ac_area_threshold_px", controller.cfg.area_threshold_px))
                    except Exception as e:
                        self.log(f"[RuleAC] 运行时同步 controller.cfg 失败：{e}")

                cycle_idx += 1

                self.log(
                    f"[RuleAC] cycle={cycle_idx}, "
                    f"threshold_px={getattr(cfg_live, 'rule_ac_area_threshold_px', None)}, "
                    f"enable_stage={getattr(cfg_live, 'rule_ac_enable_stage', None)}, "
                    f"stage_axis={getattr(cfg_live, 'rule_ac_stage_axis', None)}, "
                    f"stage_direction={getattr(cfg_live, 'rule_ac_stage_direction', None)}, "
                    f"stage_step_size={getattr(cfg_live, 'rule_ac_stage_step_size', None)}, "
                    f"max_cycles={max_cycles_live}"
                )

                last_continue = controller.run_one_cycle()

                history_len = len(getattr(controller, "history", []) or [])
                if history_len > 0:
                    last_row = controller.history[-1]
                else:
                    last_row = {}

                csv_path = str(getattr(controller, "csv_path", ""))

                self.set_var(
                    self.rule_module_status_var,
                    f"规则模块状态：RuleAC测试中 cycle={cycle_idx}, "
                    f"area_px={last_row.get('area_px')}, threshold={getattr(cfg_live, 'rule_ac_area_threshold_px', None)}"
                )

                if not last_continue:
                    self.log("[RuleAC] controller.run_one_cycle() 返回 False，停止单独测试")
                    break

                # 小间隔，防止 UI 参数刚输入时过快连续执行。
                time.sleep(0.05)
            ...
            if self.workflow is not None:
                self.workflow.context["last_rule_ac_result"] = result
            ...
            if self.workflow is not None:
                self.refresh_result_labels(self.workflow)

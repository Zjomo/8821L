...
class MeasurementWorkflowGUI:
    ...
    def _save_static_c_map_result(
        ...
        mask_layer[c_bool] = (255, 255, 0)
        overlay = cv2.addWeighted(mask_layer, 0.35, overlay, 0.65, 0)
    ...
    def segment_c_only(self):
        ...
        单独分割 C：
            ...
            3. 保存 static_c_mask.png 等固定 C 文件；
            4. 自动把保存目录写入“C标定文件夹”，后续“单独A沿C绕行”会加载该 C。
        """
        try:
            self.set_var(self.rule_module_status_var, "规则模块状态：正在单独分割C")
            self.log("========== 单独分割C：只分割并保存 C，不分割 A/B，不移动控制器 ==========")
            ...
            self.set_var(
                ...
                f"规则模块状态：C分割完成，已保存并加载：{saved_dir}",
            )
            self.log(f"[RuleAB-C] C 分割结果已保存，并设置为后续加载目录：{saved_dir}")

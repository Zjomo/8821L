...
class MeasurementWorkflow:
    ...
    def _execute_stage12_diagonal_action(self, action_code: int) -> Dict[str, Any]:
        """
        按 1/2 通道执行 Step9 运动，使“检测区域中心”向目标点靠近。

        重要修正：
            之前代码把“平台/背景的物理运动方向”直接当成了
            “图像中检测区域中心的运动方向”。实际运行日志显示二者相反：
            当目标在右上方、程序发送“右上”动作时，下一帧检测中心反而向左下远离目标。

        因此这里采用“图像中心运动方向”的反向补偿映射：
            希望检测中心向右上  -> 发送原左下命令
            希望检测中心向左下  -> 发送原右上命令
            希望检测中心向左上  -> 发送原右下命令
            希望检测中心向右下  -> 发送原左上命令
        ...
        action_code 仍然表示“希望检测区域中心在图像中移动的方向”，
        ch1_distance/ch2_distance 才是最终发送给 1/2 通道的相对步数。
        """
        step = abs(float(getattr(self.cfg, "rule_ac_stage_step_size", 20)))

        # action_name 表示“希望检测区域中心在图像中移动的方向”。
        # ch1_distance/ch2_distance 是为实现该图像方向而发送给 1/2 通道的反向补偿步数。
        mapping = {
            1:   (-step, 0.0,   "右上"),
            -1:  (step,  0.0,   "左下"),
            2:   (0.0,  -step,  "左上"),
        ...
        self.log(
            f"[Step9-BC中心] 希望检测中心向{name}移动，发送反向补偿："
            f"CH1={d1}, CH2={d2} "
            f"[已修正：以检测中心距离目标变小为准]"
        )

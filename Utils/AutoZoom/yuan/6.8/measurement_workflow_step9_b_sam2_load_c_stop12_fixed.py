...
class MeasurementWorkflow:
    ...
    def prepare_step9_b_segmentation(self):
        ...
        """
        output_dir = self.output_root / "step9_b_select"
        output_dir.mkdir(parents=True, exist_ok=True)
        ...
        # 保存初始 B 分割检查图
        try:
            out_dir = self.output_root / "step9_b_select"
    ...
    def _capture_bc_overlap_center_once(self) -> Dict[str, Any]:
        """
        截图，加载固定 C mask，并调用 SAM2 分割/更新 B，
        然后计算 B∩C 的重合区域中心。

        重要：
            - C 不再在 Step9 中重新分割，而是从“选择C文件夹”加载 static_c_mask.png；
            - B 每帧都调用 SAM2 更新；
            - 如果还没有 B 点提示，会返回 b_prompt_not_ready。
        """
        output_dir = self.output_root / "step9_bc_center_align"
        output_dir.mkdir(parents=True, exist_ok=True)
        image_rgb = self._capture_current_rule_ab_frame(output_dir=output_dir)

        c_mask = self._load_step9_c_mask_from_config(image_shape=image_rgb.shape[:2])
        ...
        # 每帧都调用 SAM2 分割/跟踪 B：
        # 初始帧：人工 B 正点/负点；
        # 后续帧：上一帧 B 中心作为正点，上一帧 bbox 作为 box 约束。
        try:
            if self.step9_b_last_center is not None:
    ...
    def _save_step9_bc_overlay(
        ...
        """
        保存 Step9 B∩C 中心对齐的检查图。
        """
        try:
            out_dir = self.output_root / "step9_bc_center_align"
            out_dir.mkdir(parents=True, exist_ok=True)
            ...
            if overlap_mask is not None:
                m = np.asarray(overlap_mask).astype(bool)
                overlay = canvas.copy()
                overlay[m] = (0, 255, 255)
                canvas = cv2.addWeighted(overlay, 0.35, canvas, 0.65, 0)
            ...
            ts = datetime.now().strftime("%Y%m%d_%H%M%S_%f")[:-3]
            path = out_dir / f"step9_bc_center_cycle_{cycle_idx:04d}_{ts}_action_{action_code}.png"
            cv2.imwrite(str(path), canvas)
            return str(path)
        except Exception as e:
            self.log(f"[Step9-BC中心] 保存检查图失败：{e}")
            return None
    ...
    def run_rule_ac_until_threshold(self) -> Dict[str, Any]:
        ...
        不再判断面积阈值，而是：
            1. 截图并更新 B/C mask；
            2. 计算 overlap = B_mask ∩ C_mask；
            3. 计算 overlap 中心；
            4. 与提前选定的目标点比较；
            5. 偏差超过容差时，按 +1/-1/+2/-2 对角方向运动 1/2 通道；
            6. 直到中心重合或达到 max_cycles。
        """
        ...
        for cycle_idx in range(1, max_cycles + 1):
            ...
            if abs_dx <= tol and abs_dy <= tol:
                ...
                overlay_path = self._save_step9_bc_overlay(
                    det.get("image_rgb"),
            ...
            time.sleep(float(getattr(self.cfg, "rule_ac_loop_interval_s", 0.10)))
...
class MeasurementWorkflowGUI:
    ...
    def test_rule_ac(self):
        ...
        新逻辑：
            1. 必须有 Step9 目标点；
            2. 必须加载 C 文件夹中的 static_c_mask.png；
            3. 测试开始时调用 SAM2 对 B 进行初始分割；
            4. 后续每帧继续用 SAM2 更新 B；
            5. 计算 B∩C 重合中心并用 1/2 通道对齐；
            6. 可通过“停止1/2轴运动”按钮立即停止。
        """
        try:
            self.set_var(self.rule_module_status_var, "规则模块状态：Step9 B∩C中心对齐测试中")
            self.log("========== Step9 单独测试：B∩C 重合中心对齐 ==========")
            ...
            # 检查 C 文件夹。Step9 单独测试必须加载固定 C mask。
            c_dir_text = str(self.rule_ab_static_c_map_dir_var.get()).strip()
            resolved_c = self._resolve_static_c_reference_dir(c_dir_text) if c_dir_text else None
            if resolved_c is None:
                msg = (
                    "Step9 对齐需要先加载 C mask。\n\n"
                    "请先点击“单独分割C”生成 C 标定结果，"
                    "或者点击“选择C文件夹”选择包含 static_c_mask.png 的文件夹。"
                )
                self.log("[Step9-BC中心] 未找到有效 C mask，无法计算 B∩C 重合区域。")
                self.root.after(0, lambda: messagebox.showwarning("Step9缺少C mask", msg))
                self.set_var(self.rule_module_status_var, "规则模块状态：Step9缺少C mask")
                return

            self.rule_ab_static_c_map_dir_var.set(str(resolved_c))
            self.rule_ab_load_static_c_map_var.set(True)
            self.rule_ab_force_reselect_c_var.set(False)
            ...
            # 重置 Step9 停止状态和上一轮 B 跟踪结果；每次单独测试都重新选择/分割 B。
            ...
            self.log(
                ...
                f"C_mask_dir={wf.cfg.rule_ab_static_c_map_dir}, "

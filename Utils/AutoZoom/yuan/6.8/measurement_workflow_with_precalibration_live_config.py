...
try:
    from config import DEFAULT_CONFIG  # 新版：GUI 左侧所有输入项的默认值
except Exception:
    try:
        from config_conditional_second_detect import DEFAULT_CONFIG  # 旧版兼容
    except Exception:
        DEFAULT_CONFIG = {}
...
class MeasurementWorkflow:
    ...
    def run_rule_ab_until_angle_delta(
        ...
        while True:
            ...
            # 每一步开始前同步 GUI 当前参数。
            # 这样运行完整循环测量过程中修改左侧第 8 区域的数值，
            # 下一步 RuleAB 动作就会按新数值执行。
            ...
            # 如果调用方没有显式传入范围，则每一步都从 GUI/配置读取最新目标范围。

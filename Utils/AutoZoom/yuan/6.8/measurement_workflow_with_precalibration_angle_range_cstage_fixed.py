...
class MeasurementConfig:
    ...
    rule_ab_angle_delta_max_deg: float = float(_cfg("rule_ab_angle_delta_max_deg", 5.5))
...
class MeasurementWorkflow:
    ...
    def apply_calibration_state(self, state: CalibrationState):
        ...
        # C map：完整测量中的 RuleAB/Step9 优先加载这个 static C。
        resolved_c_dir = self._resolve_static_c_dir_from_state(state)
        if resolved_c_dir:
    ...
    def run_rule_ab_until_angle_delta(
        ...
        # 关键修改：
        # 不再使用 rule_ab_max_steps=15 作为 Step7 的结束条件。
        # Step7 会一直执行 follower.run_one_cycle() 并检测角度，直到：
        #   1) 角度变化进入 [min_delta, max_delta]，本轮成功；
        #   2) 角度变化超过 max_delta，判定过冲并停止完整测量；
        #   3) 用户请求停止；
        #   4) RuleAB 返回 False；
        #   5) 角度检测长期失败只能由用户停止或 RuleAB 返回 False 退出。
        step_idx = 0
        while True:
            ...
            step_idx += 1
            self.log(f"[RuleAB] step={step_idx}: 执行一次 rule_ab.run_one_cycle()；不再受 rule_ab_max_steps 限制")
            ...
            if final_delta is not None:
                if min_delta <= float(final_delta) <= max_delta:
                    ...
                    self.log(
                        ...
                        f"目标范围=[{min_delta:.3f}, {max_delta:.3f}]，Step7 成功"
                    )
                    break

                if float(final_delta) > max_delta:
                    reached = False
                    stop_reason = "angle_delta_overshoot"
                    self.log(
                        f"[RuleAB] 角度变化 {final_delta:.3f}° 已超过上限 {max_delta:.3f}°，"
                        "本轮 Step7 判定失败，后续不允许进入下一轮完整测量"
                    )
                    break
    ...
    def run_one_cycle(self, cycle_index: int) -> bool:
        ...
        Step 8：当对边角度相对 Step1 进入 3.5°~5.5° 时关闭激光；否则停止完整测量
        ...
        try:
            ...
            # 关键修改：
            # Step7 必须使 B 对边角度相对本轮第一次检测角度变化进入 3.5°~5.5°。
            # 没有达到、过冲、RuleAB失败或用户停止，都不允许进入下一轮完整循环测量。
            if not bool(rule_ab_result.get("ok", False)):
                ...
                self.log(
                    "[RuleAB] Step7 未达到允许进入下一轮的角度目标，停止完整循环测量："
...
class MeasurementWorkflowGUI:
    ...
    def sync_config_from_ui_to_workflow(self) -> MeasurementWorkflow:
        ...
        if loaded_state is not None:
            ...
            # 完整测量运行中，Step7 必须真动 Stage34，Step9 必须真动 Stage12；
            # 不允许被 GUI 默认值或旧配置覆盖成 dry-run。
            if self.workflow.is_measuring:
                self.workflow.cfg.rule_ab_enable_stage = True
                self.workflow.cfg.rule_ac_enable_stage = True
    ...
    def run_workflow(self):
        ...
        try:
            ...
            # 即使 RuleAB/RuleAC 单独测试面板里还残留旧的“加载C”目录，
            # 这里也强制使用 current_calibration.json 里最新标定的 static_c_map_dir。
            self.set_var(self.rule_ab_static_c_map_dir_var, str(calibration_state.static_c_map_dir))
            self.set_var(self.rule_ab_load_static_c_map_var, True)
            self.set_var(self.rule_ab_force_reselect_c_var, False)
            self.set_var(self.rule_ab_enable_stage_var, True)
            self.set_var(self.rule_ac_enable_stage_var, True)
            ...
            self.log(
                ...
                f"本次完整测量使用标定包中的 C：{calibration_state.static_c_map_dir}"
            )

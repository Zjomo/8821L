...
class MeasurementWorkflow:
    ...
    def _predict_step9_c_mask_by_sam2(
        ...
        初始帧使用人工选择的 C 正负点；
        后续帧优先使用上一帧 C 中心点 + 上一帧 bbox 更新。
        这样 Step9 中的 C mask 会跟随当前帧变化，不再使用固定 static_c_mask。
        """
        import torch
        ...
        scores_arr = np.asarray(scores).reshape(-1)
        best_idx = int(np.argmax(scores_arr))
        c_mask = masks[best_idx].astype(bool)
        ...
        self.log(
            f"[Step9-BC中心] SAM2 C 动态分割完成：score={float(scores_arr[best_idx]):.6f}, "
            f"area={area} px, center={center}, bbox={bbox}"
        )
        return c_mask

...
class CalibrationState:
    ...
    def missing_items(self) -> List[str]:
        missing: List[str] = []
        if not self.angle_b_positive_points:
            missing.append("角度检测：B mask 正点")
        ...
        if not self.rule_ab_b_positive_points and not self.angle_b_positive_points:
            missing.append("A推动B：B mask 正点（可单独标定RuleAB-B，或复用角度检测B点）")

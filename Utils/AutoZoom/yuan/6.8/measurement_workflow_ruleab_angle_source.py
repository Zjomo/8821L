...
class CalibrationState:
    ...
    def missing_items(self) -> List[str]:
        ...
        # 关键修改：完整测量不再依赖独立 ScreenAngleDetector 的
        # “1 标定角度B/边”。Step1/Step7 的角度都直接来自 RuleAB 当前
        # B mask/annotated_frames，因此这里只要求 RuleAB-A、RuleAB-B、C、Step9。
        if not self.rule_ab_a_positive_points:
            missing.append("A推动B：A mask 正点")
        if not self.rule_ab_b_positive_points:
            missing.append("A推动B：B mask 正点")
...
class MeasurementWorkflow:
    ...
    def _complete_calibration_state_for_runtime(self, state: CalibrationState) -> CalibrationState:
        ...
        # 关键修改：完整测量角度直接来自 RuleAB-B 的当前 mask，
        # 不再把“角度检测B/边”自动复用为 RuleAB-B。
        # 因此 RuleAB-B 必须通过“RuleAB-B 标定”显式保存。
    ...
    def preflight_check_calibration(self) -> CalibrationState:
        ...
        self.log(
            "[完整测量标定] 启动前检查通过："
            "角度来源=RuleAB当前B mask/annotated_frames，"
            f"A点={len(state.rule_ab_a_positive_points)}/{len(state.rule_ab_a_negative_points)}, "
            f"B点={len(state.rule_ab_b_positive_points)}/{len(state.rule_ab_b_negative_points)}, "
            f"C点={len(state.global_c_positive_points)}/{len(state.global_c_negative_points)}, "
    ...
    def connect_measurement_devices(self):
        ...
        # 完整测量的角度已经改为来自 RuleAB 当前 B mask / annotated_frames，
        # 因此这里不再强制初始化 ScreenAngleDetector，避免完整流程产生
        # outputs/captured_frames/.../angle_results 角度来源。
        # 如需手动“单次角度检测”，对应按钮仍会按旧逻辑初始化/使用角度模块。

        self.log("========== 实验设备初始化完成：照明光 + 激光控制器；完整测量角度来源=RuleAB annotated_frames ==========")
        self.notify_update()
    ...
    def _get_latest_rule_ab_annotated_frame_path(self, follower: Any = None) -> str:
        """
        返回 RuleAB 输出目录下最新的 annotated_frames 图像路径。

        说明：角度数值本身来自 RuleAB 当前 B mask；这个路径用于把该角度
        关联到 measurement_output/rule_ab_from_measurement/run_xxx/annotated_frames，
        避免再使用 outputs/captured_frames/.../angle_results。
        """
        candidates: List[Path] = []
        base_dirs: List[Path] = []
        ...
        try:
            ...
            if od:
                base_dirs.append(Path(str(od)))
        except Exception:
            pass

        try:
            base_dirs.append(self.output_root / "rule_ab_from_measurement")
        except Exception:
            pass
        ...
        for base in base_dirs:
            try:
                if not base.exists():
                    continue
                for sub in base.rglob("annotated_frames"):
                    if not sub.is_dir():
                        continue
    ...
    def _get_rule_ab_current_bmask_angle_candidates(
        self,
        follower: Any,
        refresh_scene: bool = False,
    ) -> List[Dict[str, Any]]:
        """
        用 RuleAB 当前帧的 B mask 计算所有候选边角度。

        关键修改：
            完整测量 Step1/Step7 不再调用 ScreenAngleDetector，
            不再使用 outputs/captured_frames/.../angle_results。
            角度直接来自 RuleAB/SAM2 当前跟踪出的 B mask，并与
            rule_ab_from_measurement/run_xxx/annotated_frames 的当前帧对应。
        """
        if follower is None:
            return []

        def try_scene(scene_obj: Any) -> List[Dict[str, Any]]:
            try:
                b_mask = self._extract_mask_from_scene_object(getattr(scene_obj, "b", None))
        ...
        # 优先使用 run_one_cycle / initialize 后缓存的当前 scene，避免额外调用独立角度检测。
        for attr in (
            "last_scene", "current_scene", "scene",
            "last_valid_scene", "latest_scene", "_last_scene",
        ):
            scene = getattr(follower, attr, None)
            if scene is None:
                continue
            cands = try_scene(scene)
            if cands:
                return cands
        ...
        for obj in (follower, getattr(follower, "abc_segmenter", None), getattr(follower, "segmenter", None)):
            if obj is None:
                continue
            for attr in (
                "last_b_mask", "current_b_mask", "b_mask", "last_mask_b",
                "last_B_mask", "current_B_mask",
            ):
                try:
        ...
        # Step1 初始角度可能还没有缓存 scene；这时只允许调用 RuleAB 自己的
        # capture_and_build_scene() 建立当前 A/B/C 场景，仍然不会调用 ScreenAngleDetector。
        if refresh_scene:
            try:
                ...
                self.log(f"[RuleAB角度] 从 RuleAB capture_and_build_scene 获取 B mask 失败：{e}")
    ...
    def measure_rule_ab_bmask_angle_once(
        ...
        """
        从 RuleAB 当前 B mask 计算一次角度。

        选择边规则：
            - 如果 previous_angle 不为空，选与上一帧角度最接近的边，保持同一条边；
            - 如果 previous_angle 为空，选最长边作为初始基准边。

        这替代完整测量中的 ScreenAngleDetector 角度检测，因此完整流程不再需要
        “1 标定角度B/边”。
        """
        follower = follower or self.ensure_rule_ab_follower()
        self.log(f"========== RuleAB-Bmask角度：{label} ==========")

        candidates = self._get_rule_ab_current_bmask_angle_candidates(
            follower,
            refresh_scene=bool(refresh_scene),
        )
        annotated_path = self._get_latest_rule_ab_annotated_frame_path(follower)
        ...
        if not candidates:
            result = {
                ...
                "angle_deg": None,
                "angle_deg_raw": None,
                "reason": "no_rule_ab_bmask_edge_candidates",
                "source": "rule_ab_current_b_mask",
                "annotated_frame_path": annotated_path,
                "image_path": annotated_path,
                "candidate_angles": [],
            }
            self.log(f"[RuleAB-Bmask角度-{label}] 失败：未能从 RuleAB 当前 B mask 得到候选边角度。")
            if annotated_path:
                self.log(f"[RuleAB-Bmask角度-{label}] latest_annotated_frame={annotated_path}")
            if not allow_fail:
                raise RuntimeError(f"{label} RuleAB-Bmask角度失败：{result}")
            return result

        if previous_angle is not None:
            selected = min(candidates, key=lambda c: self.angle_diff_deg(float(c["angle_deg"]), float(previous_angle)))
            select_reason = "nearest_to_previous_angle"
        else:
            selected = max(candidates, key=lambda c: float(c.get("length_px", 0.0)))
            select_reason = "longest_edge_initial"
        ...
        result = {
            ...
            "angle_deg_raw": angle,
            "source": "rule_ab_current_b_mask",
            "source_note": "computed_from_RuleAB_SAM2_B_mask_not_ScreenAngleDetector",
            "annotated_frame_path": annotated_path,
            "image_path": annotated_path,
            "selected_edge": self._json_safe(selected),
            "candidate_count": len(candidates),
            "candidate_angles": [float(c["angle_deg"]) for c in candidates],
            "select_reason": select_reason,
            "conditional_second_detect_triggered": False,
        }
        self.log(
            f"[RuleAB-Bmask角度-{label}] angle_deg={angle:.6f}, "
            f"select_reason={select_reason}, candidates={result['candidate_angles']}"
        )
        if annotated_path:
            self.log(f"[RuleAB-Bmask角度-{label}] annotated_frame={annotated_path}")
        return result
    ...
    def run_rule_ab_until_angle_delta(
        ...
        while True:
            ...
            # 关键修改：Step7 的角度来自 RuleAB 当前 B mask / annotated_frames，
            # 不再调用 detect_angle_once()，因此不会再使用
            # outputs/captured_frames/.../angle_results 中的角度。
    ...
    def _execute_stage12_diagonal_action(
        self,
        action_code: int,
        ...
        注意：action_code 表示“希望检测区域中心在图像中移动的方向”，
        但 1/2 通道是开环并且存在耦合/迟滞，因此这个函数只负责
        发出一次候选动作；真正判断动作是否有效由 run_rule_ac_until_threshold()
        在运动后重新截图并计算距离完成。

        step_override 用于 Step9 验证式闭环的自适应步长。
        当某次动作让中心离目标更远时，主循环会立即反向回退并减小后续步长。
        """
        base_step = abs(float(getattr(self.cfg, "rule_ac_stage_step_size", 20)))
        step = abs(float(step_override)) if step_override is not None else base_step
        if not math.isfinite(step) or step <= 0:
            step = base_step if base_step > 0 else 1.0

        # action_name 表示“希望检测区域中心在图像中移动的方向”。
        # ch1_distance/ch2_distance 是当前候选映射下实际发送给 1/2 通道的相对步数。
        mapping = {
            1:   (-step, 0.0,   "右上"),
            -1:  (step,  0.0,   "左下"),
            2:   (0.0,  -step,  "左上"),
        ...
        if not bool(getattr(self.cfg, "rule_ac_enable_stage", False)):
            self.log(
                ...
                f"候选发送 CH1={d1}, CH2={d2}, step={step}；但 RuleAC真动Stage12 未勾选"
            )
            return {
                "moved": False,
                "dry_run": True,
        ...
        tag_text = f", tag={verify_tag}" if verify_tag else ""
        self.log(
            f"[Step9-BC中心] 候选动作：希望检测中心向{name}移动，"
            f"发送 CH1={d1}, CH2={d2}, step={step}{tag_text}；"
            "随后会重新检测距离，若变远则立即反向回退。"
        )
    ...
    @staticmethod
    def _step9_distance_to_target(center: Tuple[float, float], target: Tuple[float, float]) -> float:
        """计算 Step9 当前检测中心到目标点的欧氏距离。"""
        cx, cy = float(center[0]), float(center[1])
        tx, ty = float(target[0]), float(target[1])
        return float(math.hypot(tx - cx, ty - cy))

    @staticmethod
    def _step9_reverse_action_code(action_code: int) -> int:
        """Step9 动作编码的反向动作。"""
        return -int(action_code)
    ...
    def run_rule_ac_until_threshold(self) -> Dict[str, Any]:
        ...
        关键修复：
            不再相信固定的 1/2 通道方向映射。
            每次发送候选动作后，立即重新截图检测中心到目标点的距离：
                - 距离变小：接受本次动作，继续下一轮；
                - 距离变大或没有明显改善：立即执行反向动作回退，并把后续步长减半；
                - 连续多次变差：停止 Step9，避免检测区域越跑越远。

        这样可以处理 1/2 通道开环、方向映射反、耦合、迟滞、负载变化等问题。
        """
        ...
        # 自适应闭环参数：不增加 GUI 字段，优先从 cfg 读取；没有就使用安全默认值。
        current_adaptive_step: Optional[float] = None
        last_gui_step: Optional[float] = None
        consecutive_bad_moves = 0
        total_bad_moves = 0
        min_step_px = float(getattr(self.cfg, "rule_ac_verified_min_step", 2.0))
        min_improve_px = float(getattr(self.cfg, "rule_ac_verified_min_improve_px", 0.5))
        max_consecutive_bad_moves = int(getattr(self.cfg, "rule_ac_verified_max_bad_moves", 4))
        settle_s_default = max(0.20, float(getattr(self.cfg, "rule_ac_loop_interval_s", 0.10)))
        verify_settle_s = float(getattr(self.cfg, "rule_ac_verified_settle_s", settle_s_default))
        ...
        while True:
            ...
            gui_step = abs(float(getattr(self.cfg, "rule_ac_stage_step_size", 20)))
            if current_adaptive_step is None or last_gui_step is None or abs(gui_step - last_gui_step) > 1e-9:
                current_adaptive_step = max(gui_step, min_step_px)
                last_gui_step = gui_step
                consecutive_bad_moves = 0

            # 这些参数运行中允许从 cfg 更新。
            min_step_px = float(getattr(self.cfg, "rule_ac_verified_min_step", min_step_px))
            min_improve_px = float(getattr(self.cfg, "rule_ac_verified_min_improve_px", min_improve_px))
            max_consecutive_bad_moves = int(getattr(self.cfg, "rule_ac_verified_max_bad_moves", max_consecutive_bad_moves))
            verify_settle_s = float(getattr(self.cfg, "rule_ac_verified_settle_s", max(0.20, loop_interval_s)))
            ...
            # 当前版本不再用“最大循环次数”作为 Step9 的正常结束条件。
            # max_cycles 仍记录到日志中，保留为参数兼容，但不作为成功/失败边界。
            ...
            if not det.get("ok", False):
                ...
                last_record = {
                    ...
                    "verify_result": "detect_failed_before_move",
                    "adaptive_step": current_adaptive_step,
                    **save_info,
                }
            ...
            abs_dx = abs(dx)
            abs_dy = abs(dy)
            dist_before = self._step9_distance_to_target((cx, cy), target_xy)
            ...
            if abs_dx <= tol and abs_dy <= tol:
                ...
                self.log(
                    ...
                    f"dx={dx:.2f}, dy={dy:.2f}, dist={dist_before:.2f}, tol={tol}"
                )
                break
            ...
            verify_result = "not_moved"
            post_center = None
            post_dx = ""
            post_dy = ""
            dist_after: Any = ""
            distance_improve: Any = ""
            rollback_info: Dict[str, Any] = {}
            rollback_center = None
            rollback_dist: Any = ""
            rollback_improve: Any = ""
            ...
            if self.step9_stop_requested:
                ...
                verify_result = "stop_before_move"
            else:
                # 先执行一次候选动作。
                move_info = self._execute_stage12_diagonal_action(
                    action_code,
                    step_override=current_adaptive_step,
                    verify_tag="candidate",
                )

                if move_info.get("moved") and not move_info.get("dry_run"):
                    time.sleep(max(0.05, verify_settle_s))
                    post_det = self._capture_bc_overlap_center_once()

                    if post_det.get("ok", False):
                        pcx, pcy = post_det["overlap_center"]
                        post_center = (pcx, pcy)
                        post_dx = target_x - float(pcx)
                        post_dy = target_y - float(pcy)
                        dist_after = self._step9_distance_to_target((pcx, pcy), target_xy)
                        distance_improve = dist_before - float(dist_after)

                        if abs(float(post_dx)) <= tol and abs(float(post_dy)) <= tol:
                            final_ok = True
                            consecutive_bad_moves = 0
                            verify_result = "accepted_aligned_after_move"
                            self.log(
                                f"[Step9-BC中心] 验证通过并已重合：old_dist={dist_before:.2f}, "
                                f"new_dist={float(dist_after):.2f}, improve={float(distance_improve):.2f}, "
                                f"post_center=({pcx:.2f},{pcy:.2f})"
                            )
                        elif float(distance_improve) >= min_improve_px:
                            consecutive_bad_moves = 0
                            verify_result = "accepted_distance_decreased"
                            self.log(
                                f"[Step9-BC中心] 验证通过：old_dist={dist_before:.2f}, "
                                f"new_dist={float(dist_after):.2f}, improve={float(distance_improve):.2f} >= {min_improve_px:.2f}"
                            )
                        else:
                            # 候选动作让距离变大或改善不足：立即反向回退，并减小后续步长。
                            consecutive_bad_moves += 1
                            total_bad_moves += 1
                            reverse_code = self._step9_reverse_action_code(action_code)
                            self.log(
                                f"[Step9-BC中心] 验证失败：old_dist={dist_before:.2f}, "
                                f"new_dist={float(dist_after):.2f}, improve={float(distance_improve):.2f} < {min_improve_px:.2f}；"
                                f"立即执行反向回退 action={reverse_code}，并减小后续步长。"
                            )
                            rollback_info = self._execute_stage12_diagonal_action(
                                reverse_code,
                                step_override=current_adaptive_step,
                                verify_tag="rollback_after_bad_move",
                            )
                            time.sleep(max(0.05, verify_settle_s))
                            rollback_det = self._capture_bc_overlap_center_once()
                            if rollback_det.get("ok", False):
                                rcx, rcy = rollback_det["overlap_center"]
                                rollback_center = (rcx, rcy)
                                rollback_dist = self._step9_distance_to_target((rcx, rcy), target_xy)
                                rollback_improve = dist_before - float(rollback_dist)
                                self.log(
                                    f"[Step9-BC中心] 回退后检测：center=({rcx:.2f},{rcy:.2f}), "
                                    f"rollback_dist={float(rollback_dist):.2f}, 相对回退前改善={float(rollback_improve):.2f}"
                                )
                            else:
                                self.log(f"[Step9-BC中心] 回退后重新检测失败：{rollback_det.get('reason')}")

                            old_step = float(current_adaptive_step)
                            current_adaptive_step = max(min_step_px, old_step * 0.5)
                            verify_result = "reversed_distance_not_decreased"

                            if consecutive_bad_moves >= max_consecutive_bad_moves and current_adaptive_step <= min_step_px + 1e-9:
                                self.log(
                                    f"[Step9-BC中心] 连续 {consecutive_bad_moves} 次运动验证失败，且步长已降到最小 {current_adaptive_step}。"
                                    "为避免检测区域继续远离目标，本次 Step9 停止。"
                                )
                                last_record = {
                                    "cycle": cycle_idx,
                                    "time": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                                    "ok": True,
                                    "aligned": False,
                                    "reason": "verified_motion_failed_stop",
                                    "center_x": cx,
                                    "center_y": cy,
                                    "target_x": target_x,
                                    "target_y": target_y,
                                    "dx": dx,
                                    "dy": dy,
                                    "post_center_x": post_center[0] if post_center else "",
                                    "post_center_y": post_center[1] if post_center else "",
                                    "post_dx": post_dx,
                                    "post_dy": post_dy,
                                    "rollback_center_x": rollback_center[0] if rollback_center else "",
                                    "rollback_center_y": rollback_center[1] if rollback_center else "",
                                    "distance_before": dist_before,
                                    "distance_after": dist_after,
                                    "distance_rollback": rollback_dist,
                                    "distance_improve": distance_improve,
                                    "rollback_improve": rollback_improve,
                                    "tolerance_px": tol,
                                    "overlap_area_px": det.get("overlap_area_px"),
                                    "b_mask_area_px": det.get("b_mask_area_px"),
                                    "c_mask_area_px": det.get("c_mask_area_px"),
                                    ...
                                    "adaptive_step": current_adaptive_step,
                                    "consecutive_bad_moves": consecutive_bad_moves,
                        ...
                        old_step = float(current_adaptive_step)
                        current_adaptive_step = max(min_step_px, old_step * 0.5)
                        verify_result = "detect_failed_after_move_step_reduced"
                        self.log(
                            f"[Step9-BC中心] 运动后重新检测失败：{post_det.get('reason')}；"
                            f"后续步长 {old_step:.2f} -> {current_adaptive_step:.2f}"
                        )
                else:
                    # dry-run 或未启用 Stage 时，无法做真实验证。
                    verify_result = "dry_run_or_no_stage_no_verification"
            ...
            last_record = {
                ...
                "time": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                "ok": True,
                "aligned": bool(final_ok),
                "reason": "aligned_after_move" if final_ok else "moving_verified",
                "center_x": cx,
                "center_y": cy,
                "target_x": target_x,
                "target_y": target_y,
            ...
            self.log(
                ...
                f"center=({cx:.2f},{cy:.2f}), target=({target_x:.2f},{target_y:.2f}), "
                f"dx={dx:.2f}, dy={dy:.2f}, old_dist={dist_before:.2f}, "
                f"new_dist={dist_after}, rollback_dist={rollback_dist}, tol={tol}, "
                f"color_area={det.get('overlap_area_px')}, "
                f"gui_step={gui_step}, adaptive_step={current_adaptive_step}, "
                f"max_cycles={max_cycles}(ignored), enable_stage={getattr(self.cfg, 'rule_ac_enable_stage', None)}, "
                f"action={action_code}({action_name}), verify={verify_result}, moved={move_info.get('moved')}"
            )
    ...
    def run_one_cycle(self, cycle_index: int) -> bool:
        ...
        try:
            ...
            self.log("========== Step 1：RuleAB当前B mask角度 current_angle ==========")
            # 关键修改：本轮基准角度直接来自 RuleAB 当前 B mask / annotated_frames，
            # 不再使用 outputs/captured_frames/.../angle_results。
            ...
            if angle_result.get("ok", False) and angle_result.get("angle_deg") is not None:
                ...
                current_angle = None
                self.log("[RuleAB-Bmask角度-current_angle] 本轮角度无效，流程继续，但本轮不参与跨轮角度差调节")
...
class MeasurementWorkflowGUI:
    ...
    def _build_ui(self):
        ...
        ttk.Button(button_grid, text="1 可选旧角度B/边", command=self.calibrate_angle_b_thread).grid(row=2, column=0, padx=4, pady=4, sticky="ew")
    ...
    def _current_or_new_calibration_state(self) -> CalibrationState:
        ...
        # RuleAB-B 没有单独标定时，默认复用角度检测 B 点，避免完整测量 Step7 再弹 B 点选择窗口。
    ...
    def calibrate_step9_target_color(self):
        ...
        关键修正：
            1. 每次点击“Step9目标/颜色标定”都强制重新点目标位置；
            2. 旧 Step9 target/HSV 先清空为 -1，取消时不会复用旧值；
            3. 目标点直接使用 select_step9_target() 的返回值，不再依赖 GUI 变量异步刷新；
            4. HSV 直接使用 prepare_step9_color_region_detection() 的返回值保存。
        """
        try:
            state = self._current_or_new_calibration_state()

            # 先作废旧目标点和旧 HSV，避免取消选择后继续复用上一次标定。
            state.step9_target_x_px = -1.0
            state.step9_target_y_px = -1.0
            state.step9_color_h = -1
            state.step9_color_s = -1
            state.step9_color_v = -1
            self.set_var(self.rule_ac_target_x_var, -1.0)
            self.set_var(self.rule_ac_target_y_var, -1.0)
            self.set_var(self.rule_ac_color_h_var, -1)
            self.set_var(self.rule_ac_color_s_var, -1)
            self.set_var(self.rule_ac_color_v_var, -1)
            self._save_calibration_file_raw(state)
            self.log("[完整测量标定] 已清空旧 Step9 目标点/HSV；本次必须重新点选目标位置和颜色，取消则不会复用旧值。")

            selected_target = self.select_step9_target(raise_on_cancel=True)
            if selected_target is None:
                raise RuntimeError("Step9 目标点无效：请重新点击目标位置并确认。")
            tx, ty = float(selected_target[0]), float(selected_target[1])
            if tx < 0 or ty < 0:
                raise RuntimeError("Step9 目标点无效：请重新点击目标位置并确认。")

            wf = self.sync_config_from_ui_to_workflow()
            # 关键：GUI set_var 是异步的，所以同步后仍要用刚刚点击返回的坐标强制覆盖。
            wf.cfg.rule_ac_target_x_px = tx
            wf.cfg.rule_ac_target_y_px = ty
            self.set_var(self.rule_ac_target_x_var, round(tx, 3))
            self.set_var(self.rule_ac_target_y_var, round(ty, 3))
            ...
            if hsv is None or len(hsv) < 3:
                raise RuntimeError("Step9 颜色 HSV 无效：请重新点击颜色采样点并确认。")
            mode = str(color_info.get("mode", wf.cfg.rule_ac_color_mode))
            h, s_, v = int(hsv[0]), int(hsv[1]), int(hsv[2])

            self.set_var(self.rule_ac_color_mode_var, mode)
            self.set_var(self.rule_ac_color_h_var, h)
            self.set_var(self.rule_ac_color_s_var, s_)
            self.set_var(self.rule_ac_color_v_var, v)

            # 重新同步 GUI 后仍强制覆盖目标点和 HSV，避免异步变量读到旧值。
            wf = self.sync_config_from_ui_to_workflow()
            wf.cfg.rule_ac_target_x_px = tx
            wf.cfg.rule_ac_target_y_px = ty
            wf.cfg.rule_ac_color_mode = mode
            wf.cfg.rule_ac_color_h = h
            wf.cfg.rule_ac_color_s = s_
            wf.cfg.rule_ac_color_v = v

            state = self._current_or_new_calibration_state()
            state.step9_target_x_px = tx
            state.step9_target_y_px = ty
            state.step9_color_mode = mode
            state.step9_color_h = h
            state.step9_color_s = s_
    ...
    def select_step9_target(self, raise_on_cancel: bool = False) -> Optional[Tuple[float, float]]:
        """
        人工选定 Step9 的目标位置。
        ...
        返回：
            (target_x, target_y)。普通按钮调用时也会同步写入 GUI 输入框；
            完整标定调用时直接使用返回值，避免 GUI 变量异步刷新导致又读到旧值。
        """
        try:
            wf = self.ensure_workflow()

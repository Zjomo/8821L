...
class MeasurementWorkflow:
    ...
    def _run_one_c_edge_route_cycle(
        ...
        ab_guard_info = self._postprocess_scene_ab_masks(follower, scene, tag=f"step7_route_{step_idx}")
    ...
    def run_one_cycle(self, cycle_index: int) -> bool:
        ...
        try:
            ...
            self.log("========== Step 5.5 / 原Step10：保存数据（异步线程，提前到打开激光前） ==========")
            ...
            self.context["cycle_save_submitted_before_laser"] = True
            self.log(f"[Step 5.5] 异步保存任务提交耗时：{time.time() - t_save_submit0:.3f} s；随后才进入 Step6 打开激光")
            ...
            self.log("========== Step 10：保存已在 Step5.5 提交，本步不再重复保存 ==========")
            self.log(f"[流程] 第 {cycle_index} 轮保存任务已在打开激光前提交；Step9 完成后可以进入下一轮")
            self.notify_update()
            return True
...
class MeasurementWorkflowGUI:
    ...
    def calibrate_step9_target_color(self):
        ...
        try:
            # 每次点击“4 标定Step9目标/颜色”都必须重新标定目标点，
            # 不再复用之前 JSON 或 GUI 中残留的 Step9 目标。
            target_xy = self.select_step9_target()
            if target_xy is None:
                raise RuntimeError("Step9 目标点未完成选择。")
    ...
    def select_step9_target(self):
        ...
        try:
            ...
            self._last_selected_step9_target = (float(target_x), float(target_y))
    ...
    def test_rule_ab(self):
        ...
        try:
            ...
            while not self.rule_ab_only_stop_requested:
                ...
                try:
                    image_rgb, scene = follower.capture_and_build_scene()
                    ab_guard_info = self.ensure_workflow()._postprocess_scene_ab_masks(follower, scene, tag=f"single_rule_ab_frame_{frame_idx}")
                    ...
                    try:
                        status["ab_mask_guard"] = self.workflow._json_safe(ab_guard_info) if self.workflow is not None else ab_guard_info
                    except Exception:
                        pass
                    lost_count = 0
                except Exception as e:

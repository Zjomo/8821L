...
class MeasurementWorkflow:
    ...
    def _extract_mask_from_angle_result(self, angle_result: Optional[Dict[str, Any]]) -> Optional[np.ndarray]:
        """
        从“本次角度检测返回结果”中提取 B mask。

        重要：
            这个函数不截图、不调用 RuleAB 的 capture_and_build_scene()，
            只使用 angle_detect.py 在本次角度检测时已经得到/保存的 mask。

        兼容的返回字段：
            mask / b_mask / object_mask / target_mask / seg_mask / polygon_mask / binary_mask
            mask_path / b_mask_path / object_mask_path / seg_mask_path
            nested dict: result["mask_info"] / result["segmentation"] / result["result"] 等。
        """
        ...
        def _as_mask(value: Any) -> Optional[np.ndarray]:
            ...
            # 字符串路径：读取角度检测已经保存的 mask 文件。
            if isinstance(value, (str, Path)):
                s = str(value).strip()
                if not s:
                    return None
                path_candidates: List[Path] = [Path(s)]
                # 相对路径兼容：相对当前工程目录 / 输出根目录。
                try:
                    path_candidates.append(PROJECT_ROOT / s)
                    path_candidates.append(self.output_root / s)
                except Exception:
                    pass
                for mp in path_candidates:
                    try:
            ...
            # dict/object 里的 mask 字段。
            try:
                if isinstance(value, dict):
                    for k in (
                        "mask", "b_mask", "object_mask", "target_mask", "seg_mask",
                        "polygon_mask", "binary_mask", "mask_path", "b_mask_path",
                        "object_mask_path", "target_mask_path", "seg_mask_path",
                    ):
                        if k in value:
                            m = _as_mask(value.get(k))
                            if m is not None:
                                return m
            except Exception:
                pass
            ...
            try:
                for k in ("mask", "b_mask", "object_mask", "target_mask", "seg_mask", "polygon_mask", "binary_mask"):
                    m0 = getattr(value, k, None)
                    if m0 is not None:
                        m = _as_mask(m0)
                        if m is not None:
                            return m
        ...
        for key in (
            ...
            if key in angle_result:
                mask = _as_mask(angle_result.get(key))
                if mask is not None:
                    return mask

        # 再查常见嵌套字段，但不做无限递归。
        def _walk(obj: Any, depth: int = 0) -> Optional[np.ndarray]:
            ...
            if isinstance(obj, dict):
                # 优先查可能存放分割结果的字段。
                priority_keys = (
                    "mask_info", "segmentation", "seg", "sam_result", "result",
                    "detail", "details", "stats", "angle_result", "detection",
                )
                for k in priority_keys:
                    if k in obj:
                        m = _walk(obj.get(k), depth + 1)
                        if m is not None:
                            return m
                # 兜底遍历其它值。
                for v in obj.values():
        ...
        return _walk(angle_result)

    def _compact_angle_result_for_record(self, angle_result: Optional[Dict[str, Any]]) -> Dict[str, Any]:
        """
        保存 Step7 记录时使用的轻量版角度检测结果。

        angle_result 里可能包含完整 mask ndarray；完整 mask 已用于角度修复，
        但不应该写入 summary CSV/XLSX 的 JSON 字段，否则文件会非常大。
        """
        if not isinstance(angle_result, dict):
            return {"ok": False, "reason": "angle_result_not_dict"}

        keep_keys = (
            "ok", "reason", "error", "image_path",
            "angle_deg", "angle_deg_raw", "angle_deg_raw_before_repair", "angle_deg_repaired",
            "center", "center_norm", "bbox", "bbox_xywhn", "area_px", "area",
            "conditional_second_detect_triggered", "conditional_second_detect_failed",
            "first_angle_deg", "mask_path", "b_mask_path", "object_mask_path",
            "target_mask_path", "seg_mask_path", "angle_repair",
        )
        out: Dict[str, Any] = {}
        for k in keep_keys:
            if k in angle_result:
                out[k] = self._json_safe(angle_result.get(k))
        return out
    ...
    def _get_rule_ab_current_bmask_angle_candidates(
        ...
        """
        用“角度检测时已经得到的 B mask”计算所有候选边角度。

        关键修复：
            不再重新截图，不再调用 follower.capture_and_build_scene()；
            角度修复与本次 raw_angle 使用同一帧、同一个 mask，避免修复角度来自另一帧。
        """
        b_mask = self._extract_mask_from_angle_result(angle_result)
        if b_mask is not None:
            cands = self._bmask_edge_angle_candidates_from_mask(b_mask)
            if cands:
                return cands
            self.log("[RuleAB-角度修复] 已取得角度检测 mask，但未能从该 mask 得到候选边角度。")
        else:
            self.log(
                "[RuleAB-角度修复] 本次角度检测结果中没有可用 mask/mask_path；"
                "按要求不重新截图，因此本次不执行边角度修复。"
            )
    ...
    def _repair_rule_ab_angle_if_jump(
        ...
        candidates = self._get_rule_ab_current_bmask_angle_candidates(follower, angle_result=angle_result)
    ...
    def run_rule_ab_until_angle_delta(
        ...
        while True:
            ...
            record = {
                ...
                "angle_result": self._compact_angle_result_for_record(angle_result),
            }
        ...
        with self.summary_csv_path.open("a", encoding="utf-8-sig", newline="") as f:
            ...
            if not file_exists:
                writer.writerow([
                    ...
                    "step7_final_angle",
                    "step7_final_delta",
                    "step7_angle_repair_used",
                    "step7_angle_repair_json",
                    "step7_records_json",
                ])

            rule_ab_result = ctx.get("last_rule_ab_result") or {}
            if not isinstance(rule_ab_result, dict):
                rule_ab_result = {}
            rule_ab_records = rule_ab_result.get("records") or []
            final_repair_info = {}
            if isinstance(rule_ab_records, list):
                for rec in reversed(rule_ab_records):
                    if isinstance(rec, dict) and rec.get("angle_repair") is not None:
                        final_repair_info = rec.get("angle_repair") or {}
                        break
            repair_used = bool(final_repair_info.get("used_repaired_angle", False)) if isinstance(final_repair_info, dict) else False
            repair_json = json.dumps(self._json_safe(final_repair_info), ensure_ascii=False) if final_repair_info else ""
            records_json = json.dumps(self._json_safe(rule_ab_records), ensure_ascii=False) if rule_ab_records else ""
            ...
            writer.writerow([
                ...
                rule_ab_result.get("final_angle"),
                rule_ab_result.get("final_delta"),
                repair_used,
                repair_json,
                records_json,
            ])
    ...
    def _append_summary_xlsx(
        ...
        ws.cell(row=row_idx, column=7).number_format = "0.000000"
        ws.cell(row=row_idx, column=8).number_format = "0.000000"
...
class MeasurementWorkflowGUI:
    ...
    def calibrate_step9_target_color(self):
        ...
        重要修改：
            每次点击“Step9目标/颜色标定”都必须重新点选目标位置，
            不允许复用 GUI 输入框、config.py 或旧 calibration JSON 中残留的 target。

        这样可以避免出现：用户没有本次重新点目标点，但完整测量仍然
        使用旧 target=(449.4, 360.0) 之类历史坐标的问题。
        """
        try:
            # 先把旧的 Step9 target / HSV 主动作废，并立即写入标定文件。
            # 如果用户随后取消目标点或颜色选择，完整测量启动前检查会失败，
            # 不会继续复用旧 target / 旧 HSV。
            ...
            self.log(
                "[完整测量标定] 已清空旧 Step9 目标点/HSV；"
                "本次必须重新点选目标位置和颜色，取消则不会复用旧值。"
            )

            # 强制重新点目标点。select_step9_target(..., raise_on_cancel=True)
            # 会在用户 ESC/Q 取消时向外抛异常，从而终止本次 Step9 标定。
            selected_target = self.select_step9_target(raise_on_cancel=True)
            if selected_target is None:
                raise RuntimeError("Step9 目标点未重新选择，已禁止复用旧目标点。")

            # 关键修复：select_step9_target() 虽然会用 root.after() 更新 GUI 变量，
            # 但 calibrate_step9_target_color() 本身在子线程里运行，GUI 变量刷新不是同步完成的。
            # 因此这里必须直接使用函数返回的坐标作为本次唯一可信 Step9 目标点，
            # 不能马上从 GUI 变量 / sync_config_from_ui_to_workflow() 再读一次。
            ...
            # sync_config_from_ui_to_workflow() 可能仍读到尚未刷新的 -1；这里强制覆盖。
            wf.cfg.rule_ac_target_x_px = float(tx)
            wf.cfg.rule_ac_target_y_px = float(ty)
            ...
            # 再同步一次只用于读取其它 GUI 容差/面积参数。
            # 目标点和 HSV 仍然用本次函数返回/颜色选择返回的值强制覆盖，
            # 避免 root.after() 尚未执行导致保存成 -1 或旧值。
            wf = self.sync_config_from_ui_to_workflow()
            wf.cfg.rule_ac_target_x_px = float(tx)
            wf.cfg.rule_ac_target_y_px = float(ty)
            wf.cfg.rule_ac_color_mode = str(mode)
            wf.cfg.rule_ac_color_h = int(hsv[0])
            wf.cfg.rule_ac_color_s = int(hsv[1])
            wf.cfg.rule_ac_color_v = int(hsv[2])
            wf.step9_color_mode = str(mode)
            wf.step9_color_hsv = (int(hsv[0]), int(hsv[1]), int(hsv[2]))

            state = self._current_or_new_calibration_state()
            state.step9_target_x_px = float(tx)
            state.step9_target_y_px = float(ty)
            state.step9_color_mode = str(mode)
            state.step9_color_h = int(hsv[0])
            state.step9_color_s = int(hsv[1])
            state.step9_color_v = int(hsv[2])
            ...
            if state.step9_target_x_px < 0 or state.step9_target_y_px < 0:
                raise RuntimeError("Step9 目标点保存前检查失败：target 仍为负值。")
            if state.step9_color_h < 0 or state.step9_color_s < 0 or state.step9_color_v < 0:
                raise RuntimeError("Step9 颜色保存前检查失败：HSV 仍为无效值。")
    ...
    def select_step9_target(self, raise_on_cancel: bool = False):
        """
        人工选定 Step9 的目标位置。
        ...
        参数：
            raise_on_cancel:
                False：保持旧行为，异常只弹窗提示；
                True：用户取消/失败时继续向外抛异常，供“完整测量标定”阻断旧目标点复用。

        返回：
            (target_x, target_y) 或 None。
        """
        try:
            wf = self.ensure_workflow()

...
class MeasurementWorkflow:
    ...
    def _capture_bc_overlap_center_once(self) -> Dict[str, Any]:
        """
        截图并通过 SAM2 更新 A/B/C，计算 B∩C 的重合区域中心。
        """
        follower = self._get_or_create_step9_follower_for_bc()
        image_rgb, scene = follower.capture_and_build_scene()

        b_mask = self._extract_mask_from_scene_object(getattr(scene, "b", None))
        c_mask = self._extract_mask_from_scene_object(getattr(scene, "c", None))

        if b_mask is None or c_mask is None:
            return {
                "ok": False,
                "reason": "missing_b_or_c_mask",
                "image_rgb": image_rgb,
                "scene": scene,
            }
...
class MeasurementWorkflowGUI:
    ...
    def test_rule_ac(self):
        ...
        新逻辑：
            1. 如果没有提前选择目标点，则先弹窗选择 Step9 目标点；
            2. 每轮截图并用 SAM2 更新 B/C；
            3. 计算 B∩C 重合区域中心；
            4. 与目标点比较；
            5. 偏离超过容差时，根据误差方向执行 1/2 通道对角动作：
                   +1 = 右上
                   -1 = 左下
                   +2 = 左上
                   -2 = 右下
            6. 中心进入容差范围或达到最大循环后停止。
        """
        try:

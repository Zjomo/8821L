...
class MeasurementConfig:
    ...
    rule_ab_follow_c_direction: int = int(_cfg("rule_ab_follow_c_direction", 1))  # >=0：屏幕视觉逆时针；<0：屏幕视觉顺时针
    ...
    # 路线本身按 GUI 的 A-C target 距离从 C 近似四边形边界向外偏移。
    # 也就是说：紫色 A 路线不是贴着 C 边，而是离 C 边 target px。
    # min/max 则用于实时安全带判断。
    rule_ab_c_edge_route_safe_clearance_px: float = float(_cfg("rule_ab_c_edge_route_safe_clearance_px", _cfg("rule_ab_ac_target_clearance_px", 90.0)))
...
class MeasurementWorkflow:
    ...
    def _set_step7_route_safety_base_from_payload(self, payload: Dict[str, Any], route_points: Optional[List[Tuple[float, float]]] = None) -> List[Tuple[float, float]]:
        """
        为 Step7 安全距离准备“C 边界基准线”。

        注意：
            - route_points_xy 是 A 的目标路线，已经按 GUI 的 target 从 C 边界外扩；
            - 安全距离 min/target/max 应该是 A 中心到 C 边界的距离，
              不能再拿 A 中心到外扩后的 A 路线距离来判断；
            - 因此这里从 quad_points_xy / safety_base_points_xy 重建 C 边界折线，
              后续 _run_one_c_edge_route_cycle 用它计算 route_distance_px。
        """
        base_points: List[Tuple[float, float]] = []
        try:
            raw_base = payload.get("safety_base_points_xy") or payload.get("c_boundary_points_xy") or []
            if raw_base:
                for q in raw_base:
                    base_points.append((float(q[0]), float(q[1])))
            elif payload.get("quad_points_xy") is not None:
                quad = np.asarray(payload.get("quad_points_xy"), dtype=np.float32).reshape(-1, 2)[:4]
                base_points = self._generate_c_edge_route_points_from_quad(
                    quad,
                    spacing_px=float(payload.get("spacing_px", getattr(self.cfg, "rule_ab_c_edge_route_spacing_px", 12.0))),
                    follow_direction=int(payload.get("follow_direction", getattr(self.cfg, "rule_ab_follow_c_direction", 1))),
                    safe_clearance_px=0.0,
                )
        except Exception:
            base_points = []

        if not base_points and route_points:
            # 兜底：旧 JSON 没有 C 边界信息时才退回到 route_points。
            # 这种情况下 route_distance_px 是到 A 路线的距离，不是到 C 边界的距离。
            base_points = [(float(x), float(y)) for x, y in route_points]

        try:
            self.context["step7_c_edge_route_safety_base_points"] = [[float(x), float(y)] for x, y in base_points]
        except Exception:
            pass
        return base_points
    ...
    def _load_or_build_step7_c_edge_route(self, follower: Any) -> List[Tuple[float, float]]:
        ...
        if route_path.exists():
            try:
                ...
                if route and source_ok and clearance_ok and spacing_ok and follow_ok:
                    self._set_step7_route_safety_base_from_payload(route_data, route)
                    return route
                if route:
                    self.log(
        ...
        if route_source in ("quad", "quadrilateral"):
            ...
            if not route:
                ...
                route = self._build_and_save_quad_edge_route(c_path)
        try:
            route_json_path = c_path / "static_c_edge_route.json"
            if route_json_path.exists():
                with route_json_path.open("r", encoding="utf-8") as f:
                    route_payload = json.load(f)
                self._set_step7_route_safety_base_from_payload(route_payload, route)
        except Exception:
            pass
        return route
    ...
    def _build_and_save_quad_edge_route(self, c_path: Path) -> List[Tuple[float, float]]:
        ...
        try:
            ...
            follow_dir = int(getattr(self.cfg, "rule_ab_follow_c_direction", 1))
            clearance_px = float(getattr(self.cfg, "rule_ab_c_edge_route_safe_clearance_px", getattr(self.cfg, "rule_ab_ac_target_clearance_px", 90.0)))
            route = self._generate_c_edge_route_points_from_quad(
                quad,
                spacing_px=spacing_px,
                follow_direction=follow_dir,
                safe_clearance_px=clearance_px,
            )
            safety_base = self._generate_c_edge_route_points_from_quad(
                quad,
                spacing_px=spacing_px,
                follow_direction=follow_dir,
                safe_clearance_px=0.0,
            ...
            with (c_path / "static_c_edge_route.json").open("w", encoding="utf-8") as f:
                json.dump({
                    ...
                    "safe_clearance_px": float(clearance_px),
                    "follow_direction": int(follow_dir),
                    "quad_points_xy": [[float(x), float(y)] for x, y in quad.tolist()],
                    "safety_base_points_xy": [[float(x), float(y)] for x, y in safety_base],
                    "route_points_xy": [[float(x), float(y)] for x, y in route],
                }, f, ensure_ascii=False, indent=2)
            return route
        except Exception as e:
    ...
    def _generate_c_edge_route_points_from_quad(
        ...
        # 方向约定修复：
        #   GUI 里的 follow_direction >= 0 表示按屏幕显示方向“逆时针”绕 C 走；
        #   follow_direction < 0 表示按屏幕显示方向“顺时针”绕 C 走。
        # 注意图像坐标系 y 轴向下，标准 shoelace 面积符号与屏幕视觉方向相反：
        #   signed_area < 0  -> 屏幕视觉逆时针；
        #   signed_area > 0  -> 屏幕视觉顺时针。
        def _signed_area_image_xy(arr: np.ndarray) -> float:
            x = arr[:, 0].astype(np.float64)
            y = arr[:, 1].astype(np.float64)
            return 0.5 * float(np.sum(x * np.roll(y, -1) - np.roll(x, -1) * y))

        signed_area = _signed_area_image_xy(pts)
        want_ccw_on_screen = int(follow_direction) >= 0
        if want_ccw_on_screen:
            if signed_area > 0:
                pts = pts[::-1].copy()
        else:
            if signed_area < 0:
    ...
    def _run_one_c_edge_route_cycle(
        ...
        # 先检查 A 中心到 C 边界折线的距离是否处于安全带。
        # 注意：route_points 是已经按 GUI target 外扩后的 A 目标路线；
        # 安全距离 min/target/max 应该相对于 C 近似四边形边界计算，
        # 因此优先使用 step7_c_edge_route_safety_base_points。
        safety_base_raw = self.context.get("step7_c_edge_route_safety_base_points") or []
        safety_route_points: List[Tuple[float, float]] = []
        try:
            safety_route_points = [(float(p[0]), float(p[1])) for p in safety_base_raw]
        except Exception:
            safety_route_points = []
        if not safety_route_points:
            safety_route_points = route_points
        safety_info = self._route_safety_target_from_distance(a_center, safety_route_points)
        ...
        info = {
            ...
            "route_target_distance_px": safety_info.get("safe_target_distance_px"),
            "route_max_distance_px": safety_info.get("max_distance_px"),
            "route_target_offset_from_c_px": float(getattr(self.cfg, "rule_ab_c_edge_route_safe_clearance_px", getattr(self.cfg, "rule_ab_ac_target_clearance_px", 90.0))),
        ...
        self.log(
            ...
            f"safe_band=[{safety_info.get('min_distance_px')},{safety_info.get('safe_target_distance_px')},{safety_info.get('max_distance_px')}], "
...
class MeasurementWorkflowGUI:
    ...
    def _save_static_c_map_result(
        ...
        safety_base_points: List[Tuple[float, float]] = []
        if route_source in ("quad", "quadrilateral"):
            ...
            safety_base_points = self._generate_c_edge_route_points_from_quad(
                quad_points,
                spacing_px=route_spacing_px,
                follow_direction=route_follow_direction,
                safe_clearance_px=0.0,
        ...
        route_payload = {
            ...
            "safety_base_points_xy": [[float(x), float(y)] for x, y in safety_base_points],
        ...
        if route_points:
            ...
            cv2.putText(
                route_overlay,
                f"A route from {route_payload_source}: {len(route_points)} pts, spacing={route_spacing_px:.1f}px, route_offset={route_safe_clearance_px:.1f}px",
                (12, 28),
                cv2.FONT_HERSHEY_SIMPLEX,
                0.70,
                (255, 0, 255),
    ...
    def build_config_from_ui(self) -> MeasurementConfig:
        return MeasurementConfig(
            ...
            # GUI 绿框里的 A-C target 直接作为四边形路线外扩距离。

...
class MeasurementWorkflowGUI:
    ...
    def stop_step9_stage12_thread(self):
        """
        GUI按钮入口：后台执行 Step9 停止请求，避免阻塞 Tkinter 主线程。
        """
    ...
    def stop_step9_stage12(self):
        ...
        说明：
            _build_ui() 里已经创建了“停止1/2轴运动”按钮，
            但原文件漏写了 MeasurementWorkflowGUI.stop_step9_stage12_thread()，
            所以 GUI 初始化时会报 AttributeError。

            这里补齐 GUI 层方法，并调用 MeasurementWorkflow.stop_step9_stage12()。
        """
        try:
            wf = self.ensure_workflow()
            wf.stop_step9_stage12()
            self.set_var(self.rule_module_status_var, "规则模块状态：已请求停止1/2轴运动")
            self.log("[Step9-颜色中心] GUI已请求停止1/2轴运动")
        except Exception as e:
            self.set_var(self.rule_module_status_var, "规则模块状态：停止1/2轴失败")
            self.log(f"[Step9-颜色中心] 停止1/2轴失败：{e}")
            self.log(traceback.format_exc())

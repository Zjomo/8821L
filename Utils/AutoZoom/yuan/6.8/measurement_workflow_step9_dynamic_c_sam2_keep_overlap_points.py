...
class MeasurementWorkflow:
    def __init__(self, cfg: MeasurementConfig, on_log=None, on_update=None):
        ...
        # Step9：C 不再使用固定 static_c_mask，而是在 Step9 中也由 SAM2 动态分割/跟踪。
        # 这样 1/2 轴移动导致背景/样品在图像中移动时，C mask 会随当前帧更新。
        self.step9_c_positive_points: List[Tuple[float, float]] = []
        self.step9_c_negative_points: List[Tuple[float, float]] = []
        self.step9_c_last_mask: Optional[np.ndarray] = None
        self.step9_c_last_center: Optional[Tuple[float, float]] = None
        self.step9_c_last_box: Optional[Tuple[float, float, float, float]] = None

        # Step9：上一帧 B∩C 重合区域中心。
        # 后续帧会把这个点作为 B/C 的共同正点提示之一，并在候选 mask 选择时优先保留它，
        # 避免“明明点了重合区域，SAM2 最高分候选却把重合部分丢掉”。
        self.step9_overlap_last_center: Optional[Tuple[float, float]] = None

        # 旧固定 C mask 缓存字段保留兼容，但 Step9 默认不再使用固定 C mask。
        ...
        # 运行过程中所有截图、B/C/overlap mask、overlay、CSV 日志都写入该文件夹。
    ...
    def begin_step9_run_session(self) -> Path:
        ...
        for sub in ("frames", "masks", "overlays", "b_initial", "c_initial"):
    ...
    def _step9_direction_from_error(self, dx: float, dy: float, tol: float) -> int:
        """
        根据“当前 B∩C 中心 -> 目标点”的误差选择 1/2 通道对角运动命令。
    ...
    def _execute_stage12_diagonal_action(self, action_code: int) -> Dict[str, Any]:
        ...
        动作映射：
            +1: CH1 +step, CH2 +step  -> 右上
            -1: CH1 -step, CH2 -step  -> 左下
            +2: CH1 -step, CH2 +step  -> 左上
            -2: CH1 +step, CH2 -step  -> 右下
        """
        ...
        if int(action_code) not in mapping:
            return {"moved": False, "action_code": int(action_code), "reason": "unknown_action"}

        d1, d2, name = mapping[int(action_code)]

        if not bool(getattr(self.cfg, "rule_ac_enable_stage", False)):
            self.log(
                f"[Step9-BC中心] DRY-RUN：应执行 {action_code}({name})，"
                f"CH1={d1}, CH2={d2}，但 RuleAC真动Stage12 未勾选"
            )
            return {
                "moved": False,
                "dry_run": True,
        ...
        self.log(
            f"[Step9-BC中心] 执行 {action_code}({name})："
            f"CH1={d1}, CH2={d2}"
        )
    ...
    def _load_step9_c_mask_from_config(self, image_shape: Optional[Tuple[int, int]] = None) -> np.ndarray:
        ...
        """
        c_dir_text = str(getattr(self.cfg, "rule_ab_static_c_map_dir", "")).strip()
        c_dir = None
        if c_dir_text:
    ...
    def _get_step9_sam2_predictor(self):
        ...
        单独测试 Step9 时需要动态分割/跟踪 B 和 C，因此这里单独缓存一个 predictor，
        避免每一帧重复加载模型。
        """
        ...
        try:
            ...
            raise ImportError(
                "Step9 需要调用 SAM2 动态分割 B/C，但当前环境无法导入 sam2。"
        ...
        self.log(
            f"[Step9-BC中心] 正在加载 SAM2 分割 B/C：cfg={sam2_cfg}, "
    ...
    def _select_step9_c_points_interactively(
        self,
        image_rgb: np.ndarray,
        window_name: str = "Step9 select C: left positive, right negative",
        scale: float = 0.85,
    ) -> Tuple[List[Tuple[float, float]], List[Tuple[float, float]]]:
        """
        Step9 单独测试前，用 SAM2 分割 C 的点提示。
        ...
        def redraw() -> np.ndarray:
            canvas = display.copy()
            lines = [
                "Step9: select C for dynamic SAM2 segmentation",
            ...
            for j, (x, y) in enumerate(pos):
                ...
                cv2.putText(canvas, f"C+{j + 1}", (x + 8, y - 8), cv2.FONT_HERSHEY_SIMPLEX, 0.58, (255, 0, 0), 2, cv2.LINE_AA)
            for j, (x, y) in enumerate(neg):
                cv2.circle(canvas, (x, y), 7, (0, 0, 255), 2)
                cv2.line(canvas, (x - 5, y - 5), (x + 5, y + 5), (0, 0, 255), 2)
                cv2.line(canvas, (x - 5, y + 5), (x + 5, y - 5), (0, 0, 255), 2)
        ...
        try:
            while True:
                ...
                key = cv2.waitKey(30) & 0xFF
                if key in (27, ord("q"), ord("Q")):
                    raise RuntimeError("用户取消了 Step9 的 C 分割。")
                if key in (ord("r"), ord("R")):
                    pos.clear()
                    neg.clear()
                if key in (13, 10, ord("n"), ord("N")):
                    if len(pos) <= 0:
                        self.log("[Step9-BC中心] C 分割至少需要 1 个正点。")
                        continue
                    break
        finally:
            try:
    ...
    @staticmethod
    def _point_inside_mask(mask: np.ndarray, point_xy: Tuple[float, float], radius: int = 0) -> bool:
        """
        判断一个点或点附近小圆区域是否被 mask 覆盖。

        radius=0：只检查单个像素；
        radius>0：检查点附近圆形区域内是否有任意像素被覆盖。
        """
        if mask is None:
            return False
        m = np.asarray(mask).astype(bool)
    ...
    @staticmethod
    def _point_disk_coverage(mask: np.ndarray, point_xy: Tuple[float, float], radius: int = 4) -> float:
        """
        计算点附近小圆区域被 mask 覆盖的比例，用于 SAM2 多候选 mask 排序。
        """
        if mask is None:
            return 0.0
        m = np.asarray(mask).astype(bool)
        ...
        yy, xx = np.ogrid[y0:y1, x0:x1]
        disk = (xx - x) ** 2 + (yy - y) ** 2 <= r ** 2
        denom = int(np.count_nonzero(disk))
        if denom <= 0:
            return 0.0
        patch = m[y0:y1, x0:x1]
        return float(np.count_nonzero(patch[disk]) / denom)

    @staticmethod
    def _merge_near_points(points: List[Tuple[float, float]], min_dist_px: float = 3.0) -> List[Tuple[float, float]]:
        """
        合并距离很近的点，避免同一个点被重复加权太多。
        """
    ...
    def _select_sam2_mask_keep_positive_points(
        self,
        masks: Any,
        scores: Any,
        ...
        object_name: str = "B",
        force_keep_points: Optional[List[Tuple[float, float]]] = None,
        require_positive_point: bool = True,
        point_radius_px: int = 4,
    ) -> Tuple[np.ndarray, int, Dict[str, Any]]:
        """
        SAM2 返回 multimask 时，不再简单选择 score 最高的候选。

        选择原则：
            1. 优先选择覆盖用户正点的 mask；
            2. 如果有上一帧 overlap_center，则作为 B/C 共同的 force_keep_points；
            3. 负点被覆盖会降权；
            4. 在满足正点/保留点的前提下，再参考 SAM2 score。

        这样可以避免：用户把重合区域点成正点，但 argmax(score) 选出的候选 mask 把该区域丢掉。
        """
        masks_arr = np.asarray(masks)
        scores_arr = np.asarray(scores).reshape(-1)
        if masks_arr.ndim < 3 or masks_arr.shape[0] <= 0:
            raise RuntimeError(f"SAM2 没有返回 {object_name} mask。")

        positive_points = self._merge_near_points(list(positive_points or []), min_dist_px=3.0)
        negative_points = self._merge_near_points(list(negative_points or []), min_dist_px=3.0)
        force_keep_points = self._merge_near_points(list(force_keep_points or []), min_dist_px=3.0)

        # 若某个 force_keep 点已经在 positive_points 里，不重复计算。
        for fp in force_keep_points:
            if all(math.hypot(float(fp[0]) - float(pp[0]), float(fp[1]) - float(pp[1])) > 3.0 for pp in positive_points):
                positive_points.append(fp)
        ...
        for i in range(masks_arr.shape[0]):
            ...
            score_i = float(scores_arr[i]) if i < len(scores_arr) else 0.0

            pos_hits = 0
            pos_cov_sum = 0.0
            pos_missed: List[Tuple[float, float]] = []
            for p in positive_points:
                hit = self._point_inside_mask(m, p, radius=int(point_radius_px))
                cov = self._point_disk_coverage(m, p, radius=int(point_radius_px))
                pos_cov_sum += float(cov)
                if hit:
                    pos_hits += 1
                else:
                    pos_missed.append((float(p[0]), float(p[1])))
            ...
            for p in negative_points:
                hit = self._point_inside_mask(m, p, radius=int(point_radius_px))
                cov = self._point_disk_coverage(m, p, radius=int(point_radius_px))
                neg_cov_sum += float(cov)
                if hit:
                    neg_hits += 1

            pos_n = max(1, len(positive_points))
            neg_n = max(1, len(negative_points))
            pos_hit_ratio = float(pos_hits / pos_n)
            pos_cov_ratio = float(pos_cov_sum / pos_n)
            neg_hit_ratio = float(neg_hits / neg_n) if negative_points else 0.0
            neg_cov_ratio = float(neg_cov_sum / neg_n) if negative_points else 0.0

            # 强约束：有正点但完全不覆盖正点的候选大幅降权；
            # 覆盖负点的候选也降权。
            hard_penalty = 0.0
            if require_positive_point and positive_points and pos_hits <= 0:
                hard_penalty -= 1000.0
            if positive_points and pos_hits < len(positive_points):
                hard_penalty -= 100.0 * (len(positive_points) - pos_hits)
            if negative_points and neg_hits > 0:
                hard_penalty -= 80.0 * neg_hits

            # 综合评分：正点覆盖优先级最高，其次才是 SAM2 score。
            total = (
                hard_penalty
                + 200.0 * pos_hit_ratio
                + 120.0 * pos_cov_ratio
            ...
            info = {
                ...
                "pos_total": int(len(positive_points)),
                "pos_hit_ratio": pos_hit_ratio,
                "pos_cov_ratio": pos_cov_ratio,
                "neg_hits": int(neg_hits),
                "neg_total": int(len(negative_points)),
                "neg_hit_ratio": neg_hit_ratio,
                "neg_cov_ratio": neg_cov_ratio,
                "total_score": float(total),
                "missed_positive_points": pos_missed,
            }
        ...
        selected = masks_arr[best_idx].astype(bool)
        selected_info = candidate_infos[best_idx]

        # 如果最佳候选仍然没覆盖正点，明确写日志，方便排查点选位置/提示是否不合理。
        if positive_points and selected_info.get("pos_hits", 0) < len(positive_points):
            self.log(
                f"[Step9-BC中心] 警告：{object_name} 最终候选没有覆盖全部正点，"
                f"pos_hits={selected_info.get('pos_hits')}/{len(positive_points)}，"
                f"missed={selected_info.get('missed_positive_points')}。"
                "这通常说明正点落在边界/重合模糊区，建议增加主体内部正点，或减少错误负点。"
            )

        # 打印所有候选的核心信息，方便解释为什么选这个 mask。
        summary = [
            f"#{ci['idx']}:sam2={ci['sam2_score']:.3f},pos={ci['pos_hits']}/{ci['pos_total']},"
            f"neg={ci['neg_hits']}/{ci['neg_total']},area={ci['area']},total={ci['total_score']:.2f}"
            for ci in candidate_infos
        ]
        self.log(
            f"[Step9-BC中心] {object_name} mask候选选择：selected=#{best_idx}, "
            f"{'; '.join(summary)}"
        )

        return selected, int(best_idx), {
    ...
    def _get_step9_overlap_keep_points(self) -> List[Tuple[float, float]]:
        """
        返回需要 B/C 都保留的共同正点。

        第一帧：如果你在 B 与 C 点选时点了相近的重合区域正点，会自动识别为共同保留点；
        后续帧：上一帧 B∩C 的中心也会作为共同保留点，减少 overlap 突然消失。
        """
        keep: List[Tuple[float, float]] = []
        if self.step9_overlap_last_center is not None:
            keep.append((float(self.step9_overlap_last_center[0]), float(self.step9_overlap_last_center[1])))

        # B/C 正点中距离较近的点视作“共同 overlap 正点”。
        for bp in self.step9_b_positive_points or []:
            for cp in self.step9_c_positive_points or []:
                try:
                    bx, by = float(bp[0]), float(bp[1])
                    cx, cy = float(cp[0]), float(cp[1])
                except Exception:
                    continue
                if math.hypot(bx - cx, by - cy) <= 10.0:
                    keep.append(((bx + cx) / 2.0, (by + cy) / 2.0))
        return self._merge_near_points(keep, min_dist_px=4.0)
    ...
    def _predict_step9_b_mask_by_sam2(
        ...
        修改点：
            - 不再直接选择 SAM2 score 最高的候选；
            - 优先选择覆盖 B 正点和 B/C 共同 overlap 正点的候选；
            - 后续帧会把上一帧 overlap_center 作为共同保留点，避免 overlap 被候选选择丢掉。
        """
        import torch
        ...
        keep_points = self._get_step9_overlap_keep_points()
        b_mask, best_idx, select_info = self._select_sam2_mask_keep_positive_points(
            masks=masks,
            scores=scores,
            positive_points=list(positive_points or []),
            negative_points=list(negative_points or []),
            object_name="B",
            force_keep_points=keep_points,
            require_positive_point=True,
            point_radius_px=4,
        )
        ...
        selected = select_info.get("selected", {}) if isinstance(select_info, dict) else {}
        self.log(
            f"[Step9-BC中心] SAM2 B 分割完成：selected_idx={best_idx}, "
            f"sam2_score={float(scores_arr[best_idx]):.6f}, "
            f"pos_hits={selected.get('pos_hits')}/{selected.get('pos_total')}, "
            f"area={area} px, center={center}, bbox={bbox}"
        )
        return b_mask


    def _predict_step9_c_mask_by_sam2(
        self,
        image_rgb: np.ndarray,
        positive_points: List[Tuple[float, float]],
        negative_points: Optional[List[Tuple[float, float]]] = None,
        ...
        """
        调用 SAM2 ImagePredictor 动态分割 C。

        修改点：
            - 不再直接选择 SAM2 score 最高的候选；
            - 优先选择覆盖 C 正点和 B/C 共同 overlap 正点的候选；
            - 后续帧会把上一帧 overlap_center 作为共同保留点。
        """
        import torch
        ...
        if not coords or 1 not in labels:
            raise ValueError("Step9 C 分割至少需要一个正点。")
        ...
        keep_points = self._get_step9_overlap_keep_points()
        c_mask, best_idx, select_info = self._select_sam2_mask_keep_positive_points(
            masks=masks,
            scores=scores,
            positive_points=list(positive_points or []),
            negative_points=list(negative_points or []),
        ...
        area = int(np.count_nonzero(c_mask))
        if area <= 0:
            raise RuntimeError("SAM2 返回的 C mask 为空。")

        scores_arr = np.asarray(scores).reshape(-1)
        center = self._mask_center_xy(c_mask)
        bbox = self._mask_bbox_xyxy(c_mask, margin=30.0)
        self.step9_c_last_mask = c_mask.copy()
        self.step9_c_last_center = center
        self.step9_c_last_box = bbox

        selected = select_info.get("selected", {}) if isinstance(select_info, dict) else {}
        self.log(
            f"[Step9-BC中心] SAM2 C 动态分割完成：selected_idx={best_idx}, "
            f"sam2_score={float(scores_arr[best_idx]):.6f}, "
            f"pos_hits={selected.get('pos_hits')}/{selected.get('pos_total')}, "
            f"area={area} px, center={center}, bbox={bbox}"
        )
    ...
    def prepare_step9_c_segmentation(self):
        """
        单独测试 Step9 前准备 C 的动态 SAM2 分割。

        注意：
            旧版 Step9 是加载固定 static_c_mask.png；
            现在 C 也在 Step9 中用 SAM2 分割并逐帧跟踪，
            因此 1/2 轴移动后，C mask 会随当前图像位置变化。
        """
        output_dir = self._get_step9_output_dir("c_initial")
        output_dir.mkdir(parents=True, exist_ok=True)
        image_rgb = self._capture_current_rule_ab_frame(output_dir=output_dir)

        pos, neg = self._select_step9_c_points_interactively(
            image_rgb=image_rgb,
            window_name="Step9 C segmentation: left positive, right negative",
            scale=float(getattr(self.cfg, "rule_ab_confirm_window_scale", 0.85)),
        )
        self.step9_c_positive_points = list(pos)
        self.step9_c_negative_points = list(neg)

        c_mask = self._predict_step9_c_mask_by_sam2(
            image_rgb=image_rgb,
            positive_points=self.step9_c_positive_points,
            negative_points=self.step9_c_negative_points,
            box_xyxy=None,
        )

        # 保存初始 C 分割检查图
        try:
            out_dir = self._get_step9_output_dir("c_initial")
            out_dir.mkdir(parents=True, exist_ok=True)
            canvas = cv2.cvtColor(np.asarray(image_rgb).copy(), cv2.COLOR_RGB2BGR)
            overlay = canvas.copy()
            overlay[c_mask.astype(bool)] = (255, 0, 0)
            canvas = cv2.addWeighted(overlay, 0.35, canvas, 0.65, 0)
            if self.step9_c_last_center is not None:
                cx, cy = int(round(self.step9_c_last_center[0])), int(round(self.step9_c_last_center[1]))
                cv2.circle(canvas, (cx, cy), 6, (0, 0, 255), -1)
            ts = datetime.now().strftime("%Y%m%d_%H%M%S_%f")[:-3]
            cv2.imwrite(str(out_dir / f"step9_c_initial_{ts}.png"), canvas)
        except Exception as e:
            self.log(f"[Step9-BC中心] 保存 C 初始分割图失败：{e}")

        self.log(
            f"[Step9-BC中心] C 初始动态分割已准备：positive={len(pos)}, negative={len(neg)}, "
            f"center={self.step9_c_last_center}"
        )


    def reset_step9_tracking_state(self, clear_b_points: bool = False, clear_c_points: bool = False):
        """
        重置 Step9 的 B/C 动态 SAM2 跟踪缓存。

        clear_b_points=True 时，下次单独测试会重新要求人工选择 B 点。
        clear_c_points=True 时，下次单独测试会重新要求人工选择 C 点。
        """
        if clear_b_points:
            self.step9_b_positive_points = []
            self.step9_b_negative_points = []
        if clear_c_points:
            self.step9_c_positive_points = []
            self.step9_c_negative_points = []

        self.step9_b_last_mask = None
        self.step9_b_last_center = None
        self.step9_b_last_box = None

        self.step9_c_last_mask = None
        self.step9_c_last_center = None
        self.step9_c_last_box = None
        self.step9_overlap_last_center = None

        # 旧固定 C mask 缓存也清空，避免误用旧 C。
        self.step9_c_mask_cache = None
        self.step9_c_mask_path = None

    def _capture_bc_overlap_center_once(self) -> Dict[str, Any]:
        """
        截图，并对 B、C 都调用 SAM2 动态分割/跟踪，然后计算 B∩C 的重合区域中心。

        重要：
            - 旧版 Step9 使用固定 C mask，1/2 轴移动后 C mask 不会跟随图像变化；
            - 当前版本 C 也在每帧用 SAM2 动态更新；
            - 初始帧需要分别人工选择 B 点和 C 点；
            - 后续帧使用上一帧中心点 + bbox 作为 prompt/box 继续跟踪。
        """
        output_dir = self._get_step9_output_dir("frames")
        output_dir.mkdir(parents=True, exist_ok=True)
        image_rgb = self._capture_current_rule_ab_frame(output_dir=output_dir)

        if not self.step9_b_positive_points and self.step9_b_last_center is None:
            return {
                "ok": False,
                "reason": "b_prompt_not_ready",
                "image_rgb": image_rgb,
                "b_mask": None,
                "c_mask": self.step9_c_last_mask,
                "overlap_mask": None,
                "b_mask_area_px": "",
                "c_mask_area_px": int(np.count_nonzero(self.step9_c_last_mask)) if self.step9_c_last_mask is not None else "",
                "overlap_area_px": 0,
            }

        if not self.step9_c_positive_points and self.step9_c_last_center is None:
            return {
                "ok": False,
                "reason": "c_prompt_not_ready",
                "image_rgb": image_rgb,
                "b_mask": self.step9_b_last_mask,
                "c_mask": None,
                "overlap_mask": None,
                "b_mask_area_px": int(np.count_nonzero(self.step9_b_last_mask)) if self.step9_b_last_mask is not None else "",
                "c_mask_area_px": "",
                "overlap_area_px": 0,
            }
        ...
        try:
            if self.step9_b_last_center is not None:
                b_pos = [self.step9_b_last_center]
                if self.step9_overlap_last_center is not None:
                    b_pos.append(self.step9_overlap_last_center)
                b_neg = []
                b_box = self.step9_b_last_box
            else:
                b_pos = list(self.step9_b_positive_points)
                b_neg = list(self.step9_b_negative_points)
                b_box = None
            ...
            self.log(f"[Step9-BC中心] B 跟踪分割失败，尝试退回初始 B 点：{e}")
            b_mask = self._predict_step9_b_mask_by_sam2(
                image_rgb=image_rgb,
                positive_points=list(self.step9_b_positive_points),
                negative_points=list(self.step9_b_negative_points),
                box_xyxy=None,
            )

        # 每帧都调用 SAM2 分割/跟踪 C，不再加载固定 C mask。
        try:
            if self.step9_c_last_center is not None:
                c_pos = [self.step9_c_last_center]
                if self.step9_overlap_last_center is not None:
                    c_pos.append(self.step9_overlap_last_center)
                c_neg = []
                c_box = self.step9_c_last_box
            else:
                c_pos = list(self.step9_c_positive_points)
                c_neg = list(self.step9_c_negative_points)
                c_box = None
            ...
            self.log(f"[Step9-BC中心] C 跟踪分割失败，尝试退回初始 C 点：{e}")
            c_mask = self._predict_step9_c_mask_by_sam2(
                image_rgb=image_rgb,
                positive_points=list(self.step9_c_positive_points),
                negative_points=list(self.step9_c_negative_points),
                box_xyxy=None,
            )

        if b_mask.shape != c_mask.shape:
            h = min(b_mask.shape[0], c_mask.shape[0])
            w = min(b_mask.shape[1], c_mask.shape[1])
            b_mask = b_mask[:h, :w]
            c_mask = c_mask[:h, :w]

        overlap = np.logical_and(b_mask, c_mask)
        center = self._mask_center_xy(overlap)
        area_px = int(np.count_nonzero(overlap))

        if center is None:
            self.step9_overlap_last_center = None
            return {
                "ok": False,
                "reason": "empty_b_c_overlap",
                "image_rgb": image_rgb,
                "b_mask": b_mask,
                "c_mask": c_mask,
                "overlap_mask": overlap,
                "b_mask_area_px": int(np.count_nonzero(b_mask)),
                "c_mask_area_px": int(np.count_nonzero(c_mask)),
                "overlap_area_px": area_px,
                "b_center": self.step9_b_last_center,
                "c_center": self.step9_c_last_center,
            }

        self.step9_overlap_last_center = center
    ...
    def _save_step9_bc_overlay(
        ...
        try:
            ...
            if overlap_mask is not None:
                p_o = mask_dir / f"{stem}_overlap_mask.png"
            ...
            if overlap_mask is not None:
                ...
                overlay = canvas.copy()
                overlay[om] = (0, 255, 255)    # B∩C: yellow
                canvas = cv2.addWeighted(overlay, 0.45, canvas, 0.55, 0)

            if center is not None:
                cx, cy = int(round(center[0])), int(round(center[1]))
                cv2.circle(canvas, (cx, cy), 7, (0, 0, 255), -1)
                cv2.putText(canvas, "BC center", (cx + 10, cy - 8), cv2.FONT_HERSHEY_SIMPLEX, 0.55, (0, 0, 255), 2, cv2.LINE_AA)
    ...
    def run_rule_ac_until_threshold(self) -> Dict[str, Any]:
        ...
        """
        self.log("========== Step9：B∩C 重合区域中心对齐，必要时移动1/2通道 ==========")
        ...
        while True:
            ...
            if cycle_idx > max_cycles:
                self.log(
                    f"[Step9-BC中心] 当前 cycle={cycle_idx} 已超过 GUI 当前最大循环 max_cycles={max_cycles}，退出 Step9。"
                )
                break
            ...
            self.log(
                ...
                f"center=({cx:.2f},{cy:.2f}), target=({target_x:.2f},{target_y:.2f}), "
                f"dx={dx:.2f}, dy={dy:.2f}, tol={tol}, "
                f"overlap_area={det.get('overlap_area_px')}, "
                f"step={getattr(self.cfg, 'rule_ac_stage_step_size', None)}, "
                f"max_cycles={max_cycles}, enable_stage={getattr(self.cfg, 'rule_ac_enable_stage', None)}, "
        ...
        result = {
            "ok": bool(final_ok),
            "reason": "aligned" if final_ok else "not_aligned_or_stopped",
            "target": (
...
class MeasurementWorkflowGUI:
    ...
    def _build_ui(self):
        ...
        rule_buttons.grid(row=14, column=0, columnspan=4, padx=2, pady=6, sticky="ew")
        for i in range(4):
        ...
        ttk.Label(rule_frame, textvariable=self.rule_module_status_var, wraplength=450).grid(
            row=15, column=0, columnspan=4, padx=4, pady=(4, 0), sticky="w"
        )
    ...
    def select_step9_target(self):
        ...
        try:
            ...
            window_name = "Select Step9 target: B-C overlap center target"
    ...
    def test_rule_ac(self):
        """
        Step9 单独测试：B∩C 重合区域中心对齐。

        新逻辑：
            1. 必须有 Step9 目标点；
            2. 每次点击都会新建一个 Step9 run 文件夹保存过程结果；
            3. 测试开始时分别调用 SAM2 对 B 和 C 进行初始分割；
            4. 后续每帧继续用 SAM2 动态更新 B 和 C；
            5. 不再使用固定 static_c_mask.png 计算 B∩C；
            6. 运动过程中可修改 RuleAC/Step9 输入，下一轮立即读取；
            7. 可通过“停止1/2轴运动”按钮立即停止。
        """
        try:
            self.set_var(self.rule_module_status_var, "规则模块状态：Step9 B∩C动态中心对齐测试中")
            self.log("========== Step9 单独测试：B∩C 动态重合中心对齐 ==========")
            ...
            if tx < 0 or ty < 0:
                self.log("[Step9-BC中心] 尚未设置目标点，先进入目标点选择。")
            ...
            # 重置 Step9 停止状态和上一轮 B/C 跟踪结果；每次单独测试都重新选择/分割 B/C。
            wf.step9_stop_requested = False
            wf.reset_step9_tracking_state(clear_b_points=True, clear_c_points=True)

            self.log(
                f"[Step9-BC中心] 参数：target=({wf.cfg.rule_ac_target_x_px}, {wf.cfg.rule_ac_target_y_px}), "
                ...
                f"C_mode=dynamic_sam2，不再加载固定 C_mask，"
            ...
            # 先调用 SAM2 分割 B 和 C。没有这一步就无法计算动态 B∩C。
            self.log("[Step9-BC中心] 正在选择 B 点并调用 SAM2 分割 B。")
            wf.prepare_step9_b_segmentation()

            self.log("[Step9-BC中心] 正在选择 C 点并调用 SAM2 动态分割 C。")
            wf.prepare_step9_c_segmentation()
            ...
            self.log(f"[Step9-BC中心] 单独测试失败：{e}")
            self.log(traceback.format_exc())
    ...
    def stop_step9_stage12(self):
        """
        GUI按钮：停止 Step9 的 1/2 通道运动。
        """
        try:
            if self.workflow is not None:
                self.workflow.stop_step9_stage12()
            self.set_var(self.rule_module_status_var, "规则模块状态：已请求停止1/2轴运动")
            self.log("[GUI] 已请求停止 Step9 1/2轴运动")
        except Exception as e:

...
class MeasurementConfig:
    ...
    capture_area: Tuple[int, int, int, int] = tuple(_cfg("capture_area", (116, 100, 1112, 888)))  # type: ignore
...
class MeasurementWorkflow:
    ...
    def begin_step9_run_session(self) -> Path:
        ...
        该文件夹用于保存：
            1. 每轮原始截图；
            2. 每轮颜色检测区域 mask；
            3. 每轮干净 overlay 检查图（只叠加 color_mask、color center、target）；
            4. step9_history.csv；
            5. step9_meta.json。

        注意：Step9/Step8 对齐逻辑只使用颜色检测区域中心。
        本版本不再保存/叠加 A/B/C 分割调试 mask，避免保存图被大面积调试 mask 覆盖。
        """
    ...
    def _normalize_angle_0_360(angle_degrees: float) -> float:
        """把任意有符号角度统一到 [0, 360) 范围。保留兼容旧字段/旧调用。"""
        a = float(angle_degrees) % 360.0
        if a < 0:
            a += 360.0
        if abs(a - 360.0) < 1e-9:
            a = 0.0
    ...
    @staticmethod
    def _normalize_angle_0_180(angle_degrees: float) -> float:
        """把长边线段角度统一显示/保存为 [0, 180) 的无方向角度。

        YOLO-OBB 长边是一条没有箭头方向的线段，theta 与 theta+180 表示同一条边。
        因此 Step1、Step7、GUI 列表、CSV/XLSX 中显示的角度统一使用 0~180°，
        避免 277°/97° 这种同一条边反向端点导致的显示跳变。
        """
        a = float(angle_degrees) % 180.0
        if a < 0:
            a += 180.0
        if abs(a - 180.0) < 1e-9:
            a = 0.0
    ...
    @classmethod
    def _yolo_obb_normalize_angle(cls, angle_degrees: float) -> float:
        # 长边是无方向线段：277° 与 97° 等价。最终显示/保存统一为 [0, 180)。
        return cls._normalize_angle_0_180(angle_degrees)
    ...
    def _run_angle_detector_once_raw(self, use_second_detect: bool = False, label: str = "yolo_obb") -> Dict[str, Any]:
        ...
        # 按用户要求：不再保存 ab_angle_realtime_yolo_obb 下的 raw / overlay / meta 文件。
        # 角度检测仍然正常执行，只把路径字段留空，避免磁盘 IO 拖慢 Step7 检测频率。
        raw_path_str = ""
        overlay_path_str = ""
        meta_path_str = ""
        if result.obb is None or len(result.obb) == 0:
            return {"ok": False, "angle_ok": False, "angle_deg": None, "angle_deg_raw": None, "reason": "yolo_obb_no_detection", "angle_source": "yolo_obb_no_detection", "edge_selection_mode": "yolo_obb_long_edge", "image_path": raw_path_str, "raw_image_path": raw_path_str, "detection_count": 0, "timestamp": timestamp}
    ...
    def _normalize_angle_result(result: Any) -> Dict[str, Any]:
        ...
        if isinstance(result, dict):
            # 兼容其它角度入口：只要返回了角度字段，就在进入日志/CSV/GUI 前统一到 [0, 180) 无方向角度。
            # delta/阈值判断仍由 angle_diff_deg 完成，不改变原来的无向边差值逻辑。
            for _k in ("angle_deg", "angle_deg_raw", "final_angle", "baseline_angle"):
                if result.get(_k) is not None:
                    try:
                        result[_k] = MeasurementWorkflow._normalize_angle_0_180(float(result[_k]))
                    except Exception:
                        pass
            if isinstance(result.get("selected_b_edge"), dict) and result["selected_b_edge"].get("angle_deg") is not None:
                try:
                    result["selected_b_edge"]["angle_deg"] = MeasurementWorkflow._normalize_angle_0_180(float(result["selected_b_edge"]["angle_deg"]))
                except Exception:
                    pass
    ...
    def detect_angle_once(self, label: str, allow_fail: bool = True) -> Dict[str, Any]:
        ...
        if not result.get("ok", False):
            ...
            return result
        self.log(f"[YOLO-OBB角度-{label}] angle={result.get('angle_deg')}, conf={result.get('confidence')}, class={result.get('class_name')}；overlay保存已关闭")
        return result
    ...
    @classmethod
    def _edge_angle_from_endpoints(cls, p1: Tuple[float, float], p2: Tuple[float, float]) -> float:
        dx = float(p2[0]) - float(p1[0])
        dy = float(p2[1]) - float(p1[1])
        # 与 YOLO-OBB 角度保持一致：所有最终角度都显示/保存为 [0, 180) 无方向角度。
        # 注意：angle_diff_deg 仍按 0~180 无向边差值计算，所以阈值判断逻辑不变。
        return cls._normalize_angle_0_180(math.degrees(math.atan2(dy, dx)))
    ...
    def _save_step7_route_overlay_image(
        ...
        当前版本按用户要求关闭 step7_a_route_overlays 图片保存，避免磁盘 IO 拖慢 Step7。
        保留函数入口以兼容调用链，但直接返回空路径。
        """
        return ""
        try:
    ...
    def _step9_direction_from_error(self, dx: float, dy: float, tol: float) -> int:
        """
        根据“当前重合/颜色检测区域中心 -> 目标点”的误差，选择希望图像中心移动的视觉方向。
        ...
        判断原则：
            dx > 0：目标在当前中心右侧，希望图像中的重合区域中心向右移动；
            dx < 0：目标在当前中心左侧，希望图像中的重合区域中心向左移动；
            dy > 0：目标在当前中心下方，希望图像中的重合区域中心向下移动；
            dy < 0：目标在当前中心上方，希望图像中的重合区域中心向上移动。

        你实测定义的 1/2 轴“图像视觉运动方向”为：
            CH1 -  = 右上
            CH1 +  = 左下
            CH2 -  = 左上
            CH2 +  = 右下
        ...
        action_code 定义的是“希望图像中的重合区域中心移动方向”：
             1  = 右上，只动 CH1-
            -1  = 左下，只动 CH1+
             2  = 左上，只动 CH2-
            -2  = 右下，只动 CH2+
    ...
    def _execute_stage12_diagonal_action(self, action_code: int) -> Dict[str, Any]:
        """
        按你实测的 1/2 轴“图像视觉运动方向”执行 Step9 对齐。

        这里的 action_code / action_name 表示希望图像中的重合区域中心移动的方向，
        不是抽象的物理 stage 坐标方向，也不再做额外反向。
        ...
        # action_name 表示“希望图像中的重合/颜色检测区域中心移动的方向”。
        # ch1_distance/ch2_distance 是直接发送给 1/2 通道的相对步数；
        # 这里已经使用你实测的图像视觉方向表，不再额外反向。
        mapping = {
            1:   (-step,  0.0,   "右上"),
            -1:  (step,   0.0,   "左下"),
            2:   (0.0,  -step,  "左上"),
        ...
        if not bool(getattr(self.cfg, "rule_ac_enable_stage", False)):
            self.log(
                f"[Step9-颜色中心] DRY-RUN：希望图像中心向{name}移动，"
                f"直接发送 CH1={d1}, CH2={d2}；但 RuleAC真动Stage12 未勾选"
            )
            return {
                "moved": False,
                "dry_run": True,
        ...
        self.log(
            f"[Step9-颜色中心] 希望图像中心向{name}移动："
            f"直接发送 CH1={d1}, CH2={d2} "
            f"[按实测 1/2 轴图像视觉方向表执行，无额外反向]"
        )
    ...
    def _capture_step9_color_center_once(self) -> Dict[str, Any]:
        """
        Step9/Step8 当前帧检测：颜色检测区域中心对齐。

        当前版本只做颜色检测：
            1) 截取当前显微图像；
            2) 根据 Step9 标定 HSV 生成 color_mask；
            3) 计算 color_mask 中心作为 color_center。

        这里不再调用 RuleAB follower，不再分割/保存/叠加 A/B/C mask。
        Step9 的运动方向和停止条件只由 color_center、target 和 tolerance 决定。
        """
        try:
    ...
    def _save_step9_color_overlay(
        ...
        """
        保存 Step9/Step8 颜色检测区域中心对齐检查图。

        本版本只保存三类结果：
            1) 原始截图 frame；
            2) 颜色检测区域 color_mask；
            3) 干净 overlay：原始截图 + color_mask + color center + target。

        a_mask / b_mask / c_mask / c_raw_mask 参数保留只是为了兼容旧调用，
        这里不再保存，也不再叠加，避免 A/B/C 调试 mask 把保存图覆盖成大块颜色。
        """
        saved = {
            "overlay_path": None,
            "frame_path": None,
        ...
        try:
            ...
            # 只叠加颜色检测区域。这里不叠加 A/B/C mask。
            if color_mask is not None:
                cm = np.asarray(color_mask).astype(bool)
                overlay = canvas.copy()
                overlay[cm] = (255, 255, 0)  # BGR: cyan
            ...
            self.log(f"[Step9-颜色中心] 保存颜色检测检查图失败：{e}")
    ...
    def run_rule_ac_until_threshold(self) -> Dict[str, Any]:
        ...
        本版本支持：
            ...
            2. 每轮保存原始截图、颜色区域 mask、干净 overlay 和 CSV；
    ...
    def _set_step7_gui_result(self, cycle_index: int, final_angle: Any = None, final_delta: Any = None):
        """把 Step7 实时角度结果补写到右侧 GUI 列表对应轮次。

        本程序在 Step5.5 就会异步保存并追加 plot_points；Step7 的 final_angle/final_delta
        在后面才产生。因此这里按真实 cycle_index 回填，保证右侧列表能同时显示：
        Step1 角度、拟合峰值、Step7 final_angle、Step7 final_delta。
        """
        try:
            fa = None if final_angle is None else self._normalize_angle_0_180(float(final_angle))
        except Exception:
            fa = final_angle
        try:
            fd = None if final_delta is None else float(final_delta)
        except Exception:
            fd = final_delta
        ...
        try:
            for point in self.plot_points:
                try:
                    if int(point.get("cycle_index")) == int(cycle_index):
                        point["step7_final_angle"] = fa
                        point["step7_final_delta"] = fd
                        updated = True
                        break
                except Exception:
                    continue
            self.context["plot_points"] = self.plot_points
            self.context["last_rule_ab_final_angle"] = fa
            self.context["last_rule_ab_final_delta"] = fd
            self.log(
                f"[绘图] 已回填 Step7 结果：cycle={cycle_index}, "
                f"step7_final_angle={fa}, step7_final_delta={fd}, updated={updated}"
            )
        except Exception as e:
            self.log(f"[绘图] 回填 Step7 结果失败：cycle={cycle_index}, error={e}")
    ...
    def run_one_cycle(self, cycle_index: int) -> bool:
        ...
        try:
            ...
            if not bool(rule_ab_result.get("ok", False)):
                ...
                final_angle = rule_ab_result.get("final_angle")
                self._set_step7_gui_result(cycle_index, final_angle=final_angle, final_delta=final_delta)
                self.notify_update()
            ...
            self._set_step7_gui_result(cycle_index, final_angle=final_step7_angle, final_delta=final_step7_delta)
...
class MeasurementWorkflowGUI:
    ...
    def _build_ui(self):
        ...
        columns = ("cycle", "angle", "fit_peak", "step7_final_angle", "step7_final_delta")
        ...
        self.angle_fit_tree.heading("angle", text="Step1角度/deg")
        self.angle_fit_tree.heading("fit_peak", text="拟合峰值")
        self.angle_fit_tree.heading("step7_final_angle", text="Step7 final_angle/deg")
        self.angle_fit_tree.heading("step7_final_delta", text="Step7 final_delta/deg")
        self.angle_fit_tree.column("cycle", width=55, anchor="center", stretch=False)
        self.angle_fit_tree.column("angle", width=115, anchor="center", stretch=True)
        self.angle_fit_tree.column("fit_peak", width=105, anchor="center", stretch=True)
        self.angle_fit_tree.column("step7_final_angle", width=145, anchor="center", stretch=True)
        self.angle_fit_tree.column("step7_final_delta", width=145, anchor="center", stretch=True)
    ...
    def update_angle_fit_peak_plot(self, workflow: Optional[MeasurementWorkflow] = None):
        ...
        实时更新界面：
            1. 右侧列表显示每轮的：
               ...
               - Step1 无方向角度 / deg；
               - 拟合峰值；
               - Step7 final_angle / deg；
               - Step7 final_delta / deg。
            2. 中间图像显示区固定显示三张图：
               - 序号 - 拟合峰值；
               - xlsx横坐标 - 原始数据；
               - xlsx横坐标 - 中值滤波结果。
        ...
        # 右侧列表：显示 Step1 角度、拟合峰值、Step7 final_angle 和 Step7 final_delta。
        # 即使角度检测失败，相关角度为 None，也保留该轮记录。
        ...
        for p in points:
            try:
                ...
                list_points.append((record_index, angle_value, fit_peak_value, step7_final_angle_value, step7_final_delta_value))
    ...
    def update_angle_fit_xy_list(self, valid_points: List[Tuple[int, Optional[float], Optional[float], Optional[float], Optional[float]]]):
        ...
        列表包括：Step1 无方向角度、拟合峰值、Step7 final_angle、Step7 final_delta。
        如果某一项尚未产生或检测失败，则显示为 None。
        """
        ...
        try:
            ...
            for cycle_index, angle, fit_peak, step7_final_angle, step7_final_delta in valid_points:
                tree.insert(
                    "",
                    tk.END,
                    values=(
                        ...
                        self._format_optional_float(step7_final_angle),
            ...
            self.angle_fit_list_status_var.set(
                ...
                "Step1/Step7角度均为0~180°无方向长边角度，final_delta为当前轮Step7相对Step1的角度差"
            )
        except Exception as e:

...
class MeasurementWorkflow:
    def __init__(self, cfg: MeasurementConfig, on_log=None, on_update=None):
        ...
        # angle_monitor_thread 只允许设置 stop_event、关激光、emergency stop。
    ...
    def _laser_off_once_threadsafe(self, reason: str = "") -> bool:
        """多个线程可能同时触发阈值时，只允许一个线程真正执行 laser_off。"""
        with self.rule_ab_laser_lock:
            try:
                if bool(self.context.get("laser_on", False)):
                    self.log(f"[Step7实时] 触发 laser_off；reason={reason}")
                    self.laser_off()
                    return True
                self.log(f"[Step7实时] laser_on=False，跳过重复 laser_off；reason={reason}")
                return False
            except Exception as e:
                self.log(f"[Step7实时] laser_off 失败：{e}")
                return False
                    ...
                    try:
                        ...
                        if angle_ok and delta is not None:
                            if float(delta) >= float(min_delta) and float(delta) < float(max_delta):
                                ...
                                _append_record(rec)
                                self.log(f"[Step7实时角度] 达到目标范围：final_angle={angle_deg:.6f}°, baseline={baseline_angle}, delta={delta:.3f}°, target=[{min_delta:.3f},{max_delta:.3f})；停止/暂停Stage34当前推动，不关闭激光，继续完整测量。")
                                stop_event.set()
                            ...
                            if float(delta) >= float(max_delta):
                                ...
                                _append_record(rec)
                                self.log(f"[Step7实时角度] 角度变化超过目标上限，仅记录为 overshoot，不关闭激光，继续完整测量：final_angle={angle_deg:.6f}°, baseline={baseline_angle}, delta={delta:.3f}° >= {max_delta:.3f}°。")
                                stop_event.set()
        ...
        while True:
            ...
            if close_hit:
                ...
                self.log(
                    ...
                    f"目标范围=[{float(min_delta):.3f}, {float(max_delta):.3f})；停止本次 Stage34 推动，不关闭激光，继续完整测量。"
                )
                break
            ...
            if final_delta is not None and float(final_delta) < float(min_delta):
                ...
                self.log(
                    ...
                    f"{float(max_delta):.3f}°；仅记录为 overshoot，不关闭激光，停止本次 Stage34 推动并继续完整测量。"
                )
                break
            else:

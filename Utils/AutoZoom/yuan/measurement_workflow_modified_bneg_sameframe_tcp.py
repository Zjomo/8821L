...
class MeasurementConfig:
    ...
    # B 负点颜色排除：负点用于建立“明确不是 B”的 HSV seed，后续 Bmask 必须远离这些颜色。
    rule_ab_b_negative_color_seed_radius_px: int = int(_cfg("rule_ab_b_negative_color_seed_radius_px", _cfg("rule_ab_b_positive_color_seed_radius_px", 5)))
    rule_ab_b_negative_reject_threshold: float = float(_cfg("rule_ab_b_negative_reject_threshold", 1.25))
    rule_ab_b_negative_margin: float = float(_cfg("rule_ab_b_negative_margin", 0.10))
    rule_ab_b_hsv_center_count: int = int(_cfg("rule_ab_b_hsv_center_count", 4))
...
class CalibrationState:
    ...
    # B 负点颜色模板：标定 B 负点附近提取的 HSV 种子，后续用于排除 A/C/背景等非 B 颜色。
    b_negative_hsv_seeds: List[List[float]] = field(default_factory=list)
    b_negative_hsv_seed_count: int = 0
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "CalibrationState":
        ...
        neg_hsv_seeds: List[List[float]] = []
        for p in (getattr(state, "b_negative_hsv_seeds", []) or []):
            try:
                neg_hsv_seeds.append([float(p[0]), float(p[1]), float(p[2])])
            except Exception:
                continue
        state.b_negative_hsv_seeds = neg_hsv_seeds
        try:
            state.b_negative_hsv_seed_count = int(len(state.b_negative_hsv_seeds))
        except Exception:
            state.b_negative_hsv_seed_count = 0
        return state
...
class MeasurementWorkflow:
    ...
    def detect_bmask_longest_edge_angle_once(
        ...
        try:
            ...
            result = {
                ...
                "candidate_edges": self._json_safe(edge.get("candidate_edges", [])),
                "raw_edge": self._json_safe(edge.get("selected_b_edge")),
                "b_mask": np.asarray(b_mask).astype(bool),
                "image_rgb": np.asarray(image_rgb).copy(),
                "angle_frame_id": f"{datetime.now().strftime('%Y%m%d_%H%M%S_%f')}_{label}",
            ...
            result.update(paths)
            if "raw_image_path" not in result:
                result["raw_image_path"] = result.get("image_path") or result.get("overlay_path")
            self.context["bmask_longest_edge_angle_state"] = self._json_safe(result)
    ...
    def _repair_rule_ab_angle_if_jump(
        ...
        """
        Step7 每帧角度修复/确认：只以“上一帧最终角度”为连续性参考。

        规则：
            1) raw_angle 与上一帧最终角度 last_confirmed_angle 的 0~180 周期最小差 <= 2°：
               认为 Bmask 最长边检测正确，final_angle = raw_angle。
            2) raw_angle 与 last_confirmed_angle 的差 > 2°：
               直接复用 raw 检测同一帧 angle_result["candidate_edges"]；
               只接受与 last_confirmed_angle 相差 <= 2° 的同帧候选边作为 repaired_angle。
            3) 找不到候选边时，本帧角度无效；不使用 raw_angle，不关闭激光，不停止完整测量。

        注意：baseline_angle 只用于计算 final_delta_from_baseline，不作为修复参考。
        """
        ...
        info: Dict[str, Any] = {
            ...
            "selected_reference_name": "last_confirmed_angle",
            "selected_edge": None,
            "repaired_angle": None,
        ...
        if not info["enabled"]:
            final_angle = float(raw_angle) if raw_angle is not None else None
            info.update({
            ...
            try:
                ...
                angle_result["angle_deg_raw_before_repair"] = final_angle
                angle_result["angle_deg"] = final_angle
                angle_result["angle_source"] = info["angle_source"]
                angle_result["used_repaired_angle"] = False
                angle_result["repair_used"] = False
            except Exception:
                pass
        ...
        raw_angle_f = float(raw_angle)
        ref_angle = float(previous_angle) if previous_angle is not None else raw_angle_f
        info["selected_reference_angle"] = ref_angle

        try:
            raw_delta_to_last = self.angle_diff_deg(raw_angle_f, ref_angle)
        except Exception:
            raw_delta_to_last = None
        ...
        if baseline_angle is not None:
            try:
                raw_delta_from_baseline = self.angle_diff_deg(raw_angle_f, float(baseline_angle))
            except Exception:
                raw_delta_from_baseline = None
        info["raw_delta_from_baseline"] = raw_delta_from_baseline

        # raw 与上一帧最终角度连续：直接使用 raw。
        if raw_delta_to_last is not None and float(raw_delta_to_last) <= tol:
            final_delta = None
            if baseline_angle is not None:
                try:
                    final_delta = self.angle_diff_deg(raw_angle_f, float(baseline_angle))
                except Exception:
                    final_delta = None
            info.update({
                ...
                "reason": f"raw_delta_to_last_within_{tol:g}deg_use_raw",
            })
            try:
                angle_result["angle_repair"] = self._json_safe(info)
                angle_result["angle_deg_raw_before_repair"] = raw_angle_f
                angle_result["angle_deg"] = raw_angle_f
                angle_result["final_angle"] = raw_angle_f
                angle_result["angle_source"] = "bmask_longest_edge_raw_ok"
                angle_result["used_repaired_angle"] = False
                angle_result["repair_used"] = False
                angle_result["raw_delta_to_last"] = raw_delta_to_last
                angle_result["final_delta_to_baseline"] = final_delta
                angle_result["last_confirmed_angle_before"] = ref_angle
            except Exception:
                pass
            self.log(
                f"[RuleAB-角度修复] raw_angle={raw_angle_f:.6f}° 与上一帧最终角度 {ref_angle:.6f}° "
                f"差值 {float(raw_delta_to_last):.3f}° <= {tol:.3f}°；使用 raw_angle。"
            )
            return raw_angle_f, info

        # raw 跳变过大：必须使用 raw 角度检测同一帧已经保存的 candidate_edges，禁止重新截图/重新 build scene。
        candidates = []
        try:
            candidates = list((angle_result or {}).get("candidate_edges") or [])
        except Exception:
            candidates = []
        info["candidate_source"] = "angle_result_same_frame_candidate_edges"
        info["same_frame_repair"] = True
        info["repair_reuse_raw_frame"] = True
        info["raw_image_path"] = (angle_result or {}).get("raw_image_path") or (angle_result or {}).get("image_path") or (angle_result or {}).get("overlay_path")
        info["raw_overlay_path"] = (angle_result or {}).get("overlay_path") or (angle_result or {}).get("image_path")
        ...
        enriched: List[Tuple[float, float, float, Dict[str, Any]]] = []
        for c in candidates:
            try:
                a = float(c["angle_deg"])
                d_last = self.angle_diff_deg(a, ref_angle)
                d_raw = self.angle_diff_deg(a, raw_angle_f)
                cc = dict(c)
                cc["diff_to_last_confirmed_deg"] = float(d_last)
                cc["diff_to_raw_deg"] = float(d_raw)
                enriched.append((float(d_last), float(d_raw), a, cc))
            except Exception:
                continue
        ...
        info["repair_candidates"] = [self._json_safe(x[3]) for x in enriched]
        ...
        if not eligible:
            info.update({
                ...
                "raw_confirmed_correct": False,
                "final_angle": None,
                "angle_source": "bmask_same_frame_polygon_edge_repair_failed_no_candidate_close_to_last",
                "reason": f"raw_delta_to_last_gt_{tol:g}deg_and_no_polygon_edge_within_{tol:g}deg",
            })
            try:
                angle_result["angle_repair"] = self._json_safe(info)
                angle_result["angle_deg_raw_before_repair"] = raw_angle_f
                angle_result["angle_deg"] = None
                angle_result["final_angle"] = None
                angle_result["ok"] = False
                angle_result["angle_ok"] = False
                angle_result["allow_close"] = False
                angle_result["angle_source"] = info["angle_source"]
                angle_result["repair_failed"] = True
                angle_result["repair_used"] = False
                angle_result["raw_delta_to_last"] = raw_delta_to_last
            ...
            self.log(
                f"[RuleAB-角度修复] raw_angle={raw_angle_f:.6f}° 与上一帧最终角度 {ref_angle:.6f}° "
                f"差值 {raw_delta_to_last} > {tol:.3f}°；同帧多边形候选仍未找到 ±{tol:.3f}° 内的边，本帧角度无效，不关激光、不停止完整测量。"
            )
            return None, info

        best_last_diff, best_raw_diff, repaired, selected = eligible[0]
        repaired = float(repaired)
        final_delta = None
        if baseline_angle is not None:
            try:
                final_delta = self.angle_diff_deg(repaired, float(baseline_angle))
            except Exception:
                final_delta = None
        ...
        info.update({
            ...
            "repaired_delta_from_baseline": final_delta,
            "repaired_vs_raw_diff_deg": float(best_raw_diff),
            "final_angle": repaired,
            "final_delta_from_baseline": final_delta,
            "angle_source": "bmask_same_frame_polygon_edge_repaired_by_last_angle",
            "reason": f"raw_delta_to_last_gt_{tol:g}deg_repaired_by_same_frame_polygon_edge_within_{tol:g}deg",
        })
        ...
        try:
            ...
            angle_result["final_angle"] = repaired
            angle_result["angle_source"] = "bmask_same_frame_polygon_edge_repaired_by_last_angle"
            angle_result["used_repaired_angle"] = True
            angle_result["repair_used"] = True
            angle_result["repair_failed"] = False
            angle_result["raw_delta_to_last"] = raw_delta_to_last
            angle_result["final_delta_to_baseline"] = final_delta
            angle_result["last_confirmed_angle_before"] = ref_angle
            angle_result["repair_candidates"] = self._json_safe(info.get("repair_candidates", []))
            angle_result["repair_reason"] = info["reason"]
        except Exception:
            pass

        self.log(
            f"[RuleAB-角度修复] raw_angle={raw_angle_f:.6f}° 与上一帧最终角度 {ref_angle:.6f}° "
            f"差值 {raw_delta_to_last} > {tol:.3f}°；使用同帧多边形候选 repaired_angle={repaired:.6f}°，"
            f"repaired_delta_to_last={float(best_last_diff):.3f}°，final_delta={final_delta}。"
        )
    ...
    def _repair_step1_baseline_angle(
        ...
        规则：
            1) 直接复用 raw 角度检测同一帧 angle_result["candidate_edges"]；
        ...
        # Step1 baseline 修复也必须使用 raw 角度检测同一帧的候选边，禁止重新截图/重新 build scene。
        try:
            candidates = list((angle_result or {}).get("candidate_edges") or [])
        except Exception:
            candidates = []
        ...
        info["candidate_angles"] = [float(c.get("angle_deg")) for c in candidates if isinstance(c, dict) and c.get("angle_deg") is not None]
        if not candidates:
            info["reason"] = "no_same_frame_bmask_edge_candidates_use_raw_angle"
            self.log("[Step1-角度修复] 同帧 angle_result 中没有候选边，Step1 baseline 使用 raw_angle；不重新取帧。")
            return raw_angle, info
    ...
    def _extract_b_negative_hsv_seeds_from_points(
        self,
        image_rgb: np.ndarray,
        negative_points: Any,
        label: str = "B-negative",
    ) -> List[List[float]]:
        """从 B 负点附近小窗口提取 HSV median 颜色种子，用于后续排除非 B 颜色。"""
        img = np.asarray(image_rgb).astype(np.uint8)
        if img.ndim != 3 or img.shape[2] < 3:
            return []
        hsv = cv2.cvtColor(cv2.cvtColor(img, cv2.COLOR_RGB2BGR), cv2.COLOR_BGR2HSV)
        h, w = hsv.shape[:2]
        r = max(1, int(getattr(self.cfg, "rule_ab_b_negative_color_seed_radius_px", getattr(self.cfg, "rule_ab_b_positive_color_seed_radius_px", 5))))
        seeds: List[List[float]] = []
        pts = self._normalize_points_xy(negative_points)
        for i, (x, y) in enumerate(pts):
            cx, cy = int(round(float(x))), int(round(float(y)))
            x1, x2 = max(0, cx - r), min(w, cx + r + 1)
            y1, y2 = max(0, cy - r), min(h, cy + r + 1)
            if x2 <= x1 or y2 <= y1:
                self.log(f"[BNegColorSeed] {label} negative point {i} 窗口越界/为空，跳过。")
                continue
            patch = hsv[y1:y2, x1:x2].reshape(-1, 3)
            if patch.shape[0] < 3:
                self.log(f"[BNegColorSeed] {label} negative point {i} 窗口像素太少 n={patch.shape[0]}，跳过。")
                continue
            med = np.median(patch.astype(np.float32), axis=0)
            seed = [float(med[0]), float(med[1]), float(med[2])]
            seeds.append(seed)
            self.log(f"[BNegColorSeed] {label} negative point {i} hsv_seed=({seed[0]:.1f},{seed[1]:.1f},{seed[2]:.1f})")
        self.log(f"[BNegColorSeed] saved b_negative_hsv_seeds count={len(seeds)}")
        return seeds
    ...
    def _build_b_hsv_centers_from_seeds_and_mask(
        ...
        target_k = max(1, int(getattr(self.cfg, "rule_ab_b_hsv_center_count", 4)))
    ...
    def _hsv_color_distance_to_centers(
        self,
        image_rgb: np.ndarray,
        centers: Any,
        h_tol: Optional[float] = None,
        s_tol: Optional[float] = None,
        v_tol: Optional[float] = None,
    ) -> np.ndarray:
        """计算每个像素到一组 HSV center 的最小归一化距离；用于 B 负点颜色排除。"""
        img = np.asarray(image_rgb).astype(np.uint8)
        hsv = cv2.cvtColor(cv2.cvtColor(img, cv2.COLOR_RGB2BGR), cv2.COLOR_BGR2HSV)
        h_tol = max(1.0, float(h_tol if h_tol is not None else getattr(self.cfg, "rule_ab_b_feature_h_tol", getattr(self.cfg, "rule_ab_feature_h_tol", 14))))
        s_tol = max(1.0, float(s_tol if s_tol is not None else getattr(self.cfg, "rule_ab_b_feature_s_tol", getattr(self.cfg, "rule_ab_feature_s_tol", 75))))
        v_tol = max(1.0, float(v_tol if v_tol is not None else getattr(self.cfg, "rule_ab_b_feature_v_tol", getattr(self.cfg, "rule_ab_feature_v_tol", 75))))
        best = np.full(hsv.shape[:2], 9999.0, dtype=np.float32)
        for c in (centers or []):
            try:
                h0, s0, v0 = float(c[0]), float(c[1]), float(c[2])
    ...
    def _raw_color_mask_from_profile(self, image_rgb: np.ndarray, profile: Dict[str, Any]) -> np.ndarray:
        """根据模板颜色阈值得到粗 mask；B 额外应用负点 HSV seed 排除 A/C/背景颜色。"""
        dist = self._hsv_color_distance_to_profile(image_rgb, profile)
        # dist<=sqrt(3) 等价于 H/S/V 都在容差量级内；适当收紧到 1.65 减少串色。
        raw = dist <= 1.65
        label = str(profile.get("label", "")).lower()
        if label == "b":
            neg_centers = profile.get("b_negative_hsv_centers") or profile.get("b_negative_hsv_seeds") or []
            neg_dist = self._hsv_color_distance_to_centers(image_rgb, neg_centers) if neg_centers else None
            neg_threshold = float(getattr(self.cfg, "rule_ab_b_negative_reject_threshold", 1.25))
            neg_margin = float(getattr(self.cfg, "rule_ab_b_negative_margin", 0.10))
            if neg_dist is not None and len(neg_centers) > 0:
                # 保留条件：像 B 正点颜色，并且不像 B 负点颜色。
                # neg_dist 越小越像 A/C/背景等负点颜色；负点过近时从 B 粗 mask 中排除。
                raw = raw & (neg_dist > neg_threshold) & ((neg_dist - dist) > neg_margin)
            try:
                pos_pixels = dist[np.asarray(raw).astype(bool)]
                neg_pixels = neg_dist[np.asarray(raw).astype(bool)] if neg_dist is not None else np.asarray([], dtype=np.float32)
                profile["_last_b_color_debug"] = {
                    "b_positive_hsv_centers": self._json_safe_simple(profile.get("b_hsv_centers", [])),
                    "b_negative_hsv_centers": self._json_safe_simple(neg_centers),
                    "b_negative_reject_threshold": float(neg_threshold),
                    "b_negative_margin": float(neg_margin),
                    "positive_color_score": None if pos_pixels.size <= 0 else float(np.mean(np.exp(-0.5 * pos_pixels.astype(np.float32) * pos_pixels.astype(np.float32)))),
                    "negative_color_score": None if neg_pixels.size <= 0 else float(np.mean(np.exp(-0.5 * neg_pixels.astype(np.float32) * neg_pixels.astype(np.float32)))),
                    "positive_color_mean_distance": None if pos_pixels.size <= 0 else float(np.mean(pos_pixels)),
                    "negative_color_mean_distance": None if neg_pixels.size <= 0 else float(np.mean(neg_pixels)),
                    "raw_area_after_negative_exclusion_px": int(np.count_nonzero(raw)),
                }
            except Exception:
                pass
    ...
    def _track_abc_by_saved_features(
        ...
        for lab, raw in raw_masks.items():
            ...
            if lab == "b":
                try:
                    b_color_debug = dict(profiles[lab].get("_last_b_color_debug", {}) or {})
                    debug[lab].update(b_color_debug)
                    # 同步到 profile，便于 _save_bmask_runtime_debug 写入 CSV。
                    for _k in ("positive_color_score", "negative_color_score", "positive_color_mean_distance", "negative_color_mean_distance"):
                        if _k in b_color_debug:
                            profiles[lab][_k] = b_color_debug.get(_k)
                    debug[lab]["bmask_area"] = int(np.count_nonzero(m))
                    debug[lab]["bmask_center"] = [float(center[0]), float(center[1])] if center is not None else None
                    debug[lab]["bmask_ok"] = bool(np.any(m))
                    debug[lab]["bmask_fail_reason"] = "" if np.any(m) else "empty_after_positive_negative_color_filter_or_component_selection"
                except Exception as _e:
                    debug[lab]["bmask_debug_error"] = str(_e)
            if np.any(m):
    ...
    def _save_bmask_runtime_debug(
        ...
        try:
            ...
            row = {
                ...
                "b_negative_hsv_seed_count": int(prof_b.get("b_negative_hsv_seed_count", len(prof_b.get("b_negative_hsv_seeds", []) or [])) or 0),
                "b_positive_hsv_centers": json.dumps(self._json_safe_simple(prof_b.get("b_hsv_centers", [])), ensure_ascii=False),
                "b_negative_hsv_centers": json.dumps(self._json_safe_simple(prof_b.get("b_negative_hsv_centers", prof_b.get("b_negative_hsv_seeds", []))), ensure_ascii=False),
                "positive_color_score": "" if prof_b.get("positive_color_score") is None else prof_b.get("positive_color_score"),
                "negative_color_score": "" if prof_b.get("negative_color_score") is None else prof_b.get("negative_color_score"),
    ...
    def _install_feature_tracker_on_rule_ab_follower(self, follower: Any, state: Optional[CalibrationState] = None):
        ...
        def _build_templates_from_original_scene() -> Tuple[np.ndarray, Any]:
            ...
            b_neg_seeds: List[List[float]] = []
            try:
                if state is not None:
                    b_neg_seeds = [
                        [float(v[0]), float(v[1]), float(v[2])]
                        for v in getattr(state, "b_negative_hsv_seeds", [])
                        if len(v) >= 3
                    ]
            except Exception:
                b_neg_seeds = []
            if not b_neg_seeds:
                neg_points = []
                try:
                    neg_points = negative_points.get("b", []) if isinstance(negative_points, dict) else []
                except Exception:
                    neg_points = []
                b_neg_seeds = self._extract_b_negative_hsv_seeds_from_points(image_rgb, neg_points, label="B")
                if state is not None:
                    try:
                        state.b_negative_hsv_seeds = [[float(x), float(y), float(z)] for x, y, z in b_neg_seeds]
                        state.b_negative_hsv_seed_count = int(len(b_neg_seeds))
                    except Exception:
                        pass
            b_neg_centers = self._dedupe_hsv_centers(b_neg_seeds)
            profiles["b"]["b_negative_hsv_seeds"] = b_neg_seeds
            profiles["b"]["b_negative_hsv_seed_count"] = int(len(b_neg_seeds))
            profiles["b"]["b_negative_hsv_centers"] = b_neg_centers
            profiles["b"]["b_negative_hsv_center_count"] = int(len(b_neg_centers))
        ...
        def _feature_capture_and_build_scene():
            ...
            #   5) 只改变 Bmask 来源，不改变后续 raw longest-edge angle 与角度修复逻辑。
        ...
        self.log("[ABC特征跟踪] 已安装：SAM2 仅用于首帧模板建立；后续帧仅使用 feature tracker；Bmask 失败不 fallback、不 hold_last；角度修复逻辑保持不变。")
    ...
    def close_all(self):
        ...
        当前要求：
            ...
            4. TCP 从“关闭全部设备”中拆出：这里不关闭 LabVIEW TCP，便于下一次完整测量直接复用。
        """
        self.log("========== 关闭全部设备：关闭运动/激光相关设备，TCP保持连接 ==========")
        self.save_summary_xlsx()
        ...
        # 注意：TCP 已从“关闭全部设备”中拆出。这里不 close self.tcp_server，
        # 不清空 tcp_started/labview_ready，下一次完整循环测量可直接复用已有 LabVIEW TCP。
        try:
            if self.tcp_server is not None:
                self.log("[TCP] 关闭全部设备时保持连接；如需释放端口，请单独点击‘关闭TCP’。")
        except Exception as e:
            self.log(f"[TCP] 保持连接状态提示失败：{e}")
        ...
        self.log("========== 关闭全部设备完成：运动/激光相关设备已关闭，TCP保持连接；运行态已重置 ==========")
        self.notify_update()
...
class MeasurementWorkflowGUI:
    ...
    def _build_ui(self):
        ...
        ttk.Button(button_grid, text="加载完整标定", command=self.load_calibration_to_gui_thread).grid(row=5, column=0, padx=4, pady=4, sticky="ew")
        ttk.Button(button_grid, text="重置后重新标定", command=self.reset_after_stop_for_recalibration_thread).grid(row=5, column=1, padx=4, pady=4, sticky="ew")
        ttk.Button(button_grid, text="暂停运动并重新标定ABC", command=self.pause_midrun_recalibration_thread).grid(row=6, column=0, padx=4, pady=4, sticky="ew")
        ttk.Button(button_grid, text="完成重标定并继续测量", command=self.resume_midrun_recalibration_thread).grid(row=6, column=1, padx=4, pady=4, sticky="ew")
    ...
    def _extract_b_negative_hsv_seeds_from_points(
        ...
        """从 B 负点附近小窗口提取 HSV median 颜色种子，用于排除 A/C/背景等非 B 颜色。"""
        img = np.asarray(image_rgb).astype(np.uint8)
        if img.ndim != 3 or img.shape[2] < 3:
            return []
        try:
            cfg = self.build_config_from_ui()
            radius_px = int(getattr(cfg, "rule_ab_b_negative_color_seed_radius_px", getattr(cfg, "rule_ab_b_positive_color_seed_radius_px", 5)))
        except Exception:
            radius_px = 5
        r = max(1, radius_px)
        hsv = cv2.cvtColor(cv2.cvtColor(img, cv2.COLOR_RGB2BGR), cv2.COLOR_BGR2HSV)
        h_img, w_img = hsv.shape[:2]
        seeds: List[List[float]] = []
        pts = self._normalize_points_xy_for_gui(negative_points)
        for i, (x, y) in enumerate(pts):
            cx, cy = int(round(float(x))), int(round(float(y)))
            x1, x2 = max(0, cx - r), min(w_img, cx + r + 1)
            y1, y2 = max(0, cy - r), min(h_img, cy + r + 1)
    ...
    def calibrate_angle_b(self):
        ...
        try:
            ...
            seeds = self._extract_b_positive_hsv_seeds_from_points(image_rgb, pos, label="Angle-B")
            neg_seeds = self._extract_b_negative_hsv_seeds_from_points(image_rgb, neg, label="Angle-B")
            state.b_positive_hsv_seeds = [[float(h), float(ss), float(v)] for h, ss, v in seeds]
            state.b_positive_hsv_seed_count = int(len(seeds))
            state.b_negative_hsv_seeds = [[float(h), float(ss), float(v)] for h, ss, v in neg_seeds]
            state.b_negative_hsv_seed_count = int(len(neg_seeds))
            ...
            self.log(
                "[完整测量标定] Bmask 标定完成："
                f"B+={len(pos)}, B-={len(neg)}, hsv_seeds={len(seeds)}, neg_hsv_seeds={len(neg_seeds)}；角度边=当前帧Bmask最长边自动选择，不再人工输入边编号。"
            )
        except Exception as e:
            self.log(f"[完整测量标定] Bmask 标定失败：{e}")
            self.log(traceback.format_exc())
            error_msg = str(e)
            self.root.after(0, lambda msg=error_msg: messagebox.showerror("Bmask 标定失败", msg))
    ...
    def calibrate_rule_ab_b(self):
        ...
        try:
            self._prepare_for_manual_calibration(reason="calibrate_rule_ab_b")
            cfg = self.build_config_from_ui()
            ...
            state.rule_ab_b_positive_points = [[float(x), float(y)] for x, y in pos]
            state.rule_ab_b_negative_points = [[float(x), float(y)] for x, y in neg]
            seeds = self._extract_b_positive_hsv_seeds_from_points(image_rgb, pos, label="RuleAB-B")
            neg_seeds = self._extract_b_negative_hsv_seeds_from_points(image_rgb, neg, label="RuleAB-B")
            state.b_positive_hsv_seeds = [[float(h), float(ss), float(v)] for h, ss, v in seeds]
            state.b_positive_hsv_seed_count = int(len(seeds))
            state.b_negative_hsv_seeds = [[float(h), float(ss), float(v)] for h, ss, v in neg_seeds]
            state.b_negative_hsv_seed_count = int(len(neg_seeds))
            self._save_calibration_file_raw(state)
            self.log(f"[完整测量标定] RuleAB-B 标定完成：B+={len(pos)}, B-={len(neg)}, hsv_seeds={len(seeds)}, neg_hsv_seeds={len(neg_seeds)}")
        except Exception as e:
            self.log(f"[完整测量标定] RuleAB-B 标定失败：{e}")
            self.log(traceback.format_exc())
            error_msg = str(e)
            self.root.after(0, lambda msg=error_msg: messagebox.showerror("RuleAB-B标定失败", msg))
    ...
    def run_workflow(self):
        ...
        try:
            ...
            #    这里不能再把 wf.tcp_server 置 None，也不能把 labview_ready 伪造为 True；
            #    否则 Step 4 会和“单次光谱采集”走不同链路，甚至没有真实 1024 点光谱。
            if bool(globals().get("SPECTROMETER_TCP_DISABLED", False)):
                raise RuntimeError(
    ...
    def close_tcp(self):
        try:
            ...
            if self.workflow is not None and self.workflow.tcp_server is not None:
                self.set_var(self.tcp_status_var, "TCP状态：保持连接，可复用")
            else:
                self.set_var(self.tcp_status_var, "TCP状态：未连接")
            self.log("[TCP] 已关闭")
        except Exception as e:
            if self.workflow is not None:
                self.workflow.context["tcp_started"] = False

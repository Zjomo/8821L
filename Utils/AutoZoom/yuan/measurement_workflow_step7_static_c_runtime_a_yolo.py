...
class MeasurementWorkflow:
    ...
    def ensure_rule_ab_follower(self) -> ActualNanoBoundaryFollower:
        ...
        if state is not None:
            ...
            # A 不再依赖人工 RuleAB-A 标定；这里用 YOLO detect 自动生成一个初始化 seed，
            # 仅用于兼容外部 RuleAB 首帧初始化接口，后续 A 均由 YOLO bbox 写回 scene。
            state = self._ensure_calibration_state_has_yolo_a_seed(state, reason="ensure_rule_ab_follower")
    ...
    def _capture_and_build_scene_with_ab_retry(
        ...
        if follower is None:
            msg = "RuleAB follower 为空，无法取帧构建 C-only/A-YOLO scene"
            info["reason"] = msg
            if allow_fail:
        ...
        for attempt in range(1, attempts + 1):
            ...
            try:
                ...
                self.log(
                    f"[Step7-A-YOLO] {label}: 当前帧 A 检测/scene 构建失败 attempt={attempt}/{attempts}: {msg}；"
        ...
        final_msg = f"{label}: Step7 当前帧 A-YOLO/scene 构建连续失败 {attempts} 次，最后错误：{last_exc}"

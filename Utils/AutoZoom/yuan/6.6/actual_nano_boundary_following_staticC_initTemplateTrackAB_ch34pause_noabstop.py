...
class RuntimeConfig:
    ...
    a_temporal_score_weight: float = 0.15
    a_temporal_iou_weight: float = 0.05
    a_temporal_shape_iou_weight: float = 0.50
    a_temporal_area_weight: float = 0.20
    a_temporal_center_weight: float = 0.20
    a_temporal_aspect_weight: float = 0.25
    a_temporal_bbox_size_weight: float = 0.20
    a_temporal_c_leak_weight: float = 0.80

    # A 可以移动，但中心突跳越大得分越低；值越小，约束越严格。
    a_temporal_center_sigma_px: float = 80.0

    # A 形状基本不变，因此面积、宽高比、bbox 尺寸都要严格约束。
    a_temporal_area_ratio_min: float = 0.75
    a_temporal_area_ratio_max: float = 1.35
    a_temporal_min_shape_iou: float = 0.35
    a_temporal_min_aspect_similarity: float = 0.65
    a_temporal_min_bbox_size_similarity: float = 0.65

    # A 不应大面积吸入 C 的独有区域；但允许上一帧 A/C 已经真实接近或接触的区域。
    # C 独有区域定义为 static_C - last_A。
    a_temporal_max_c_unique_leak_ratio: float = 0.12

    # 如果所有候选都很差，直接退回上一帧 A，避免错误 mask 继续传递。
    a_temporal_fallback_to_last_if_bad: bool = True
    a_temporal_min_accept_score: float = 0.35
    ...
    # 综合评分权重。score 只作为弱参考；shape IoU / bbox 形状约束 / C 泄漏硬拒绝是主约束。
    b_temporal_score_weight: float = 0.15
    b_temporal_iou_weight: float = 0.05
    b_temporal_shape_iou_weight: float = 0.50
    b_temporal_area_weight: float = 0.20
    b_temporal_center_weight: float = 0.20
    b_temporal_aspect_weight: float = 0.25
    b_temporal_bbox_size_weight: float = 0.20
    b_temporal_c_leak_weight: float = 0.80

    # B 可以移动，但中心突跳越大得分越低；值越小，约束越严格。
    b_temporal_center_sigma_px: float = 80.0

    # B 形状基本不变，因此面积、宽高比、bbox 尺寸都要严格约束。
    b_temporal_area_ratio_min: float = 0.75
    b_temporal_area_ratio_max: float = 1.35
    b_temporal_min_shape_iou: float = 0.35
    b_temporal_min_aspect_similarity: float = 0.65
    b_temporal_min_bbox_size_similarity: float = 0.65

    # B 不能大面积吸入 C 的独有区域；但允许真实 B/C 重合区域。
    # C 独有区域定义为 static_C - last_B。
    b_temporal_max_c_unique_leak_ratio: float = 0.12

    # 如果所有候选都很差，直接退回上一帧 B，避免错误 mask 继续传递。
    b_temporal_fallback_to_last_if_bad: bool = True
    b_temporal_min_accept_score: float = 0.35
    ...
    temporal_position_match_margin_px: float = 260.0
    temporal_position_match_min_score: float = 0.45
    # False：默认使用第一帧人工确认后的初始化模板定位 A/B，避免上一帧错误模板污染后续帧。
    temporal_position_match_use_last_image_template: bool = False
    ...
    # --------------------------------------------------------
    # 需求：
    #   action=LEFT/RIGHT：CH3 执行一次动作，然后强制停顿 2 s。
    #   action=UP/DOWN   ：CH4 执行一次动作，然后强制停顿 2 s。
    # 停顿发生在每次 Stage 指令之后，防止连续动作过快。
...
class SAM2ABCSegmenter:
    ...
    def _estimate_bbox_by_template_match(
        ...
        # 当默认使用初始化模板时，上一帧位置可能已经错了，因此局部失败后做整图兜底。
        global_match = None
        if (local is None) or (float(local.get("score", -1.0)) < float(self.temporal_position_match_min_score)):
    ...
    def _score_temporal_candidate_mask(
        ...
        shape_iou_weight: float = 0.50,
        area_weight: float = 0.20,
        center_weight: float = 0.20,
        aspect_weight: float = 0.25,
        bbox_size_weight: float = 0.20,
        c_leak_weight: float = 0.0,
        center_sigma_px: float = 80.0,
        area_ratio_min: float = 0.75,
        area_ratio_max: float = 1.35,
        min_shape_iou: float = 0.35,
        min_aspect_similarity: float = 0.65,
        min_bbox_size_similarity: float = 0.65,
        ...
        这版是真正的“形状锁定”：
            - 普通 IoU 只弱参考，因为 A/B 会移动；
            - 中心对齐 shape-IoU 是主指标，允许平移但不允许形状乱变；
            - bbox 宽高比和 bbox 尺寸相似度用于拒绝长条、大片 C 等形状突变；
            - C 独有区域泄漏超过阈值直接硬拒绝；
            - 硬拒绝候选的 total 会被扣 100，基本不会被选中。
        """
        ...
        leak_ratio = c_unique_leak_ratio(cand, static_c_mask, ref)
        ...
        if shape_iou < float(min_shape_iou):
            hard_reject = True
            reject_reasons.append(f"shape_iou_low({shape_iou:.3f})")

        if aspect_sim < float(min_aspect_similarity):
            hard_reject = True
            reject_reasons.append(f"aspect_low({aspect_sim:.3f})")

        if bbox_size_sim < float(min_bbox_size_similarity):
            hard_reject = True
            reject_reasons.append(f"bbox_size_low({bbox_size_sim:.3f})")

        if static_c_mask is not None and leak_ratio > float(max_c_unique_leak_ratio):
            hard_reject = True
            reject_reasons.append(f"c_unique_leak_high({leak_ratio:.3f})")

        # 同时必须像“初始化人工确认的 A/B 形状”。
        # 只像上一帧是不够的，因为上一帧可能已经跟错目标。
        if tmpl is not None:
            if template_shape_iou < float(min_shape_iou):
                hard_reject = True
                reject_reasons.append(f"init_shape_iou_low({template_shape_iou:.3f})")
            if template_area_ratio < float(area_ratio_min) or template_area_ratio > float(area_ratio_max):
                hard_reject = True
                reject_reasons.append(f"init_area_ratio_out({template_area_ratio:.3f})")
            if template_aspect_sim < float(min_aspect_similarity):
                hard_reject = True
                reject_reasons.append(f"init_aspect_low({template_aspect_sim:.3f})")
            if template_bbox_size_sim < float(min_bbox_size_similarity):
                hard_reject = True
                reject_reasons.append(f"init_bbox_size_low({template_bbox_size_sim:.3f})")

        total = (
            float(score_weight) * score_norm
            + float(iou_weight) * raw_iou
            + float(shape_iou_weight) * (0.55 * shape_iou + 0.45 * template_shape_iou)
            + float(area_weight) * (0.50 * area_sim + 0.50 * template_area_sim)
            + float(center_weight) * center_sim
            + float(aspect_weight) * (0.50 * aspect_sim + 0.50 * template_aspect_sim)
            + float(bbox_size_weight) * (0.50 * bbox_size_sim + 0.50 * template_bbox_size_sim)
            - float(c_leak_weight) * leak_ratio
        )
...
class ActualNanoBoundaryFollower:
    ...
    def execute_action(self, action_id: int) -> None:
        ...
        if is_ch3_horizontal_action and ch3_pause_s > 0:
            need_pause = True
            pause_s = ch3_pause_s
            force_wait = ch3_force_wait or force_wait
            stop_after = ch3_stop_after
            pause_tag = "CH3"
        elif is_ch4_vertical_action and ch4_pause_s > 0:
            need_pause = True
            pause_s = ch4_pause_s

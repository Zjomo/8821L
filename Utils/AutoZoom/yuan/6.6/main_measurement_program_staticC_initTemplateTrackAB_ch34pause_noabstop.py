...
from logic.actual_nano_boundary_following_staticC_initTemplateTrackAB_ch34pause_noabstop import RuntimeConfig as RuleABRuntimeConfig
from logic.actual_nano_boundary_following_staticC_initTemplateTrackAB_ch34pause_noabstop import ActualNanoBoundaryFollower
...
class MeasurementConfig:
    ...
    rule_ab_a_temporal_score_weight: float = float(_cfg("rule_ab_a_temporal_score_weight", 0.15))
    rule_ab_a_temporal_iou_weight: float = float(_cfg("rule_ab_a_temporal_iou_weight", 0.05))
    rule_ab_a_temporal_shape_iou_weight: float = float(_cfg("rule_ab_a_temporal_shape_iou_weight", 0.50))
    rule_ab_a_temporal_area_weight: float = float(_cfg("rule_ab_a_temporal_area_weight", 0.20))
    rule_ab_a_temporal_center_weight: float = float(_cfg("rule_ab_a_temporal_center_weight", 0.20))
    rule_ab_a_temporal_aspect_weight: float = float(_cfg("rule_ab_a_temporal_aspect_weight", 0.25))
    rule_ab_a_temporal_bbox_size_weight: float = float(_cfg("rule_ab_a_temporal_bbox_size_weight", 0.20))
    rule_ab_a_temporal_c_leak_weight: float = float(_cfg("rule_ab_a_temporal_c_leak_weight", 0.80))
    rule_ab_a_temporal_center_sigma_px: float = float(_cfg("rule_ab_a_temporal_center_sigma_px", 80.0))
    rule_ab_a_temporal_area_ratio_min: float = float(_cfg("rule_ab_a_temporal_area_ratio_min", 0.75))
    rule_ab_a_temporal_area_ratio_max: float = float(_cfg("rule_ab_a_temporal_area_ratio_max", 1.35))
    rule_ab_a_temporal_min_shape_iou: float = float(_cfg("rule_ab_a_temporal_min_shape_iou", 0.35))
    rule_ab_a_temporal_min_aspect_similarity: float = float(_cfg("rule_ab_a_temporal_min_aspect_similarity", 0.65))
    rule_ab_a_temporal_min_bbox_size_similarity: float = float(_cfg("rule_ab_a_temporal_min_bbox_size_similarity", 0.65))
    rule_ab_a_temporal_max_c_unique_leak_ratio: float = float(_cfg("rule_ab_a_temporal_max_c_unique_leak_ratio", 0.12))
    rule_ab_a_temporal_fallback_to_last_if_bad: bool = bool(_cfg("rule_ab_a_temporal_fallback_to_last_if_bad", True))
    rule_ab_a_temporal_min_accept_score: float = float(_cfg("rule_ab_a_temporal_min_accept_score", 0.35))
    ...
    rule_ab_b_temporal_score_weight: float = float(_cfg("rule_ab_b_temporal_score_weight", 0.15))
    rule_ab_b_temporal_iou_weight: float = float(_cfg("rule_ab_b_temporal_iou_weight", 0.05))
    rule_ab_b_temporal_shape_iou_weight: float = float(_cfg("rule_ab_b_temporal_shape_iou_weight", 0.50))
    rule_ab_b_temporal_area_weight: float = float(_cfg("rule_ab_b_temporal_area_weight", 0.20))
    rule_ab_b_temporal_center_weight: float = float(_cfg("rule_ab_b_temporal_center_weight", 0.20))
    rule_ab_b_temporal_aspect_weight: float = float(_cfg("rule_ab_b_temporal_aspect_weight", 0.25))
    rule_ab_b_temporal_bbox_size_weight: float = float(_cfg("rule_ab_b_temporal_bbox_size_weight", 0.20))
    rule_ab_b_temporal_c_leak_weight: float = float(_cfg("rule_ab_b_temporal_c_leak_weight", 0.80))
    rule_ab_b_temporal_center_sigma_px: float = float(_cfg("rule_ab_b_temporal_center_sigma_px", 80.0))
    rule_ab_b_temporal_area_ratio_min: float = float(_cfg("rule_ab_b_temporal_area_ratio_min", 0.75))
    rule_ab_b_temporal_area_ratio_max: float = float(_cfg("rule_ab_b_temporal_area_ratio_max", 1.35))
    rule_ab_b_temporal_min_shape_iou: float = float(_cfg("rule_ab_b_temporal_min_shape_iou", 0.35))
    rule_ab_b_temporal_min_aspect_similarity: float = float(_cfg("rule_ab_b_temporal_min_aspect_similarity", 0.65))
    rule_ab_b_temporal_min_bbox_size_similarity: float = float(_cfg("rule_ab_b_temporal_min_bbox_size_similarity", 0.65))
    rule_ab_b_temporal_max_c_unique_leak_ratio: float = float(_cfg("rule_ab_b_temporal_max_c_unique_leak_ratio", 0.12))
    rule_ab_b_temporal_fallback_to_last_if_bad: bool = bool(_cfg("rule_ab_b_temporal_fallback_to_last_if_bad", True))
    rule_ab_b_temporal_min_accept_score: float = float(_cfg("rule_ab_b_temporal_min_accept_score", 0.35))
    ...
    rule_ab_temporal_position_match_margin_px: float = float(_cfg("rule_ab_temporal_position_match_margin_px", 260.0))
    rule_ab_temporal_position_match_min_score: float = float(_cfg("rule_ab_temporal_position_match_min_score", 0.45))
    rule_ab_temporal_position_match_use_last_image_template: bool = bool(_cfg("rule_ab_temporal_position_match_use_last_image_template", False))
...
class MeasurementWorkflowGUI:
    ...
    def _build_ui(self):
        ...
        ttk.Button(rule_buttons, text="停止A推B控制器", command=self.stop_rule_ab_controller_thread).grid(row=0, column=1, padx=4, pady=3, sticky="ew")
        ttk.Button(rule_buttons, text="单独测试Step9面积", command=self.test_rule_ac_thread).grid(row=0, column=2, padx=4, pady=3, sticky="ew")
    ...
    def choose_static_c_map_dir(self):
        ...
        可以直接选择旧版 static_c_reference 文件夹，里面只要有：
            static_c_mask.png
            static_c_pixels_yx.npy（可选但推荐）
            static_c_reference.csv（可选）

        也可以选择 run_xxx 文件夹；程序会自动使用其中的 static_c_reference 子目录。
        """
        folder = filedialog.askdirectory(title="选择包含 static_c_mask.png 的 C 标定文件夹，或选择 run_xxx 文件夹")
        if not folder:
            return
        ...
        self.log(f"[RuleAB-C] 已选择 C 标定文件夹：{folder}")
    ...
    def test_rule_ab(self):
        ...
        try:
            ...
            rule_cfg = RuleABRuntimeConfig(
                ...
                temporal_position_match_margin_px=float(getattr(cfg_gui, "rule_ab_temporal_position_match_margin_px", 260.0)),
                temporal_position_match_min_score=float(getattr(cfg_gui, "rule_ab_temporal_position_match_min_score", 0.45)),
                temporal_position_match_use_last_image_template=bool(getattr(cfg_gui, "rule_ab_temporal_position_match_use_last_image_template", False)),

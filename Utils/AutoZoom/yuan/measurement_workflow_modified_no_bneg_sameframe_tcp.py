...
class MeasurementConfig:
    ...
    # B 负点只保留为 SAM2 首帧 prompt，不再参与后续 feature tracker 颜色排除。
    # 以下旧参数仅为兼容旧配置文件，不在 Bmask 后续帧检测中使用。
...
class CalibrationState:
    ...
    # 兼容旧标定文件：B 负点 HSV seed 不再用于后续 feature tracker 颜色排除。
...
class MeasurementWorkflow:
    ...
    def _raw_color_mask_from_profile(self, image_rgb: np.ndarray, profile: Dict[str, Any]) -> np.ndarray:
        """根据模板颜色阈值得到粗 mask；B 只使用正点/clean Bmask 生成的正向颜色模板。"""
        dist = self._hsv_color_distance_to_profile(image_rgb, profile)
        # dist<=sqrt(3) 等价于 H/S/V 都在容差量级内；适当收紧到 1.65 减少串色。
        raw = dist <= 1.65
        label = str(profile.get("label", "")).lower()
        if label == "b":
            # B 负点只用于 SAM2 首帧 prompt，不参与后续 feature tracker 颜色排除。
            # 后续帧 Bmask 仅由 B 正点 HSV seed + 首帧 clean Bmask 内多 HSV center 判断。
            try:
                ...
                profile["_last_b_color_debug"] = {
                    ...
                    "raw_area_after_positive_color_filter_px": int(np.count_nonzero(raw)),
                    "b_negative_color_exclusion_enabled": False,
                }
            except Exception:
                pass
        ...
        for lab, raw in raw_masks.items():
            ...
            if lab == "b":
                try:
                    ...
                    for _k in ("positive_color_score", "positive_color_mean_distance"):
                        if _k in b_color_debug:
                            profiles[lab][_k] = b_color_debug.get(_k)
                    # 明确清空上一版负点颜色排除留下的运行态字段，避免后续 debug/CSV 误认为仍在使用负点。
                    profiles[lab].pop("negative_color_score", None)
                    profiles[lab].pop("negative_color_mean_distance", None)
                    ...
                    debug[lab]["bmask_fail_reason"] = "" if np.any(m) else "empty_after_positive_color_filter_or_component_selection"
    ...
    def _install_feature_tracker_on_rule_ab_follower(self, follower: Any, state: Optional[CalibrationState] = None):
        ...
        def _build_templates_from_original_scene() -> Tuple[np.ndarray, Any]:
            ...
            # B 负点只作为 SAM2 首帧 prompt 使用，不再提取/保存为 feature tracker 颜色排除模板。
            profiles["b"].pop("b_negative_hsv_seeds", None)
            profiles["b"].pop("b_negative_hsv_seed_count", None)
            profiles["b"].pop("b_negative_hsv_centers", None)
            profiles["b"].pop("b_negative_hsv_center_count", None)
            profiles["b"]["b_negative_color_exclusion_enabled"] = False
...
class MeasurementWorkflowGUI:
    ...
    def calibrate_angle_b(self):
        ...
        try:
            ...
            # B 负点坐标仍保存给 SAM2 prompt，但不再提取/保存负点 HSV seed 给 feature tracker。
            state.b_negative_hsv_seeds = []
            state.b_negative_hsv_seed_count = 0
            ...
            self.log(
                "[完整测量标定] Bmask 标定完成："
                f"B+={len(pos)}, B-={len(neg)}, hsv_seeds={len(seeds)}；B负点仅用于SAM2 prompt，不参与后续feature tracker颜色排除；角度边=当前帧Bmask最长边自动选择，不再人工输入边编号。"
            )
        except Exception as e:
    ...
    def calibrate_rule_ab_b(self):
        ...
        try:
            ...
            state.b_negative_hsv_seed_count = 0
            self._save_calibration_file_raw(state)
            self.log(f"[完整测量标定] RuleAB-B 标定完成：B+={len(pos)}, B-={len(neg)}, hsv_seeds={len(seeds)}；B负点仅用于SAM2 prompt，不参与后续feature tracker颜色排除")
        except Exception as e:

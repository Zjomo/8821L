...
class MeasurementWorkflow:
    ...
    def _run_one_c_edge_route_cycle(
        ...
        try:
            ...
            scene_src = ""
        if "c_only_scene_a_yolo_b_disabled" not in scene_src:
            with self.rule_ab_vision_lock:
    ...
    def _install_feature_tracker_on_rule_ab_follower(self, follower: Any, state: Optional[CalibrationState] = None):
        ...
        def _build_c_template_from_original_scene() -> Tuple[np.ndarray, Any]:
            ...
            # A 首帧也只用 YOLO detect 生成运行时 bbox mask/center；不建立 A 模板。
            a_det = self._run_a_yolo_detect_on_image(image_rgb, label="initial_a_yolo")
            if not a_det.get("ok"):
                raise RuntimeError(f"首帧 A YOLO 检测失败，无法提供 Step7 A center：{a_det.get('reason')}")
            a_mask = self._a_bbox_to_rect_mask(image_rgb.shape[:2], a_det["best"]["box_xyxy"])
            self.log(
                f"[A-YOLO-Step7] initial A center={a_det['best'].get('center_xy')}, "
            ...
            profiles["c"].update({
                ...
                "runtime_source": "static_c_or_c_feature_profile",
            ...
            feature_scene = self._apply_feature_tracker_masks_to_scene(
                ...
                source="c_only_scene_a_yolo_b_disabled",
            )
            ...
            try:
                self.context["abc_feature_tracker"] = {
                    ...
                    "out_dir": str(out_dir),
                    "mode": "c_only_template_a_yolo_b_yolo_obb_no_original_capture",
                    "scene_source": "c_only_scene_a_yolo_b_disabled",
                }
            ...
            self.log(
                ...
                "A=YOLO detect bbox/center；B 不建模板，B角度=YOLO-OBB；"
        ...
        def _feature_capture_and_build_scene():
            if not tracker_state.get("initialized", False):
                return _build_c_template_from_original_scene()
            ...
            # C 可以继续用保存的 C profile 更新；如果跟踪失败，则保留 base_scene 中的静态 C mask。
            masks: Dict[str, np.ndarray] = {}
            debug: Dict[str, Dict[str, Any]] = {}
            try:
                c_masks, c_debug = self._track_abc_by_saved_features(image_rgb, {"c": profiles.get("c", {})} if isinstance(profiles, dict) and "c" in profiles else {})
                if isinstance(c_masks, dict) and c_masks.get("c") is not None and np.any(np.asarray(c_masks.get("c")).astype(bool)):
                    masks["c"] = np.asarray(c_masks["c"]).astype(bool)
                    debug["c"] = c_debug.get("c", {}) if isinstance(c_debug, dict) else {}
                    debug["c"]["source"] = "c_feature_profile"
                    if "c" in profiles:
                        profiles["c"] = self._update_feature_profile_runtime(image_rgb, profiles["c"], masks["c"])
                else:
                    ...
                    if base_c is not None and np.any(np.asarray(base_c).astype(bool)):
                        ...
                        debug["c"] = {"source": "static_c_from_base_scene", "reason": "c_feature_failed_or_empty"}
            except Exception as e:
                base_c = self._extract_mask_from_scene_object(self._get_scene_object_by_label(tracker_state.get("base_scene"), "c"))
                if base_c is not None and np.any(np.asarray(base_c).astype(bool)):
                    masks["c"] = np.asarray(base_c).astype(bool)
                    debug["c"] = {"source": "static_c_from_base_scene", "reason": f"c_feature_exception: {e}"}
            ...
            if not a_det.get("ok"):
                raise RuntimeError(f"A YOLO 检测失败：{a_det.get('reason')}；本帧不允许回退旧 A mask/Amask。")

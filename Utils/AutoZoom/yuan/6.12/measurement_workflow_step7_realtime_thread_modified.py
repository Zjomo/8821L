...
class MeasurementConfig:
    ...
    rule_ab_pause_check_interval_s: float = float(_cfg("rule_ab_pause_check_interval_s", 0.02))
...
class MeasurementWorkflow:
    ...
    def _write_step7_realtime_records_csv(self, records: List[Dict[str, Any]], csv_path: Path) -> None:
        ...
        fieldnames = ["timestamp", "frame_index", "step", "phase", "angle_deg", "baseline_angle", "delta_from_baseline", "angle_source", "edge_length_px", "endpoints", "trigger_type", "controller_action", "raw_image_path", "overlay_image_path", "reason"]
        with csv_path.open("w", encoding="utf-8-sig", newline="") as f:
            writer = csv.DictWriter(f, fieldnames=fieldnames, extrasaction="ignore")
            writer.writeheader()
            for r in records:
                rr = dict(r)
                if isinstance(rr.get("endpoints"), (list, dict)):
                    rr["endpoints"] = json.dumps(self._json_safe(rr.get("endpoints")), ensure_ascii=False)
                writer.writerow(rr)
                    ...
                    try:
                        ...
                        if angle_ok and delta is not None:
                            if float(delta) >= float(min_delta) and float(delta) < float(max_delta):
                                ...
                                result_box.update({"trigger_type": "target_range", "stop_reason": "realtime_bmask_longest_edge_delta_in_target_range_close", "final_angle": float(angle_deg), "final_delta": float(delta)})
                                _append_record(rec)
                                self.log(f"[Step7实时角度] 达到目标范围：angle={angle_deg:.6f}°, baseline={baseline_angle}, delta={delta:.3f}°, target=[{min_delta:.3f},{max_delta:.3f})；立即停止Stage34并关激光。")
                                stop_event.set()
                                self._stop_rule_ab_stage34_if_possible(follower, reason="angle_delta_in_target_range")
                                self._laser_off_once_threadsafe(reason="angle_delta_in_target_range")
                                break
                            if float(delta) >= float(max_delta):
                                rec["trigger_type"] = "overshoot"
                                result_box.update({"trigger_type": "overshoot", "stop_reason": "realtime_bmask_longest_edge_delta_ge_target_max_overshoot", "final_angle": float(angle_deg), "final_delta": float(delta)})
                                _append_record(rec)
                                self.log(f"[Step7实时角度] 角度过冲：angle={angle_deg:.6f}°, baseline={baseline_angle}, delta={delta:.3f}° >= {max_delta:.3f}°；立即停止Stage34并关激光。")
                                stop_event.set()
                                self._stop_rule_ab_stage34_if_possible(follower, reason="angle_delta_overshoot")
                                self._laser_off_once_threadsafe(reason="angle_delta_overshoot")
                                break
                        _append_record(rec)
                    except Exception as e:
    ...
    def run_rule_ab_until_angle_delta(
        ...
        while True:
            ...
            angle_ok = bool(angle_result.get("ok", False) and angle_result.get("angle_deg") is not None)
            angle_source = str(angle_result.get("angle_source", "bmask_longest_edge_failed"))
            ...
            if close_hit:
                ...
                stop_reason = "bmask_longest_edge_delta_in_target_range_close"
                self.log(
                ...
                try:
                    if bool(self.context.get("laser_on", False)):
                        self.laser_off()
                        self.log("[RuleAB-Bmask最长边] 角度关闭条件满足后已立即执行 laser_off()")
                    else:
                        self.log("[RuleAB-Bmask最长边] 角度关闭条件满足时 context 显示 laser_on=False，跳过 laser_off()")
                except Exception as e:
                    self.log(f"[RuleAB-Bmask最长边] 角度达到关闭条件后关闭激光失败：{e}")
                break
            ...
            if final_delta is not None and float(final_delta) < float(min_delta):
                ...
                stop_reason = "bmask_longest_edge_delta_ge_target_max_no_close_continue"
                self.log(

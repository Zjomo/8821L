...
class RuleABRuntimeConfig:
    def __init__(self, **kwargs):
        defaults = {
            ...
            "stage_x_channel": 3,
            "stage_y_channel": 4,
            "stage_default_velocity": 10,
            "stage_default_acceleration": 10,
            "stage_default_max_voltage": 50,
            "stage_ch3_velocity": 10,
            "stage_ch3_acceleration": 10,
            "stage_ch3_max_voltage": 50,
            "stage_ch4_velocity": 10,
            "stage_ch4_acceleration": 10,
            "stage_ch4_max_voltage": 50,
            "stage_step_x": 20,
            "stage_step_y": 20,
            "action_step": 20,
            "stage_ch3_pause_after_move_s": 2.0,
            "stage_ch4_pause_after_move_s": 2.0,
            "stage_x_sign": -1,
            "stage_y_sign": 1,
        }
...
class ActualNanoBoundaryFollower:
    def __init__(self, cfg: RuleABRuntimeConfig):
        self.cfg = cfg
        self.cycle_index = 0
        self.stage = None
        if bool(getattr(cfg, "enable_stage", False)):
            if Thorlabs is None:
                raise RuntimeError("当前环境无法导入 pylablib.devices.Thorlabs；请安装 pylablib 或切换 hardware_mode=virtual。")
            self.stage = Thorlabs.KinesisPiezoMotor(str(getattr(cfg, "stage_conn", "97101208")))
            self._setup_stage_channel(3)
            self._setup_stage_channel(4)

    def _setup_stage_channel(self, channel: int):
        if self.stage is None:
            return
        velocity = int(getattr(self.cfg, f"stage_ch{channel}_velocity", getattr(self.cfg, "stage_default_velocity", 10)))
        acceleration = int(getattr(self.cfg, f"stage_ch{channel}_acceleration", getattr(self.cfg, "stage_default_acceleration", 10)))
        max_voltage = int(getattr(self.cfg, f"stage_ch{channel}_max_voltage", getattr(self.cfg, "stage_default_max_voltage", 50)))
        if hasattr(self.stage, "setup_channel"):
            self.stage.setup_channel(channel=channel, max_voltage=max_voltage, velocity=velocity, acceleration=acceleration)
        elif hasattr(self.stage, "setup_drive"):
            self.stage.setup_drive(max_voltage=max_voltage, velocity=velocity, acceleration=acceleration, channel=channel)

    def execute_action(self, action_id: int) -> Dict[str, Any]:
        action_id = int(action_id)
        action_map = {
            0: (None, 0, 0.0, "STAY"),
            1: (int(getattr(self.cfg, "stage_y_channel", 4)), int(getattr(self.cfg, "stage_y_sign", 1)), float(getattr(self.cfg, "stage_step_y", 20)), "UP"),
            2: (int(getattr(self.cfg, "stage_y_channel", 4)), -int(getattr(self.cfg, "stage_y_sign", 1)), float(getattr(self.cfg, "stage_step_y", 20)), "DOWN"),
            3: (int(getattr(self.cfg, "stage_x_channel", 3)), int(getattr(self.cfg, "stage_x_sign", -1)), float(getattr(self.cfg, "stage_step_x", 20)), "LEFT"),
            4: (int(getattr(self.cfg, "stage_x_channel", 3)), -int(getattr(self.cfg, "stage_x_sign", -1)), float(getattr(self.cfg, "stage_step_x", 20)), "RIGHT"),
        }
        channel, sign, step_abs, name = action_map.get(action_id, action_map[0])
        if channel is None or step_abs == 0:
            return {"ok": True, "action_id": action_id, "action_name": name, "channel": None, "channel_name": "STAY", "move_step": 0.0, "pause_s": 0.0, "dry_run": self.stage is None}

        move_step = float(sign) * abs(float(step_abs))
        pause_s = float(
            getattr(
                self.cfg,
                f"stage_ch{channel}_pause_after_move_s",
                getattr(self.cfg, "stage_ch3_pause_after_move_s", 0.0),
            )
        )
        dry_run = self.stage is None
        if self.stage is not None:
            if hasattr(self.stage, "execute_rule_action"):
                return self.stage.execute_rule_action(action_id=action_id, channel=channel, direction=sign, move_step=abs(float(step_abs)), pause_s=pause_s)
            if hasattr(self.stage, "move_by"):
                self.stage.move_by(distance=move_step, channel=channel)
            if pause_s > 0:
                time.sleep(pause_s)
        ...
        return {
            ...
            "action_name": name,
            "channel": int(channel),
            "channel_name": f"CH{int(channel)}",
            "move_step": float(move_step),
            "pause_s": float(pause_s),
            "dry_run": bool(dry_run),
            "voltage": getattr(self.cfg, f"stage_ch{channel}_max_voltage", None),
            "velocity": getattr(self.cfg, f"stage_ch{channel}_velocity", None),
            "acceleration": getattr(self.cfg, f"stage_ch{channel}_acceleration", None),
        }

    def close(self):
        if self.stage is not None and hasattr(self.stage, "close"):
            self.stage.close()
...
class MeasurementConfig:
    ...
    # Step7 A 检测：每一帧重新调用 YOLO detect，只使用当前帧 bbox center。
    rule_ab_a_detect_model_path: str = str(_cfg("rule_ab_a_detect_model_path", "runs/detect/best.pt"))
    rule_ab_a_detect_conf: float = float(_cfg("rule_ab_a_detect_conf", 0.5))
    rule_ab_a_detect_iou: float = float(_cfg("rule_ab_a_detect_iou", 0.7))
    rule_ab_a_detect_imgsz: int = int(_cfg("rule_ab_a_detect_imgsz", 640))
    rule_ab_a_detect_device: str = str(_cfg("rule_ab_a_detect_device", ""))
    rule_ab_a_detect_class_id: int = int(_cfg("rule_ab_a_detect_class_id", -1))
    rule_ab_a_detect_min_area_px: float = float(_cfg("rule_ab_a_detect_min_area_px", 20.0))
    rule_ab_a_detect_save_debug: bool = bool(_cfg("rule_ab_a_detect_save_debug", False))

    # Step7 C 检测：unet_mask + latest valid C contour。
    rule_ab_c_unet_model_path: str = str(_cfg("rule_ab_c_unet_model_path", "runs/unet/best.pt"))
    rule_ab_c_unet_img_size: int = int(_cfg("rule_ab_c_unet_img_size", 0))
    rule_ab_c_unet_base_channels: int = int(_cfg("rule_ab_c_unet_base_channels", 0))
    rule_ab_c_unet_device: str = str(_cfg("rule_ab_c_unet_device", ""))
    rule_ab_c_unet_class_id: int = int(_cfg("rule_ab_c_unet_class_id", 1))
    rule_ab_c_unet_threshold: float = float(_cfg("rule_ab_c_unet_threshold", 0.5))
    rule_ab_c_unet_min_area_px: float = float(_cfg("rule_ab_c_unet_min_area_px", 50.0))
    rule_ab_c_unet_min_contour_points: int = int(_cfg("rule_ab_c_unet_min_contour_points", 12))
    rule_ab_c_unet_morph_kernel: int = int(_cfg("rule_ab_c_unet_morph_kernel", 5))
    ...
    #   "mask_outer_edge"：可选，使用 legacy 原始 C mask 最大外轮廓像素生成路线。
    rule_ab_c_edge_route_source: str = "quad"
    ...
    # RuleAB：第一帧 legacy 分割确认与 C 四边形拟合
    ...
    # A/B 位置跟踪：先通过上一帧图像块模板匹配预测当前帧位置，再交给 legacy。
    ...
    # legacy video predictor：仅保留首帧模板建立相关入口；完整测量后续帧不再启用 legacy video tracking。
    # 后续帧 Bmask 只能来自 feature tracker，失败即记录失败，不 fallback。


    # ============================================================
    # 新方案：legacy 只用于首帧三目标分割；后续用颜色 + 形状模板自动识别 A/B/C
    # ============================================================
    # 逻辑：
    #   1) 完整标定/初始化时，仍然允许 legacy 根据 A/B/C 正点负点分割一次；
    #   2) 从这一次 mask 内提取每个目标的 HSV 颜色模板、面积/中心/外接框/形状模板；
    #   3) 后续完整循环测量不再每帧调用 legacy，而是用模板自动识别 A/B/C；
    ...
    # 后续帧只允许 feature tracker 作为 Bmask 来源；失败即记录失败，不调用 legacy fallback，不 hold_last。
    ...
    rule_ab_bmask_max_center_jump_px: float = float(_cfg("rule_ab_bmask_max_center_jump_px", _cfg("rule_ab_b_center_jump_max_px", 220.0)))
...
class CalibrationState:
    ...
    # 新方案：首帧 legacy 分割后保存的 A/B/C 颜色与形状模板。
    ...
    def missing_items(self) -> List[str]:
        ...
        # Step7 A 改为每帧 YOLO bbox 检测，C 改为 UNet + latest-valid contour；
        # 完整循环启动前不再强制要求 A mask 点、A 模板、C 正点/负点或旧 static C map。
        # 如果用户提供了 static C，保留路径有效性提示；未提供时由 Step7 首帧动态检测 C。
...
class MeasurementWorkflow:
    def __init__(self, cfg: MeasurementConfig, on_log=None, on_update=None):
        ...
        # Step7：A 每帧 YOLO bbox 检测；C 使用 unet latest-valid contour。
        self.a_detect_model: Optional[Any] = None
        self.a_detect_model_path_resolved: Optional[Path] = None
        self.c_unet_model: Optional[Any] = None
        self.c_unet_device: Optional[Any] = None
        self.c_unet_img_size: int = 0
        self.c_unet_classes: Dict[int, str] = {}
        self.c_unet_model_path_resolved: Optional[Path] = None
        self.current_c_mask: Optional[np.ndarray] = None
        self.current_c_contour: Optional[np.ndarray] = None
        self.c_mask_dirty: bool = True
        self.stage12_moved_since_last_c_update: bool = False

        # Step9：颜色检测区域中心对齐专用运行状态。
        # 单独测试 Step9 时，C 从“选择C文件夹”加载 static_c_mask.png；
        # B 由 legacy 分割/跟踪得到。停止按钮会把 step9_stop_requested 置 True，
        # 并尝试停止 1/2 通道。
        ...
        # 人工重新标定 Step9 后，让当前 cycle 从 Step1 重新开始。
    ...
    @staticmethod
    def _resolve_model_path_with_fallback(path_text: str, fallback_rel: str = "") -> Path:
        ...
        if p.exists() and p.is_file():
            return p
        if fallback_rel:
            fp = Path(str(fallback_rel)).expanduser()
    ...
    def _complete_calibration_state_for_runtime(self, state: CalibrationState) -> CalibrationState:
        ...
        # 则角度检测 B 也复用 RuleAB-B，避免 B 缺失导致 ABC 初始化/legacy prompt 顺序不稳定。
    ...
    def _force_cfg_to_strict_full_calibration_c(self, reason: str = "") -> str:
        ...
        self.cfg.rule_ab_load_static_c_map_if_exists = False
    ...
    def apply_calibration_state(self, state: CalibrationState):
        ...
        关键修复：
            ...
            现在会把 A/B/C 正点负点也保存到 context，并准备注入 RuleAB/legacy 跟踪模块。
        """
        if state is None:
            return
        ...
        self.cfg.rule_ab_static_c_map_dir = ""
        self.cfg.rule_ab_load_static_c_map_if_exists = False
        self.cfg.rule_ab_force_reselect_c_each_run = False
        self.cfg.rule_ab_reuse_static_c_map_in_memory = False
        state.static_c_map_dir = ""
        state.static_c_mask_path = ""
        state.static_c_edge_route_path = ""
        ...
        # Step9 如果以后仍需要 B 点/legacy，则这里也预置，避免单独准备时再弹 B 点选择。
    ...
    def prepare_runtime_from_full_calibration(self, state: CalibrationState):
        ...
        has_static_c = False
        try:
            has_static_c = bool(self._resolve_static_c_dir_from_state(state))
        except Exception:
            has_static_c = False
        if has_static_c:
            ...
            self.cfg.rule_ab_load_static_c_map_if_exists = False
            self.cfg.rule_ab_force_reselect_c_each_run = False
            self.current_c_mask = None
            self.current_c_contour = None
            self.c_mask_dirty = True
            self.stage12_moved_since_last_c_update = False
            self.log("[完整测量标定] 未提供 static C；完整测量允许启动，Step7 将使用 UNet 动态检测 C。")
        self.apply_calibration_state(state)
    ...
    def preflight_check_calibration(self) -> CalibrationState:
        ...
        a_model_path = self._resolve_model_path_with_fallback(getattr(self.cfg, "rule_ab_a_detect_model_path", ""), "runs/detect/best.pt")
        c_model_path = self._resolve_model_path_with_fallback(getattr(self.cfg, "rule_ab_c_unet_model_path", ""), "runs/unet/best.pt")
        b_obb_model_path = self._resolve_model_path_with_fallback(getattr(self.cfg, "angle_model_path", ""), "runs/obb/best.pt")
        if not (a_model_path.exists() and a_model_path.is_file()):
            missing.append(f"Step7：A YOLO detect 模型不存在：{getattr(self.cfg, 'rule_ab_a_detect_model_path', '')}")
        else:
            self.cfg.rule_ab_a_detect_model_path = str(a_model_path)
        if not (c_model_path.exists() and c_model_path.is_file()):
            missing.append(f"Step7：C unet_mask 模型不存在：{getattr(self.cfg, 'rule_ab_c_unet_model_path', '')}")
        else:
            self.cfg.rule_ab_c_unet_model_path = str(c_model_path)
        if not (b_obb_model_path.exists() and b_obb_model_path.is_file()):
            missing.append(f"Step1/Step7：B YOLO-OBB 角度模型不存在：{getattr(self.cfg, 'angle_model_path', '')}")
        else:
            self.cfg.angle_model_path = str(b_obb_model_path)
        if missing:
    ...
    def init_angle_module(self):
        ...
        model_path = self._resolve_model_path_with_fallback(str(self.cfg.angle_model_path), "runs/obb/best.pt")
        if not model_path.exists():
            raise FileNotFoundError(f"YOLO-OBB模型不存在：{model_path}")
        self.cfg.angle_model_path = str(model_path)
    ...
    def ensure_rule_ab_follower(self) -> ActualNanoBoundaryFollower:
        ...
        cfg = RuleABRuntimeConfig(
            ...
            # A 后续帧使用“最像上一帧 A”的候选 mask，而不是只选 legacy 最高分。
            a_use_temporal_mask_selection=False if bool(getattr(self.cfg, "rule_ab_use_feature_tracker", True)) else bool(self.cfg.rule_ab_a_use_temporal_mask_selection),
            ...
            # B 后续帧使用“最像上一帧 B”的候选 mask，而不是只选 legacy 最高分。
            b_use_temporal_mask_selection=False if bool(getattr(self.cfg, "rule_ab_use_feature_tracker", True)) else bool(self.cfg.rule_ab_b_use_temporal_mask_selection),
        ...
        setattr(self.rule_ab_follower, "_skip_abc_feature_tracker_install", True)
        self.log("[RuleAB] Skipping legacy A/B/C mask initialization; Step7 uses YOLO-A + UNet-C and B uses YOLO-OBB.")
    ...
    def apply_runtime_rule_ab_params_to_follower(self, follower: Optional[Any] = None, reason: str = ""):
        ...
        feature_tracker_enabled = bool(getattr(self.cfg, "rule_ab_use_feature_tracker", True))
        ...
        # A/B 跟踪和 legacy video tracking 运行参数。
        tracking_map = {
        ...
        for target_attr, cfg_attr in tracking_map.items():
            ...
            if feature_tracker_enabled:
                # 完整循环测量启用“首帧 legacy 建模板 + 颜色/形状跟踪”后，
                # 旧的 temporal mask selection / legacy video tracking 只能用于首帧初始化，
        ...
        if feature_tracker_enabled:
            ...
            # 防止外部模块保存 overlay 时仍把 A/B 标成 legacy。
    ...
    def _clear_rule_ab_bad_scene_cache(self, follower: Any, reason: str = ""):
        """
        清理 RuleAB/legacy A/B 跟踪失败后可能残留的坏 scene 缓存。
    ...
    def _capture_and_build_scene_with_ab_retry(
        ...
        if bool(getattr(self.cfg, "rule_ab_use_feature_tracker", True)):
    ...
    def request_midrun_recalibration_pause(self, cycle_index: Optional[int] = None) -> None:
        ...
        self.log(
            "[中途重标定] 已请求暂停当前 cycle 并重新标定 Step9；"
    ...
    def prepare_runtime_for_midrun_recalibration_waiting(self) -> None:
        ...
        # 释放旧 RuleAB / RuleAC / Step9 运行态，避免旧 legacy prompt、feature template、static C 路线污染新标定。
        for attr, label in (
        ...
        self.log("[中途重标定] 已进入等待重标定状态：请重新标定 Step9，完成后点击“完成重标定并继续测量”。")
        self.notify_update()

    def resume_after_midrun_recalibration(self, state: Optional[CalibrationState] = None) -> None:
        """
        用户完成 Step9 标定后调用。重新加载标定并准备从当前 cycle 的 Step1 重跑。
        """
        if state is None:
    ...
    def _load_or_build_step7_c_edge_route(self, follower: Any) -> List[Tuple[float, float]]:
        ...
        如果确实要恢复原始 legacy mask 外轮廓路线，可在 config.py 显式设置
        rule_ab_c_edge_route_source="mask_outer_edge"。
        """
        ...
        if route_source not in ("mask_outer_edge", "mask_outer_edge", "mask", "quad", "quadrilateral"):
            route_source = "quad"
        ...
        if route_path.exists():
            try:
                ...
                if route_source in ("quad", "quadrilateral"):
                    ...
                    source_ok = ("legacy_outer" in existing_source) or ("mask_outer" in existing_source) or ("mask" in existing_source)
        ...
        if route_source in ("quad", "quadrilateral"):
            ...
            route = self._build_and_save_mask_outer_edge_route(c_path)
            if not route:
                self.log("[RuleAB-Route] legacy 原始 mask 外轮廓路线生成失败，尝试从 static_c_mask.png 外轮廓兜底生成。")
                route = self._build_and_save_mask_outer_edge_route(c_path, prefer_mask=False)
            if not route:
    ...
    def _load_preferred_c_mask_for_route_shape(self, c_path: Path) -> Tuple[Optional[np.ndarray], str]:
        """路线生成时优先使用原始 legacy C mask 判断形状；没有时再用 static_c_mask.png 兜底。"""
        c_path = Path(c_path)
        for mp in (
    ...
    def _build_and_save_quad_edge_route(self, c_path: Path) -> List[Tuple[float, float]]:
        ...
        新增圆形 C mask 分支：
            - 优先读取 static_c_mask.png 判断原始 C 是否近似圆形；
        ...
        # 只要原始 legacy C 是圆形，也强制使用圆形外接矩形生成路径。
        try:
    ...
    def _ensure_step7_a_detect_model(self) -> Any:
        model_path = Path(str(getattr(self.cfg, "rule_ab_a_detect_model_path", "runs/detect/best.pt") or "")).expanduser()
        if not model_path.is_absolute():
            model_path = (PROJECT_ROOT / model_path).resolve()
        if not model_path.exists():
            raise FileNotFoundError(f"Step7 A YOLO model not found: {model_path}")
        if YOLO is None:
            raise ImportError("ultralytics.YOLO is not available for Step7 A detection")
        if self.a_detect_model is None or self.a_detect_model_path_resolved != model_path:
            self.a_detect_model = YOLO(str(model_path))
            self.a_detect_model_path_resolved = model_path
            self.log(f"[Step7-A] loaded YOLO detect model: {model_path}")
        return self.a_detect_model

    def _detect_step7_a_bbox_current_frame(self, image_rgb: np.ndarray) -> Dict[str, Any]:
        h, w = image_rgb.shape[:2]
        try:
            model = self._ensure_step7_a_detect_model()
            kwargs = {
                "conf": float(getattr(self.cfg, "rule_ab_a_detect_conf", 0.5)),
                "iou": float(getattr(self.cfg, "rule_ab_a_detect_iou", 0.7)),
                "imgsz": int(getattr(self.cfg, "rule_ab_a_detect_imgsz", 640)),
                "verbose": False,
            }
            device = str(getattr(self.cfg, "rule_ab_a_detect_device", "") or "").strip()
            if device:
                kwargs["device"] = device
            results = model.predict(image_rgb, **kwargs)
            result = results[0] if results else None
            boxes = getattr(result, "boxes", None) if result is not None else None
            if boxes is None or len(boxes) <= 0:
                return {"ok": False, "reason": "a_yolo_no_detection", "A_detect_ok": False}
            xyxy = boxes.xyxy.detach().cpu().numpy() if hasattr(boxes.xyxy, "detach") else np.asarray(boxes.xyxy)
            confs = boxes.conf.detach().cpu().numpy() if getattr(boxes, "conf", None) is not None and hasattr(boxes.conf, "detach") else np.asarray(getattr(boxes, "conf", np.ones((len(xyxy),), dtype=float)))
            classes = boxes.cls.detach().cpu().numpy() if getattr(boxes, "cls", None) is not None and hasattr(boxes.cls, "detach") else np.full((len(xyxy),), -1)
            class_id_required = int(getattr(self.cfg, "rule_ab_a_detect_class_id", -1))
            min_area = float(getattr(self.cfg, "rule_ab_a_detect_min_area_px", 20.0))
            best = None
            for i, box in enumerate(xyxy):
                ...
                if class_id_required >= 0 and cls != class_id_required:
                    continue
                if area < min_area or cx < 0 or cx >= w or cy < 0 or cy >= h:
                    continue
                cand = {"idx": int(i), "bbox_xyxy": [x1, y1, x2, y2], "center": (float(cx), float(cy)), "conf": conf, "class_id": cls, "area_px": float(area)}
                if best is None or conf > best["conf"]:
                    best = cand
            if best is None:
                return {"ok": False, "reason": "a_yolo_no_valid_bbox", "A_detect_ok": False}
            best.update({"ok": True, "A_detect_ok": True})
            return best
        except Exception as e:
            return {"ok": False, "reason": f"a_yolo_exception:{e}", "A_detect_ok": False, "error": str(e)}

    def _ensure_step7_c_unet_model(self) -> Any:
        model_path = Path(str(getattr(self.cfg, "rule_ab_c_unet_model_path", "runs/unet/best.pt") or "")).expanduser()
        if not model_path.is_absolute():
            model_path = (PROJECT_ROOT / model_path).resolve()
        if not model_path.exists():
            raise FileNotFoundError(f"Step7 C UNet model not found: {model_path}")
        if self.c_unet_model is not None and self.c_unet_model_path_resolved == model_path:
            return self.c_unet_model
        import torch
        from train_unet import UNet
        device_text = str(getattr(self.cfg, "rule_ab_c_unet_device", "") or "").strip()
        ...
        base_channels = int(getattr(self.cfg, "rule_ab_c_unet_base_channels", 0) or args.get("base_channels", 32))
        img_size = int(getattr(self.cfg, "rule_ab_c_unet_img_size", 0) or args.get("img_size", 512))
        ...
        model.eval()
        self.c_unet_model = model
        self.c_unet_device = device
        self.c_unet_img_size = int(img_size)
        self.c_unet_classes = {int(k): str(v) for k, v in ckpt.get("classes", {}).items()} if isinstance(ckpt, dict) else {}
        self.c_unet_model_path_resolved = model_path
        self.log(f"[Step7-C] loaded UNet model: {model_path}, img_size={img_size}, device={device}")
        return self.c_unet_model

    def _detect_step7_c_unet_current_frame(self, image_rgb: np.ndarray) -> Dict[str, Any]:
        try:
            import torch
            model = self._ensure_step7_c_unet_model()
            device = self.c_unet_device
            img_size = int(self.c_unet_img_size or 512)
            ...
            with torch.no_grad():
                logits = model(((tensor.to(device) - mean) / std).float())
                ...
                if logits.ndim == 4 and logits.shape[1] > 1:
                    cls_id = int(getattr(self.cfg, "rule_ab_c_unet_class_id", 1))
                    prob = torch.softmax(logits, dim=1)[0, cls_id].detach().cpu().numpy()
                elif logits.ndim == 4:
            ...
            mask = prob >= float(getattr(self.cfg, "rule_ab_c_unet_threshold", 0.5))
            k = int(getattr(self.cfg, "rule_ab_c_unet_morph_kernel", 5))
            if k > 1:
                if k % 2 == 0:
                    k += 1
            ...
            min_area = float(getattr(self.cfg, "rule_ab_c_unet_min_area_px", 50.0))
            best_lab, best_area = -1, 0
            for lab in range(1, num):
                area = int(stats[lab, cv2.CC_STAT_AREA])
                if area >= min_area and area > best_area:
                    best_lab, best_area = lab, area
            if best_lab < 0:
                return {"ok": False, "reason": "c_unet_no_valid_component", "c_area": 0}
            c_mask = (labels == best_lab)
            contours, _ = cv2.findContours(c_mask.astype(np.uint8) * 255, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)
            if not contours:
                return {"ok": False, "reason": "c_unet_no_contour", "c_area": int(best_area)}
            contour = max(contours, key=cv2.contourArea).reshape(-1, 2).astype(np.float32)
            min_pts = int(getattr(self.cfg, "rule_ab_c_unet_min_contour_points", 12))
            if len(contour) < min_pts:
                return {"ok": False, "reason": "c_unet_contour_too_short", "c_area": int(best_area), "contour_points": int(len(contour))}
            self.current_c_mask = c_mask.copy()
            self.current_c_contour = contour.copy()
            self.c_mask_dirty = False
            self.stage12_moved_since_last_c_update = False
            return {"ok": True, "c_mask": c_mask, "c_contour": contour, "c_area": int(best_area), "contour_points": int(len(contour))}
        except Exception as e:
            return {"ok": False, "reason": f"c_unet_exception:{e}", "error": str(e), "c_area": 0}

    @staticmethod
    def _nearest_point_on_contour(point_xy: Tuple[float, float], contour: np.ndarray) -> Dict[str, Any]:
        pts = np.asarray(contour, dtype=np.float32).reshape(-1, 2)
        if pts.size == 0:
            return {"ok": False, "reason": "empty_contour"}
        px, py = float(point_xy[0]), float(point_xy[1])
        d2 = (pts[:, 0] - px) ** 2 + (pts[:, 1] - py) ** 2
        idx = int(np.argmin(d2))
        qx, qy = float(pts[idx, 0]), float(pts[idx, 1])
        return {"ok": True, "idx": idx, "point": (qx, qy), "distance_px": float(math.hypot(px - qx, py - qy))}

    @staticmethod
    def _contour_tangent_at(contour: np.ndarray, idx: int, route_direction: int, k: int = 5) -> Tuple[float, float]:
        pts = np.asarray(contour, dtype=np.float32).reshape(-1, 2)
        n = len(pts)
        if n < 2:
            return (1.0, 0.0)
        kk = max(1, min(int(k), max(1, n // 4)))
        p0 = pts[(int(idx) - kk) % n]
        p1 = pts[(int(idx) + kk) % n]
        ...
        if norm <= 1e-9:
            return (1.0, 0.0)
        sign = 1.0 if int(route_direction) >= 0 else -1.0
        return (sign * vx / norm, sign * vy / norm)

    def image_vector_to_stage34_action(self, vx: float, vy: float) -> Dict[str, Any]:
        norm = math.hypot(float(vx), float(vy))
        if norm <= 1e-9:
            return {"action_id": 0, "action_name": "STAY", "vector": [0.0, 0.0]}
        ux, uy = float(vx) / norm, float(vy) / norm
        action_id = self._route_action_from_error(ux, uy, tol=0.0)
        name = {0: "STAY", 1: "UP", 2: "DOWN", 3: "LEFT", 4: "RIGHT"}.get(action_id, f"ACTION_{action_id}")
        return {"action_id": int(action_id), "action_name": name, "vector": [float(ux), float(uy)]}
    ...
    def _run_one_c_edge_route_cycle(
        ...
        """Step7 closed loop: current-frame YOLO A plus UNet/latest-valid C contour."""
        if stop_event is not None and stop_event.is_set():
            return False, {"route_enabled": True, "reason": "stop_event_set_before_step7_frame", "step": int(step_idx)}

        out_base = self.run_session_dir if self.run_session_dir is not None else self.output_root
        image_rgb = self._capture_current_rule_ab_frame(output_dir=Path(out_base) / "step7_ac_yolo_unet_frames")
        try:
            follower.cycle_index = int(getattr(follower, "cycle_index", 0) or 0) + 1
        except Exception:
            pass

        a_det = self._detect_step7_a_bbox_current_frame(image_rgb)
        if not bool(a_det.get("ok", False)):
            self.log("[Step7] 当前帧 A 未检测到，不运动，继续等待 A")
            return False, {
                "route_enabled": True,
                "reason": "a_yolo_not_detected_wait_next_frame",
                "retry_next_frame": True,
                "A_detect_ok": False,
                "a_detect": self._json_safe(a_det),
                "step": int(step_idx),
            }

        a_center = (float(a_det["center"][0]), float(a_det["center"][1]))
        c_from_last = False
        c_det: Dict[str, Any] = {"ok": False, "reason": "not_attempted"}
        if self.current_c_contour is None or bool(self.c_mask_dirty):
            c_det = self._detect_step7_c_unet_current_frame(image_rgb)
            if not bool(c_det.get("ok", False)):
                if self.current_c_contour is not None:
                    c_from_last = True
                    self.log("[Step7] 当前帧 C 检测失败，继续使用 latest valid C contour")
                else:
                    self.log("[Step7] 无有效 C contour，不运动，继续尝试检测 C")
                    return False, {
                        "route_enabled": True,
                        "reason": "no_valid_c_contour_wait_next_frame",
                        "retry_next_frame": True,
                        "A_detect_ok": True,
                        "a_detect": self._json_safe(a_det),
                        "c_detect": self._json_safe(c_det),
                        "c_contour_valid": False,
                        "c_mask_dirty": bool(self.c_mask_dirty),
                        "stage12_moved_since_last_c_update": bool(self.stage12_moved_since_last_c_update),
                        "step": int(step_idx),
                    }

        contour = np.asarray(self.current_c_contour, dtype=np.float32).reshape(-1, 2)
        c_mask = self.current_c_mask
        c_area = int(np.count_nonzero(c_mask)) if c_mask is not None else 0
        near = self._nearest_point_on_contour(a_center, contour)
        if not bool(near.get("ok", False)):
            return False, {"route_enabled": True, "reason": "c_contour_nearest_failed", "retry_next_frame": True, "step": int(step_idx)}

        nearest_c_point = (float(near["point"][0]), float(near["point"][1]))
        dist_to_c = float(near["distance_px"])
        vx_away = float(a_center[0] - nearest_c_point[0])
        vy_away = float(a_center[1] - nearest_c_point[1])
        vn = math.hypot(vx_away, vy_away)
        if vn <= 1e-9:
            vx_away, vy_away, vn = 0.0, -1.0, 1.0
        normal_away = (vx_away / vn, vy_away / vn)
        normal_toward = (-normal_away[0], -normal_away[1])

        min_dist = float(getattr(self.cfg, "rule_ab_ac_min_clearance_px", getattr(self.cfg, "rule_ab_route_min_distance_px", 80.0)))
        target_dist = float(getattr(self.cfg, "rule_ab_ac_target_clearance_px", getattr(self.cfg, "rule_ab_route_safe_target_distance_px", 90.0)))
        max_dist = float(getattr(self.cfg, "rule_ab_ac_max_clearance_px", getattr(self.cfg, "rule_ab_route_max_distance_px", 100.0)))
        route_direction = int(getattr(self.cfg, "rule_ab_follow_c_direction", 1))

        if dist_to_c < min_dist:
            safety_state = "TOO_CLOSE"
            move_mode = "NORMAL_AWAY"
            move_vector = normal_away
        elif dist_to_c > max_dist:
            safety_state = "TOO_FAR"
            move_mode = "NORMAL_TOWARD"
            move_vector = normal_toward
        else:
            safety_state = "INSIDE_SAFE_ZONE"
            move_mode = "TANGENT"
            move_vector = self._contour_tangent_at(contour, int(near["idx"]), route_direction=route_direction, k=5)

        stage_action_info = self.image_vector_to_stage34_action(float(move_vector[0]), float(move_vector[1]))
        action_id = int(stage_action_info.get("action_id", 0))
        action_name = str(stage_action_info.get("action_name", "STAY"))
        stage_action = "STAY/no_stage_move"
        move_info: Dict[str, Any] = {}

        self.log(
            "[Step7-debug] "
            f"A_detect_ok=True, A_center=({a_center[0]:.2f},{a_center[1]:.2f}), A_conf={float(a_det.get('conf', 0.0)):.4f}, "
            f"C_contour_valid=True, C_area={c_area}, c_mask_dirty={bool(self.c_mask_dirty)}, "
            f"stage12_moved_since_last_c_update={bool(self.stage12_moved_since_last_c_update)}, c_from_last={bool(c_from_last)}, "
            f"nearest_C_point=({nearest_c_point[0]:.2f},{nearest_c_point[1]:.2f}), dist_to_C={dist_to_c:.2f}, "
            f"min_dist_px={min_dist:.2f}, target_dist_px={target_dist:.2f}, max_dist_px={max_dist:.2f}, "
            f"safety_state={safety_state}, move_mode={move_mode}, move_vector=({move_vector[0]:.4f},{move_vector[1]:.4f}), "
            f"stage_action={stage_action_info}, route_direction={route_direction}"
        )

        if action_id != 0:
            if stop_event is not None and stop_event.is_set():
                return False, {"route_enabled": True, "reason": "stop_event_set_before_stage34_move", "step": int(step_idx), "action_id": action_id, "action_name": action_name}
            if phase_state is not None:
                phase_state["phase"] = "moving"
                phase_state["controller_action"] = str(action_name)
        ...
        return True, {
            ...
            "A_conf": float(a_det.get("conf", 0.0)),
            "A_bbox_xyxy": self._json_safe(a_det.get("bbox_xyxy")),
            "c_contour_valid": True,
            "C_area": int(c_area),
            "c_mask_dirty": bool(self.c_mask_dirty),
            "stage12_moved_since_last_c_update": bool(self.stage12_moved_since_last_c_update),
            "c_from_last": bool(c_from_last),
            "c_detect": self._json_safe(c_det),
            "nearest_C_point": [float(nearest_c_point[0]), float(nearest_c_point[1])],
            "dist_to_C": float(dist_to_c),
            "min_dist_px": float(min_dist),
            "target_dist_px": float(target_dist),
            "max_dist_px": float(max_dist),
            "safety_state": safety_state,
            "move_mode": move_mode,
            "move_vector": [float(move_vector[0]), float(move_vector[1])],
            "stage_action": stage_action,
            "stage_action_info": self._json_safe(stage_action_info),
            "action_id": int(action_id),
            "image_action": action_name,
            "action_name": action_name,
            "route_direction": int(route_direction),
    ...
    def _postprocess_scene_ab_masks(self, follower: Any, scene: Any, tag: str = "") -> Dict[str, Any]:
        ...
        if bool(getattr(self.cfg, "rule_ab_use_feature_tracker", True)) and bool(getattr(self.cfg, "rule_ab_feature_ab_color_separation", True)):
    ...
    # --------------------------------------------------------
    # 新方案：legacy 首帧分割 -> 保存 A/B/C 颜色与形状模板 -> 后续特征识别
    # --------------------------------------------------------
    ...
    def _extract_feature_profile_from_mask(self, image_rgb: np.ndarray, mask: np.ndarray, label: str) -> Dict[str, Any]:
        """从 legacy 首帧 mask 内提取颜色、面积、中心、bbox、形状模板。"""
        m = np.asarray(mask).astype(bool)
        if not np.any(m):
    ...
    def _save_feature_templates_debug(
        ...
        约定：
            - *_initial_mask_raw.png：原始 legacy 输出；
            - *_initial_mask_cleaned.png：按正点连通域清理后的 mask；
            - *_initial_mask.png：兼容旧文件名，保存 cleaned mask；
        ...
        try:
            ...
            for lab in ("a", "b", "c"):
                if lab in raw_masks:
                    ...
                    raw_path = out_dir / f"{lab}_initial_mask_raw.png"
                ...
                cleaned_path = out_dir / f"{lab}_initial_mask_cleaned.png"
                legacy_path = out_dir / f"{lab}_initial_mask.png"
                ...
                paths[f"{lab}_mask"] = str(legacy_path)
            ...
            canvas = cv2.addWeighted(overlay, 0.35, canvas, 0.65, 0)
            overlay_path = out_dir / "abc_initial_feature_overlay.png"
            cv2.imwrite(str(overlay_path), canvas)
    ...
    def _is_bmask_valid_for_feature_tracker(
        ...
        if last_center:
            try:
                ...
                max_jump = float(getattr(self.cfg, "rule_ab_bmask_max_center_jump_px", 220.0))
    ...
    def _save_bmask_runtime_debug(
        ...
        b_fallback_mask: Optional[np.ndarray] = None,
        ...
        try:
            ...
            paths["fallback_mask_path"] = _write_mask("b_fallback_mask", b_fallback_mask)
            ...
            if b_fallback_mask is not None:
                overlay[np.asarray(b_fallback_mask).astype(bool)] = (255, 0, 255)
            ...
            row = {
                ...
                "fallback_mask_path": paths.get("fallback_mask_path", ""),
    ...
    def _apply_feature_tracker_masks_to_scene(
        ...
        关键点：
          ...
          因此这里是统一 A 身份的唯一出口，禁止再从旧 legacy scene 单独取 A。
        """
        debug = debug or {}
        if scene is None:
            return scene
        for lab, mask in masks.items():
            ...
            # 同步常见缓存字段，避免外部模块仍读取旧 legacy last_a_mask/current_a_mask。
    ...
    def _select_initial_component_by_positive_points(
        ...
        """
        从 legacy 初始 mask 的多个连通域中，只保留对应目标正点所在的主体连通域。
    ...
    def _clean_initial_abc_masks_by_positive_components(
        ...
        """
        首帧 legacy 建模板前清理 A/B/C 初始 mask。
        ...
        if "a" in raw_masks:
            ...
            if not np.any(a_clean):
                raise RuntimeError(
                    ...
                    "A 初始 legacy mask 可能没有覆盖 A 正点，或被 B/C 排除后无有效区域；"
                    f"debug={a_info}"
                )
    ...
    def _get_or_create_step9_follower_for_bc(self) -> ActualNanoBoundaryFollower:
        """
        Step9 需要 B/C mask，因此复用 RuleAB 的 legacy A/B/C 分割模块。
    ...
    def _select_step9_color_interactively(
        ...
        左键选择颜色点；I/E 切换 include/exclude；Enter/N 确认；R 重选；ESC/Q 取消。
        """
        if scale <= 0:
            scale = 1.0
        ...
        def redraw() -> np.ndarray:
            ...
            for idx, (x, y) in enumerate(selected):
                cv2.drawMarker(canvas, (x, y), (0, 0, 255), markerType=cv2.MARKER_CROSS, markerSize=18, thickness=2)
                cv2.circle(canvas, (x, y), 5, (0, 255, 255), 2)
                cv2.putText(canvas, str(idx + 1), (x + 8, y - 8), cv2.FONT_HERSHEY_SIMPLEX, 0.65, (0, 0, 255), 2, cv2.LINE_AA)

            hsv_text = ""
            if selected_hsv:
                hh, ss, vv = selected_hsv[-1]
                hsv_text = f" HSV=({hh},{ss},{vv})"
            cv2.rectangle(canvas, (0, 0), (min(w - 1, 760), 58), (0, 0, 0), -1)
            cv2.putText(
                canvas,
                f"Mode={mode}{hsv_text} | click color, I include, E exclude, Enter confirm, R reset, ESC cancel",
                (10, 35),
            ...
            if scale != 1.0:
                canvas = cv2.resize(canvas, (show_w, show_h), interpolation=cv2.INTER_AREA)
            return canvas

        def on_mouse(event, x, y, flags, param):
            if event != cv2.EVENT_LBUTTONDOWN:
                return
            ix = int(round(x / scale)) if scale != 1.0 else int(x)
            iy = int(round(y / scale)) if scale != 1.0 else int(y)
            ix = max(0, min(w - 1, ix))
            iy = max(0, min(h - 1, iy))
            hsv_val = image_hsv[iy, ix]
            selected.append((ix, iy))
            selected_hsv.append((int(hsv_val[0]), int(hsv_val[1]), int(hsv_val[2])))
            self.log(f"[Step9-颜色中心] 已选择颜色点 ({ix},{iy}) HSV={selected_hsv[-1]} mode={mode}")
    ...
    def _build_step9_color_mask_from_hsv(
        ...
        include：mask = 接近采样颜色的像素；exclude：mask = 不接近采样颜色的像素。
        """
        ...
        mask = ~close if mode == "exclude" else close
    ...
    def run_rule_ac_until_threshold(self) -> Dict[str, Any]:
        ...
        while True:
            ...
            if bool(move_info.get("moved", False)):
                self.c_mask_dirty = True
                self.stage12_moved_since_last_c_update = True
                self.log("[Step9] CH1/CH2 已运动，已设置 c_mask_dirty=True，下次 Step7 前尝试重新检测 C")
    ...
    def reset_runtime_state_for_new_calibration(self, close_followers: bool = True):
        """
        停止/失败/关闭设备之后，允许重新标定 Step9 并重新运行完整测量。
        ...
        目的：
            ...
            3. 关闭并释放上一轮 RuleAB/RuleAC/Stage12 缓存，避免旧 legacy prompt、旧 mask、
               旧 C 路线污染新一轮。
        """
        self.stop_requested = False
        self.is_measuring = False
...
class MeasurementWorkflowGUI:
    ...
    def _build_ui(self):
        ...
        # Step7 已改为 runs/detect/best.pt 检 A、runs/unet/best.pt 检 C；
        # 不再提供旧的 RuleAB-A / legacy-C 人工标定入口。
        ...
        ttk.Button(button_grid, text="暂停运动并重新标定Step9", command=self.pause_midrun_recalibration_thread).grid(row=6, column=1, padx=4, pady=4, sticky="ew")
        ttk.Button(button_grid, text="完成Step9重标定并继续测量", command=self.resume_midrun_recalibration_thread).grid(row=7, column=0, columnspan=2, padx=4, pady=4, sticky="ew")
        ...
        self.rule_ab_static_c_map_name_var = tk.StringVar(value="")
        self.rule_ab_static_c_map_dir_var = tk.StringVar(value="")
        self.rule_ab_load_static_c_map_var = tk.BooleanVar(value=False)
        self.rule_ab_force_reselect_c_var = tk.BooleanVar(value=False)
    ...
    def choose_static_c_map_dir(self):
        self.log("[Step7] Static C/legacy map selection is disabled; C is detected from runs/unet/best.pt.")
        self.set_var(self.rule_module_status_var, "Step7: C uses runs/unet/best.pt; static C/legacy selection disabled")
        return
    ...
    def segment_c_only_thread(self):
        self.log("[Step7] C-only legacy segmentation is disabled; C is detected from runs/unet/best.pt.")
        self.set_var(self.rule_module_status_var, "Step7: C uses runs/unet/best.pt; C-only legacy segmentation disabled")
    ...
    def _fit_quadrilateral_from_mask(mask_bool: np.ndarray) -> Tuple[np.ndarray, np.ndarray, str]:
        """
        将 legacy 原始 C mask 近似为四边形，并生成四边形 mask。
    ...
    def segment_c_only(self):
        self.log("[Step7] C-only segmentation is disabled; C is detected from runs/unet/best.pt.")
        self.set_var(self.rule_module_status_var, "Step7: C uses runs/unet/best.pt; C-only segmentation disabled")
    ...
    def build_config_from_ui(self) -> MeasurementConfig:
        return MeasurementConfig(
            ...
            rule_ab_static_c_map_name="",
            rule_ab_static_c_map_dir="",
            rule_ab_reuse_static_c_map_in_memory=False,
            rule_ab_load_static_c_map_if_exists=False,
            rule_ab_save_static_c_map_library=False,
    ...
    def sync_config_from_ui_to_workflow(self) -> MeasurementWorkflow:
        ...
        # 导致完整测量中后续 RuleAB/legacy 又弹出标点窗口。
    ...
    def calibrate_rule_ab_a_thread(self):
        self.log("[Step7] RuleAB-A manual calibration is disabled; A is detected from runs/detect/best.pt.")
        self.set_var(self.rule_module_status_var, "Step7: A uses runs/detect/best.pt; manual A calibration disabled")
    ...
    def calibrate_global_c_thread(self):
        self.log("[Step7] Global C/legacy calibration is disabled; C is detected from runs/unet/best.pt.")
        self.set_var(self.rule_module_status_var, "Step7: C uses runs/unet/best.pt; manual C/legacy calibration disabled")

    def calibrate_global_c(self):
        self.log("[Step7] Global C calibration is disabled; C is detected from runs/unet/best.pt.")
        self.set_var(self.rule_module_status_var, "Step7: C uses runs/unet/best.pt; manual C calibration disabled")
    ...
    def calibrate_step9_target(self):
        ...
        try:
            self._prepare_for_step9_calibration_only(reason="calibrate_step9_target")
    ...
    def calibrate_step9_color(self):
        ...
        try:
            self._prepare_for_step9_calibration_only(reason="calibrate_step9_color")
    ...
    def load_calibration_to_gui(self):
        ...
        try:
            ...
            # 也真正进入后续 RuleAB/legacy/角度/Step9 的运行上下文。
    ...
    def pause_midrun_recalibration(self):
        ...
        try:
            ...
            self.set_var(self.flow_status_var, "流程状态：已请求暂停当前cycle，请等待运动停止后重新标定Step9")
            self.set_var(self.rule_module_status_var, "规则模块状态：中途重标定请求已发出，正在停止Stage34/Step9")
            self.log("[GUI] 已请求完整测量中途暂停重标定；请等待状态变为等待重标定后，再点击 Step9 标定按钮。")
        except Exception as e:
    ...
    def resume_midrun_recalibration(self):
        """用户重新标定 Step9 后，通知完整测量线程继续当前 cycle。"""
        try:
            wf = self.workflow
            if wf is None:
            ...
            # 保存并检查当前 GUI/标定文件，确保 Step9 是最新的；A/C 由 runs 模型自动检测。
            state = self._current_or_new_calibration_state()
            self._save_calibration_file_raw(state)
            state = self._load_calibration_file_raw()
            missing = state.missing_items()
    ...
    def reset_after_stop_for_recalibration(self):
        ...
        这个函数不会删除旧的标定 JSON；后续重新点 Step9 会覆盖对应字段。
        """
        try:
            self.rule_ab_only_stop_requested = False
            self.rule_ab_controller_stop_requested = False
            ...
            self.is_busy = False
            self.set_var(self.flow_status_var, "流程状态：已重置运行态，可以重新标定Step9并重新运行")
    ...
    def _prepare_for_step9_calibration_only(self, reason: str = ""):
        """Step9 目标/颜色标定只清自己的停止标志，不改照明光或完整测量状态。"""
        try:
            if self.workflow is not None:
                self.workflow.stop_requested = False
                self.workflow.step9_stop_requested = False
            if reason:
                self.log(f"[GUI] Step9标定前仅解除停止锁；不改变照明光/完整测量状态；reason={reason}")
        except Exception as e:
            self.log(f"[GUI] Step9标定前清理停止标志失败：{e}")
    ...
    def run_workflow(self):
        ...
        try:
            ...
            if str(getattr(wf.cfg, "rule_ab_static_c_map_dir", "") or "").strip():
                ...
                c_source_note = f"本次完整测量使用运行专用 static C：{wf.cfg.rule_ab_static_c_map_dir}"
            else:
                wf.context["strict_full_calibration_c_locked"] = False
                self.root.after(0, lambda: self.rule_ab_static_c_map_dir_var.set(""))
                self.root.after(0, lambda: self.rule_ab_load_static_c_map_var.set(False))
                c_source_note = "本次完整测量未锁定 static C；Step7 将在每轮用 UNet 动态检测 C"
            ...
            self.log(
                ...
                f"{c_source_note}；不会再使用 RuleAB/RuleAC 单独测试区域残留的 C 文件夹。"
            )
            ...
            while True:
                ...
                if bool(getattr(wf, "restart_current_cycle_after_recalibration", False)):
                    ...
                    self.set_var(
                        ...
                        f"流程状态：第 {cycle_index} 轮已暂停，等待重新标定Step9；完成后点“完成重标定并继续测量”"
                    )
    ...
    def select_step9_target(self):
        ...
        """
        try:
            self._prepare_for_step9_calibration_only(reason="select_step9_target")
            wf = self.ensure_workflow()
    ...
    def test_rule_ab_thread(self):
        self.log("[Step7] Standalone RuleAB loop is disabled; full workflow Step7 uses runs/detect, runs/unet, and YOLO-OBB.")
        self.set_var(self.rule_module_status_var, "Step7: standalone RuleAB test disabled")

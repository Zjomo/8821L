...
class MeasurementConfig:
    ...
    # A 检测：使用独立 YOLO detect 模型，替代原来的 A mask 人工正/负点标定与后续 A 颜色/形状模板跟踪。
    # 输出为 A 的 bbox、center，并把 bbox 转成矩形 A mask 写回 RuleAB scene。
    ...
    # -1 表示不限制类别；如果 A 模型有多个类别，可在配置里指定 A 的 class id。
    ...
    rule_ab_a_detect_prefer_nearest_last: bool = bool(_cfg("rule_ab_a_detect_prefer_nearest_last", True))
...
class CalibrationState:
    ...
    def missing_items(self) -> List[str]:
        missing: List[str] = []
        # 当前 Step1/Step7 角度来源为 YOLO-OBB；A 位置来源为 YOLO detect，
        # 不再需要标定 Bmask、人工选 B 边、RuleAB-A 点或 RuleAB-B 点。
...
class MeasurementWorkflow:
    def __init__(self, cfg: MeasurementConfig, on_log=None, on_update=None):
        ...
        # A 检测 YOLO detect 模型缓存。A 不再通过人工 Amask/RuleAB-A 标定进入主程序。
        self.a_detect_model: Optional[Any] = None
        self.a_detect_model_path_resolved: Optional[Path] = None
        self.a_detect_last_center: Optional[Tuple[float, float]] = None
        self.a_detect_last_box: Optional[Tuple[float, float, float, float]] = None
        ...
        # GUI 右侧列表显示用：Step7 在 Step5.5 异步保存之后才完成，
        # 因此单独缓存每个 cycle 的 Step7 final_angle/final_delta，
        # 并在 plot_points 已存在或稍后追加时同步进去。
        self.step7_display_by_cycle: Dict[int, Dict[str, Any]] = {}
        self.context["step7_display_by_cycle"] = self.step7_display_by_cycle
    ...
    def init_a_detect_model(self):
        """初始化 A 检测 YOLO detect 模型。"""
        if self.a_detect_model is not None:
            return
        if YOLO is None:
            raise RuntimeError("无法导入 ultralytics.YOLO，请先安装：pip install ultralytics")
        model_text = str(getattr(self.cfg, "rule_ab_a_detect_model_path", "runs/detect/best.pt") or "").strip()
        if not model_text:
            raise ValueError("未设置 rule_ab_a_detect_model_path，不能执行 A YOLO 检测。")
        model_path = Path(model_text).expanduser()
        if not model_path.is_absolute():
            model_path = PROJECT_ROOT / model_path
        if not model_path.exists():
            raise FileNotFoundError(f"A检测YOLO模型不存在：{model_path}")
        self.log(f"[A-YOLO] 初始化 A 检测模型：{model_path}")
        self.a_detect_model = YOLO(str(model_path))
        self.a_detect_model_path_resolved = model_path
        self.context["a_detector_type"] = "yolo_detect_bbox"
        self.context["a_detect_model_path_resolved"] = str(model_path)

    def _run_a_yolo_detect_on_image(self, image_rgb: np.ndarray, label: str = "a_yolo") -> Dict[str, Any]:
        """按第一个 A 检测程序的方式，对当前截图区域图像执行 YOLO detect，返回最佳 bbox/center。"""
        if image_rgb is None:
            return {"ok": False, "reason": "image_none", "label": label}
        self.init_a_detect_model()
        frame = np.asarray(image_rgb).astype(np.uint8)
        class_filter = int(getattr(self.cfg, "rule_ab_a_detect_class_id", -1))
        ...
        result = self.a_detect_model.predict(**predict_kwargs)[0]
        if result.boxes is None or len(result.boxes) <= 0:
            return {"ok": False, "reason": "no_detection", "label": label, "detection_count": 0}
        ...
        last_center = self.a_detect_last_center
        prefer_nearest = bool(getattr(self.cfg, "rule_ab_a_detect_prefer_nearest_last", True)) and last_center is not None
        for i, box in enumerate(boxes):
            cls_id = int(classes[i])
            if class_filter >= 0 and cls_id != class_filter:
                continue
            ...
            if last_center is not None:
                dist = math.hypot(cx - float(last_center[0]), cy - float(last_center[1]))
            # 首帧/无 last_center：按置信度优先；后续：置信度 + 距上一帧近综合排序。
            score = float(confs[i]) if not prefer_nearest else float(confs[i]) - 0.002 * float(dist)
            candidates.append({
                "index": int(i), "box_xyxy": [x1, y1, x2, y2], "confidence": float(confs[i]),
                "class_id": cls_id, "class_name": str(getattr(self.a_detect_model, "names", {}).get(cls_id, cls_id)),
                "center_xy": [float(cx), float(cy)], "width": float(w), "height": float(h),
                "area_px": float(area), "distance_to_last_px": float(dist), "score": float(score),
            })
        if not candidates:
            return {"ok": False, "reason": "no_valid_detection", "label": label, "detection_count": int(len(boxes))}
        best = max(candidates, key=lambda d: float(d.get("score", 0.0)))
        self.a_detect_last_center = (float(best["center_xy"][0]), float(best["center_xy"][1]))
        self.a_detect_last_box = tuple(float(v) for v in best["box_xyxy"])
        left, top, width, height = [int(v) for v in tuple(getattr(self.cfg, "capture_area", (0, 0, frame.shape[1], frame.shape[0])))]
        best["center_screen"] = [float(best["center_xy"][0] + left), float(best["center_xy"][1] + top)]
        best["box_screen_xyxy"] = [
            float(best["box_xyxy"][0] + left), float(best["box_xyxy"][1] + top),
            float(best["box_xyxy"][2] + left), float(best["box_xyxy"][3] + top),
        ]
        return {"ok": True, "reason": "ok", "label": label, "method": "yolo_detect_bbox_from_first_code", "best": best, "candidates": candidates, "detection_count": int(len(boxes))}

    @staticmethod
    def _a_bbox_to_rect_mask(image_shape: Tuple[int, int], box_xyxy: List[float]) -> np.ndarray:
        """把 A YOLO bbox 转为矩形 mask，供原 RuleAB scene 统一读取 A mask/center。"""
        h, w = int(image_shape[0]), int(image_shape[1])
        x1, y1, x2, y2 = [float(v) for v in box_xyxy]
        xi1 = max(0, min(w - 1, int(math.floor(x1))))
        yi1 = max(0, min(h - 1, int(math.floor(y1))))
        xi2 = max(0, min(w - 1, int(math.ceil(x2))))
        yi2 = max(0, min(h - 1, int(math.ceil(y2))))
        mask = np.zeros((h, w), dtype=bool)
        if xi2 >= xi1 and yi2 >= yi1:
            mask[yi1:yi2 + 1, xi1:xi2 + 1] = True
        return mask

    def _ensure_scene_object_for_label(self, scene: Any, label: str) -> Any:
        """确保 scene 中存在 label 对象；若缺失则创建 dict 形式对象。"""
        obj = self._get_scene_object_by_label(scene, label)
        if obj is not None:
            return obj
        lab = str(label).lower()
        new_obj: Dict[str, Any] = {"label": lab.upper(), "mask": None, "source": "created_for_yolo_a"}
        try:
            if isinstance(scene, dict):
                scene[lab] = new_obj
                scene[lab.upper()] = new_obj
                return new_obj
        except Exception:
            pass
        try:
            setattr(scene, lab, new_obj)
            return new_obj
        except Exception:
            return new_obj

    def _apply_yolo_a_detection_to_scene(self, follower: Any, scene: Any, image_rgb: np.ndarray, label: str = "a_yolo_scene") -> Tuple[Any, Dict[str, Any]]:
        """用第一个程序的 YOLO detect 方式检测 A，并写回 scene.a 的 mask/center/bbox。"""
        det = self._run_a_yolo_detect_on_image(image_rgb, label=label)
        if not det.get("ok"):
            raise RuntimeError(f"A YOLO 检测失败：{det.get('reason')}")
        best = det["best"]
        mask = self._a_bbox_to_rect_mask(np.asarray(image_rgb).shape[:2], best["box_xyxy"])
        a_obj = self._ensure_scene_object_for_label(scene, "a")
        self._set_mask_to_scene_object(a_obj, mask)
        cx, cy = float(best["center_xy"][0]), float(best["center_xy"][1])
        x1, y1, x2, y2 = [float(v) for v in best["box_xyxy"]]
        try:
            if isinstance(a_obj, dict):
                a_obj.update({
                    "label": "A", "source": "yolo_detect_bbox", "mask_source": "yolo_detect_bbox",
                    "segmentation_source": "yolo_detect_bbox", "center": (cx, cy), "center_xy": (cx, cy),
                    "bbox": (x1, y1, x2, y2), "bbox_xyxy": (x1, y1, x2, y2),
                    "confidence": float(best.get("confidence", 0.0)), "class_id": int(best.get("class_id", -1)),
                    "class_name": str(best.get("class_name", "")),
                })
            else:
                setattr(a_obj, "label", "A")
                setattr(a_obj, "source", "yolo_detect_bbox")
                setattr(a_obj, "mask_source", "yolo_detect_bbox")
                setattr(a_obj, "segmentation_source", "yolo_detect_bbox")
                setattr(a_obj, "center", (cx, cy))
                setattr(a_obj, "center_xy", (cx, cy))
                setattr(a_obj, "bbox", (x1, y1, x2, y2))
                setattr(a_obj, "bbox_xyxy", (x1, y1, x2, y2))
                setattr(a_obj, "confidence", float(best.get("confidence", 0.0)))
        except Exception:
            pass
        for holder in (follower, getattr(follower, "abc_segmenter", None), getattr(follower, "segmenter", None)):
            ...
            for attr in ("last_a_mask", "current_a_mask", "a_mask", "last_mask_a"):
                try:
                    setattr(holder, attr, mask.copy())
                except Exception:
                    pass
            for attr in ("last_a_center", "current_a_center", "a_center"):
                try:
                    setattr(holder, attr, (cx, cy))
                except Exception:
                    pass
        try:
            setattr(scene, "a_source", "yolo_detect_bbox")
            if isinstance(scene, dict):
                scene["a_source"] = "yolo_detect_bbox"
        except Exception:
            pass
        det["mask_area_px"] = int(np.count_nonzero(mask))
        self.context["a_yolo_last_detection"] = self._json_safe(det)
        self.context["feature_tracker_a_center"] = [cx, cy]
        return scene, det

    def _ensure_calibration_state_has_yolo_a_seed(self, state: Optional[CalibrationState], reason: str = "") -> Optional[CalibrationState]:
        """给外部 RuleAB 初始化提供自动 A 正点种子，避免再要求人工 RuleAB-A 标定。"""
        if state is None:
            return state
        if getattr(state, "rule_ab_a_positive_points", None):
            return state
        try:
            image_rgb = self._capture_current_rule_ab_frame(output_dir=self.output_root / "a_yolo_seed")
            det = self._run_a_yolo_detect_on_image(image_rgb, label=f"a_seed_{reason or 'runtime'}")
            if det.get("ok"):
                cx, cy = det["best"]["center_xy"]
                state.rule_ab_a_positive_points = [[float(cx), float(cy)]]
                state.rule_ab_a_negative_points = []
                self.context["calibration_state"] = state.to_dict()
                self.log(f"[A-YOLO] 已自动生成 RuleAB 初始化用 A 正点 seed=({float(cx):.1f},{float(cy):.1f})；不需要人工 Amask/RuleAB-A 标定。")
            else:
                self.log(f"[A-YOLO] 自动生成 A seed 失败：{det.get('reason')}；后续仍会尝试直接 YOLO 写回 scene。")
        except Exception as e:
            self.log(f"[A-YOLO] 自动生成 A seed 异常：{e}")
        return state

    def _save_a_yolo_runtime_debug(self, image_rgb: np.ndarray, det: Dict[str, Any], frame_index: int = 0, label: str = "") -> None:
        if not bool(getattr(self.cfg, "rule_ab_a_detect_save_debug", False)):
            return
        try:
            base = self.output_root / "a_yolo_runtime_debug"
            base.mkdir(parents=True, exist_ok=True)
            img = cv2.cvtColor(np.asarray(image_rgb).astype(np.uint8), cv2.COLOR_RGB2BGR)
            best = det.get("best", {}) if isinstance(det, dict) else {}
            if best:
                x1, y1, x2, y2 = [int(round(float(v))) for v in best.get("box_xyxy", [0, 0, 0, 0])]
                cv2.rectangle(img, (x1, y1), (x2, y2), (0, 220, 255), 2)
                cx, cy = [int(round(float(v))) for v in best.get("center_xy", [0, 0])]
                cv2.circle(img, (cx, cy), 5, (0, 0, 255), -1)
                cv2.putText(img, f"A-YOLO {float(best.get('confidence', 0.0)):.2f}", (x1, max(20, y1 - 8)), cv2.FONT_HERSHEY_SIMPLEX, 0.65, (0, 220, 255), 2, cv2.LINE_AA)
            stem = f"frame_{int(frame_index):06d}_{label or 'a_yolo'}"
            cv2.imwrite(str(base / f"{stem}.png"), img)
            with (base / f"{stem}.json").open("w", encoding="utf-8") as f:
                json.dump(self._json_safe(det), f, ensure_ascii=False, indent=2)
        except Exception as e:
            self.log(f"[A-YOLO] 保存 debug 失败：{e}")
    ...
    def ensure_rule_ab_follower(self) -> ActualNanoBoundaryFollower:
        ...
        if state is not None:
            ...
            # Step7 初始化阶段不再执行 A-YOLO，也不再自动生成 A seed。
            # 当前完整测量只要求 static_c_mask.png 可用；A 位置只在 Step7 控制循环内逐帧 YOLO detect。
            # 因此这里保留 state 原样，不把首帧 A-YOLO 成功作为 RuleAB/Step7 前置条件。
        ...
        if state is not None:
            if bool(getattr(self.cfg, "rule_ab_use_feature_tracker_after_sam2_init", True)):
                # C-only / runtime A-YOLO 模式下，不调用外部 RuleAB 的 ABC 首帧初始化。
                # 该旧初始化链路可能会强制要求 A/B 点、A/B raw mask 或首帧 A-YOLO 成功，
                # 与当前需求冲突。这里只注入完整标定包中的 static C 信息；真正的 scene 由
                # _install_feature_tracker_on_rule_ab_follower() 接管后的 capture_and_build_scene() 建立。
                self._inject_calibration_into_rule_ab_follower(self.rule_ab_follower, state)
                self.log(
                    "[C-only场景接管] 已跳过外部 RuleAB ABC 首帧初始化；"
                    "Step7 初始化只要求 static_c_mask.png，A/B 初始 mask 均为空。"
                )
            else:
            ...
            if bool(getattr(self.cfg, "rule_ab_use_feature_tracker_after_sam2_init", True)):
                self.log(
                    "[C-only场景接管] 未加载完整标定 state；跳过外部 ABC 初始化。"
                    "后续若 static C 不可用，将在 C-only scene 初始化时报错。"
                )
            else:
                self.rule_ab_follower.initialize_abc_with_first_frame()

        # 安装 C-only scene 接管：初始化只读 static C，Step7 控制循环逐帧 A-YOLO。
        ...
        for obj in objs:
            ...
            for attr in cache_attrs:
                try:
                    ...
                    pass
        if reason:
            self.log(f"[Step7-A-YOLO] 已清理当前帧坏 scene 缓存；reason={reason}")
    ...
    def _capture_and_build_scene_with_ab_retry(
        ...
        """
        完整测量中所有需要 Step7 当前帧 A-YOLO / C-only scene 的地方统一通过这个函数取帧。

        解决的问题：
            - 当前帧 A-YOLO detect 无检测框；
            - 当前帧截图失败；
            - C-only capture_and_build_scene() 构建 scene 时抛异常。

        行为：
            失败时不直接终止完整测量，而是等待下一帧后重新截图并检测 A。
            达到重试次数仍失败时：
        ...
        if follower is None:
            msg = "RuleAB follower 为空，无法取帧执行 Step7 当前帧 A-YOLO"
            info["reason"] = msg
            if allow_fail:
        ...
        for attempt in range(1, attempts + 1):
            ...
            try:
                ...
                if attempt > 1:
                    self.log(f"[Step7-A-YOLO] {label}: 第 {attempt}/{attempts} 次重新取帧并检测 A 成功。")
                ...
                self.log(
                    f"[Step7-A-YOLO] {label}: 当前帧 A 检测失败 attempt={attempt}/{attempts}: {msg}；"
                    "本帧不执行 Stage34 运动，不停止完整测量，准备重新取下一帧。"
                )
                self._clear_rule_ab_bad_scene_cache(follower, reason=f"{label}/attempt={attempt}: {msg}")
                if attempt < attempts:
                    time.sleep(interval_s)

        final_msg = f"{label}: Step7 当前帧 A-YOLO 连续失败 {attempts} 次，最后错误：{last_exc}"
            ...
            try:
                ...
                while not stop_event.is_set() and not self.stop_requested and not self._is_midrun_recalibration_requested():
                    ...
                    if stop_event.is_set():
                        break
                    if not ok:
                        result_box["stop_reason"] = "step7_a_yolo_failed_retry_next_frame"
                        self.log(f"[Step7实时控制] 当前帧 A-YOLO 未成功或路线本帧未执行，继续等待下一帧：{route_info}")
    ...
    def run_rule_ab_until_angle_delta(
        ...
        while True:
            ...
            if use_route:
                ...
                if not ok:
                    self.log(f"[Step7-A-YOLO] 当前帧 A 检测失败或路线本帧未执行：{route_info}；不停止完整测量，重新取下一帧继续。")
                    records.append({
                        "step": step_idx,
                        "rule_ab_ok": False,
                        "angle_deg": None,
                    ...
                    stop_reason = "step7_a_yolo_failed_retry_next_frame"
                ...
                if not ok:
                    self.log(f"[RuleAB] run_one_cycle/当前帧检测失败：{route_info}；重新取下一帧继续。")
                    records.append({
                        "step": step_idx,
                        "rule_ab_ok": False,
                        "angle_deg": None,
    ...
    def _run_one_c_edge_route_cycle(
        ...
        # 捕获当前帧并获得 C-only + runtime A-YOLO scene；不再调用 follower.rule_policy(scene)。
        # 当前帧 A-YOLO 失败时不允许异常直接冒泡终止完整测量，而是返回 False，
        ...
        try:
            ...
            if image_rgb is None or scene is None:
                self.log(
                    "[Step7-A-YOLO] 当前帧 A 检测失败，跳过本帧运动，等待下一帧；"
                    f"step={int(step_idx)}, reason={seg_retry_info.get('reason') if isinstance(seg_retry_info, dict) else 'unknown'}"
                )
                return False, {
                    "route_enabled": True,
                    "reason": "step7_a_yolo_failed_retry_next_frame",
                    "error": seg_retry_info.get("reason") if isinstance(seg_retry_info, dict) else "current_frame_a_yolo_failed",
                    "step": int(step_idx),
            ...
            return False, {
                "route_enabled": True,
                "reason": "step7_a_yolo_exception_retry_next_frame",
                "error": str(e),
                "step": int(step_idx),
            }
        try:
        ...
        # 当前 C-only 方案下：
        #   A = YOLO detect bbox/center；
        #   B = 不建立 mask，角度由 YOLO-OBB；
        # 因此不能再执行旧的 A/B mask guard（它会检查 A 面积突变、A-B 排除等旧逻辑）。
        # 只有在非 C-only/旧来源 scene 时才保留旧 guard。
        ab_guard_info = {"enabled": False, "reason": "c_only_a_yolo_b_disabled_skip_ab_mask_guard"}
        try:
            scene_src = str(getattr(scene, "segmentation_source", getattr(scene, "source", "")) or "")
            if isinstance(scene, dict):
                scene_src = str(scene.get("segmentation_source", scene.get("source", scene_src)) or scene_src)
        except Exception:
            scene_src = ""
        if "c_only_scene_static_c_runtime_a_yolo_b_disabled" not in scene_src:
            with self.rule_ab_vision_lock:
    ...
    def _track_abc_by_saved_features(
        ...
        for lab in ("a", "b", "c"):
            if lab not in profiles:
                continue
            # A 已替换为 YOLO detect bbox，不再参与旧颜色/形状 feature tracker。
            if lab == "a" and bool(profiles.get(lab, {}).get("disabled_for_feature_tracking", False)):
                continue
    ...
    def _apply_feature_tracker_masks_to_scene(
        ...
        for lab, mask in masks.items():
            # C-only 模式下 scene 可能是最小 dict，不再依赖外部 RuleAB 原始 scene。
            # 因此缺少 a/b/c 对象时必须现场创建，不能 continue，否则 A center 会丢失。
            obj = self._ensure_scene_object_for_label(scene, lab)
            if obj is None:
                continue
            ...
            obj_source = "yolo_detect_bbox" if str(lab).lower() == "a" and str(debug.get("a", {}).get("source", "")) == "yolo_detect_bbox" else source
            try:
                setattr(obj, "source", obj_source)
                setattr(obj, "mask_source", obj_source)
                setattr(obj, "segmentation_source", obj_source)
            ...
            try:
                if isinstance(obj, dict):
                    ...
                    obj["mask_source"] = obj_source
                    obj["segmentation_source"] = obj_source
        ...
        try:
            ...
            setattr(scene, "a_source", "yolo_detect_bbox" if str(debug.get("a", {}).get("source", "")) == "yolo_detect_bbox" else source)
        ...
        if isinstance(scene, dict):
            try:
                ...
                scene["a_source"] = "yolo_detect_bbox" if str(debug.get("a", {}).get("source", "")) == "yolo_detect_bbox" else source
    ...
    def _install_feature_tracker_on_rule_ab_follower(self, follower: Any, state: Optional[CalibrationState] = None):
        """
        把 RuleAB follower 的 capture_and_build_scene() 替换成 C-only 版本。

        当前设计：
            - A：不再使用首帧 raw mask / Amask / SAM2 / feature template；
                 每次需要 A 位置时都直接用 YOLO detect 得到 bbox/center。
            - B：不再使用首帧 raw mask / SAM2 / feature template；
                 Step1/Step7 的 B 角度来源为 YOLO-OBB 原始检测，不依赖 Bmask 模板。
            - C：保留首帧/静态 C mask 和 C 路线，用于 Step7 安全区/路线运动。
        """
        if follower is None:
            return
        if not bool(getattr(self.cfg, "rule_ab_use_feature_tracker_after_sam2_init", True)):
            self.log("[C-only场景接管] 已关闭 rule_ab_use_feature_tracker_after_sam2_init，仍使用原 RuleAB 路径。")
            return
        if getattr(follower, "_abc_feature_tracker_installed", False):
            return
        if not hasattr(follower, "capture_and_build_scene"):
            self.log("[C-only场景接管] follower 没有 capture_and_build_scene，无法安装。")
            return
        ...
        tracker_state: Dict[str, Any] = {
            ...
            "mode": "c_only_static_or_feature_plus_a_yolo",
        }

        def _empty_mask_like(image_rgb: np.ndarray) -> np.ndarray:
            return np.zeros(np.asarray(image_rgb).shape[:2], dtype=bool)

        def _build_c_template_from_original_scene() -> Tuple[np.ndarray, Any]:
            """
            C-only 初始化入口。

            重要：这里不再调用 original_capture() / 外部 RuleAB 的 SAM2 A/B/C
            capture_and_build_scene()。原因是外部旧链路仍会检查 A/B raw mask、
            A 面积突变、ABC feature template 等旧条件，失败后又会回退到
            [ABC特征跟踪]。当前版本中：
              - A 只来自 YOLO detect bbox/center；
              - B 不建立 mask 模板，角度只来自 YOLO-OBB；
              - C 只来自完整测量锁定的 static_c_mask.png / static_c_edge_route.json。
            """
            out_dir = Path(tracker_state["out_dir"])
            runtime_dir = out_dir / "c_only_initial_capture"
            runtime_dir.mkdir(parents=True, exist_ok=True)

            # 直接取当前屏幕帧，不经过 RuleAB 原始 SAM2/ABC 分割链路。
            image_rgb = self._capture_current_rule_ab_frame(output_dir=runtime_dir)
            if image_rgb is None:
                raise RuntimeError("[C-only场景接管] 当前帧截图为空。")
            image_rgb = np.asarray(image_rgb).astype(np.uint8)
            if image_rgb.ndim != 3 or image_rgb.shape[0] <= 0 or image_rgb.shape[1] <= 0:
                raise RuntimeError(f"[C-only场景接管] 当前帧图像尺寸异常：shape={getattr(image_rgb, 'shape', None)}")

            # C mask 只能从完整测量锁定的 static C 目录读取；不再依赖 original_capture 的 scene.c。
            c_mask_path = ""
            try:
                locked_dir = str(getattr(self.cfg, "rule_ab_static_c_map_dir", "") or "")
                if locked_dir:
                    p0 = Path(locked_dir) / "static_c_mask.png"
                    p1 = Path(locked_dir) / "static_c_reference" / "static_c_mask.png"
                    if p0.exists():
                        c_mask_path = str(p0)
                    elif p1.exists():
                        c_mask_path = str(p1)
            ...
            if not c_mask_path:
                raise RuntimeError(
                    "[C-only场景接管] 未找到完整测量锁定的 static_c_mask.png；"
                    "A/B 不需要 raw mask，但 C 必须先完成全局 C 标定。"
                )
            loaded = cv2.imread(c_mask_path, cv2.IMREAD_GRAYSCALE)
            if loaded is None:
                raise RuntimeError(f"[C-only场景接管] static_c_mask.png 读取失败：{c_mask_path}")
            if loaded.shape[:2] != image_rgb.shape[:2]:
                loaded = cv2.resize(loaded, (image_rgb.shape[1], image_rgb.shape[0]), interpolation=cv2.INTER_NEAREST)
            raw_c_mask = loaded > 0
            if raw_c_mask is None or not np.any(raw_c_mask):
                raise RuntimeError(f"[C-only场景接管] static C mask 为空：{c_mask_path}")
            self.log(f"[C-only场景接管] 已从 static_c_mask 读取 C：{c_mask_path}；area={int(np.count_nonzero(raw_c_mask))} px")

            positive_points = self._feature_initial_positive_points_from_state(state)

            # C-only 版本：这里绝对不能再调用 _clean_initial_abc_masks_by_positive_components()。
            # 该旧函数的语义是“必须同时拥有 A/B/C raw mask 才能建立 ABC 特征模板”，
            # 即使只传入 {"c": raw_c_mask}，也会在最后检查缺少 A raw mask / B raw mask 后报错。
            # 当前版本 A/B 都不建首帧模板，因此 C 只按自己的正点连通域单独清理。
            c_pts = positive_points.get("c", [])
            c_final, c_info = self._select_initial_component_by_positive_points(
                raw_c_mask, c_pts, "c", int(getattr(self.cfg, "rule_ab_feature_min_area_px", 20))
            )
            clean_debug = {
                "created_at": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                "mode": "c_only_static_mask_clean",
                "labels": {"c": c_info},
            }
            c_final = np.asarray(c_final).astype(bool)
            if not np.any(c_final):
                c_final = raw_c_mask.astype(bool)
                clean_debug.setdefault("labels", {}).setdefault("c", {})["fallback"] = "raw_c_mask"

            # C-only 初始化阶段不再强制执行 A-YOLO。
            # Step7 真正进入路线控制循环后，才会在每一帧重新截图并检测 A。
            # 因此这里 A/B 都先写入空 mask，避免“首帧 A YOLO 检测失败”成为 Step7 前置硬条件。
            a_det: Dict[str, Any] = {
                "ok": False,
                "reason": "not_required_in_c_only_initialization",
                "label": "initial_a_yolo_skipped",
            }
            a_mask = _empty_mask_like(image_rgb)
            self.log(
                "[C-only场景接管] 初始化阶段只读取 static C；"
                "不执行首帧 A-YOLO，不要求 A mask/A center。"
            )

            # B 不再建模板，运行时 Bmask 固定为空；Step1/Step7 角度由 YOLO-OBB 模型负责。
            b_mask = _empty_mask_like(image_rgb)

            profiles: Dict[str, Dict[str, Any]] = {
                "c": self._extract_feature_profile_from_mask(image_rgb, c_final, "c")
            }
            profiles["c"].update({
                "initial_mask_source": "static_c_only",
                "runtime_source": "static_c_fixed_no_runtime_detection",
                "static_c_mask_path": str(c_mask_path),
                "positive_points_used_for_cleaning": [
                    [float(x), float(y)] for x, y in positive_points.get("c", [])
                ],
                "clean_debug": clean_debug.get("labels", {}).get("c", {}),
            })

            masks_for_debug = {"a": a_mask, "b": b_mask, "c": c_final}
            clean_debug.setdefault("labels", {}).setdefault("a", {}).update({
                "source": "empty_initial_mask",
                "reason": "A-YOLO is intentionally skipped during C-only initialization; runtime Step7 detects A per frame",
                "box_xyxy": None,
                "center_xy": None,
                "confidence": None,
                "template_created": False,
            })
            clean_debug.setdefault("labels", {}).setdefault("b", {}).update({
                "source": "disabled_empty_mask",
                "template_created": False,
                "reason": "B angle uses YOLO-OBB; B feature/raw mask template disabled",
            })

            saved = self._save_feature_templates_debug(
                image_rgb=image_rgb,
                profiles=profiles,
                masks=masks_for_debug,
                out_dir=out_dir,
                raw_masks={"c": raw_c_mask},
            ...
            debug = {
                "a": {
                    "source": "empty_initial_mask",
                    "template_created": False,
                    "selected_area_px": int(np.count_nonzero(a_mask)),
                    "center_xy": None,
                    "box_xyxy": None,
                    "confidence": None,
                    "reason": "A is detected only during Step7 runtime frames",
                },
                "b": {
                    "source": "disabled_empty_mask",
                    "template_created": False,
                    "selected_area_px": 0,
                    "reason": "B uses YOLO-OBB angle detector, not B feature template",
                },
                "c": {
                    "source": "static_c_only",
                    "template_created": True,
                    "raw_static_area_px": int(np.count_nonzero(raw_c_mask)),
                    "cleaned_area_px": int(np.count_nonzero(c_final)),
                    "used_initial_cleaned_mask": True,
                    "clean_debug": clean_debug.get("labels", {}).get("c", {}),
                },
            }

            # 构造最小 scene，不再复制/依赖外部 RuleAB 原始 scene。
            feature_scene: Dict[str, Any] = {
                "a": {"label": "A"},
                "b": {"label": "B"},
                "c": {"label": "C"},
            }
            feature_scene = self._apply_feature_tracker_masks_to_scene(
                ...
                scene=feature_scene,
                image_rgb=image_rgb,
                masks={"a": a_mask, "b": b_mask, "c": c_final},
                debug=debug,
                tracker_state=tracker_state,
                source="c_only_scene_static_c_runtime_a_yolo_b_disabled",
            )
            ...
            tracker_state["first_frame_c_only_scene"] = True
            try:
                self.context["abc_feature_tracker"] = {
                    ...
                    "out_dir": str(out_dir),
                    "mode": "c_only_static_c_runtime_a_yolo_b_yolo_obb_no_original_capture",
                    "scene_source": "c_only_scene_static_c_runtime_a_yolo_b_disabled",
                }
                ...
                pass
            self.log(
                "[C-only场景接管] 已建立 C-only 运行状态：只读 static C；"
                "A 初始化为空，Step7 运行时每帧 YOLO detect bbox/center；B 不建模板，B角度=YOLO-OBB；"
                "未调用外部 RuleAB 原始 ABC capture。"
                f"调试目录={out_dir}"
            )
        ...
        def _feature_capture_and_build_scene():
            if not tracker_state.get("initialized", False):
                # 只建立 C-only base_scene，不在初始化阶段检测 A；
                # 建立完成后继续进入下面的 runtime 分支，在当前 Step7 帧实时检测 A。
                _build_c_template_from_original_scene()
            ...
            # C 严格固定读取初始化阶段保存的 static C mask。
            # Step7 运行时不再对 C 做每帧 feature tracking / SAM2 / 重新分割，
            # C 只作为固定路线、边界和安全距离参考。
            masks: Dict[str, np.ndarray] = {}
            debug: Dict[str, Dict[str, Any]] = {}
            base_c = self._extract_mask_from_scene_object(self._get_scene_object_by_label(tracker_state.get("base_scene"), "c"))
            if base_c is not None and np.any(np.asarray(base_c).astype(bool)):
                masks["c"] = np.asarray(base_c).astype(bool)
                debug["c"] = {
                    "source": "static_c_from_base_scene",
                    "reason": "C is fixed; no runtime C detection/tracking",
                }
            else:
                raise RuntimeError("C-only base_scene 中 static C mask 缺失，无法执行 Step7 路线运动。")

            # A 检测流程：每帧 YOLO detect bbox -> rect mask/center。
            # Step7 路线运动使用的 A center 就来自这里的当前帧 YOLO detect 结果。
            a_det = self._run_a_yolo_detect_on_image(image_rgb, label=f"runtime_a_yolo_{frame_index}")
            if not a_det.get("ok"):
                self.log(
                    f"[Step7-A-YOLO] 当前帧未检测到 A，跳过本帧运动，等待下一帧；"
                    f"frame={frame_index}, reason={a_det.get('reason')}"
                )
                raise RuntimeError(f"[Step7-A-YOLO] 当前帧 A 检测失败：{a_det.get('reason')}；跳过本帧运动。")
            masks["a"] = self._a_bbox_to_rect_mask(image_rgb.shape[:2], a_det["best"]["box_xyxy"])
            self.log(
                f"[A-YOLO-Step7] frame={frame_index}: A center={a_det['best'].get('center_xy')}, "
                f"bbox={a_det['best'].get('box_xyxy')}, conf={a_det['best'].get('confidence')}"
            )
            debug["a"] = {
                "source": "yolo_detect_bbox",
                "template_created": False,
                "selected_area_px": int(np.count_nonzero(masks["a"])),
                "center_xy": a_det["best"].get("center_xy"),
                "box_xyxy": a_det["best"].get("box_xyxy"),
                "confidence": a_det["best"].get("confidence"),
                "class_id": a_det["best"].get("class_id"),
            }
            self._save_a_yolo_runtime_debug(image_rgb, a_det, frame_index=frame_index, label="runtime")

            # B 不再建立/运行 feature mask。写入空 Bmask，避免任何旧 Bmask 被误用。
            masks["b"] = _empty_mask_like(image_rgb)
            debug["b"] = {
                "source": "disabled_empty_mask",
                "template_created": False,
                "selected_area_px": 0,
                "bmask_source": "disabled",
                "fail_reason": "B feature template disabled; angle uses YOLO-OBB detector",
            }

            scene = copy.deepcopy(tracker_state.get("base_scene"))
            if scene is None:
                raise RuntimeError("C-only base_scene 缺失；为避免 A/B 回到旧 SAM2，本帧不允许回退原始 SAM2。")
        ...
        self.log(
            "[C-only场景接管] 已安装：只保留 C 首帧/static mask；"
            "A 每帧 YOLO detect；B 不建模板，B角度由 YOLO-OBB 检测。"
        )
    ...
    def _clean_initial_abc_masks_by_positive_components(
        ...
        # 如果还有 raw_masks 里提供但尚未清理的标签，则按原始 mask 清理一次并要求有效。
        # 注意：旧版本这里强制要求 A/B/C 三者都存在，因此 C-only 场景会因为缺少 A/B raw mask 报错。
        # 当前版本允许只清理传入的标签；A/B 不再参与首帧模板建立。
        for lab in tuple(raw_masks.keys()):
            lab = str(lab).lower()
            if lab in cleaned:
                continue
            pts = positive_points.get(lab, [])
    ...
    def _append_plot_point(self, cycle_index: int, context_snapshot: Optional[Dict[str, Any]] = None):
        ...
        step7_info = self.step7_display_by_cycle.get(int(cycle_index), {})

        point = {
            "cycle_index": int(cycle_index),
            "angle_deg": angle_value,
            "step7_final_angle": ctx.get("last_rule_ab_final_angle", step7_info.get("step7_final_angle")),
            "step7_final_delta": ctx.get("last_rule_ab_final_delta", step7_info.get("step7_final_delta")),
            "fit_peak": fit_peak,
    ...
    def _record_step7_display_values(self, cycle_index: int, rule_ab_result: Optional[Dict[str, Any]]):
        """
        记录 GUI 右侧列表需要显示的 Step7 final_angle/final_delta。

        注意：本程序在 Step5.5 已经提交异步保存/绘图点，Step7 结果是在之后才产生。
        因此这里同时写入 step7_display_by_cycle 缓存，并尽量回填已经存在的 plot_points。
        只影响 GUI 显示，不改变 CSV/XLSX 中原来的保存角度。
        """
        if rule_ab_result is None:
            return
        try:
            ci = int(cycle_index)
        except Exception:
        ...
        info = {
            "step7_final_angle": rule_ab_result.get("final_angle"),
            "step7_final_delta": rule_ab_result.get("final_delta"),
        }
        self.step7_display_by_cycle[ci] = info
        self.context["step7_display_by_cycle"] = self.step7_display_by_cycle

        try:
            for p in self.plot_points:
                if int(p.get("cycle_index", -1)) == ci:
                    p.update(info)
        except Exception:
            pass
    ...
    def run_one_cycle(self, cycle_index: int) -> bool:
        ...
        try:
            ...
            self._record_step7_display_values(cycle_index, rule_ab_result)
    ...
    def close_all(self, close_light: bool = True):
        """
        关闭硬件安全相关对象，但保留 LabVIEW TCP Server。

        参数：
            close_light:
                True  ：用于 GUI 的“关闭全部设备”按钮，按原逻辑关闭/释放照明光对象；
                False ：用于直接关闭 GUI 窗口，只停止测量并释放其它运行对象，
                        不对照明光发送 OFF，也不 close/release 照明光对象。

        当前要求：
            1. 关闭运动设备、激光等硬件安全相关对象；
            2. GUI 窗口关闭时照明光保持当前状态，不额外 OFF/close/release；
            3. 不关闭 LabVIEW TCP Server；
            4. 不把 self.tcp_server 置 None；
            5. 不把 context["tcp_started"] / context["labview_ready"] 改成 False；
            6. TCP 只由 GUI 的“关闭TCP”按钮单独关闭。
        """
        if close_light:
            self.log("========== 关闭全部设备：关闭硬件安全相关对象，保留 LabVIEW TCP ==========")
        else:
            self.log("========== 关闭GUI：停止测量/释放运动与激光对象，照明光保持当前状态，保留 LabVIEW TCP ==========")
        self.save_summary_xlsx()
        ...
        try:
            if self.light is not None:
                if close_light:
                    ...
                    self.log("[照明光] GUI关闭时保持当前状态：不发送 OFF，不 close/release，不修改 light_on 状态")
        except Exception as e:
            self.log(f"[照明光] 关闭处理失败：{e}")
...
class MeasurementWorkflowGUI:
    ...
    def _build_ui(self):
        ...
        ttk.Label(button_grid, text="2 A检测：YOLO自动，无需RuleAB-A标定").grid(row=2, column=0, padx=4, pady=4, sticky="ew")
        ...
        columns = ("cycle", "angle", "step7_final_angle", "step7_final_delta", "fit_peak")
        ...
        self.angle_fit_tree.heading("angle", text="Step1角度(0-180°)")
        self.angle_fit_tree.heading("step7_final_angle", text="Step7 final_angle(0-180°)")
        self.angle_fit_tree.heading("step7_final_delta", text="Step7 final_delta/deg")
        ...
        self.angle_fit_tree.column("step7_final_angle", width=160, anchor="center", stretch=True)
        self.angle_fit_tree.column("step7_final_delta", width=150, anchor="center", stretch=True)
    ...
    def update_angle_fit_peak_plot(self, workflow: Optional[MeasurementWorkflow] = None):
        ...
        # 右侧列表：显示 Step1 角度、Step7 final_angle、Step7 final_delta 和拟合峰值。
        # 角度显示统一转换为 0–180° 的无方向角度；内部保存/计算仍保持原始角度。
        list_points: List[Tuple[int, Optional[float], Optional[float], Optional[float], Optional[float]]] = []
        fit_plot_x: List[int] = []
        ...
        for p in points:
            try:
                ...
                step7_final_angle = p.get("step7_final_angle")
                step7_final_delta = p.get("step7_final_delta")
                fit_peak = p.get("fit_peak")

                angle_value = None if angle is None else float(angle)
                step7_final_angle_value = None if step7_final_angle is None else float(step7_final_angle)
                step7_final_delta_value = None if step7_final_delta is None else float(step7_final_delta)
                fit_peak_value = None if fit_peak is None else float(fit_peak)

                list_points.append((record_index, angle_value, step7_final_angle_value, step7_final_delta_value, fit_peak_value))
    ...
    @staticmethod
    def _angle_to_undirected_0_180(value: Optional[float]) -> Optional[float]:
        """
        GUI 显示用：把有方向角度转换成 0–180° 的无方向角度。

        例如 -82.8° 显示为 97.2°，因为这两者表示同一条无方向直线。
        只改变 GUI 显示，不改变程序内部的 YOLO-OBB 原始角度、delta 计算或保存字段。
        """
        if value is None:
            return None
        try:
            v = float(value) % 180.0
            if math.isclose(v, 180.0, abs_tol=1e-9):
                v = 0.0
            return float(v)
        except Exception:
            return None

    def update_angle_fit_xy_list(self, valid_points: List[Tuple[int, Optional[float], Optional[float], Optional[float], Optional[float]]]):
        """
        ...
        try:
            ...
            for cycle_index, angle, step7_final_angle, step7_final_delta, fit_peak in valid_points:
                tree.insert(
                    "",
                    tk.END,
                    values=(
                        cycle_index,
                        self._format_optional_float(self._angle_to_undirected_0_180(angle)),
                        self._format_optional_float(self._angle_to_undirected_0_180(step7_final_angle)),
                        self._format_optional_float(step7_final_delta),
            ...
            self.angle_fit_list_status_var.set(
                ...
                "Step1角度和Step7 final_angle按0–180°无方向角度显示；final_delta为Step7相对Step1的角度变化"
            )
        except Exception as e:
    ...
    def calibrate_rule_ab_a_thread(self):
        # A 已改为 YOLO detect 自动检测；保留空入口仅兼容旧快捷调用。
        self.run_in_thread(self.calibrate_rule_ab_a)

    def calibrate_rule_ab_a(self):
        """A 已改为 YOLO detect 自动检测，不再进行 RuleAB-A/Amask 人工标定。"""
        try:
            self.log("[完整测量标定] A检测已改为 YOLO detect 自动检测；RuleAB-A/Amask 标定入口已停用。")
            self.root.after(0, lambda: messagebox.showinfo("A检测", "A 已改为 YOLO detect 自动检测，不需要再标定 RuleAB-A/Amask。"))
        except Exception:
            pass
    ...
    def on_close():
        ...
        try:
            if app.workflow is not None:
                ...
                app.workflow.close_all(close_light=False)
        except Exception:
            pass

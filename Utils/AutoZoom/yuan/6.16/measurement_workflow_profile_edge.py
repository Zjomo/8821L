...
class MeasurementConfig:
    ...
    rule_ab_use_profile_selected_b_edge_angle: bool = bool(_cfg("rule_ab_use_profile_selected_b_edge_angle", True))
...
class MeasurementWorkflow:
    ...
    def detect_b_klt_selected_edge_angle_once(
        ...
        """Step1/Step7 默认角度入口：动态一维灰度导数扫描跟踪指定 B 物理边。"""
        self.log(f"========== Profile指定B边角度检测：{label} ==========")
        try:
            if follower is None:
                raise RuntimeError("RuleAB follower 为空，无法做 profile 指定边角度检测")

            image_rgb = None
            b_mask = None
            seg_retry_info: Dict[str, Any] = {"ok": True, "mode": "direct_frame_after_profile_initialized"}

            # 初始化阶段仍需要当前 B mask 来让你人工确认指定边；初始化完成后，角度检测只直接截当前帧，
            # 不再因为 B mask 分割失败而改变/重选指定边。
            if not self.b_edge_klt_state.get("initialized", False):
                image_rgb, scene, seg_retry_info = self._capture_and_build_scene_with_ab_retry(
                    follower=follower,
                    label=f"Profile指定B边初始化/{label}",
                    allow_fail=False,
                )
                try:
                    self._postprocess_scene_ab_masks(follower=follower, scene=scene, tag=f"profile_angle_init_{label}")
                except Exception:
                    pass
                b_mask = self._extract_mask_from_scene_object(getattr(scene, "b", None))
                if b_mask is None or not np.any(np.asarray(b_mask).astype(bool)):
                    raise RuntimeError("当前帧 B mask 为空，无法进行首帧 profile 指定边人工选边")
            else:
                image_rgb = self._capture_current_rule_ab_frame(self._profile_output_dir())
                # 仅用于 overlay 可视化；失败不影响 profile 角度。
                try:
                    _img2, scene, seg_retry_info = self._capture_and_build_scene_with_ab_retry(
                        follower=follower,
                        label=f"Profile指定B边可视化Bmask/{label}",
                        max_attempts=1,
                        allow_fail=True,
                    )
            ...
            result["scene_source"] = "direct_frame_profile_edge" if self.b_edge_klt_state.get("initialized", False) else "feature_tracker_initialization"
            result["edge_selection_mode"] = "initial_selected_b_edge_dynamic_1d_profile"
            if result.get("ok", False) and result.get("angle_deg") is not None:
                self.log(
                    f"[Profile指定B边-{label}] angle={float(result.get('angle_deg')):.6f}°, "
                ...
                self.log(f"[Profile指定B边-{label}] 失败：{result.get('reason')}")
            return result
        except Exception as e:
            msg = f"Profile指定B边角度检测失败：{e}"
            self.log(f"[Profile指定B边-{label}] {msg}")
            if not allow_fail:
                raise
        ...
        while True:
            ...
            if use_route:
                ...
                self.log(
                    ...
                    f"目标角度范围=[{min_delta}, {max_delta}]；角度来源=Profile指定B边。"
                )
                try:
            ...
            # 每一步运动后，使用同一个跨轮次 Profile 动态扫描器计算当前 B 指定物理边角度。
            angle_result = self.detect_b_klt_selected_edge_angle_once(
                follower=follower,
                label=f"rule_ab_profile_edge_step_{step_idx}",
                baseline_angle=baseline_angle,
                allow_fail=True,
    ...
    def run_one_cycle(self, cycle_index: int) -> bool:
        ...
        try:
            ...
            self.log("========== Step 1：Profile指定B边角度检测 current_angle ==========")
            angle_result = {"ok": False, "angle_deg": None, "reason": "profile_selected_b_edge_not_run"}
            follower_for_angle = None
            try:
                follower_for_angle = self.ensure_rule_ab_follower()
                self.apply_runtime_rule_ab_params_to_follower(follower_for_angle, reason="step1_profile_selected_b_edge_angle")
                ...
                angle_result = self.detect_b_klt_selected_edge_angle_once(
                    ...
                    label="current_angle_profile_selected_b_edge",
                    baseline_angle=None,
                    allow_fail=True,
                ...
                if angle_result.get("ok", False) and angle_result.get("angle_deg") is not None:
                    self.log(
                        f"[角度检测-current_angle] 已使用跨轮次 Profile 指定 B 边角度作为本轮 baseline："
                        f"{angle_result.get('angle_deg')}°；source={angle_result.get('angle_source')}"
                    )
                else:
                    self.log(
                        f"[角度检测-current_angle] Profile 指定 B 边 baseline 计算失败，"
                        f"不再回退 A 最近边作为主角度来源；reason={angle_result.get('reason')}"
                    )
            except Exception as e:
                self.log(f"[角度检测-current_angle] Profile 指定 B 边 baseline 计算异常：{e}")
            ...
            if angle_result.get("ok", False) and angle_result.get("angle_deg") is not None:
                ...
                current_angle = None
                self.log("[角度检测-current_angle] 本轮 Profile 指定边角度无效，流程停止，避免使用错误边角度进入 Step7。")

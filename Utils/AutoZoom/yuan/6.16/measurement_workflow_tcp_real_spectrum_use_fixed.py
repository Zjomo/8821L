...
class MeasurementWorkflow:
    ...
    def _collect_labview_result_after_request(self, server: Any, cycle_index: int, command: str, method_name: str = "") -> Optional[Dict[str, Any]]:
        """
        request_measure() 等接口可能返回 None，但把结果存在属性或 CSV 文件中。
        这里统一做一次兜底读取，避免误判为“没有采集接口”。
        """
            ...
            try:
                ...
                if isinstance(value, (str, Path)):
                    return {
                        ...
                        "index": cycle_index,
                        "command": command,
                        "method": f"{method_name}+attr:{attr}",
                        "csv_path": str(value),
                    }
                if not isinstance(value, dict):
                    value = {"values": value}
                value.setdefault("ok", True)
                value.setdefault("index", cycle_index)
                value.setdefault("command", command)
                value.setdefault("method", f"{method_name}+attr:{attr}")
                return value
            except Exception:
                continue
    ...
    def _request_labview_spectrum_compat(self, cycle_index: int) -> Dict[str, Any]:
        ...
        for name in request_method_names:
            ...
            try:
                # 依次尝试常见签名。
                try:
                    result = method(command=command, index=cycle_index)
                except TypeError:
                    try:
                        result = method(command)
                    except TypeError:
                        try:
                            result = method(cycle_index)
                        except TypeError:
                            result = method()

                if result is None:
                    # 有些 request_measure() 只负责发命令/等待 LabVIEW 保存 CSV，
                    # 返回 None，但会把结果写入 server 属性或 output_dir 中。
                    result = self._collect_labview_result_after_request(server, cycle_index, command, method_name=name)
                if result is None:
                    result = {}
                ...
                self.log(f"[TCP] 已通过 LabVIEWTCPServer.{name}() 获取光谱结果")
                return result
            except Exception as e:
                last_exc = e
                self.log(f"[TCP] 调用 {name}() 失败，尝试下一个接口：{e}")

        # 有些 Server 用最新结果属性保存数据。
        for attr in ("last_result", "result", "latest_result", "last_data", "data"):
            try:
                value = getattr(server, attr, None)
                value = value() if callable(value) else value
                if value is not None:
                    ...
                    value.setdefault("index", cycle_index)
                    value.setdefault("command", command)
                    value.setdefault("method", f"attr:{attr}")
                    self.log(f"[TCP] 已从 LabVIEWTCPServer.{attr} 获取光谱结果")
                    return value
            except Exception as e:
                last_exc = e
        ...
        raise RuntimeError(
            ...
            f"{request_method_names} 和 last_result/result/latest_result/last_data/data；"
    ...
    def request_labview_spectrum(self, cycle_index: int) -> Dict[str, Any]:
        ...
        if not parsed.get("raw_values"):
            raise RuntimeError(
                ...
                "请确认 LabVIEWTCPServer.request_measure() 返回 raw_y/intensity/values，"
                "或返回/保存一个可读取的CSV。程序不会再用1000代替真实数据。"
            )

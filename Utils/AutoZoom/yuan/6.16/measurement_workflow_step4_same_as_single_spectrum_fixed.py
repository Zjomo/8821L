...
class MeasurementWorkflow:
    ...
    def _labview_result_has_usable_spectrum(self, result: Any) -> bool:
        ...
        这里故意不把 ok/reason/index/num_points 这类状态字段当作光谱；
        因为完整测量 Step 4 必须和“单次光谱采集”一样，拿到真实数组或真实 CSV，
        不能只拿到 request_measure() 的状态包就结束。
        """
        if result is None:
            return False
    ...
    def request_labview_spectrum(self, cycle_index: int) -> Dict[str, Any]:
        """请求一次真实 LabVIEW 光谱采集，并计算/保存真实峰值。"""
        if bool(globals().get("SPECTROMETER_TCP_DISABLED", False)):
        ...
        self.log("========== Step 4：LabVIEW TCP 真实光谱采集 ==========")
        raw_result = self._request_labview_spectrum_compat(cycle_index)
        self.log(
            "[TCP] 返回摘要："
            f"ok={raw_result.get('ok')}, "
            f"reason={raw_result.get('reason')}, "
            f"method={raw_result.get('method')}, "
            f"num_points={raw_result.get('num_points')}, "
            f"csv_path={raw_result.get('csv_path')}, "
            f"values_type={type(raw_result.get('values')).__name__}, "
            f"values_preview={self._preview_labview_value(raw_result.get('values'))}"
        )
        parsed = self._parse_labview_result(raw_result)

        result = dict(raw_result)
        result.update(parsed)
        result["ok"] = bool(result.get("ok", True))
        result["index"] = cycle_index

        # 真实 TCP 模式下不允许再静默退回 1000。
        # 如果 LabVIEW 已连接但没有返回 raw_y/intensity/values，也没有可读 csv_path，
        # 直接报错，避免 Summary/Excel/绘图继续显示假峰值。
        if not parsed.get("raw_values"):
            debug_path = ""
            try:
                err_dir = self.output_root / "errors"
                err_dir.mkdir(parents=True, exist_ok=True)
                debug_path = str(err_dir / f"labview_raw_result_cycle_{cycle_index:04d}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
                with open(debug_path, "w", encoding="utf-8") as f:
                    json.dump(raw_result, f, ensure_ascii=False, indent=2, default=str)
            except Exception:
                debug_path = ""
            raise RuntimeError(
                "Step 4 已执行 LabVIEW TCP 请求，但没有解析到任何光谱原始数组。"
                f" TCP返回keys={list(raw_result.keys())}; "
                f"ok={raw_result.get('ok')}; reason={raw_result.get('reason')}; "
                f"num_points={raw_result.get('num_points')}; csv_path={raw_result.get('csv_path')}; "
                f"values_type={type(raw_result.get('values')).__name__}; "
                f"values_preview={self._preview_labview_value(raw_result.get('values'))}. "
                f"raw_result_debug_json={debug_path}. "
                "请确认 LabVIEWTCPServer.request_measure() 的 values 是否确实包含光谱点。程序不会再用1000代替真实数据。"
            )

        self.context["last_tcp_result"] = result
        self.context["wavelength"] = parsed.get("wavelength") or []
        self.context["x_axis_values"] = parsed.get("wavelength") or self.load_x_axis_values_from_xlsx()
        self.context["x_axis_xlsx_path"] = self.cfg.x_axis_xlsx_path
        self.context["labview_csv_path"] = raw_result.get("csv_path") or ""
        ...
        result["num_points"] = len(self.context["raw_values"])
        ...
        self.log(
            "[光谱] 真实采集完成："
            f"num_points={result.get('num_points')}, "
            f"raw_original_peak={self.context.get('raw_original_peak')}, "
            f"raw_filtered_peak={self.context.get('raw_filtered_peak')}, "
            f"raw_median_peak={self.context.get('raw_median_peak')}, "
            f"fit_peak={self.context.get('fit_peak')}"
        )
        self.notify_update()
        return result
    ...
    def _parse_labview_result(self, result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # 先从整个 result 递归提取。这样即使 LabVIEWTCPServer 返回：
        #   {"values": [[x,y], ...]}、{"values": "x,y\n..."}、
        #   {"data": {"intensity": [...]}}，也能解析。
        wavelength, raw_values = self._extract_spectrum_arrays_from_any(result)

        # 明确字段优先级更高，避免 result 中其它数值字段污染光谱。
        if "wavelength" in result:
            wavelength = self._to_float_list(result.get("wavelength")) or wavelength

        for key in ("raw_y", "RAW_Y", "raw_values", "intensity"):
            if key in result:
                _, y2 = self._extract_spectrum_arrays_from_any(result.get(key))
                if y2:
                    raw_values = y2
                    break

        # values 是 LabVIEWTCPServer 当前返回里最关键的字段；单独再解析一次。
        if not raw_values and "values" in result:
            x2, y2 = self._extract_spectrum_arrays_from_any(result.get("values"))
            wavelength = wavelength or x2
            raw_values = y2

        if not raw_values and "spectrum" in result:
            x2, y2 = self._extract_spectrum_arrays_from_any(result.get("spectrum"))
            wavelength = wavelength or x2
            raw_values = y2

        if not raw_values and "data" in result:
            x2, y2 = self._extract_spectrum_arrays_from_any(result.get("data"))
            wavelength = wavelength or x2
            raw_values = y2
        ...
        # 有些 LabVIEW 只返回峰值，不返回完整数组。这里不伪造 1000，
        # 但允许用真实 raw_peak/fit_peak 作为单点值，保证保存和显示真实数据。
        if not raw_values:
            peak_candidate = None
            for key in ("raw_peak", "raw_original_peak", "peak", "max", "fit_peak"):
                if key in result:
                    peak_candidate = self._numeric_from_any(result.get(key))
                    if peak_candidate is not None:
                        break
            if peak_candidate is not None:
                raw_values = [float(peak_candidate)]

...
class MeasurementConfig:
    ...
    # 说明：单轮流程中的 Step 6 / Step 8 不再使用 Newport 激光轴，
    ...
    # A 推动 B：调用 RuleAB/Step7 路线推进，每一步后用当前帧 Bmask 最长边检测角度。
    rule_ab_enable_stage: bool = bool(_cfg("rule_ab_enable_stage", True))
    rule_ab_max_steps: int = int(_cfg("rule_ab_max_steps", 15))
    # Step7 的目标角度变化范围。注意：这不是旧 ScreenAngleDetector 的 angle_delta_min/max，
    # 也不是 delta_w 判断；它仍然是当前完整测量 Step7 的真实停止阈值。
    ...
    # Step7 每一次角度检测都执行一次“全边候选修复/确认”：
    #   - 先从当前 B mask 的全部候选边中，按上一帧确定角度选择 repaired_angle；
    #   - 若 |repaired_angle - raw_angle| <= 2°，认为检测角度正确，使用 raw_angle；
    #   - 若 |repaired_angle - raw_angle| > 2°，认为检测边选错，使用 repaired_angle。
    rule_ab_angle_repair_raw_tolerance_deg: float = float(_cfg("rule_ab_angle_repair_raw_tolerance_deg", 2.0))
    rule_ab_angle_repair_min_edge_length_px: float = float(_cfg("rule_ab_angle_repair_min_edge_length_px", 8.0))

    # Step1 baseline 角度修复/确认：
    #   Step1 也会基于当前 B mask 全边候选修复一次 baseline；
    #   若修复角度与检测角度相差 <= 1.5°，认为 Step1 检测角度正确。
    step1_angle_repair_raw_tolerance_deg: float = float(_cfg("step1_angle_repair_raw_tolerance_deg", 1.5))

    # Step7 raw_delta >= 6° 时的真假过冲判据：
    #   repaired_delta 与 raw_delta 相差 <= 2° 且 repaired_delta >= 6°：视为真实过冲，关激光并保存修复角度；
    #   repaired_delta < 6° 且 repaired_delta 与 raw_delta 相差 >= 6°：视为选错边，继续推动。
    rule_ab_overshoot_repair_delta_tolerance_deg: float = float(_cfg("rule_ab_overshoot_repair_delta_tolerance_deg", 2.0))
    rule_ab_wrong_edge_delta_jump_deg: float = float(_cfg("rule_ab_wrong_edge_delta_jump_deg", 6.0))
    ...
    rule_ab_ac_target_clearance_px: float = float(_cfg("rule_ab_ac_target_clearance_px", 150.0))
    rule_ab_ac_min_clearance_px: float = float(_cfg("rule_ab_ac_min_clearance_px", 120.0))
    rule_ab_ac_max_clearance_px: float = float(_cfg("rule_ab_ac_max_clearance_px", 180.0))
    ...
    # ============================================================
    # 说明：为了尽量不改原有调用链，Step1/Step7 仍复用
    # detect_b_klt_selected_edge_angle_once() 这个入口函数名，但函数内部
    # 不再调用 calcOpticalFlowPyrLK，而是使用上一帧指定边生成动态扫描带，
    # 沿法线做 1D 灰度导数峰值检测，再用 RANSAC 拟合当前指定边。
    ...
    # 保存前角度质量检查：只标记疑似跳边，不阻止保存、不停止完整测量。
    rule_ab_save_angle_jump_warn_deg: float = float(_cfg("rule_ab_save_angle_jump_warn_deg", 8.0))
    ...
    rule_ab_realtime_save_every_angle_frame: bool = bool(_cfg("rule_ab_realtime_save_every_angle_frame", True))
    ...
    rule_ac_center_tolerance_px: float = float(_cfg("rule_ac_center_tolerance_px", 10.0))
    ...
    rule_ac_color_min_area_px: int = int(_cfg("rule_ac_color_min_area_px", 200))
    rule_ac_color_morph_kernel: int = int(_cfg("rule_ac_color_morph_kernel", 5))


    # 完整测量前统一标定包。
    # 该文件用于保存：Bmask点/检测边、RuleAB的A点、全局C点/static C、Step9目标点与颜色。
...
class CalibrationState:
    ...
    b_mask_positive_points: List[List[float]] = field(default_factory=list)
    b_mask_negative_points: List[List[float]] = field(default_factory=list)
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "CalibrationState":
        ...
        for k in cls.__dataclass_fields__.keys():  # type: ignore[attr-defined]
            if k in data:
                kwargs[k] = data[k]
        state = cls(**kwargs)
        # 兼容旧标定文件：旧字段 angle_b_* 现在改名为 b_mask_*。
        if not state.b_mask_positive_points and data.get("angle_b_positive_points"):
            state.b_mask_positive_points = cls._points_to_lists(data.get("angle_b_positive_points"))
        if not state.b_mask_negative_points and data.get("angle_b_negative_points"):
            state.b_mask_negative_points = cls._points_to_lists(data.get("angle_b_negative_points"))
        state.b_mask_positive_points = cls._points_to_lists(state.b_mask_positive_points)
        state.b_mask_negative_points = cls._points_to_lists(state.b_mask_negative_points)
    ...
    def missing_items(self) -> List[str]:
        ...
        # B mask 与 RuleAB-B 现在做双向兜底：
        #   - 点了“1 标定Bmask/边”但没点 RuleAB-B：RuleAB-B 复用角度 B；
        #   - 点了 RuleAB-B 但没点“1 标定Bmask/边”：角度 B 复用 RuleAB-B。
        # 因此这里不再把“必须点击 1 标定Bmask/边中的 B 点”作为硬条件，
        # 只要求至少有一套 B 点，避免 C/ABC 初始化因为 B 缺失而不稳定。
        has_any_b_points = bool(self.b_mask_positive_points or self.rule_ab_b_positive_points)
        if not has_any_b_points:
            missing.append("B mask 正点（可通过 1 标定Bmask/边 或 单独 RuleAB-B 标定提供）")
        # 当前版本角度检测使用“当前帧 Bmask 最长边”，不再需要人工选择某一条 B 边。
...
class MeasurementWorkflow:
    def __init__(self, cfg: MeasurementConfig, on_log=None, on_update=None):
        ...
        self.context: Dict[str, Any] = {
            ...
            # 180° 周期角度展开与保存质量标记。
            "last_saved_final_angle_unwrapped": None,
            "last_confirmed_angle_unwrapped": None,
            "final_angle_raw_0_180": None,
            "final_angle_unwrapped": None,
            "angle_cross_boundary": False,
            "angle_delta_unwrapped_to_last": None,
            "final_delta_unwrapped_to_baseline": None,
            "angle_quality": "",
            "angle_quality_reason": "",
            "angle_quality_jump_deg": None,
        }
        ...
        self.summary_xlsx_headers = [
            ...
            "本次拟合数据峰值",
            "文件名",
            "final_angle_raw_0_180",
            "final_angle_unwrapped",
            "last_saved_final_angle_unwrapped",
            "angle_cross_boundary",
            "angle_quality",
            "angle_quality_reason",
            "angle_quality_jump_deg",
            "raw_angle",
            "repaired_angle",
            "repair_used",
            "repair_source",
            "repair_reason",
            "raw_delta_to_last_confirmed",
            "repaired_delta_to_last_confirmed",
            "final_delta_to_baseline_raw_0_180",
            "final_delta_unwrapped_to_baseline",
        ]
    ...
    def _complete_calibration_state_for_runtime(self, state: CalibrationState) -> CalibrationState:
        ...
        关键修复：
            ...
            3. 如果 RuleAB-B 未单独标定，则自动复用“B mask”的正负点作为 RuleAB-B。
        """
        if state is None:
            return state

        # RuleAB-B 没有单独标定时，复用B mask 点。
        if not state.rule_ab_b_positive_points and state.b_mask_positive_points:
            state.rule_ab_b_positive_points = self._points_to_plain_lists(state.b_mask_positive_points)
            state.rule_ab_b_negative_points = self._points_to_plain_lists(state.b_mask_negative_points)
            self.log(
                "[完整测量标定] RuleAB-B 未单独标定，已自动复用B mask 点作为 RuleAB-B："
                f"B+={len(state.rule_ab_b_positive_points)}, B-={len(state.rule_ab_b_negative_points)}"
            )

        # 反向兜底：没有点击“1 标定Bmask/边”中的 B 点，但已经单独标定了 RuleAB-B，
        # 则B mask 也复用 RuleAB-B，避免 B 缺失导致 ABC 初始化/SAM2 prompt 顺序不稳定。
        if not state.b_mask_positive_points and state.rule_ab_b_positive_points:
            state.b_mask_positive_points = self._points_to_plain_lists(state.rule_ab_b_positive_points)
            state.b_mask_negative_points = self._points_to_plain_lists(state.rule_ab_b_negative_points)
            self.log(
                "[完整测量标定] Bmask未单独标定，已自动复用 RuleAB-B 点作为B mask："
                f"B+={len(state.b_mask_positive_points)}, B-={len(state.b_mask_negative_points)}"
            )
    ...
    def begin_step9_run_session(self) -> Path:
        ...
        meta = {
            ...
            "tolerance_px": float(getattr(self.cfg, "rule_ac_center_tolerance_px", 10.0)),
    ...
    def connect_measurement_devices(self):
        ...
        self.log("========== 实验设备初始化完成：照明光 + Newport激光控制器 ==========")
        self.notify_update()
    ...
    def laser_on(self):
        """
        打开激光：替代原单轮流程 Step 6 的 Newport 激光轴 ON。
    ...
    def laser_off(self):
        """
        关闭激光：替代原单轮流程 Step 8 的 Newport 激光轴 OFF。
    ...
    def start_tcp_server(self):
        """启动 LabVIEW TCP Server，恢复真实光谱仪连接。"""
        if self.tcp_server is not None:
            self.log("[TCP] LabVIEW TCP Server 已启动")
        ...
        Path(str(self.cfg.tcp_output_dir)).mkdir(parents=True, exist_ok=True)
        self.tcp_server = LabVIEWTCPServer(
            host=self.cfg.tcp_host,
            port=int(self.cfg.tcp_port),
            output_dir=self.cfg.tcp_output_dir,
        )
        if hasattr(self.tcp_server, "start"):
            self.tcp_server.start()
        elif hasattr(self.tcp_server, "start_server"):
            self.tcp_server.start_server()
        else:
            raise RuntimeError("LabVIEWTCPServer 缺少 start/start_server 方法")
        self.context["tcp_started"] = True
        self.context["labview_ready"] = False
        self.log("[TCP] LabVIEW TCP Server 已启动，等待 LabVIEW READY")
        self.notify_update()
        return {"ok": True, "disabled": False}

    def wait_labview_ready(self):
        """等待 LabVIEW 客户端 READY，恢复真实光谱仪流程。"""
        if self.tcp_server is None:
            self.start_tcp_server()
        assert self.tcp_server is not None
        self.log("========== 等待 LabVIEW READY ==========")
        if hasattr(self.tcp_server, "wait_ready"):
            ret = self.tcp_server.wait_ready()
        elif hasattr(self.tcp_server, "wait_for_ready"):
            ret = self.tcp_server.wait_for_ready()
        elif hasattr(self.tcp_server, "wait_labview_ready"):
            ret = self.tcp_server.wait_labview_ready()
        else:
            raise RuntimeError("LabVIEWTCPServer 缺少 wait_ready/wait_for_ready/wait_labview_ready 方法")
        self.context["tcp_started"] = True
        self.context["labview_ready"] = True
        self.log("[TCP] LabVIEW READY，可进行真实光谱采集")
        self.notify_update()
        return {"ok": True, "disabled": False, "ready_result": self._json_safe(ret)}
    ...
    @staticmethod
    def unwrap_angle_180(current_angle: Optional[float], reference_unwrapped_angle: Optional[float]) -> Optional[float]:
        """
        180° 周期方向角展开。

        current_angle 为当前 0~180° 显示角；reference_unwrapped_angle 为上一帧/上一轮
        已展开角。返回与 reference_unwrapped_angle 最连续的展开角。
        """
        if current_angle is None:
            return None
        a = MeasurementWorkflow._normalize_angle_0_180(float(current_angle))
        if reference_unwrapped_angle is None:
            return float(a)
        ref = float(reference_unwrapped_angle)
        candidates = [a - 360.0, a - 180.0, a, a + 180.0, a + 360.0]
        return float(min(candidates, key=lambda x: abs(float(x) - ref)))

    @staticmethod
    def _angle_cross_boundary_180(current_raw_0_180: Optional[float], reference_unwrapped_angle: Optional[float]) -> bool:
        """判断 0~180° 显示角是否跨过边界但展开后连续。"""
        if current_raw_0_180 is None or reference_unwrapped_angle is None:
            return False
        cur = MeasurementWorkflow._normalize_angle_0_180(float(current_raw_0_180))
        ref_raw = MeasurementWorkflow._normalize_angle_0_180(float(reference_unwrapped_angle))
        raw_abs = abs(cur - ref_raw)
        periodic = MeasurementWorkflow.angle_diff_deg(cur, ref_raw)
        return bool(raw_abs > 90.0 and periodic < 30.0)
    ...
    def _save_rule_ab_repair_edge_overlay(
        self,
        angle_result: Dict[str, Any],
        repair_info: Dict[str, Any],
    ) -> str:
        """
        在 AB 最近边角度检测图上额外标注“修复后重新选择的 B 边”。

        原始 AB 最近边角度检测图会画出 raw 检测选中的边；
        每帧修复逻辑会从当前 B mask 的全部候选边中重新选择 repaired_edge。
        为了排查 raw 边和 repaired 边是否一致，这里直接读取 raw overlay，
        再把 repaired_edge 用青色粗线叠加保存到同一个 ab_angle_nearest_edge 文件夹。
        """
        try:
            if not isinstance(angle_result, dict) or not isinstance(repair_info, dict):
                return ""

            selected_edge = repair_info.get("selected_edge") or repair_info.get("repaired_edge")
            if not isinstance(selected_edge, dict):
                return ""

            overlay_path = (
                angle_result.get("overlay_path")
                or angle_result.get("image_path")
                or angle_result.get("ab_nearest_edge_overlay_path")
            )
            if not overlay_path:
                return ""

            p_overlay = Path(str(overlay_path))
            if not p_overlay.exists():
                return ""

            canvas = cv2.imread(str(p_overlay), cv2.IMREAD_COLOR)
            if canvas is None:
                return ""

            # 重新选择的修复边：cyan，和原 raw 最近边的 magenta 区分。
            x1, y1 = selected_edge["p1"]
            x2, y2 = selected_edge["p2"]
            pt1 = (int(round(float(x1))), int(round(float(y1))))
            pt2 = (int(round(float(x2))), int(round(float(y2))))
            repair_color = (255, 255, 0)  # BGR: cyan
            endpoint_color = (0, 255, 0)  # BGR: green

            cv2.line(canvas, pt1, pt2, repair_color, 5, cv2.LINE_AA)
            cv2.circle(canvas, pt1, 6, endpoint_color, -1, cv2.LINE_AA)
            cv2.circle(canvas, pt2, 6, endpoint_color, -1, cv2.LINE_AA)

            # 在线段中点附近写标签。
            mx = int(round((pt1[0] + pt2[0]) / 2.0))
            my = int(round((pt1[1] + pt2[1]) / 2.0))
            repaired_angle = repair_info.get("repaired_angle", selected_edge.get("angle_deg", None))
            raw_angle = repair_info.get("raw_angle", angle_result.get("angle_deg_raw", angle_result.get("angle_deg", None)))
            diff = repair_info.get("repaired_vs_raw_diff_deg", None)
            used = bool(repair_info.get("used_repaired_angle", False))
            try:
                label = f"REPAIR edge angle={float(repaired_angle):.3f} deg"
            except Exception:
                label = "REPAIR edge"
            cv2.putText(canvas, label, (max(5, mx - 120), max(20, my - 12)), cv2.FONT_HERSHEY_SIMPLEX, 0.62, repair_color, 2, cv2.LINE_AA)

            # 左上角追加完整说明，避免和原 overlay 的 raw 信息混淆。
            y0 = 90
            cv2.rectangle(canvas, (6, y0 - 22), (760, y0 + 72), (0, 0, 0), -1)
            cv2.putText(canvas, "MAGENTA = raw detected nearest B edge", (12, y0), cv2.FONT_HERSHEY_SIMPLEX, 0.58, (255, 0, 255), 2, cv2.LINE_AA)
            cv2.putText(canvas, "CYAN = repaired selected B edge", (12, y0 + 24), cv2.FONT_HERSHEY_SIMPLEX, 0.58, repair_color, 2, cv2.LINE_AA)
            try:
                cv2.putText(
                    canvas,
                    f"raw={float(raw_angle):.3f}, repaired={float(repaired_angle):.3f}, diff={float(diff):.3f}, used_repair={used}",
                    (12, y0 + 50),
                ...
                cv2.putText(canvas, f"used_repair={used}", (12, y0 + 50), cv2.FONT_HERSHEY_SIMPLEX, 0.58, (255, 255, 255), 2, cv2.LINE_AA)

            out_path = p_overlay.with_name(p_overlay.stem + "_WITH_REPAIR_EDGE.png")
            cv2.imwrite(str(out_path), canvas)

            # 同目录保存一个轻量 meta，方便后续批量查找。
            try:
                meta_path = out_path.with_suffix(".json")
                meta = {
                    "source_overlay_path": str(p_overlay),
                    "repair_overlay_path": str(out_path),
                    "raw_selected_edge": self._json_safe(angle_result.get("selected_b_edge")),
                    "repair_selected_edge": self._json_safe(selected_edge),
                    "raw_angle": raw_angle,
                    "repaired_angle": repaired_angle,
                    "repaired_vs_raw_diff_deg": diff,
                    "used_repaired_angle": used,
                    "repair_info": self._json_safe(repair_info),
                }
                with meta_path.open("w", encoding="utf-8") as f:
                    json.dump(meta, f, ensure_ascii=False, indent=2)
                repair_info["repair_edge_meta_path"] = str(meta_path)
                angle_result["repair_edge_meta_path"] = str(meta_path)
            except Exception:
                pass
            ...
            try:
                self.log(f"[RuleAB-角度修复] 保存修复边 overlay 失败：{e}")
            except Exception:
                pass
            return ""
    ...
    def detect_bmask_longest_edge_angle_once(
        self,
        image_rgb: Optional[np.ndarray] = None,
        b_mask: Optional[np.ndarray] = None,
        label: str = "bmask_longest_edge",
        baseline_angle: Optional[float] = None,
        allow_fail: bool = True,
        follower: Any = None,
        save_overlay: bool = True,
    ) -> Dict[str, Any]:
        """Step1 / Step7 / simple_angle_loop 统一角度入口：只使用当前帧 Bmask 的最长边。"""
        self.log(f"========== Bmask最长边角度检测：{label} ==========")
        seg_retry_info: Dict[str, Any] = {"ok": True, "mode": "provided_image_and_bmask"}
        scene = None
        try:
            if image_rgb is None or b_mask is None:
                if follower is None:
                    follower = self.ensure_rule_ab_follower()
                self._ensure_rule_ab_feature_tracker_installed(follower, self._get_loaded_calibration_state())
                image_rgb, scene, seg_retry_info = self._capture_and_build_scene_with_ab_retry(
                    follower=follower,
                    label=f"Bmask最长边角度/{label}",
                    allow_fail=False,
                )
                try:
                    self._postprocess_scene_ab_masks(follower=follower, scene=scene, tag=f"bmask_longest_edge_{label}")
                except Exception as e:
                    seg_retry_info["ab_guard_warning"] = str(e)
                b_mask = self._extract_mask_from_scene_object(getattr(scene, "b", None))

            if image_rgb is None:
                raise RuntimeError("当前帧 image_rgb 为空")
            if b_mask is None or not np.any(np.asarray(b_mask).astype(bool)):
                raise RuntimeError("当前帧 B mask 为空，无法计算 Bmask 最长边角度")

            edge = self._select_longest_edge_from_bmask(b_mask)
            if not bool(edge.get("ok", False)):
                raise RuntimeError(str(edge.get("reason", "bmask_longest_edge_failed")))

            angle = float(edge["angle_deg"])
            delta = self.angle_diff_deg(angle, float(baseline_angle)) if baseline_angle is not None else None
            paths = {}
            if bool(save_overlay):
                paths = self._save_bmask_longest_edge_overlay(
                    image_rgb=image_rgb,
                    b_mask=b_mask,
                    edge_result=edge,
                    label=label,
                    extra_info={"baseline_angle": baseline_angle, "delta": delta, "ab_seg_retry": self._json_safe(seg_retry_info)},
                )
            polygon_candidates = self._json_safe(edge.get("candidate_edges", []))
            result = {
                "ok": True,
                "angle_ok": True,
                "angle_deg": angle,
                "angle_deg_raw": angle,
                "raw_angle": angle,
                "final_angle_raw_0_180": angle,
                "polygon_candidates": polygon_candidates,
                "same_frame_bmask_available": True,
                "baseline_angle": baseline_angle,
                "delta": delta,
                "label": label,
                "angle_source": edge.get("angle_source", "bmask_longest_edge_polygon"),
                "edge_selection_mode": "bmask_longest_edge_current_frame_only",
                "edge_length_px": float(edge.get("edge_length_px", edge.get("length_px", 0.0))),
                "endpoints": self._json_safe(edge.get("endpoints")),
                "selected_b_edge": self._json_safe(edge.get("selected_b_edge")),
                "candidate_count": int(edge.get("candidate_count", 0)),
                "candidate_edges": polygon_candidates,
                "b_mask_area_px": int(edge.get("b_mask_area_px", np.count_nonzero(np.asarray(b_mask).astype(bool)))),
                "contour_area_px": edge.get("contour_area_px"),
                "approx_vertex_count": edge.get("approx_vertex_count"),
                "allow_close": True,
                "reason": "ok",
                "ab_seg_retry": self._json_safe(seg_retry_info),
                "scene_source": "current_frame_bmask",
            }
            result.update(paths)
            # 仅用于本次修复调用的同帧原始数据；不要写入 context/JSON，避免巨大数组进入保存快照。
            result["__same_frame_image_rgb"] = image_rgb
            result["__same_frame_b_mask"] = b_mask
            result["__same_frame_polygon_candidates"] = polygon_candidates
            result_public = {k: v for k, v in result.items() if not str(k).startswith("__same_frame_")}
            self.context["bmask_longest_edge_angle_state"] = self._json_safe(result_public)
            self.context["last_angle_result"] = self._json_safe(result_public)
            self.log(
                f"[Bmask最长边-{label}] angle={angle:.6f}°, baseline={baseline_angle}, "
                f"delta={delta}, length={result['edge_length_px']:.3f}px, "
                f"source={result['angle_source']}, overlay={result.get('overlay_path')}"
            )
            return result
        except Exception as e:
            msg = f"Bmask最长边角度检测失败：{e}"
            self.log(f"[Bmask最长边-{label}] {msg}")
            if not allow_fail:
                raise
            return {
                "ok": False,
                ...
                "angle_deg_raw": None,
                "label": label,
                "angle_source": "bmask_longest_edge_failed",
                "edge_selection_mode": "bmask_longest_edge_current_frame_only",
                "allow_close": False,
                "reason": msg,
                "error": str(e),
    ...
    def detect_b_klt_selected_edge_angle_once(
        self,
        follower: Any,
        label: str,
        baseline_angle: Optional[float] = None,
        ...
        """
        兼容旧调用名的角度入口。当前版本不再执行 KLT/Profile，
        直接转到当前帧 Bmask 最长边角度检测。
        """
        return self.detect_bmask_longest_edge_angle_once(
            image_rgb=None,
            b_mask=None,
            label=str(label).replace("profile_selected_b_edge", "bmask_longest_edge").replace("profile_edge", "bmask_longest_edge"),
            baseline_angle=baseline_angle,
    ...
    def detect_ab_nearest_b_edge_angle_once(
        ...
        try:
            ...
            saved = self._save_ab_nearest_edge_angle_debug(
                ...
                label=label,
                extra_info={
                    "ab_guard": ab_guard_info,
                    "ab_seg_retry": self._json_safe(seg_retry_info),
                    "scene_source": "feature_tracker",
                    "a_source": "feature_tracker_scene.a",
                },
            )
            if selected is None:
            ...
            result = {
                ...
                "angle_deg_raw": angle,
                "label": label,
                "edge_selection_mode": "nearest_b_edge_to_feature_tracker_scene_a_center",
                "scene_source": "feature_tracker",
                "a_source": "feature_tracker_scene.a",
    ...
    def _get_rule_ab_current_bmask_angle_candidates(self, follower: Any) -> List[Dict[str, Any]]:
        ...
        注意：这里不弹窗、不重新标定。B 的初始化仍来自“1 标定Bmask/边”
        ...
        try:
            if hasattr(follower, "capture_and_build_scene"):
                _image_rgb, scene, _seg_retry_info = self._capture_and_build_scene_with_ab_retry(
                    follower=follower,
                    label="RuleAB角度修复/当前Bmask",
                    allow_fail=False,
                )
                b_mask = self._extract_mask_from_scene_object(getattr(scene, "b", None))
                if b_mask is not None:
    ...
    def _repair_rule_ab_angle_if_jump(
        ...
        """
        Step7 每帧角度修复/确认。修复只使用 raw 检测同一帧返回的 Bmask/候选边，
        不重新截图、不重新 build scene。

        previous_angle 现在优先表示上一帧/上一轮已展开角度；为了兼容旧调用，传入
        0~180° 角度也可以直接作为初始展开参考。
        """
        tol = float(getattr(self.cfg, "rule_ab_angle_repair_raw_tolerance_deg", 2.0))
        if previous_angle is None and baseline_angle is not None:
            previous_angle = float(baseline_angle)

        info: Dict[str, Any] = {
            "enabled": bool(getattr(self.cfg, "rule_ab_enable_angle_repair", True)),
            "checked_every_frame": True,
            "repair_used": False,
            "used_repaired_angle": False,
            "raw_confirmed_correct": False,
            "repair_failed": False,
            "repair_out_of_tolerance": False,
            "raw_angle": raw_angle,
            "raw_angle_0_180": raw_angle,
            "previous_confirmed_angle": previous_angle,
            "last_confirmed_angle_before": previous_angle,
            "last_confirmed_angle_unwrapped": previous_angle,
            "baseline_angle": baseline_angle,
            "baseline_angle_unwrapped": float(baseline_angle) if baseline_angle is not None else None,
            "tolerance_to_last_confirmed_deg": tol,
            "raw_delta_to_last": None,
            "raw_delta_to_last_confirmed": None,
            "raw_jump_from_previous_deg": None,
            "raw_delta_from_baseline": None,
            "raw_delta_unwrapped_to_baseline": None,
            "candidate_count": 0,
            "candidate_angles": [],
            "candidate_diffs_to_last": [],
            "repair_candidates": [],
            "eligible_candidate_angles_within_last_tolerance": [],
            "selected_reference_angle": previous_angle,
            "selected_reference_name": "last_confirmed_angle_unwrapped",
            "selected_edge": None,
            "repaired_angle": None,
            "repaired_angle_unwrapped": None,
            "repaired_delta_to_last": None,
            "repaired_delta_to_last_confirmed": None,
            "repaired_delta_from_baseline": None,
            "repaired_vs_raw_diff_deg": None,
            "final_angle": raw_angle,
            "final_angle_raw_0_180": raw_angle,
            "final_angle_unwrapped": None,
            "final_delta_from_baseline": None,
            "final_delta_to_baseline_raw_0_180": None,
            "final_delta_unwrapped_to_baseline": None,
            "angle_cross_boundary": False,
            "angle_quality": "",
            "angle_quality_reason": "",
            "angle_source": "bmask_longest_edge_raw_unchecked",
            "repair_source": "same_frame_bmask_polygon_candidates",
            "reason": "not_checked",
        }

        if raw_angle is None:
            info.update({"reason": "raw_angle_none", "angle_quality": "invalid_no_same_frame_bmask"})
            try:
                angle_result["angle_repair"] = self._json_safe(info)
                angle_result["repair_failed"] = True
                angle_result["angle_quality"] = "invalid_no_same_frame_bmask"
                angle_result["angle_source"] = "bmask_longest_edge_raw_none_no_repair"
            except Exception:
                pass
            return None, info

        raw_angle_f = self._normalize_angle_0_180(float(raw_angle))
        ref_unwrapped = float(previous_angle) if previous_angle is not None else raw_angle_f
        baseline_unwrapped = self.unwrap_angle_180(baseline_angle, ref_unwrapped) if baseline_angle is not None else None
        raw_unwrapped = self.unwrap_angle_180(raw_angle_f, ref_unwrapped)
        info["selected_reference_angle"] = ref_unwrapped
        info["last_confirmed_angle_before"] = ref_unwrapped
        info["last_confirmed_angle_unwrapped"] = ref_unwrapped
        info["baseline_angle_unwrapped"] = baseline_unwrapped
        info["raw_angle_unwrapped"] = raw_unwrapped
        info["angle_cross_boundary"] = self._angle_cross_boundary_180(raw_angle_f, ref_unwrapped)

        raw_delta_to_last = abs(float(raw_unwrapped) - ref_unwrapped) if raw_unwrapped is not None else None
        info["raw_delta_to_last"] = raw_delta_to_last
        info["raw_delta_to_last_confirmed"] = raw_delta_to_last
        info["raw_jump_from_previous_deg"] = raw_delta_to_last
        if baseline_unwrapped is not None and raw_unwrapped is not None:
            info["raw_delta_from_baseline"] = self.angle_diff_deg(raw_angle_f, float(baseline_angle))
            info["raw_delta_unwrapped_to_baseline"] = abs(float(raw_unwrapped) - float(baseline_unwrapped))

        if not info["enabled"]:
            final_delta_raw = self.angle_diff_deg(raw_angle_f, float(baseline_angle)) if baseline_angle is not None else None
            final_delta_unwrapped = abs(float(raw_unwrapped) - float(baseline_unwrapped)) if baseline_unwrapped is not None and raw_unwrapped is not None else None
            info.update({
                "reason": "disabled_use_raw_angle",
                "final_angle": raw_angle_f,
                "final_angle_raw_0_180": raw_angle_f,
                "final_angle_unwrapped": raw_unwrapped,
                "final_delta_from_baseline": final_delta_raw,
                "final_delta_to_baseline_raw_0_180": final_delta_raw,
                "final_delta_unwrapped_to_baseline": final_delta_unwrapped,
                "angle_source": "bmask_longest_edge_raw_repair_disabled",
            })
            angle_result.update({
                "angle_repair": self._json_safe(info), "angle_deg_raw_before_repair": raw_angle_f,
                "angle_deg": raw_angle_f, "final_angle": raw_angle_f,
                "final_angle_raw_0_180": raw_angle_f, "final_angle_unwrapped": raw_unwrapped,
                "angle_source": info["angle_source"], "used_repaired_angle": False, "repair_used": False,
            })
            return raw_angle_f, info

        # raw 与上一帧展开角连续：直接使用 raw。
        if raw_delta_to_last is not None and float(raw_delta_to_last) <= tol:
            final_delta_raw = self.angle_diff_deg(raw_angle_f, float(baseline_angle)) if baseline_angle is not None else None
            final_delta_unwrapped = abs(float(raw_unwrapped) - float(baseline_unwrapped)) if baseline_unwrapped is not None and raw_unwrapped is not None else None
            q = "ok_cross_boundary" if info["angle_cross_boundary"] else "ok"
            info.update({
                "raw_confirmed_correct": True,
                "repair_used": False,
                "used_repaired_angle": False,
                "repair_failed": False,
                "final_angle": raw_angle_f,
                ...
                "final_delta_unwrapped_to_baseline": final_delta_unwrapped,
                "angle_quality": q,
                "angle_quality_reason": "raw_angle_continuous_after_unwrap",
                "angle_source": "bmask_longest_edge_raw_ok",
                "reason": f"raw_delta_to_last_unwrapped_within_{tol:g}deg_use_raw",
            })
            angle_result.update({
                "angle_repair": self._json_safe(info),
                "angle_deg_raw_before_repair": raw_angle_f,
                "angle_deg": raw_angle_f,
                "final_angle": raw_angle_f,
                "final_angle_raw_0_180": raw_angle_f,
                "final_angle_unwrapped": raw_unwrapped,
                "angle_cross_boundary": bool(info["angle_cross_boundary"]),
                "angle_quality": q,
                "angle_quality_reason": info["angle_quality_reason"],
                "angle_source": "bmask_longest_edge_raw_ok",
                "used_repaired_angle": False,
                "repair_used": False,
                "raw_delta_to_last": raw_delta_to_last,
                "raw_delta_to_last_confirmed": raw_delta_to_last,
                "final_delta_to_baseline": final_delta_raw,
                "final_delta_unwrapped_to_baseline": final_delta_unwrapped,
                "last_confirmed_angle_before": ref_unwrapped,
                "last_confirmed_angle_unwrapped": ref_unwrapped,
            })
            self.log(
                f"[RuleAB-角度修复] raw_angle={raw_angle_f:.6f}° 展开为 {float(raw_unwrapped):.6f}°，"
                f"与上一帧展开角 {ref_unwrapped:.6f}° 差值 {float(raw_delta_to_last):.3f}° <= {tol:.3f}°；使用 raw_angle。"
            )
            return raw_angle_f, info

        # raw 跳变过大：只允许使用 raw 检测同一帧带回来的候选边修复。
        candidates = angle_result.get("__same_frame_polygon_candidates") or angle_result.get("polygon_candidates") or angle_result.get("candidate_edges") or []
        if not candidates:
            # 有 raw，但没有同帧 Bmask/候选边，不能可靠修复；本帧不触发关闭激光。
            info.update({
                "repair_failed": True,
                "repair_out_of_tolerance": True,
                "repair_used": False,
                "used_repaired_angle": False,
                "raw_confirmed_correct": False,
                "final_angle": None,
                "final_angle_raw_0_180": None,
                "final_angle_unwrapped": None,
                "angle_quality": "invalid_no_same_frame_bmask",
                "angle_quality_reason": "no_same_frame_bmask_or_polygon_candidates_for_repair",
                "angle_source": "bmask_polygon_edge_repair_failed_no_same_frame_bmask",
                "reason": "invalid_no_same_frame_bmask",
            })
            angle_result.update({
                "angle_repair": self._json_safe(info),
                "angle_deg_raw_before_repair": raw_angle_f,
                ...
                "angle_quality": "invalid_no_same_frame_bmask",
                "angle_quality_reason": info["angle_quality_reason"],
                "angle_source": info["angle_source"],
                "repair_failed": True,
                "repair_used": False,
                "raw_delta_to_last": raw_delta_to_last,
                "raw_delta_to_last_confirmed": raw_delta_to_last,
            ...
            self.log(
                f"[RuleAB-角度修复] raw_angle={raw_angle_f:.6f}° 跳变 {raw_delta_to_last}°，"
                "但没有同帧 Bmask/候选边，标记 invalid_no_same_frame_bmask；本帧不关激光、不停止完整测量。"
            )
            return None, info

        info["candidate_count"] = len(candidates)
        info["candidate_angles"] = [float(c.get("angle_deg")) for c in candidates if isinstance(c, dict) and c.get("angle_deg") is not None]

        enriched: List[Tuple[float, float, float, float, Dict[str, Any]]] = []
        for c in candidates:
            try:
                a_raw = self._normalize_angle_0_180(float(c["angle_deg"]))
                a_unwrapped = self.unwrap_angle_180(a_raw, ref_unwrapped)
                d_last = abs(float(a_unwrapped) - ref_unwrapped)
                d_raw = abs(float(a_unwrapped) - float(raw_unwrapped)) if raw_unwrapped is not None else self.angle_diff_deg(a_raw, raw_angle_f)
                cc = dict(c)
                cc["angle_unwrapped"] = float(a_unwrapped)
                cc["diff_to_last_confirmed_deg"] = float(d_last)
                cc["diff_to_raw_deg"] = float(d_raw)
                enriched.append((float(d_last), float(d_raw), a_raw, float(a_unwrapped), cc))
            except Exception:
                continue
        enriched.sort(key=lambda x: x[0])
        info["candidate_diffs_to_last"] = [float(x[0]) for x in enriched]
        info["candidate_diffs_to_reference"] = info["candidate_diffs_to_last"]
        info["candidate_diffs_to_raw"] = [float(x[1]) for x in enriched]
        info["repair_candidates"] = [self._json_safe(x[4]) for x in enriched]

        eligible = [x for x in enriched if float(x[0]) <= tol]
        info["eligible_candidate_angles_within_last_tolerance"] = [float(x[2]) for x in eligible]
        ...
        if not eligible:
            info.update({
                ...
                "final_angle_raw_0_180": None,
                "final_angle_unwrapped": None,
                "angle_quality": "suspected_jump",
                "angle_quality_reason": "same_frame_polygon_candidates_no_edge_close_to_last_confirmed",
                "angle_source": "bmask_polygon_edge_repair_failed_no_candidate_close_to_last",
                "reason": f"raw_delta_to_last_unwrapped_gt_{tol:g}deg_and_no_same_frame_polygon_edge_within_{tol:g}deg",
            })
            angle_result.update({
                "angle_repair": self._json_safe(info),
                "angle_deg_raw_before_repair": raw_angle_f,
            ...
            self.log(
                f"[RuleAB-角度修复] raw_angle={raw_angle_f:.6f}° 展开后跳变 {raw_delta_to_last}°；"
                f"同帧候选边中没有 ±{tol:.3f}° 内连续边，标记 suspected_jump，本帧不关激光。"
            )
            return None, info

        best_last_diff, best_raw_diff, repaired_raw, repaired_unwrapped, selected = eligible[0]
        final_delta_raw = self.angle_diff_deg(repaired_raw, float(baseline_angle)) if baseline_angle is not None else None
        final_delta_unwrapped = abs(float(repaired_unwrapped) - float(baseline_unwrapped)) if baseline_unwrapped is not None else None
        cross = self._angle_cross_boundary_180(repaired_raw, ref_unwrapped)
        q = "ok_cross_boundary" if cross else "ok"
        info.update({
            "repair_used": True,
            ...
            "repair_failed": False,
            "repair_out_of_tolerance": False,
            "selected_edge": self._json_safe(selected),
            "nearest_candidate_angle": repaired_raw,
            "nearest_candidate_diff_to_reference_deg": float(best_last_diff),
            "repaired_angle": repaired_raw,
            "repaired_angle_unwrapped": repaired_unwrapped,
            "repaired_delta_to_last": float(best_last_diff),
            "repaired_delta_to_last_confirmed": float(best_last_diff),
            "repaired_jump_from_previous_deg": float(best_last_diff),
            "repaired_delta_from_baseline": final_delta_raw,
            "repaired_vs_raw_diff_deg": float(best_raw_diff),
            "final_angle": repaired_raw,
            "final_angle_raw_0_180": repaired_raw,
            "final_angle_unwrapped": repaired_unwrapped,
            "final_delta_from_baseline": final_delta_raw,
            "final_delta_to_baseline_raw_0_180": final_delta_raw,
            "final_delta_unwrapped_to_baseline": final_delta_unwrapped,
            "angle_cross_boundary": bool(cross),
            "angle_quality": q,
            "angle_quality_reason": "repaired_by_same_frame_polygon_edge_continuous_after_unwrap",
            "angle_source": "bmask_polygon_edge_repaired_by_same_frame_last_angle",
            "repair_source": "same_frame_bmask_polygon_candidates",
            "reason": f"raw_delta_to_last_unwrapped_gt_{tol:g}deg_repaired_by_same_frame_polygon_edge_within_{tol:g}deg",
        })

        try:
            repair_overlay_path = self._save_rule_ab_repair_edge_overlay(angle_result, info)
            if repair_overlay_path:
                info["repair_edge_overlay_path"] = repair_overlay_path
                angle_result["repair_edge_overlay_path"] = repair_overlay_path
                self.log(f"[RuleAB-角度修复] 已保存 raw/polygon repaired 边叠加图：{repair_overlay_path}")
        except Exception as e:
            self.log(f"[RuleAB-角度修复] 保存 raw/repaired 边叠加图失败：{e}")

        angle_result.update({
            "angle_repair": self._json_safe(info),
            "angle_deg_raw_before_repair": raw_angle_f,
            "angle_deg_repaired": repaired_raw,
            "angle_deg": repaired_raw,
            "final_angle": repaired_raw,
            "final_angle_raw_0_180": repaired_raw,
            "final_angle_unwrapped": repaired_unwrapped,
            "angle_cross_boundary": bool(cross),
            ...
            "angle_quality_reason": info["angle_quality_reason"],
            "angle_source": info["angle_source"],
            "repair_source": info["repair_source"],
            "used_repaired_angle": True,
            "repair_used": True,
            "repair_failed": False,
            "raw_delta_to_last": raw_delta_to_last,
            ...
            "last_confirmed_angle_before": ref_unwrapped,
            "last_confirmed_angle_unwrapped": ref_unwrapped,
            "repair_candidates": self._json_safe(info.get("repair_candidates", [])),
            "repair_reason": info["reason"],
        })

        self.log(
            f"[RuleAB-角度修复] raw_angle={raw_angle_f:.6f}° 展开后与上一帧差 {raw_delta_to_last} > {tol:.3f}°；"
            f"使用同帧候选 repaired_angle={repaired_raw:.6f}° / unwrapped={repaired_unwrapped:.6f}°，"
            f"repaired_delta_to_last={float(best_last_diff):.3f}°，final_delta_unwrapped={final_delta_unwrapped}。"
        )
        return repaired_raw, info

    def _repair_step1_baseline_angle(
        self,
        raw_angle: Optional[float],
        follower: Any,
        angle_result: Dict[str, Any],
        ...
        目的：
            Step1 的角度会作为 Step7 的 baseline_angle。
            如果 Step1 baseline 本身选错边，Step7 后续所有 delta 判断都会错。
            因此 Step1 也基于当前 B mask 的所有边候选做一次修复/确认。

        规则：
            1) 从当前 B mask 计算所有候选边角度；
            2) 如果上一轮完整测量角度 previous_cycle_angle 存在，则优先选择
               与 previous_cycle_angle 最接近的候选作为 repaired_angle；
               如果不存在上一轮角度，则选择与 raw_angle 最接近的候选；
            3) repaired_angle 与 raw_angle 的差值 <= step1_angle_repair_raw_tolerance_deg
               默认 1.5°，视作 Step1 检测角度正确；
            4) 若差值 >1.5° 且存在 previous_cycle_angle，则认为 Step1 raw_angle 可疑，
               使用 repaired_angle 作为本轮 baseline；
               若没有 previous_cycle_angle，则没有可靠参考，不强行替换 raw_angle。
        """
        tol = float(getattr(self.cfg, "step1_angle_repair_raw_tolerance_deg", 1.5))
        info: Dict[str, Any] = {
            "enabled": bool(getattr(self.cfg, "rule_ab_enable_angle_repair", True)),
            "stage": "step1_baseline",
            "raw_angle": raw_angle,
            "previous_cycle_angle": self.previous_cycle_angle,
            "tolerance_repaired_vs_raw_deg": tol,
            "candidate_count": 0,
            "candidate_angles": [],
            "selected_repaired_angle": None,
            "selected_reference_angle": None,
        ...
        info["candidate_count"] = len(candidates)
        info["candidate_angles"] = [float(c.get("angle_deg")) for c in candidates if c.get("angle_deg") is not None]
        if not candidates:
            info["reason"] = "no_bmask_edge_candidates_use_raw_angle"
            self.log("[Step1-角度修复] 当前 B mask 没有候选边，Step1 baseline 使用 raw_angle。")
            return raw_angle, info

        # Step1 没有“推动前上一帧”，因此优先用上一轮完整测量角度作为参考；
        # 第一轮没有上一轮角度时，用 raw_angle 自身作为参考，只做一致性确认。
        if self.previous_cycle_angle is not None:
            ref_angle = float(self.previous_cycle_angle)
            ref_name = "previous_cycle_angle"
        else:
            ref_angle = float(raw_angle)
            ref_name = "raw_angle_no_previous_cycle"

        enriched = []
        for c in candidates:
            try:
                a = float(c["angle_deg"])
                d_ref = self.angle_diff_deg(a, ref_angle)
                d_raw = self.angle_diff_deg(a, float(raw_angle))
                enriched.append((float(d_ref), float(d_raw), a, c))
            except Exception:
                continue

        if not enriched:
            info["reason"] = "candidate_angles_invalid_use_raw_angle"
            return raw_angle, info

        enriched.sort(key=lambda x: x[0])
        _d_ref, d_raw, repaired_angle, selected = enriched[0]
        repaired_angle = float(repaired_angle)
        d_raw = float(d_raw)

        raw_confirmed = bool(d_raw <= tol)
        use_repaired = bool((not raw_confirmed) and self.previous_cycle_angle is not None)
        final_angle = repaired_angle if use_repaired else float(raw_angle)

        info.update({
            "selected_repaired_angle": repaired_angle,
            "selected_reference_angle": ref_angle,
            "selected_reference_name": ref_name,
            "repaired_vs_raw_diff_deg": d_raw,
            "raw_confirmed_correct": raw_confirmed,
            "used_repaired_angle": use_repaired,
            "selected_edge": self._json_safe(selected),
            "reason": (
                "raw_confirmed_by_bmask_candidate_within_1p5deg"
                if raw_confirmed else
                ("raw_suspect_replaced_by_repaired_angle_from_previous_cycle_reference"
                 if use_repaired else
                 "raw_not_confirmed_but_no_previous_cycle_reference_use_raw_angle")
            ),
        })

        try:
            angle_result["step1_angle_repair"] = self._json_safe(info)
            angle_result["angle_deg_raw_before_step1_repair"] = float(raw_angle)
            angle_result["angle_deg_step1_repaired"] = repaired_angle
            angle_result["step1_raw_confirmed_correct"] = bool(raw_confirmed)
            angle_result["step1_used_repaired_angle"] = bool(use_repaired)
        ...
        if raw_confirmed:
            self.log(
                f"[Step1-角度修复] raw_angle={float(raw_angle):.6f}°，"
                f"repaired_angle={repaired_angle:.6f}°，两者差={d_raw:.3f}° <= {tol:.3f}°，"
                "判定 Step1 检测角度正确。"
            )
        elif use_repaired:
            self.log(
                f"[Step1-角度修复] raw_angle={float(raw_angle):.6f}° 与 repaired_angle={repaired_angle:.6f}° "
                f"相差 {d_raw:.3f}° > {tol:.3f}°；以上一轮角度={ref_angle:.6f}° 为参考，"
                "使用 repaired_angle 作为本轮 Step7 baseline。"
            )
        else:
            self.log(
                f"[Step1-角度修复] raw_angle={float(raw_angle):.6f}° 与候选 repaired_angle={repaired_angle:.6f}° "
                f"相差 {d_raw:.3f}° > {tol:.3f}°，但没有上一轮角度参考，仍使用 raw_angle 作为 baseline。"
            )
    ...
    def _write_step7_realtime_records_csv(self, records: List[Dict[str, Any]], csv_path: Path) -> None:
        ...
        fieldnames = [
            ...
            "final_angle_raw_0_180", "final_angle_unwrapped", "last_confirmed_angle_unwrapped",
            "angle_cross_boundary", "angle_quality", "angle_quality_reason", "angle_quality_jump_deg",
            "last_confirmed_angle_before", "raw_delta_to_last", "raw_delta_to_last_confirmed",
            "repaired_angle", "repaired_delta_to_last_confirmed",
            "delta_from_baseline", "final_delta_to_baseline", "final_delta_to_baseline_raw_0_180",
            "final_delta_unwrapped_to_baseline",
            "angle_source", "repair_used", "repair_candidates", "repair_reason",
            "edge_length_px", "endpoints", "trigger_type", "controller_action",
            "raw_image_path", "overlay_image_path", "repair_overlay_image_path", "reason",
        ]
        with csv_path.open("w", encoding="utf-8-sig", newline="") as f:
            ...
            for r in records:
                rr = dict(r)
                for key in ("endpoints", "repair_candidates"):
    ...
    def run_rule_ab_realtime_until_angle_delta(
        ...
        # Step7 角度修复只参考“上一帧最终角度”。第一帧用 Step1 baseline 初始化。
                ...
                while not stop_event.is_set() and not self.stop_requested:
                    ...
                    try:
                        with self.rule_ab_vision_lock:
                            angle_result = self.detect_b_klt_selected_edge_angle_once(
                                follower=follower,
                        ...
                        angle_ok_raw = bool(angle_result.get("ok", False) and angle_result.get("angle_deg") is not None)
                        raw_angle = float(angle_result["angle_deg"]) if angle_ok_raw else None
                        last_before = last_confirmed_angle_box.get("value")
                        final_angle_for_judge: Optional[float] = None
                        repair_info: Dict[str, Any] = {}
                        if angle_ok_raw:
                            final_angle_for_judge, repair_info = self._repair_rule_ab_angle_if_jump(
                                raw_angle=raw_angle,
                                previous_angle=float(last_before) if last_before is not None else None,
                                baseline_angle=baseline_angle,
                                follower=follower,
                                angle_result=angle_result,
                            )
                        angle_ok = bool(final_angle_for_judge is not None)
                        angle_deg = float(final_angle_for_judge) if angle_ok else None
                        final_unwrapped = repair_info.get("final_angle_unwrapped") if repair_info else None
                        baseline_unwrapped = repair_info.get("baseline_angle_unwrapped") if repair_info else baseline_angle
                        delta = abs(float(final_unwrapped) - float(baseline_unwrapped)) if final_unwrapped is not None and baseline_unwrapped is not None else (self.angle_diff_deg(float(angle_deg), float(baseline_angle)) if angle_deg is not None and baseline_angle is not None else None)
                        raw_delta_to_last = repair_info.get("raw_delta_to_last")
                        rec = {
                            ...
                            "angle_deg": angle_deg,
                            "baseline_angle": baseline_angle,
                            "final_angle_raw_0_180": repair_info.get("final_angle_raw_0_180", angle_deg),
                            "final_angle_unwrapped": repair_info.get("final_angle_unwrapped"),
                            "last_confirmed_angle_unwrapped": last_before,
                            "angle_cross_boundary": repair_info.get("angle_cross_boundary", False),
                            "angle_quality": repair_info.get("angle_quality", ""),
                            "angle_quality_reason": repair_info.get("angle_quality_reason", ""),
                            "angle_quality_jump_deg": None,
                            "last_confirmed_angle_before": last_before,
                            "raw_delta_to_last": raw_delta_to_last,
                            "raw_delta_to_last_confirmed": repair_info.get("raw_delta_to_last_confirmed", raw_delta_to_last),
                            "repaired_angle": repair_info.get("repaired_angle"),
                            "repaired_delta_to_last_confirmed": repair_info.get("repaired_delta_to_last_confirmed"),
                            "delta_from_baseline": delta,
                            "final_delta_to_baseline": delta,
                            "final_delta_to_baseline_raw_0_180": repair_info.get("final_delta_to_baseline_raw_0_180"),
                            "final_delta_unwrapped_to_baseline": repair_info.get("final_delta_unwrapped_to_baseline", delta),
                            "angle_source": repair_info.get("angle_source") or angle_result.get("angle_source"),
                            "repair_used": bool(repair_info.get("repair_used", repair_info.get("used_repaired_angle", False))),
                            "repair_candidates": repair_info.get("repair_candidates", []),
                            "repair_reason": repair_info.get("reason", angle_result.get("reason")),
                            ...
                            "repair_overlay_image_path": angle_result.get("repair_edge_overlay_path"),
                            "reason": repair_info.get("reason", angle_result.get("reason")),
                        }
                        if not angle_ok:
                            rec["trigger_type"] = "angle_invalid_after_repair"
                            _append_record(rec)
                            self.log("[Step7实时角度] 本帧角度修复失败，raw_angle 不作为最终角度；不关闭激光、不停止完整测量，继续下一帧。")
                            self._interruptible_pause(float(getattr(self.cfg, "rule_ab_angle_watch_interval_s", 0.05)), stop_event, phase_state=None)
                            continue

                        # 成功得到最终角度后，才更新上一帧最终角度。
                        if repair_info.get("final_angle_unwrapped") is not None:
                            last_confirmed_angle_box["value"] = float(repair_info.get("final_angle_unwrapped"))
                        else:
                        ...
                        if angle_ok and delta is not None:
                            if float(delta) >= float(min_delta) and float(delta) < float(max_delta):
                                rec["trigger_type"] = "target_range"
                                result_box.update({"trigger_type": "target_range", "stop_reason": "realtime_bmask_longest_edge_delta_in_target_range_continue_measurement", "final_angle": float(angle_deg), "final_delta": float(delta)})
                                _append_record(rec)
                                self.log(f"[Step7实时角度] 达到目标范围：final_angle={angle_deg:.6f}°, baseline={baseline_angle}, delta={delta:.3f}°, target=[{min_delta:.3f},{max_delta:.3f})；停止/暂停Stage34当前推动，不在Step7角度线程关闭激光；后续由完整测量主流程正常关激光，并继续完整测量。")
                                stop_event.set()
                                self._stop_rule_ab_stage34_if_possible(follower, reason="angle_delta_in_target_range_no_laser_off")
                                break
                            if float(delta) >= float(max_delta):
                                rec["trigger_type"] = "overshoot"
                                result_box.update({"trigger_type": "overshoot", "stop_reason": "realtime_bmask_longest_edge_delta_ge_target_max_overshoot_continue_measurement", "final_angle": float(angle_deg), "final_delta": float(delta)})
                                _append_record(rec)
                                self.log(f"[Step7实时角度] 角度变化超过目标上限，仅记录为 overshoot；Step7角度线程不直接关闭激光，后续由完整测量主流程正常关激光，并继续完整测量：final_angle={angle_deg:.6f}°, baseline={baseline_angle}, delta={delta:.3f}° >= {max_delta:.3f}°。")
                                stop_event.set()
                                self._stop_rule_ab_stage34_if_possible(follower, reason="angle_delta_overshoot_no_laser_off")
                                break
                        _append_record(rec)
                    except Exception as e:
        ...
        ok = bool(trigger_type == "target_range")
        overshoot = bool(trigger_type == "overshoot")
        allow_continue = bool(trigger_type in ("target_range", "overshoot"))
        result = {"ok": bool(allow_continue), "step7_success_in_target_range": bool(ok), "allow_continue_measurement": bool(allow_continue), "overshoot": bool(overshoot), "reason": str(result_box.get("stop_reason") or "stopped"), "baseline_angle": baseline_angle, "final_angle": result_box.get("final_angle"), "final_delta": result_box.get("final_delta"), "target_min": min_delta, "target_max": max_delta, "angle_source": "bmask_longest_edge_realtime_thread", "records": self._json_safe(records), "realtime_csv_path": str(csv_path), "realtime_dir": str(realtime_dir), "controller_error": result_box.get("controller_error"), "angle_error": result_box.get("angle_error")}
    ...
    def run_rule_ab_until_angle_delta(
        ...
        # 串行兼容模式同样使用“上一帧最终角度”做 Step7 角度修复参考。
        ...
        while True:
            ...
            repair_info: Dict[str, Any] = {}
            if angle_ok_raw:
                current_angle, repair_info = self._repair_rule_ab_angle_if_jump(
                    raw_angle=raw_angle,
                    previous_angle=last_confirmed_angle,
                    baseline_angle=baseline_angle,
                    follower=follower,
                    angle_result=angle_result,
                )
                ...
                current_angle = None
            angle_ok = bool(current_angle is not None)
            angle_source = str((repair_info or {}).get("angle_source") or angle_result.get("angle_source", "bmask_longest_edge_failed"))
            allow_close_by_angle = bool(angle_result.get("allow_close", False))
            final_angle = current_angle
            final_delta = None
            final_angle_unwrapped = (repair_info or {}).get("final_angle_unwrapped")
            baseline_unwrapped = (repair_info or {}).get("baseline_angle_unwrapped", baseline_angle)
            if current_angle is not None and baseline_angle is not None:
                if final_angle_unwrapped is not None and baseline_unwrapped is not None:
                    final_delta = abs(float(final_angle_unwrapped) - float(baseline_unwrapped))
                else:
                    final_delta = self.angle_diff_deg(float(current_angle), float(baseline_angle))
            if angle_ok:
                last_confirmed_angle = float(final_angle_unwrapped) if final_angle_unwrapped is not None else float(current_angle)
            ...
            record = {
                ...
                "baseline_angle": baseline_angle,
                "final_angle_raw_0_180": (repair_info or {}).get("final_angle_raw_0_180", current_angle),
                "final_angle_unwrapped": (repair_info or {}).get("final_angle_unwrapped"),
                "last_confirmed_angle_unwrapped": (repair_info or {}).get("last_confirmed_angle_unwrapped"),
                "angle_cross_boundary": (repair_info or {}).get("angle_cross_boundary", False),
                "angle_quality": (repair_info or {}).get("angle_quality", ""),
                "angle_quality_reason": (repair_info or {}).get("angle_quality_reason", ""),
                "angle_quality_jump_deg": None,
                "last_confirmed_angle_before": (repair_info or {}).get("last_confirmed_angle_before"),
                "raw_delta_to_last": (repair_info or {}).get("raw_delta_to_last"),
                "raw_delta_to_last_confirmed": (repair_info or {}).get("raw_delta_to_last_confirmed", (repair_info or {}).get("raw_delta_to_last")),
                "repaired_angle": (repair_info or {}).get("repaired_angle"),
                "repaired_delta_to_last_confirmed": (repair_info or {}).get("repaired_delta_to_last_confirmed"),
                "final_delta_to_baseline_raw_0_180": (repair_info or {}).get("final_delta_to_baseline_raw_0_180"),
                "final_delta_unwrapped_to_baseline": (repair_info or {}).get("final_delta_unwrapped_to_baseline", final_delta),
                "angle_source": angle_source,
                "repair_used": bool((repair_info or {}).get("repair_used", (repair_info or {}).get("used_repaired_angle", False))),
                "repair_candidates": (repair_info or {}).get("repair_candidates", []),
                "repair_reason": (repair_info or {}).get("reason"),
                "edge_length_px": angle_result.get("edge_length_px"),
                "endpoints": angle_result.get("endpoints"),
            ...
            # 关闭激光只允许 KLT 成功角度触发；单帧 relocated/repaired 角度默认只用于恢复 KLT。
            close_hit = bool(
                allow_close_by_angle
                and final_delta is not None
                and float(final_delta) >= float(min_delta)
                and float(final_delta) < float(max_delta)
            )
            if close_hit:
                reached = True
                allow_continue_measurement = True
                stop_reason = "bmask_longest_edge_delta_in_target_range_continue_measurement"
                self.log(
                    f"[RuleAB-Bmask最长边] 角度变化进入目标范围：delta={float(final_delta):.3f}°，"
                    f"目标范围=[{float(min_delta):.3f}, {float(max_delta):.3f})；停止本次 Stage34 推动；不在Step7分支直接关闭激光，后续由完整测量主流程正常关激光并继续完整测量。"
                )
                break
            ...
            elif final_delta is not None and float(final_delta) >= float(max_delta):
                allow_continue_measurement = True
                stop_reason = "bmask_longest_edge_delta_ge_target_max_overshoot_continue_measurement"
                self.log(
                    f"[RuleAB-Bmask最长边] 当前角度变化 {float(final_delta):.3f}° 已超过目标上限 "
                    f"{float(max_delta):.3f}°；仅记录为 overshoot，停止本次 Stage34 推动；不在Step7分支直接关闭激光，后续由完整测量主流程正常关激光并继续完整测量。"
                )
                break
            else:
    ...
    def _hsv_color_distance_to_profile(self, image_rgb: np.ndarray, profile: Dict[str, Any]) -> np.ndarray:
        """计算每个像素到某个模板 HSV 均值的归一化颜色距离，越小越像。"""
        img = np.asarray(image_rgb).astype(np.uint8)
    ...
    def _select_component_by_color_shape_center(self, raw_mask: np.ndarray, image_rgb: np.ndarray, profile: Dict[str, Any]) -> np.ndarray:
        ...
        min_area = max(int(getattr(self.cfg, "rule_ab_feature_min_area_px", 20)), int(ref_area * float(getattr(self.cfg, "rule_ab_feature_area_ratio_min", 0.25))))
        max_area = int(ref_area * float(getattr(self.cfg, "rule_ab_feature_area_ratio_max", 3.5)))
    ...
    def _install_feature_tracker_on_rule_ab_follower(self, follower: Any, state: Optional[CalibrationState] = None):
        ...
        self.log("[ABC特征跟踪] 已安装：SAM2 首帧分割 + 颜色/形状模板后续识别 + A/B 重叠颜色分离。")
    ...
    def _feature_initial_positive_points_from_state(
        ...
        if not pts["b"]:
            pts["b"] = self._normalize_points_xy(getattr(state, "b_mask_positive_points", []))
    ...
    def run_rule_ac_until_threshold(self) -> Dict[str, Any]:
        ...
        while True:
            ...
            tol = float(getattr(self.cfg, "rule_ac_center_tolerance_px", 10.0))
    ...
    def request_labview_spectrum(self, cycle_index: int) -> Dict[str, Any]:
        """通过 LabVIEW TCP 请求一次真实光谱采集，并恢复真实峰值计算/保存。"""
        self.log("========== Step 4：LabVIEW 真实光谱采集 ==========")
        if self.tcp_server is None:
            self.start_tcp_server()
        ...
        command = str(getattr(self.cfg, "tcp_command", "MEASURE") or "MEASURE")
        self.log(f"[LabVIEW] 发送采集命令：{command}，cycle={cycle_index}")

        # 兼容不同版本 LabVIEWTCPServer 的方法名。
        if hasattr(self.tcp_server, "measure"):
            result = self.tcp_server.measure(command=command, index=cycle_index)
        elif hasattr(self.tcp_server, "request_measurement"):
            result = self.tcp_server.request_measurement(command=command, index=cycle_index)
        elif hasattr(self.tcp_server, "send_measure_command"):
            result = self.tcp_server.send_measure_command(command, cycle_index)
        elif hasattr(self.tcp_server, "send_command"):
            result = self.tcp_server.send_command(command)
        else:
            raise RuntimeError("LabVIEWTCPServer 缺少 measure/request_measurement/send_measure_command/send_command 方法")

        if result is None:
            result = {"ok": False, "message": "labview_returned_none"}
        if not isinstance(result, dict):
            result = {"ok": True, "raw_result": result, "message": "labview_returned_non_dict"}
        ...
        result.update({
            ...
            "disabled": False,
            "index": cycle_index,
            "num_points": len(parsed.get("raw_values") or []),
            "raw_original_peak": parsed.get("raw_original_peak"),
            "raw_filtered_peak": parsed.get("raw_filtered_peak"),
            "raw_median_peak": parsed.get("raw_median_peak"),
            "raw_peak": parsed.get("raw_peak"),
            "fit_peak": parsed.get("fit_peak"),
        })

        self.context["last_tcp_result"] = self._json_safe(result)
        self.context["wavelength"] = parsed.get("wavelength") or []
        self.context["intensity"] = parsed.get("raw_values") or []
        self.context["raw_values"] = parsed.get("raw_values") or []
        self.context["raw_filtered_values"] = parsed.get("threshold_filtered_intensity") or []
        self.context["raw_median_values"] = parsed.get("median_filtered_intensity") or []
        self.context["fit_values"] = parsed.get("fit_values") or []
        self.context["fit_params"] = self._json_safe(parsed.get("fit_params"))
        ...
        self.context["x_axis_values"] = self.load_x_axis_values_from_xlsx()

        self.log(
            f"[光谱] 真实采集完成：num_points={result.get('num_points')}, "
            f"raw_original_peak={parsed.get('raw_original_peak')}, "
            f"raw_filtered_peak={parsed.get('raw_filtered_peak')}, "
            f"raw_median_peak={parsed.get('raw_median_peak')}, fit_peak={parsed.get('fit_peak')}, "
            f"labview_csv={result.get('csv_path')}"
        )
        self.notify_update()
        return result
    ...
    def begin_new_run_session(self):
        ...
        self.context["last_saved_final_angle_unwrapped"] = None
        self.context["last_confirmed_angle_unwrapped"] = None
    ...
    def _prepare_and_store_save_angle_quality(
        self,
        final_angle_raw_0_180: Optional[float],
        baseline_angle_raw_0_180: Optional[float],
        angle_result: Optional[Dict[str, Any]] = None,
    ) -> Dict[str, Any]:
        """保存前角度质量标记：只写标记，不阻止保存、不终止测量。"""
        last_saved = self.context.get("last_saved_final_angle_unwrapped")
        reference = float(last_saved) if last_saved is not None else None
        final_raw = self._normalize_angle_0_180(float(final_angle_raw_0_180)) if final_angle_raw_0_180 is not None else None
        final_unwrapped = self.unwrap_angle_180(final_raw, reference) if final_raw is not None else None
        baseline_unwrapped = self.unwrap_angle_180(baseline_angle_raw_0_180, final_unwrapped) if baseline_angle_raw_0_180 is not None and final_unwrapped is not None else None
        angle_delta_unwrapped_to_last = abs(float(final_unwrapped) - float(last_saved)) if final_unwrapped is not None and last_saved is not None else None
        final_delta_unwrapped_to_baseline = abs(float(final_unwrapped) - float(baseline_unwrapped)) if final_unwrapped is not None and baseline_unwrapped is not None else None
        final_delta_raw = self.angle_diff_deg(final_raw, float(baseline_angle_raw_0_180)) if final_raw is not None and baseline_angle_raw_0_180 is not None else None
        cross = self._angle_cross_boundary_180(final_raw, float(last_saved)) if last_saved is not None else False
        warn = float(getattr(self.cfg, "rule_ab_save_angle_jump_warn_deg", 8.0))

        quality = "ok_cross_boundary" if cross else "ok"
        reason = "angle_continuous_after_unwrap"
        jump = angle_delta_unwrapped_to_last
        if jump is not None and float(jump) > warn:
            quality = "suspected_jump"
            reason = "final_angle_jump_to_last_saved_too_large"
        if isinstance(angle_result, dict) and angle_result.get("angle_quality") == "invalid_no_same_frame_bmask":
            quality = "invalid_no_same_frame_bmask"
            reason = str(angle_result.get("angle_quality_reason") or "no_same_frame_bmask_or_polygon_candidates_for_repair")

        repair_info = angle_result.get("angle_repair") if isinstance(angle_result, dict) else {}
        if not isinstance(repair_info, dict):
            repair_info = {}
        out = {
            "final_angle_raw_0_180": final_raw,
            "final_angle_unwrapped": final_unwrapped,
            # 记录字段保存“本轮保存前”的上一轮最终展开角；内部 context 稍后会更新为本轮值。
            "last_saved_final_angle_unwrapped": last_saved,
            "last_confirmed_angle_unwrapped": final_unwrapped,
            "angle_cross_boundary": bool(cross),
            "angle_delta_unwrapped_to_last": angle_delta_unwrapped_to_last,
            "final_delta_unwrapped_to_baseline": final_delta_unwrapped_to_baseline,
            "final_delta_to_baseline_raw_0_180": final_delta_raw,
            "angle_quality": quality,
            "angle_quality_reason": reason,
            "angle_quality_jump_deg": jump,
            "raw_angle": (angle_result or {}).get("angle_deg_raw_before_repair", (angle_result or {}).get("angle_deg_raw", final_raw)) if isinstance(angle_result, dict) else final_raw,
            "repaired_angle": (angle_result or {}).get("angle_deg_repaired", repair_info.get("repaired_angle")) if isinstance(angle_result, dict) else None,
            "repair_used": bool((angle_result or {}).get("repair_used", repair_info.get("repair_used", False))) if isinstance(angle_result, dict) else False,
            "repair_source": (angle_result or {}).get("repair_source", repair_info.get("repair_source", "")) if isinstance(angle_result, dict) else "",
            "repair_reason": (angle_result or {}).get("repair_reason", repair_info.get("reason", "")) if isinstance(angle_result, dict) else "",
            "raw_delta_to_last_confirmed": (angle_result or {}).get("raw_delta_to_last_confirmed", repair_info.get("raw_delta_to_last_confirmed", repair_info.get("raw_delta_to_last"))) if isinstance(angle_result, dict) else None,
            "repaired_delta_to_last_confirmed": (angle_result or {}).get("repaired_delta_to_last_confirmed", repair_info.get("repaired_delta_to_last_confirmed", repair_info.get("repaired_delta_to_last"))) if isinstance(angle_result, dict) else None,
        }
        for k, v in out.items():
            self.context[k] = v
        self.context["last_saved_final_angle_unwrapped_record"] = last_saved
        # 只有保存前质量评估完成后，更新内部 last_saved，供下一轮判断 suspected_jump。
        if final_unwrapped is not None:
            self.context["last_saved_final_angle_unwrapped"] = float(final_unwrapped)
        if isinstance(angle_result, dict):
            angle_result.update(self._json_safe(out))
        return out
    ...
    def _make_save_context_snapshot(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "final_angle_raw_0_180": ctx.get("final_angle_raw_0_180"),
            "final_angle_unwrapped": ctx.get("final_angle_unwrapped"),
            "last_saved_final_angle_unwrapped": ctx.get("last_saved_final_angle_unwrapped_record", ctx.get("last_saved_final_angle_unwrapped")),
            "last_confirmed_angle_unwrapped": ctx.get("last_confirmed_angle_unwrapped"),
            "angle_cross_boundary": ctx.get("angle_cross_boundary"),
            "angle_delta_unwrapped_to_last": ctx.get("angle_delta_unwrapped_to_last"),
            "angle_quality": ctx.get("angle_quality"),
            "angle_quality_reason": ctx.get("angle_quality_reason"),
            "angle_quality_jump_deg": ctx.get("angle_quality_jump_deg"),
            "raw_angle": ctx.get("raw_angle"),
            "repaired_angle": ctx.get("repaired_angle"),
            "repair_used": ctx.get("repair_used"),
            "repair_source": ctx.get("repair_source"),
            "repair_reason": ctx.get("repair_reason"),
            "raw_delta_to_last_confirmed": ctx.get("raw_delta_to_last_confirmed"),
            "repaired_delta_to_last_confirmed": ctx.get("repaired_delta_to_last_confirmed"),
            "final_delta_to_baseline_raw_0_180": ctx.get("final_delta_to_baseline_raw_0_180"),
            "final_delta_unwrapped_to_baseline": ctx.get("final_delta_unwrapped_to_baseline"),
        }
    ...
    def _make_angle_result_snapshot(result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        keep_keys = [
            ...
            "final_angle_raw_0_180", "final_angle_unwrapped", "last_confirmed_angle_unwrapped",
            "angle_cross_boundary", "angle_quality", "angle_quality_reason", "angle_quality_jump_deg",
            "raw_angle", "angle_deg_raw_before_repair", "angle_deg_repaired",
            "repair_used", "repair_source", "repair_reason",
            "raw_delta_to_last", "raw_delta_to_last_confirmed", "repaired_delta_to_last_confirmed",
            "final_delta_to_baseline_raw_0_180", "final_delta_unwrapped_to_baseline",
        ]
    ...
    def save_cycle_result(
        ...
        with self.save_io_lock:
            ...
            if raw_values or filtered_values or median_values:
                ...
                self.log("[保存] 本轮未获得可写入的真实光谱数组；汇总仍写入当前真实峰值字段（可能为空）。")
    ...
    def _append_summary_csv(
        ...
        with self.summary_csv_path.open("a", encoding="utf-8-sig", newline="") as f:
            ...
            writer.writerow([
                ...
                ctx.get("final_angle_raw_0_180"),
                ctx.get("final_angle_unwrapped"),
                ctx.get("last_saved_final_angle_unwrapped"),
                ctx.get("angle_cross_boundary"),
                ctx.get("angle_quality"),
                ctx.get("angle_quality_reason"),
                ctx.get("angle_quality_jump_deg"),
                ctx.get("raw_angle"),
                ctx.get("repaired_angle"),
                ctx.get("repair_used"),
                ctx.get("repair_source"),
                ctx.get("repair_reason"),
                ctx.get("raw_delta_to_last_confirmed"),
                ctx.get("repaired_delta_to_last_confirmed"),
                ctx.get("final_delta_to_baseline_raw_0_180"),
                ctx.get("final_delta_unwrapped_to_baseline"),
            ])
    ...
    def _ensure_summary_xlsx(self):
        ...
        for col_idx in range(7, len(self.summary_xlsx_headers) + 1):
            ws.column_dimensions[chr(64 + col_idx) if col_idx <= 26 else "A" + chr(64 + col_idx - 26)].width = 24
    ...
    def _append_summary_xlsx(
        ...
        向 Excel 汇总文件追加一轮结果。
        表头：
            序号、本次角度、本次原始数据峰值、拟合数据峰值、文件名、Step1/Step7角度质量标记。
        """
        ctx = context_snapshot if context_snapshot is not None else self.context
        self._ensure_summary_xlsx()
        ...
        for col_idx in range(1, len(self.summary_xlsx_headers) + 1):
    ...
    def run_one_cycle(self, cycle_index: int) -> bool:
        ...
        Step 4：LabVIEW 光谱采集
        Step 5：照明光 ON
        Step 5.5：保存本轮数据；保存角度使用 Step1 的最终 baseline 角度，
                  若 Step1 触发修复并采用修复结果，则保存 Step1 修复角度；否则保存 Step1 检测角度。
        Step 6：打开激光
        ...
        try:
            ...
            if angle_result.get("ok", False) and angle_result.get("angle_deg") is not None:
                ...
                current_angle = None
                self.log("[角度检测-current_angle] 本轮 Bmask 最长边角度无效，流程停止，避免使用错误角度进入 Step7。")
                self.context["last_angle_result"] = angle_result
                self.context["rule_ab_failed_stop"] = True
                self.context["rule_ab_failed_reason"] = angle_result.get("reason")
                self.notify_update()
                return False
            ...
            self.context["angle_before_after_delta"] = None
            # 保存前角度展开和质量标记。Step1 baseline 逻辑不变，只增加记录字段。
            self._prepare_and_store_save_angle_quality(
                final_angle_raw_0_180=current_angle,
                baseline_angle_raw_0_180=current_angle,
                angle_result=angle_result,
            )
            self.context["last_angle_result"] = self._json_safe({k: v for k, v in angle_result.items() if not str(k).startswith("__same_frame_")})
            ...
            # 当前版本按要求：本轮数据保存提前到 Step5 和 Step6 之间。
            # 保存角度固定使用 Step1 最终角度 current_angle，而不是 Step7 最终角度。
            # 因为 start_save_cycle_result_async() 会立即创建 context_snapshot，
            # 所以后续 Step7 即使更新 angle_after，也不会影响本轮已提交的保存数据。
            if self.stop_requested:
                return False
            ...
            angle_after_result_for_save = {
                "ok": False,
                "angle_deg": None,
                "reason": "save_moved_before_step6_use_step1_final_angle",
            }
            ...
            # Step7 后只记录最终角度用于排查/GUI显示，不再覆盖本轮保存角度。
    ...
    def run(self):
        ...
        # 恢复真实光谱仪流程：完整测量启动前启动 TCP 并等待 LabVIEW READY。
        self.start_tcp_server()
        self.wait_labview_ready()
    ...
    def run_simple_angle_loop_one_cycle(self, cycle_index: int, state: Optional[CalibrationState] = None) -> bool:
        ...
        try:
            ...
            self._ensure_rule_ab_feature_tracker_installed(follower, self._get_loaded_calibration_state())
            result = self.detect_bmask_longest_edge_angle_once(
                image_rgb=None,
                b_mask=None,
    ...
    def request_stop(self):
        ...
        # 停止测量后，兜底关闭激光；当前版本不再操作 Newport激光。
        try:
...
class MeasurementWorkflowGUI:
    def __init__(self, root: tk.Tk):
        self.root = root
        self.root.title("循环测量 GUI：照明光 + Newport激光开关 + Bmask最长边角度 + LabVIEW TCP光谱")
    ...
    def _build_ui(self):
        ...
        ttk.Button(button_grid, text="1 标定Bmask/边", command=self.calibrate_angle_b_thread).grid(row=2, column=0, padx=4, pady=4, sticky="ew")
        ...
        ttk.Label(basic_frame, text="稳定等待/ms").grid(row=0, column=2, padx=(4, 6), pady=4, sticky="w")
        ttk.Entry(basic_frame, textvariable=self.stable_wait_ms_var, width=14).grid(row=0, column=3, padx=(0, 10), pady=4, sticky="ew")

        ttk.Label(basic_frame, text="原始数据去除阈值").grid(row=1, column=0, padx=(4, 6), pady=4, sticky="w")
        ttk.Entry(basic_frame, textvariable=self.raw_remove_above_var, width=14).grid(row=1, column=1, padx=(0, 4), pady=4, sticky="ew")
        ttk.Label(basic_frame, text="中值滤波窗口").grid(row=1, column=2, padx=(4, 6), pady=4, sticky="w")
        ttk.Entry(basic_frame, textvariable=self.median_filter_window_var, width=14).grid(row=1, column=3, padx=(0, 10), pady=4, sticky="ew")

        ttk.Separator(basic_frame, orient=tk.HORIZONTAL).grid(row=4, column=0, columnspan=4, padx=4, pady=(8, 6), sticky="ew")

        ttk.Label(basic_frame, text="保存目录").grid(row=5, column=0, padx=(4, 6), pady=4, sticky="w")
        ttk.Entry(basic_frame, textvariable=self.save_root_var).grid(row=5, column=1, columnspan=2, padx=(0, 10), pady=4, sticky="ew")
        ttk.Button(basic_frame, text="选择", command=self.select_save_root, width=8).grid(row=5, column=3, padx=(0, 4), pady=4, sticky="ew")

        ttk.Label(basic_frame, text="图2/图3横坐标xlsx").grid(row=6,column=0, padx=(4, 6), pady=4, sticky="w")
        ttk.Entry(basic_frame, textvariable=self.x_axis_xlsx_path_var).grid(row=6, column=1, columnspan=2, padx=(0, 10), pady=4, sticky="ew")
        ttk.Button(basic_frame, text="选择", command=self.select_x_axis_xlsx, width=8).grid(row=6, column=3, padx=(0, 4), pady=4, sticky="ew")
        ...
        # =====================================================
        laser_frame = ttk.LabelFrame(left_inner, text="4. Newport 8743-CL 激光开关控制器", padding=10, style="Panel.TLabelframe")
        laser_frame.pack(fill=tk.X, padx=4, pady=8)

        self.laser_status_var = tk.StringVar(value="激光控制器状态：未连接")

        laser_frame.columnconfigure(1, weight=1)

        laser_buttons = ttk.Frame(laser_frame)
        laser_buttons.grid(row=1, column=0, columnspan=3, padx=2, pady=4, sticky="ew")
        for i in range(2):
            laser_buttons.columnconfigure(i, weight=1)
        ttk.Button(laser_buttons, text="连接激光控制器", command=self.connect_laser_thread).grid(row=0, column=0, padx=4, pady=3, sticky="ew")
        ttk.Button(laser_buttons, text="设置激光轴速度/加速度", command=self.configure_laser_thread).grid(row=0, column=1, padx=4, pady=3, sticky="ew")
        ttk.Button(laser_buttons, text="激光 ON", command=self.laser_on_thread).grid(row=1, column=0, padx=4, pady=3, sticky="ew")
        ttk.Button(laser_buttons, text="激光 OFF", command=self.laser_off_thread).grid(row=1, column=1, padx=4, pady=3, sticky="ew")
        ttk.Label(laser_frame, textvariable=self.laser_status_var, wraplength=450).grid(row=2, column=0, columnspan=3, padx=4, pady=(4, 0), sticky="w")
    ...
    def sync_config_from_ui_to_workflow(self) -> MeasurementWorkflow:
        ...
        laser_param_changed = (
            old_cfg.laser_axis != new_cfg.laser_axis or
            old_cfg.laser_on_steps != new_cfg.laser_on_steps or
            old_cfg.laser_off_steps != new_cfg.laser_off_steps or
            old_cfg.laser_speed != new_cfg.laser_speed or
            old_cfg.laser_accel != new_cfg.laser_accel
        )
        ...
        if laser_param_changed and self.workflow.laser_stage is not None:
            self.workflow.set_laser_velocity_accel(
                axis=int(new_cfg.laser_axis),
                speed=int(new_cfg.laser_speed),
                accel=int(new_cfg.laser_accel),
            )
            self.log("[配置同步] Newport 激光轴参数已更新")
        ...
        self.log(
            ...
            f"laser_axis={new_cfg.laser_axis}, laser_on/off_steps=[{new_cfg.laser_on_steps}, {new_cfg.laser_off_steps}], "
    ...
    def refresh_result_labels(self, workflow: Optional[MeasurementWorkflow] = None):
        ...
        if wf.laser_stage is not None:
            self.set_var(
                self.laser_status_var,
                f"激光控制器状态：已连接，激光 {laser_state}，axis={wf.cfg.laser_axis}"
            )
        else:
            self.set_var(self.laser_status_var, "激光控制器状态：未连接")
    ...
    def init_all_devices(self):
        try:
            ...
            self.set_var(self.laser_status_var, "激光控制器状态：已连接，激光轴速度/加速度已设置")
    ...
    def _current_or_new_calibration_state(self) -> CalibrationState:
        ...
        # B 点双向兜底，避免“不点击 1 标定Bmask/边”时 RuleAB-B/Bmask 缺失导致 ABC 初始化不稳定。
        if not state.rule_ab_b_positive_points and state.b_mask_positive_points:
            state.rule_ab_b_positive_points = [[float(x), float(y)] for x, y in state.b_mask_positive_points]
            state.rule_ab_b_negative_points = [[float(x), float(y)] for x, y in state.b_mask_negative_points]
        if not state.b_mask_positive_points and state.rule_ab_b_positive_points:
            state.b_mask_positive_points = [[float(x), float(y)] for x, y in state.rule_ab_b_positive_points]
            state.b_mask_negative_points = [[float(x), float(y)] for x, y in state.rule_ab_b_negative_points]
        return state
    ...
    def calibrate_angle_b(self):
        """标定角度检测/RuleAB 共用的 B mask 正点/负点；当前版本不再要求选择某条 B 边。"""
        try:
            cfg = self.build_config_from_ui()
            ...
            pos, neg = self._select_object_points_interactively(
                image_rgb,
                "Angle-B / RuleAB-B",
                "Calibration B mask: left positive, right negative",
            )

            state = self._current_or_new_calibration_state()
            state.b_mask_positive_points = [[float(x), float(y)] for x, y in pos]
            state.b_mask_negative_points = [[float(x), float(y)] for x, y in neg]
            # 当前角度来源是当前帧 Bmask 最长边，以下旧字段只用于兼容旧 JSON / GUI 显示。

            # B mask 和 RuleAB-B 是同一个 B。若 RuleAB-B 尚未单独标定，自动复用这里的 B 点。
            ...
            self.log(
                "[完整测量标定] Bmask 标定完成："
                f"B+={len(pos)}, B-={len(neg)}；角度边=当前帧Bmask最长边自动选择，不再人工输入边编号。"
            )
        except Exception as e:
            self.log(f"[完整测量标定] Bmask 标定失败：{e}")
            self.log(traceback.format_exc())
            self.root.after(0, lambda: messagebox.showerror("Bmask 标定失败", str(e)))
    ...
    def run_workflow(self):
        ...
        try:
            ...
            # 2. 确保实验设备已经初始化
            if wf.light is None or wf.laser_stage is None:
                self.log("[流程] 实验设备尚未完整初始化，先初始化实验设备")
                wf.connect_measurement_devices()
            # 2. 恢复真实光谱仪 TCP：完整测量前启动 TCP 并等待 LabVIEW READY。
            ...
            self.set_var(self.tcp_status_var, "TCP状态：已启动并等待到 LabVIEW READY")
            self.log("[流程] 光谱仪 TCP 已启用：完整测量将采集真实光谱数据")
            ...
            while True:
                ...
                self.log(
                    ...
                    f"rule_ab_target=[{wf.cfg.rule_ab_angle_delta_min_deg}, {wf.cfg.rule_ab_angle_delta_max_deg}]"
                )
    ...
    def stop_rule_ab_controller(self):
        ...
        作用：
            ...
            3. 不关闭 Newport激光、LabVIEW TCP、照明光，也不刷新光谱图。
        """
        self.rule_ab_controller_stop_requested = True
        self.rule_ab_only_stop_requested = True
    ...
    def stop_step9_stage12(self):
        ...
        作用：
            ...
            3. 不关闭照明光、Newport激光、LabVIEW TCP，也不影响 A推B 的 Stage34。
        """
        try:
            if self.workflow is None:
    ...
    def connect_laser_thread(self):
        self.run_in_thread(self.connect_laser)
    ...
    def configure_laser_thread(self):
        self.run_in_thread(self.configure_laser)
    ...
    def laser_off_thread(self):
        self.run_in_thread(self.laser_off)
    ...
    def test_rule_ab(self):
        ...
        单独点击按钮时，只执行新的 RuleAB 视觉闭环：
            ...
            7. 不连接 Newport激光、不采集光谱、不读取 xlsx、不刷新光谱图。
        """
        follower = None
    ...
    def close_all(self):
        try:
            ...
            self.set_var(self.laser_status_var, "激光控制器状态：已关闭")
            self.set_var(self.tcp_status_var, "TCP状态：已关闭")

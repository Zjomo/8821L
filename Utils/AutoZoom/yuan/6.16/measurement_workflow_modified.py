...
class CalibrationState:
    ...
    def missing_items(self) -> List[str]:
        ...
        # 因此 angle_edge_index / angle_edge_name 只作为旧标定文件兼容字段，不再作为 preflight 硬条件.
...
class MeasurementWorkflowGUI:
    ...
    def calibrate_angle_b(self):
        ...
        try:
            ...
            state.angle_edge_index = -1
            state.angle_edge_name = "Bmask_longest_edge_auto"
            state.angle_num = int(self.num_var.get())
            state.angle_cw = int(self.cw_var.get())

            # 角度检测 B 和 RuleAB-B 是同一个 B。若 RuleAB-B 尚未单独标定，自动复用这里的 B 点。

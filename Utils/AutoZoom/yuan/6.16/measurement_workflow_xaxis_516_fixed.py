...
class MeasurementWorkflow:
    ...
    def request_labview_spectrum(self, cycle_index: int) -> Dict[str, Any]:
        ...
        # 横坐标固定使用 GUI/config 中指定的 516中心波长.xlsx。
        # LabVIEW/TCP 返回的 wavelength 只作为备用信息保存，不允许覆盖每轮 CSV/绘图的 x 轴。
        x_axis_values = self.load_x_axis_values_from_xlsx()
        labview_wavelength = parsed.get("wavelength") or []
        self.context["wavelength"] = x_axis_values
        self.context["x_axis_values"] = x_axis_values
        self.context["labview_wavelength"] = labview_wavelength

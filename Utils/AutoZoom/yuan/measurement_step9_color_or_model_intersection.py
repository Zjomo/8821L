...
class MeasurementConfig:
    ...
    # Step9 检测模式：
    #   color_region_center：沿用当前 HSV 颜色区域中心；
    #   model_intersection：C 用 U-Net .pt 得到 mask，B 用 YOLO-OBB .pt 得到旋转框 mask，
    #                       再取 C_mask ∩ B_mask 的中心点去对齐 target。
    rule_ac_detection_mode: str = str(_cfg("rule_ac_detection_mode", "color_region_center"))
    rule_ac_c_unet_model_path: str = str(_cfg("rule_ac_c_unet_model_path", ""))
    rule_ac_c_unet_device: str = str(_cfg("rule_ac_c_unet_device", "cuda"))
    rule_ac_c_unet_imgsz: int = int(_cfg("rule_ac_c_unet_imgsz", 0))  # 0 表示按当前截图原尺寸推理
    rule_ac_c_unet_threshold: float = float(_cfg("rule_ac_c_unet_threshold", 0.5))
    rule_ac_c_unet_min_area_px: int = int(_cfg("rule_ac_c_unet_min_area_px", 50))
    rule_ac_c_unet_keep_largest: bool = bool(_cfg("rule_ac_c_unet_keep_largest", True))
    rule_ac_c_unet_morph_kernel: int = int(_cfg("rule_ac_c_unet_morph_kernel", 3))
    rule_ac_b_yolo_obb_model_path: str = str(_cfg("rule_ac_b_yolo_obb_model_path", _cfg("angle_model_path", r"runs/obb/bn_up_obb/weights/best.pt")))
    rule_ac_b_yolo_obb_conf: float = float(_cfg("rule_ac_b_yolo_obb_conf", 0.25))
    rule_ac_b_yolo_obb_iou: float = float(_cfg("rule_ac_b_yolo_obb_iou", 0.7))
    rule_ac_b_yolo_obb_imgsz: int = int(_cfg("rule_ac_b_yolo_obb_imgsz", 640))
    rule_ac_b_yolo_obb_device: str = str(_cfg("rule_ac_b_yolo_obb_device", ""))
    rule_ac_b_yolo_obb_keep_largest: bool = bool(_cfg("rule_ac_b_yolo_obb_keep_largest", True))
    rule_ac_model_intersection_min_area_px: int = int(_cfg("rule_ac_model_intersection_min_area_px", 10))
    rule_ac_model_intersection_morph_kernel: int = int(_cfg("rule_ac_model_intersection_morph_kernel", 1))
...
class CalibrationState:
    ...
    step9_detection_mode: str = "color_region_center"
    step9_c_unet_model_path: str = ""
    step9_b_yolo_obb_model_path: str = ""
    ...
    def missing_items(self) -> List[str]:
        ...
        # 颜色模式需要 HSV；模型交集模式只需要 Step9 目标点，模型路径由 GUI 配置在运行前检查。
        mode = str(getattr(self, "step9_detection_mode", "color_region_center") or "color_region_center").strip().lower()
        if mode not in ("model_intersection", "models", "unet_yolo_intersection"):
...
class MeasurementWorkflow:
    def __init__(self, cfg: MeasurementConfig, on_log=None, on_update=None):
        ...
        # Step9 当前 C：只加载完整标定/静态 C mask，用于保存和 overlay 调试；
        # Step9 控制本身只使用颜色区域 mask 的中心，不再做 Step9-C SAM2 分割。
        ...
        # 仅供 Step9 单独测试中人工 SAM2-B 分割使用；完整测量 Step9 不调用 C-SAM2。
        ...
        # Step9：模型交集模式缓存。C 用 U-Net，B 用 YOLO-OBB，取二者交集中心。
        self.step9_c_unet_model: Optional[Any] = None
        self.step9_c_unet_model_path: str = ""
        self.step9_c_unet_model_device: str = ""
        self.step9_b_yolo_obb_model: Optional[Any] = None
        self.step9_b_yolo_obb_model_path: str = ""
        self.step9_model_intersection_last_mask: Optional[np.ndarray] = None
        self.step9_model_intersection_last_center: Optional[Tuple[float, float]] = None
    ...
    def _capture_current_frame_rgb(self) -> np.ndarray:
        """
        Step9 专用：截取当前屏幕区域，返回 RGB 图像。
        使用 cfg.capture_area = (left, top, width, height)。
        """
        cfg = self.cfg

        left, top, width, height = tuple(getattr(cfg, "capture_area", (116, 100, 1112, 850)))
        ...
        try:
            ...
            raise RuntimeError(f"Step9 当前帧截图失败：{e}") from e
    ...
    def _force_step9_target_from_loaded_calibration(self, reason: str = "") -> Optional[Tuple[float, float]]:
        """
        完整测量中 Step9 target 只允许来自最新加载的完整标定包，
        不允许被 GUI 输入框旧值覆盖。
        """
        if not (self.is_measuring or bool(self.context.get("full_measurement_mode", False))):
            return None
        state = self._get_loaded_calibration_state()
        if state is None:
            return None
        try:
            tx = float(getattr(state, "step9_target_x_px", -1.0))
            ty = float(getattr(state, "step9_target_y_px", -1.0))
        except Exception:
            return None
        if tx < 0 or ty < 0:
            return None
        old_tx = float(getattr(self.cfg, "rule_ac_target_x_px", -1.0))
        old_ty = float(getattr(self.cfg, "rule_ac_target_y_px", -1.0))
        self.cfg.rule_ac_target_x_px = tx
        self.cfg.rule_ac_target_y_px = ty
        if reason and (abs(old_tx - tx) > 1e-6 or abs(old_ty - ty) > 1e-6):
            self.log(
                f"[完整测量标定] Step9 target 已恢复为最新加载标定值："
                f"({tx:.3f},{ty:.3f})；覆盖 GUI 临时值 ({old_tx:.3f},{old_ty:.3f})；reason={reason}"
            )
        return (tx, ty)
    ...
    def preflight_check_calibration(self) -> CalibrationState:
        ...
        state = self._complete_calibration_state_for_runtime(state)
        state.step9_detection_mode = str(getattr(self.cfg, "rule_ac_detection_mode", "color_region_center") or "color_region_center")
        state.step9_c_unet_model_path = str(getattr(self.cfg, "rule_ac_c_unet_model_path", "") or "")
        state.step9_b_yolo_obb_model_path = str(getattr(self.cfg, "rule_ac_b_yolo_obb_model_path", "") or "")

        missing = state.missing_items()
        mode_for_check = str(getattr(self.cfg, "rule_ac_detection_mode", "color_region_center") or "color_region_center").strip().lower()
        if mode_for_check in ("model_intersection", "models", "unet_yolo_intersection"):
            try:
                self._step9_resolve_model_path(getattr(self.cfg, "rule_ac_c_unet_model_path", ""), "Step9 C-U-Net")
            except Exception as e:
                missing.append(f"Step9：C-U-Net模型路径无效（{e}）")
            try:
                self._step9_resolve_model_path(getattr(self.cfg, "rule_ac_b_yolo_obb_model_path", ""), "Step9 B YOLO-OBB")
            except Exception as e:
                missing.append(f"Step9：B YOLO-OBB模型路径无效（{e}）")
        if missing:
    ...
    def validate_step7_ac_calibration_preflight(self) -> Dict[str, Any]:
        ...
        route_points = (
            route_band.get("execution_base_route_points_xy")
            or route_band.get("execution_route_points_xy")
    ...
    def begin_step9_run_session(self) -> Path:
        ...
        meta = {
            ...
            "step9_detection_mode": str(getattr(self.cfg, "rule_ac_detection_mode", "color_region_center")),
            "c_unet_model_path": str(getattr(self.cfg, "rule_ac_c_unet_model_path", "")),
            "b_yolo_obb_model_path": str(getattr(self.cfg, "rule_ac_b_yolo_obb_model_path", "")),
            "model_intersection_min_area_px": int(getattr(self.cfg, "rule_ac_model_intersection_min_area_px", 10)),
    ...
    def _load_c_edge_route_band_from_dir(c_dir: str) -> Dict[str, Any]:
        ...
        execution_target = _pts("execution_route_points_xy") or target
    ...
    def _execute_stage12_diagonal_action(self, action_code: int) -> Dict[str, Any]:
        ...
        # 根据实测日志，原 Step9 1/2 通道输出会让检测中心远离目标；
        # 因此这里把最终发送给 CH1/CH2 的距离整体取反。
        d1 = -float(d1)
        d2 = -float(d2)
        ...
        if not bool(getattr(self.cfg, "rule_ac_enable_stage", False)):
            self.log(
                ...
                f"应发送 CH1={d1}, CH2={d2}；已按实测结果整体取反；但 RuleAC真动Stage12 未勾选"
            )
            return {
                "moved": False,
                "dry_run": True,
    ...
    def _get_step9_sam2_predictor(self):
        """
        Step9 单独测试专用 SAM2 ImagePredictor。

        注意：完整测量 Step9 已彻底删除 Step9-C SAM2 分割；
        该 predictor 只允许用于 Step9 单独测试里的 B mask 人工分割。
        """
        ...
        try:
            ...
            raise ImportError(
                "Step9 单独测试需要调用 SAM2 分割 B，但当前环境无法导入 sam2。"
        ...
        self.log(
            f"[Step9-颜色中心] 正在加载 SAM2-B 单独测试分割器：cfg={sam2_cfg}, "
    ...
    def prepare_step9_color_region_detection(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if center is not None:
                ...
                cv2.putText(canvas, "region center", (cx + 10, cy - 8), cv2.FONT_HERSHEY_SIMPLEX, 0.55, (0, 0, 255), 2, cv2.LINE_AA)
            ts = datetime.now().strftime("%Y%m%d_%H%M%S_%f")[:-3]
    ...
    def _get_or_update_step9_current_c_mask(self, image_rgb: np.ndarray) -> Dict[str, Any]:
        """
        Step9 当前 C mask 获取入口。

        当前版本已删除 Step9-C SAM2 分割：
            - 不再根据上一帧中心/box 重新分割 C；
            - 不再维护 Step9-C 运行状态；
            - 只加载完整标定锁定的 static_c_mask.png，作为保存/overlay 调试用；
            - Step9 控制仍然只使用 color_region_mask 的中心。
        """
        info: Dict[str, Any] = {
        ...
        try:
            ...
            m = np.asarray(static_c).astype(bool)
            center = self._mask_center_xy(m)
            area = int(np.count_nonzero(m))
            self.step9_current_c_mask = m.copy()
            self.step9_current_c_raw_mask = m.copy()
            self.step9_current_c_center = center
            self.step9_current_c_box = self._mask_bbox_xyxy(m, margin=35.0)
            self.step9_current_c_source = "static_full_calibration_c"
            self.step9_current_c_update_reason = "static_c_only_step9_c_sam2_removed"
            info.update({
                "step9_c_source": self.step9_current_c_source,
                ...
                "step9_c_update_ok": True,
                "step9_c_update_reason": self.step9_current_c_update_reason,
                "c_mask": m,
                "c_raw_mask": None,
            })
            return info
        except Exception as e:
            self.log(f"[Step9-C] static C 加载失败；Step9 仍继续颜色中心检测：{e}")
            info.update({
                "step9_c_source": "none",
                "step9_c_update_ok": False,
                "step9_c_update_reason": f"static_c_load_failed:{e}",
                "c_mask": None,
                "c_raw_mask": None,
            })
            return info
    def _step9_resolve_model_path(self, path_text: Any, label: str) -> Path:
        """解析 Step9 模型路径，支持绝对路径、相对主程序目录、相对当前工作目录。"""
        s = str(path_text or "").strip().strip('"').strip("'")
        if not s:
            raise RuntimeError(f"{label} 模型路径为空。")
        p = Path(s)
        candidates = [p]
        if not p.is_absolute():
            candidates.extend([PROJECT_ROOT / s, Path.cwd() / s])
        for c in candidates:
            try:
                if c.exists() and c.is_file():
                    return c.resolve()
                ...
                continue
        raise RuntimeError(f"{label} 模型文件不存在：{s}")

    def _get_step9_c_unet_model(self) -> Tuple[Any, str]:
        """
        加载 Step9 C-U-Net .pt。

        支持两类 .pt：
            1) torch.jit.trace/script 保存的 TorchScript；
            2) torch.save(model) 保存的完整 nn.Module。
        如果 .pt 只是 state_dict，因为主程序不知道 U-Net 网络结构，无法直接恢复，
        会给出明确报错；这种情况需要把第二个测试代码里的网络结构类复制进来再加载 state_dict。
        """
        if torch is None:
            raise RuntimeError("当前环境无法 import torch，不能使用 Step9 模型交集模式的 C-U-Net。")

        model_path = self._step9_resolve_model_path(getattr(self.cfg, "rule_ac_c_unet_model_path", ""), "Step9 C-U-Net")
        device_text = str(getattr(self.cfg, "rule_ac_c_unet_device", "cuda") or "cuda").strip()
        ...
        if self.step9_c_unet_model is not None and self.step9_c_unet_model_path == str(model_path) and self.step9_c_unet_model_device == str(device):
            return self.step9_c_unet_model, str(device)
        ...
        try:
            ...
            try:
                ...
                if hasattr(obj, "eval") and hasattr(obj, "to"):
                    ...
                    raise RuntimeError(
                        "该 .pt 看起来不是 TorchScript 或完整 nn.Module，可能只是 state_dict。"
                    )
            except Exception as e2:
                raise RuntimeError(
                    f"Step9 C-U-Net 加载失败：{model_path}。"
                    f"请确认 .pt 是 torch.jit.save() 的 TorchScript 或 torch.save(model) 的完整模型；"
                    f"如果是 state_dict，需要把 U-Net 网络结构类加入本程序。"
                    f"jit_error={load_error}; load_error={e2}"
                )

        model.to(device)
        model.eval()
        self.step9_c_unet_model = model
        self.step9_c_unet_model_path = str(model_path)
        self.step9_c_unet_model_device = str(device)
        self.log(f"[Step9-模型交集] 已加载 C-U-Net：{model_path}，device={device}")
        return model, str(device)

    def _get_step9_b_yolo_obb_model(self) -> Any:
        """加载 Step9 B YOLO-OBB .pt。"""
        if YOLO is None:
            raise RuntimeError("当前环境无法 import ultralytics.YOLO，不能使用 Step9 模型交集模式的 B YOLO-OBB。")
        model_path = self._step9_resolve_model_path(getattr(self.cfg, "rule_ac_b_yolo_obb_model_path", ""), "Step9 B YOLO-OBB")
        if self.step9_b_yolo_obb_model is not None and self.step9_b_yolo_obb_model_path == str(model_path):
            return self.step9_b_yolo_obb_model
        model = YOLO(str(model_path))
        self.step9_b_yolo_obb_model = model
        self.step9_b_yolo_obb_model_path = str(model_path)
        self.log(f"[Step9-模型交集] 已加载 B YOLO-OBB：{model_path}")
        return model

    @staticmethod
    def _step9_cleanup_binary_mask(mask: np.ndarray, min_area_px: int = 1, morph_kernel: int = 1, keep_largest: bool = False) -> np.ndarray:
        """二值 mask 后处理：开闭运算、最小面积过滤、可选保留最大连通域。"""
        m = np.asarray(mask).astype(bool)
        if m.ndim != 2:
        ...
        if k >= 2:
            ...
            m = u8 > 0
        num, labels, stats, _centroids = cv2.connectedComponentsWithStats(m.astype(np.uint8), connectivity=8)
        if num <= 1:
        ...
        for lab in range(1, num):
            area = int(stats[lab, cv2.CC_STAT_AREA])
            if area >= min_area:
                valid.append((area, lab))
        if not valid:
            return np.zeros(m.shape, dtype=bool)
        if keep_largest:
            lab = max(valid, key=lambda x: x[0])[1]
            return labels == lab
        out = np.zeros(m.shape, dtype=bool)
        for _area, lab in valid:
            out |= (labels == lab)
        return out

    def _predict_step9_c_mask_by_unet(self, image_rgb: np.ndarray) -> np.ndarray:
        """用 C-U-Net .pt 预测当前帧 C mask。"""
        model, device_text = self._get_step9_c_unet_model()
        device = torch.device(device_text)
        img = np.asarray(image_rgb)
        if img.ndim == 2:
            img = cv2.cvtColor(img, cv2.COLOR_GRAY2RGB)
        h0, w0 = img.shape[:2]
        imgsz = int(getattr(self.cfg, "rule_ac_c_unet_imgsz", 0))
        if imgsz > 0 and (h0 != imgsz or w0 != imgsz):
            inp_img = cv2.resize(img, (imgsz, imgsz), interpolation=cv2.INTER_AREA)
        else:
            inp_img = img
        arr = inp_img.astype(np.float32) / 255.0
        tensor = torch.from_numpy(arr.transpose(2, 0, 1)).unsqueeze(0).to(device)
        with torch.no_grad():
            out = model(tensor)
        if isinstance(out, dict):
            for k in ("out", "logits", "mask", "masks", "pred"):
                if k in out:
                    out = out[k]
                    break
            else:
        ...
        if not hasattr(out, "detach"):
            raise RuntimeError(f"C-U-Net 输出类型无法解析：{type(out).__name__}")
        y = out.detach().float().cpu()
        # 常见输出：[1,1,H,W] / [1,2,H,W] / [1,H,W] / [H,W]
        if y.ndim == 4:
            if y.shape[1] == 1:
                y = y[0, 0]
            else:
                # 多类输出默认取前景概率最大的非 0 类；二分类 softmax 时取 class 1。
                prob = torch.softmax(y, dim=1)
                cls_idx = 1 if y.shape[1] > 1 else 0
                y = prob[0, cls_idx]
        elif y.ndim == 3:
            y = y[0]
        elif y.ndim != 2:
            raise RuntimeError(f"C-U-Net 输出维度无法解析：shape={tuple(y.shape)}")
        y_np = y.numpy().astype(np.float32)
        if float(np.nanmin(y_np)) < 0.0 or float(np.nanmax(y_np)) > 1.0:
            y_np = 1.0 / (1.0 + np.exp(-y_np))
        th = float(getattr(self.cfg, "rule_ac_c_unet_threshold", 0.5))
        mask = y_np >= th
        if mask.shape[:2] != (h0, w0):
            mask = cv2.resize(mask.astype(np.uint8), (w0, h0), interpolation=cv2.INTER_NEAREST).astype(bool)
        mask = self._step9_cleanup_binary_mask(
            mask,
            min_area_px=int(getattr(self.cfg, "rule_ac_c_unet_min_area_px", 50)),
            morph_kernel=int(getattr(self.cfg, "rule_ac_c_unet_morph_kernel", 3)),
            keep_largest=bool(getattr(self.cfg, "rule_ac_c_unet_keep_largest", True)),
        )
        return mask.astype(bool)

    def _predict_step9_b_mask_by_yolo_obb(self, image_rgb: np.ndarray) -> Tuple[np.ndarray, Dict[str, Any]]:
        """用 B YOLO-OBB .pt 预测当前帧 B mask；OBB 四角填充成二值区域。"""
        model = self._get_step9_b_yolo_obb_model()
        img = np.asarray(image_rgb)
        if img.ndim == 2:
            img = cv2.cvtColor(img, cv2.COLOR_GRAY2RGB)
        h, w = img.shape[:2]
        kwargs = dict(
            conf=float(getattr(self.cfg, "rule_ac_b_yolo_obb_conf", 0.25)),
            iou=float(getattr(self.cfg, "rule_ac_b_yolo_obb_iou", 0.7)),
            imgsz=int(getattr(self.cfg, "rule_ac_b_yolo_obb_imgsz", 640)),
            verbose=False,
        )
        dev = str(getattr(self.cfg, "rule_ac_b_yolo_obb_device", "") or "").strip()
        if dev:
            kwargs["device"] = dev
        results = model.predict(img, **kwargs)
        if not results:
            return np.zeros((h, w), dtype=bool), {"det_count": 0, "conf": ""}
        r = results[0]
        dets: List[Tuple[float, float, np.ndarray]] = []

        # YOLO-OBB：优先使用 obb.xyxyxyxy 四角。
        obb = getattr(r, "obb", None)
        if obb is not None and getattr(obb, "xyxyxyxy", None) is not None:
            pts_arr = obb.xyxyxyxy.detach().cpu().numpy()
            conf_arr = obb.conf.detach().cpu().numpy() if getattr(obb, "conf", None) is not None else np.ones((len(pts_arr),), dtype=np.float32)
            for pts, conf in zip(pts_arr, conf_arr):
                poly = np.asarray(pts, dtype=np.float32).reshape(-1, 2)
                if len(poly) < 3:
                    continue
                area = abs(float(cv2.contourArea(poly.astype(np.float32))))
                dets.append((float(conf), area, poly))

        # 兼容 YOLO-seg：如果误用了 seg 模型，也允许直接取 masks。
        if not dets and getattr(r, "masks", None) is not None and getattr(r.masks, "data", None) is not None:
            masks = r.masks.data.detach().cpu().numpy()
            conf_arr = r.boxes.conf.detach().cpu().numpy() if getattr(r, "boxes", None) is not None and getattr(r.boxes, "conf", None) is not None else np.ones((len(masks),), dtype=np.float32)
            candidates = []
            for m, conf in zip(masks, conf_arr):
                mm = cv2.resize((m > 0.5).astype(np.uint8), (w, h), interpolation=cv2.INTER_NEAREST).astype(bool)
                area = int(np.count_nonzero(mm))
                if area > 0:
                    candidates.append((float(conf), float(area), mm))
            if candidates:
                if bool(getattr(self.cfg, "rule_ac_b_yolo_obb_keep_largest", True)):
                    conf, area, mm = max(candidates, key=lambda x: x[1])
                    return mm.astype(bool), {"det_count": len(candidates), "conf": conf, "area": area, "source": "yolo_seg_mask"}
                out = np.zeros((h, w), dtype=bool)
                for _conf, _area, mm in candidates:
                    out |= mm
                return out, {"det_count": len(candidates), "conf": max(c[0] for c in candidates), "area": int(np.count_nonzero(out)), "source": "yolo_seg_mask"}

        if not dets:
            return np.zeros((h, w), dtype=bool), {"det_count": 0, "conf": "", "source": "none"}

        if bool(getattr(self.cfg, "rule_ac_b_yolo_obb_keep_largest", True)):
            dets = [max(dets, key=lambda x: x[1])]
        out = np.zeros((h, w), dtype=np.uint8)
        for conf, area, poly in dets:
            pts = np.round(poly).astype(np.int32)
            cv2.fillPoly(out, [pts], 255)
        return (out > 0), {
            "det_count": len(dets),
            "conf": float(max(d[0] for d in dets)),
            "area": int(np.count_nonzero(out)),
            "source": "yolo_obb_polygon",
        }

    def _capture_step9_model_intersection_center_once(self) -> Dict[str, Any]:
        """Step9 模型交集模式：C-U-Net mask ∩ B-YOLO-OBB mask 的中心对齐。"""
        try:
            image_rgb = self._capture_current_frame_rgb()
        except Exception as e:
            return {
                "ok": False,
                "reason": f"capture_failed:{e}",
                "image_rgb": None,
                "color_mask": None,
                "color_area_px": 0,
                "a_mask": None,
                ...
                "c_mask": None,
                "c_raw_mask": None,
                "step9_c_source": "c_unet_failed_before_capture",
            }

        try:
            c_mask = self._predict_step9_c_mask_by_unet(image_rgb)
        except Exception as e:
            return {
                "ok": False,
                "reason": f"c_unet_failed:{e}",
                "image_rgb": image_rgb,
                "color_mask": None,
                "color_area_px": 0,
                "a_mask": None,
                ...
                "c_mask": None,
                "c_raw_mask": None,
                "step9_c_source": "c_unet_failed",
                "step9_c_update_ok": False,
                "step9_c_update_reason": str(e),
            }

        try:
            b_mask, b_debug = self._predict_step9_b_mask_by_yolo_obb(image_rgb)
        except Exception as e:
            b_debug = {"error": str(e)}
            b_mask = np.zeros(c_mask.shape, dtype=bool)

        if b_mask.shape[:2] != c_mask.shape[:2]:
            b_mask = cv2.resize(b_mask.astype(np.uint8), (c_mask.shape[1], c_mask.shape[0]), interpolation=cv2.INTER_NEAREST).astype(bool)

        inter = np.asarray(c_mask).astype(bool) & np.asarray(b_mask).astype(bool)
        inter = self._step9_cleanup_binary_mask(
            inter,
            min_area_px=int(getattr(self.cfg, "rule_ac_model_intersection_min_area_px", 10)),
            morph_kernel=int(getattr(self.cfg, "rule_ac_model_intersection_morph_kernel", 1)),
            keep_largest=True,
        )
        center = self._mask_center_xy(inter)
        area = int(np.count_nonzero(inter))
        c_center = self._mask_center_xy(c_mask)
        if center is None or area <= 0:
            return {
                "ok": False,
                "reason": "empty_model_intersection_mask",
                "image_rgb": image_rgb,
                "color_mask": inter,
                "color_area_px": area,
                "a_mask": None,
                "b_mask": b_mask,
                "a_mask_area_px": "",
                "b_mask_area_px": int(np.count_nonzero(b_mask)),
                "c_mask": c_mask,
                "c_raw_mask": c_mask,
                "step9_c_source": "c_unet_model",
                "step9_c_area_px": int(np.count_nonzero(c_mask)),
                "step9_c_center_x": float(c_center[0]) if c_center is not None else "",
                "step9_c_center_y": float(c_center[1]) if c_center is not None else "",
                "step9_c_dirty_before_update": False,
                "step9_c_update_ok": True,
                "step9_c_update_reason": "model_intersection_empty",
                "step9_b_yolo_debug": b_debug,
            }

        self.step9_model_intersection_last_mask = inter.copy()
        self.step9_model_intersection_last_center = center
        self.step9_current_c_mask = c_mask.copy()
        self.step9_current_c_raw_mask = c_mask.copy()
        self.step9_current_c_center = c_center
        self.step9_current_c_source = "c_unet_model"
        self.step9_current_c_update_reason = "model_intersection"
        return {
            "ok": True,
            "reason": "ok_model_intersection_center",
            "image_rgb": image_rgb,
            "color_mask": inter,
            "color_center": center,
            "color_area_px": area,
            ...
            "step9_c_dirty_before_update": False,
            "step9_c_update_ok": True,
            "step9_c_update_reason": "model_intersection",
            "step9_b_yolo_debug": b_debug,
        }
    ...
    def _capture_step9_color_center_once(self) -> Dict[str, Any]:
        ...
        # Step9 当前 C：只加载完整标定锁定的 static C，用于保存/overlay 调试；不再动态 SAM2 分割 C。
        c_info = self._get_or_update_step9_current_c_mask(image_rgb)
    ...
    def _save_step9_color_overlay(
        ...
        c_raw_mask 当前不再由 Step9 生成；保留字段只是兼容旧 CSV/overlay 结构。
        """
        saved = {
            "overlay_path": None,
            "frame_path": None,
    ...
    def run_rule_ac_until_threshold(self) -> Dict[str, Any]:
        ...
        """
        mode_now = str(getattr(self.cfg, "rule_ac_detection_mode", "color_region_center") or "color_region_center").strip().lower()
        mode_label = "模型交集中心" if mode_now in ("model_intersection", "models", "unet_yolo_intersection") else "颜色检测区域中心"
        self.log(f"========== Step9：{mode_label}对齐，必要时移动1/2通道 ==========")
        self.step9_stop_requested = False
        ...
        while True:
            ...
            # 完整测量中 target 只使用最新加载的完整标定包，避免 GUI 旧输入框覆盖。
            self._force_step9_target_from_loaded_calibration(reason="run_rule_ac_until_threshold")
            ...
            mode_now = str(getattr(self.cfg, "rule_ac_detection_mode", "color_region_center") or "color_region_center").strip().lower()
            if mode_now in ("model_intersection", "models", "unet_yolo_intersection"):
                det = self._capture_step9_model_intersection_center_once()
            else:
...
class MeasurementWorkflowGUI:
    ...
    def _build_ui(self):
        ...
        self.rule_ac_detection_mode_var = tk.StringVar(value=getattr(cfg0, "rule_ac_detection_mode", "color_region_center"))
        self.rule_ac_c_unet_model_path_var = tk.StringVar(value=getattr(cfg0, "rule_ac_c_unet_model_path", ""))
        self.rule_ac_b_yolo_obb_model_path_var = tk.StringVar(value=getattr(cfg0, "rule_ac_b_yolo_obb_model_path", getattr(cfg0, "angle_model_path", "")))
        self.rule_ac_c_unet_threshold_var = tk.DoubleVar(value=getattr(cfg0, "rule_ac_c_unet_threshold", 0.5))
        self.rule_ac_model_intersection_min_area_var = tk.IntVar(value=getattr(cfg0, "rule_ac_model_intersection_min_area_px", 10))
        ...
        ttk.Label(rule_frame, text="Step9检测模式").grid(row=14, column=0, padx=4, pady=3, sticky="w")
        ttk.Combobox(
            rule_frame,
            textvariable=self.rule_ac_detection_mode_var,
            values=("color_region_center", "model_intersection"),
            width=18,
            state="readonly",
        ).grid(row=14, column=1, columnspan=3, padx=4, pady=3, sticky="ew")

        ttk.Label(rule_frame, text="C-U-Net.pt / B-YOLO-OBB.pt").grid(row=15, column=0, padx=4, pady=3, sticky="w")
        ttk.Entry(rule_frame, textvariable=self.rule_ac_c_unet_model_path_var, width=18).grid(row=15, column=1, padx=4, pady=3, sticky="ew")
        ttk.Entry(rule_frame, textvariable=self.rule_ac_b_yolo_obb_model_path_var, width=18).grid(row=15, column=2, padx=4, pady=3, sticky="ew")
        ttk.Entry(rule_frame, textvariable=self.rule_ac_c_unet_threshold_var, width=8).grid(row=15, column=3, padx=4, pady=3, sticky="ew")

        ttk.Label(rule_frame, text="Step9颜色模式/HSV").grid(row=16, column=0, padx=4, pady=3, sticky="w")
        ttk.Combobox(rule_frame, textvariable=self.rule_ac_color_mode_var, values=("include", "exclude"), width=9, state="readonly").grid(row=16, column=1, padx=4, pady=3, sticky="ew")
        ttk.Entry(rule_frame, textvariable=self.rule_ac_color_h_var, width=6).grid(row=16, column=2, padx=4, pady=3, sticky="ew")
        ttk.Entry(rule_frame, textvariable=self.rule_ac_color_s_var, width=6).grid(row=16, column=3, padx=4, pady=3, sticky="ew")
        ttk.Label(rule_frame, text="HSV容差H/S/V/最小面积").grid(row=17, column=0, padx=4, pady=3, sticky="w")
        ttk.Entry(rule_frame, textvariable=self.rule_ac_color_h_tol_var, width=6).grid(row=17, column=1, padx=4, pady=3, sticky="ew")
        ttk.Entry(rule_frame, textvariable=self.rule_ac_color_s_tol_var, width=6).grid(row=17, column=2, padx=4, pady=3, sticky="ew")
        color_extra = ttk.Frame(rule_frame)
        color_extra.grid(row=17, column=3, padx=4, pady=3, sticky="ew")
        ...
        rule_buttons.grid(row=18, column=0, columnspan=4, padx=2, pady=6, sticky="ew")
        for i in range(4):
        ...
        ttk.Label(rule_frame, textvariable=self.rule_module_status_var, wraplength=450).grid(
            row=19, column=0, columnspan=4, padx=4, pady=(4, 0), sticky="w"
        )
    ...
    def _save_static_c_map_result(
        ...
        route_payload = {
            ...
            "route_geometry_mode": "mitered_rectangle" if route_source in ("quad", "quadrilateral") else "sam2_outer_edge_pixels",
            "execution_route_mode": "axis_aligned_hv_chunked_manhattan",
            "execution_segment_px": float(execution_segment_px),
            ...
            "note": ("检测到 C_mask 近似圆形：Step7 路线使用圆形外接矩形/四边形。" if route_circle_like else "") + "Step7 三线安全带：route_points_xy 是拟合矩形可视化路径；execution_*_points_xy 是控制器实际分段水平/垂直执行路径，避免每个采样点频繁转向。",
        }
    ...
    def build_config_from_ui(self) -> MeasurementConfig:
        return MeasurementConfig(
            ...
            rule_ac_detection_mode=str(self.rule_ac_detection_mode_var.get()).strip() or "color_region_center",
            rule_ac_c_unet_model_path=str(self.rule_ac_c_unet_model_path_var.get()).strip(),
            rule_ac_b_yolo_obb_model_path=str(self.rule_ac_b_yolo_obb_model_path_var.get()).strip(),
            rule_ac_c_unet_threshold=float(self.rule_ac_c_unet_threshold_var.get()),
            rule_ac_model_intersection_min_area_px=int(self.rule_ac_model_intersection_min_area_var.get()),
    ...
    def sync_config_from_ui_to_workflow(self) -> MeasurementWorkflow:
        ...
        rule_ac_param_changed = (
            ...
            getattr(old_cfg, "rule_ac_detection_mode", "color_region_center") != getattr(new_cfg, "rule_ac_detection_mode", "color_region_center") or
            getattr(old_cfg, "rule_ac_c_unet_model_path", "") != getattr(new_cfg, "rule_ac_c_unet_model_path", "") or
            getattr(old_cfg, "rule_ac_b_yolo_obb_model_path", "") != getattr(new_cfg, "rule_ac_b_yolo_obb_model_path", "") or
            old_cfg.capture_area != new_cfg.capture_area
        )
        ...
        if loaded_state is not None:
            if self.workflow.is_measuring or bool(self.workflow.context.get("full_measurement_mode", False)):
                ...
                # 这样 A-C距离、CH3/CH4速度/加速度/步数/停顿、Step9 HSV/容差等
                # 都可以在 GUI 中实时修改并在下一步生效；Step9 target 固定使用最新加载的完整标定包。
                ...
                self.workflow._force_step9_target_from_loaded_calibration(reason="sync_config_from_ui_to_workflow")
            else:
    ...
    def _current_or_new_calibration_state(self) -> CalibrationState:
        ...
        state.step9_detection_mode = str(getattr(cfg, "rule_ac_detection_mode", "color_region_center"))
        state.step9_c_unet_model_path = str(getattr(cfg, "rule_ac_c_unet_model_path", ""))
        state.step9_b_yolo_obb_model_path = str(getattr(cfg, "rule_ac_b_yolo_obb_model_path", ""))
    ...
    def load_calibration_to_gui(self):
        ...
        try:
            ...
            self.set_var(self.rule_ac_detection_mode_var, str(getattr(state, "step9_detection_mode", "color_region_center") or "color_region_center"))
            if getattr(state, "step9_c_unet_model_path", ""):
                self.set_var(self.rule_ac_c_unet_model_path_var, str(state.step9_c_unet_model_path))
            if getattr(state, "step9_b_yolo_obb_model_path", ""):
                self.set_var(self.rule_ac_b_yolo_obb_model_path_var, str(state.step9_b_yolo_obb_model_path))
    ...
    def run_workflow(self):
        ...
        try:
            ...
            # Step9 仍允许运行中同步步数/速度/容差/HSV 等参数；
            # 但 target_x/y 会在 workflow 内部强制恢复为最新加载完整标定包的值。
    ...
    def test_rule_ac(self):
        """
        Step9 单独测试：支持颜色检测区域中心 / 模型交集中心两种模式。

        color_region_center：使用 HSV 颜色区域中心；
        model_intersection：C-U-Net mask 与 B-YOLO-OBB mask 取交集中心。
        """
        try:
            mode_gui = str(self.rule_ac_detection_mode_var.get() or "color_region_center")
            mode_label = "模型交集中心" if mode_gui == "model_intersection" else "颜色检测区域中心"
            self.set_var(self.rule_module_status_var, f"规则模块状态：Step9 {mode_label}对齐测试中")
            self.log(f"========== Step9 单独测试：{mode_label}对齐 ==========")
            ...
            # 重置 Step9 停止状态和 B/颜色跟踪结果。
            wf.step9_stop_requested = False
            wf.reset_step9_tracking_state(clear_b_points=False)
            wf.step9_color_last_mask = None
            wf.step9_color_last_center = None
            mode_now = str(getattr(wf.cfg, "rule_ac_detection_mode", "color_region_center") or "color_region_center").strip().lower()
            is_model_mode = mode_now in ("model_intersection", "models", "unet_yolo_intersection")

            self.log(
                f"[Step9] 参数：mode={mode_now}, target=({wf.cfg.rule_ac_target_x_px}, {wf.cfg.rule_ac_target_y_px}), "
                ...
                f"C-U-Net={getattr(wf.cfg, 'rule_ac_c_unet_model_path', '')}, "
                f"B-YOLO-OBB={getattr(wf.cfg, 'rule_ac_b_yolo_obb_model_path', '')}, "
            ...
            if is_model_mode:
                # 单独测试时提前检查路径，避免进入闭环后才失败。
                wf._step9_resolve_model_path(getattr(wf.cfg, "rule_ac_c_unet_model_path", ""), "Step9 C-U-Net")
                wf._step9_resolve_model_path(getattr(wf.cfg, "rule_ac_b_yolo_obb_model_path", ""), "Step9 B YOLO-OBB")
            else:

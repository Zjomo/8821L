...
DEFAULT_CONFIG["rule_ab_angle_watch_interval_s"] = 0.05
...
class MeasurementConfig:
    ...
    # 角度最终显示/保存统一为 [0,360)；Step7 delta 仍按 180° 轴向差计算。
    # Step7 只判断 abs_axis_delta(current_angle, Step1 baseline) >= rule_ab_angle_delta_min_deg。
    ...
    rule_ab_angle_watch_interval_s: float = float(_cfg("rule_ab_angle_watch_interval_s", 0.05))
...
class MeasurementWorkflow:
    ...
    def angle_diff_deg(a: float, b: float) -> float:
        """
        OBB 长边是无方向轴，阈值判断仍按 180° 周期计算最小轴向差值。

        注意：显示/保存角度已统一为 [0, 360)，但 Step7 的 delta 仍应把
        相差 180° 的同一条长边视作同一方向，避免 OBB 顶点顺序翻转导致误判。
        """
    ...
    def _normalize_angle_0_360(angle_degrees: float) -> float:
        """把任意角度统一归一化到 [0, 360)。用于最终显示、保存、日志。"""
        a = float(angle_degrees) % 360.0
        if a < 0.0:
            a += 360.0
        # 避免 360.000000 这类边界值出现在 GUI/CSV 中。
        if a >= 360.0:
            a -= 360.0
        return float(a)
    ...
    def _yolo_obb_normalize_angle(cls, angle_degrees: float) -> float:
        """YOLO-OBB 最长边角度的最终输出格式：0 <= angle < 360。"""
        return cls._normalize_angle_0_360(angle_degrees)
    ...
    def _normalize_angle_result(result: Any) -> Dict[str, Any]:
        ...
        if isinstance(result, dict):
            # 所有对外显示/保存/判断使用的角度统一为 [0, 360)。
            # 若底层检测函数返回了负角度，这里也会兜底转换，避免 GUI/CSV 中正负混用。
            if result.get("angle_deg") is not None:
                try:
                    normalized_angle = MeasurementWorkflow._normalize_angle_0_360(float(result.get("angle_deg")))
                    result["angle_deg"] = normalized_angle
                    result["angle_deg_raw"] = normalized_angle
                    if result.get("final_angle") is not None:
                        result["final_angle"] = MeasurementWorkflow._normalize_angle_0_360(float(result.get("final_angle")))
                    selected_edge = result.get("selected_b_edge")
                    if isinstance(selected_edge, dict) and selected_edge.get("angle_deg") is not None:
                        selected_edge["angle_deg"] = normalized_angle
                except Exception:

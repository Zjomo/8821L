...
class MeasurementWorkflow:
    ...
    def _point_to_segment_distance_info(
        ...
        return {
            ...
            "nearest_point_xy": [float(qx), float(qy)],
            "segment_t": float(t),
        }
    ...
    def _bmask_edge_angle_candidates_from_mask(
        ...
        for i in range(n):
            ...
            out.append({
                ...
                "mid_xy": [float((float(x1) + float(x2)) / 2.0), float((float(y1) + float(y2)) / 2.0)],
            })
    ...
    def _select_bmask_angle_candidate(
        ...
        prefer_nearest_to_a: bool = True,
    ) -> Optional[Dict[str, Any]]:
        """
        从当前 B mask 的候选边中选择用于完整测量的 B 边角度。

        当前版本的主规则：
            1. 如果能获得 A 中心，则选择“距离 A 中心最近的 B 边”作为当前 B 边角度；
            2. 该角度随后在 Step7 中与上一帧角度比较，若跳变超过
               rule_ab_angle_repair_jump_threshold_deg，继续调用角度修复功能；
            3. 只有在 A 中心不可用时，才回退到旧逻辑：先按 previous_angle，
               再按完整标定的 edge_index，最后选最长边。
        """
        if not candidates:
            return None
        cands = list(candidates)

        if prefer_nearest_to_a and a_center_xy is not None:
            ...
            for c in cands:
                try:
                    ...
                    if p1 is None or p2 is None:
                        continue
                    dist_info = self._point_to_segment_distance_info(a_center_xy, p1, p2)
                    scored.append((float(dist_info.get("distance_px", 1e18)), c, dist_info))
                except Exception:
                    continue
            if scored:
                scored.sort(key=lambda t: t[0])
                _, best, dist_info = scored[0]
                item = dict(best)
                item.update({
                    "selection_reason": "nearest_b_edge_to_a_center",
                    "a_center_xy": [float(a_center_xy[0]), float(a_center_xy[1])],
                    "a_edge_distance_px": float(dist_info.get("distance_px", 0.0)),
                    "nearest_point_on_selected_edge_xy": dist_info.get("nearest_point_xy"),
                    "nearest_point_segment_t": float(dist_info.get("segment_t", 0.0)),
                })
                return item

        # 兜底逻辑：仅当 A 中心不可用/候选边端点异常时使用。
        if previous_angle is not None:
            try:
                prev = float(previous_angle)
                best = min(cands, key=lambda c: self.angle_diff_deg(float(c.get("angle_deg", 0.0)), prev))
                item = dict(best)
                item["selection_reason"] = "fallback_nearest_to_previous_angle"
                return item
            except Exception:
                pass
        ...
        if 0 <= edge_index < len(cands):
            item = dict(cands[edge_index])
            item["selection_reason"] = "fallback_calibration_edge_index"
            return item

        try:
            item = dict(max(cands, key=lambda c: float(c.get("length_px", 0.0))))
            item["selection_reason"] = "fallback_longest_edge"
            return item
        except Exception:
            item = dict(cands[0])
            item["selection_reason"] = "fallback_first_edge"
            return item
    ...
    def measure_rule_ab_bmask_angle_once(
        ...
        try:
            ...
            a_center_for_angle = self._get_scene_center_xy_by_name(scene, "a")
            selected = self._select_bmask_angle_candidate(
                candidates,
                previous_angle=previous_angle,
                a_center_xy=a_center_for_angle,
                prefer_nearest_to_a=True,
            )
            if selected is None:
            ...
            angle_info.update({
                "angle_deg": angle,
                "source": "rule_ab_guarded_bmask_nearest_to_a_center",
                "label": label,
    ...
    def _run_one_c_edge_route_cycle(
        ...
        try:
            ...
            sel_now = self._select_bmask_angle_candidate(
                cand_now,
                previous_angle=previous_angle,
                a_center_xy=a_center,
                prefer_nearest_to_a=True,
            )
            if sel_now is not None:
                ang_now_raw = float(sel_now.get("angle_deg"))
                route_frame_angle_info = dict(sel_now)
                route_frame_angle_info.update({
                    "ok": True,
                    "angle_deg_raw": ang_now_raw,
                    "angle_deg": ang_now_raw,
                    "source": "route_internal_pre_move_guarded_bmask_nearest_to_a_center",
                    "candidate_count": len(cand_now),
                    "candidate_angles": [float(c.get("angle_deg", 0.0)) for c in cand_now],
                })

                ang_now, route_repair_info = self._repair_rule_ab_angle_if_jump(
                    raw_angle=ang_now_raw,
                    previous_angle=previous_angle,
                    baseline_angle=baseline_angle,
                    follower=follower,
                    angle_result=route_frame_angle_info,
                )
                if ang_now is None:
                    ang_now = ang_now_raw
                delta_now = self.angle_diff_deg(float(ang_now), float(baseline_angle)) if baseline_angle is not None else None
                route_frame_angle_info.update({
                    "angle_deg": float(ang_now),
                    "delta_from_baseline": delta_now,
                    "angle_repair": self._json_safe(route_repair_info),
                })
        ...
        try:
            ...
            if not angle_info_for_overlay.get("ok", False) or angle_info_for_overlay.get("angle_deg") is None:
                ...
                sel_for_overlay = self._select_bmask_angle_candidate(
                    cand_for_overlay,
                    previous_angle=previous_angle,
                    a_center_xy=a_center,
                    prefer_nearest_to_a=True,
                ...
                if sel_for_overlay is not None:
                    angle_info_for_overlay.update({
                        ...
                        "source": "rule_ab_guarded_bmask_route_frame_nearest_to_a_fallback",

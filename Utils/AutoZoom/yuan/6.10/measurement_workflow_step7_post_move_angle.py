...
class MeasurementWorkflow:
    ...
    def _select_bmask_angle_candidate(
        ...
        选择顺序：
            1. 如果有 previous_angle，选择最接近 previous_angle 的候选边，保证跨帧跟踪同一条边；
            2. 如果完整标定包里有 angle_edge_index，则按该编号选候选边；
            3. 否则选择最长候选边。
        """
        if not candidates:
            return None
        ...
        if previous_angle is not None:
            try:
                ...
                return min(cands, key=lambda c: self.angle_diff_deg(float(c.get("angle_deg", 0.0)), prev))
            except Exception:
                pass
        ...
        if 0 <= edge_index < len(cands):
            return cands[edge_index]

        try:
            return max(cands, key=lambda c: float(c.get("length_px", 0.0)))
        except Exception:
            return cands[0]
    ...
    def measure_rule_ab_bmask_angle_once(
        ...
        try:
            ...
            selected = self._select_bmask_angle_candidate(candidates, previous_angle=previous_angle)
            if selected is None:
    ...
    def _run_one_c_edge_route_cycle(
        ...
        try:
            ...
            sel_now = self._select_bmask_angle_candidate(cand_now, previous_angle=previous_angle)
        ...
        try:
            ...
            if not angle_info_for_overlay.get("ok", False) or angle_info_for_overlay.get("angle_deg") is None:
                ...
                sel_for_overlay = self._select_bmask_angle_candidate(cand_for_overlay, previous_angle=previous_angle)

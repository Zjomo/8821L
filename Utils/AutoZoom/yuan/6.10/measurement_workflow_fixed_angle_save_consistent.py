...
class MeasurementConfig:
    ...
    # 随后强制执行 A = A - dilate(B)，避免 A mask 把 B 包进去。
...
class MeasurementWorkflow:
    ...
    def _postprocess_scene_ab_masks(self, follower: Any, scene: Any, tag: str = "") -> Dict[str, Any]:
        ...
        规则：
            1. 优先保护 B：用面积、中心点、主轴角度约束当前 B；异常时回退上一帧 B；
            2. 保护 B 后，强制 A = A - dilate(B)，避免 A mask 包含 B；
            3. 修正结果写回 scene.a / scene.b，后续 A 中心、路线控制和角度修复都使用修正后的 mask。
        """
        info: Dict[str, Any] = {

...
DEFAULT_CONFIG["rule_ab_c_edge_route_source"] = "circle"
DEFAULT_CONFIG.setdefault("rule_ab_circle_tangent_quantize_mode", "4way")
DEFAULT_CONFIG.setdefault("rule_ab_circle_tangent_segment_steps", 5)
DEFAULT_CONFIG.setdefault("rule_ab_circle_tangent_preview_segments", 12)
DEFAULT_CONFIG.setdefault("rule_ab_octagon_diagonal_chunk_steps", 3)
DEFAULT_CONFIG.setdefault("rule_ab_octagon_start_angle_deg", 22.5)
...
class MeasurementConfig:
    ...
    # 根据 C mask 的拟合圆预生成 A 的圆周运动路线；Step7 每一帧都让 A 追踪这条路线。
    rule_ab_use_c_edge_route: bool = bool(_cfg("rule_ab_use_c_edge_route", True))
    # Step7 路线来源：
    #   "circle"：默认使用 C mask 拟合圆，生成以 C 圆心为中心的圆周路线；
    #   "quad"：使用 C 近似四边形四条边生成路线；
    #   "sam2_outer_edge"：可选，使用 SAM2 原始 C mask 最大外轮廓像素生成路线。
    rule_ab_c_edge_route_source: str = str(_cfg("rule_ab_c_edge_route_source", "circle"))
    rule_ab_c_edge_route_spacing_px: float = float(_cfg("rule_ab_c_edge_route_spacing_px", 12.0))
    # circle 模式下，圆周切向运动量化为 4 个图像方向（RIGHT/LEFT/UP/DOWN）。
    # 进入安全带后，同一个切向方向连续执行 N 次，再重新检测 A center 并重新选择下一段方向。
    rule_ab_circle_tangent_quantize_mode: str = str(_cfg("rule_ab_circle_tangent_quantize_mode", "4way"))
    rule_ab_circle_tangent_segment_steps: int = int(_cfg("rule_ab_circle_tangent_segment_steps", 5))
    rule_ab_circle_tangent_preview_segments: int = int(_cfg("rule_ab_circle_tangent_preview_segments", 12))
    rule_ab_octagon_diagonal_chunk_steps: int = int(_cfg("rule_ab_octagon_diagonal_chunk_steps", 3))
    rule_ab_octagon_start_angle_deg: float = float(_cfg("rule_ab_octagon_start_angle_deg", 22.5))
    rule_ab_route_execution_segment_px: float = float(_cfg("rule_ab_route_execution_segment_px", 48.0))
    # circle 模式下该字段只保留兼容；实际使用 rule_ab_route_safe_target_distance_px 作为圆路线半径。
    # quad/mask_outer_edge 模式下仍表示外扩安全距离。
    rule_ab_c_edge_route_safe_clearance_px: float = float(_cfg("rule_ab_c_edge_route_safe_clearance_px", 0.0))

    # Step7 圆路线安全带：circle 模式下，A center 到 C 圆心的距离必须保持在 [min, max]。
    # 若距离 < min：沿半径方向远离圆心；若距离 > max：沿半径方向靠近圆心；
    # 只有在安全带内，才沿圆周切向/目标点方向推进。
...
class MeasurementWorkflow:
    def __init__(self, cfg: MeasurementConfig, on_log=None, on_update=None):
        ...
        # Step9 当前/动态 C：旧版兼容字段保留，但当前 Step9 不再主动更新动态 C。
        # Step9 只使用 HSV 颜色区域中心与目标点对齐；C mask 不参与 Step9 检测、方向判断或停止条件。
        self.step9_current_c_mask: Optional[np.ndarray] = None
        self.step9_current_c_raw_mask: Optional[np.ndarray] = None
        self.step9_current_c_center: Optional[Tuple[float, float]] = None
        self.step9_current_c_box: Optional[Tuple[float, float, float, float]] = None
        self.step9_current_c_source: str = ""
        self.step9_current_c_update_reason: str = ""
        self.step9_c_mask_dirty: bool = True
    ...
    @staticmethod
    def _should_save_runtime_debug_image(step_idx: Any, force_save: bool = False, interval: int = 20) -> bool:
        if force_save:
            return True
        try:
            i = int(step_idx)
    ...
    def _complete_calibration_state_for_runtime(self, state: CalibrationState) -> CalibrationState:
        ...
        当前 Step7 不再要求 B 标定点；这里只保留 static C 文件夹规范化。
        """
        if state is None:
            return state
    ...
    def _materialize_full_calibration_c_for_runtime(self, state: CalibrationState) -> CalibrationState:
        ...
        try:
            route_source_rt = str(getattr(self.cfg, "rule_ab_c_edge_route_source", "circle") or "circle").lower().strip()
            if route_source_rt in ("circle", "c_circle", "fit_circle"):
                route_rt = self._build_and_save_circle_edge_route(runtime_dir)
            elif route_source_rt in ("quad", "quadrilateral"):
        ...
        mask_bool_2d = self._mask_to_2d_bool(mask)
        if mask_bool_2d is None:
            raise RuntimeError("完整标定 C mask 不是有效二维/图像 mask，不能生成运行时 C 辅助文件。")
        yx = np.column_stack(np.where(mask_bool_2d))
        try:
    ...
    def _create_labview_tcp_server(self):
        """创建 LabVIEWTCPServer，兼容不同版本的构造函数参数名。"""
        if self._is_virtual_hardware_mode():
    ...
    def _run_angle_detector_once_raw(self, use_second_detect: bool = False, label: str = "yolo_obb") -> Dict[str, Any]:
        ...
        save_yolo_debug_image = True
        m_step7_rt = re.search(r"step7_realtime_frame_(\d+)", safe_label)
        if m_step7_rt:
            save_yolo_debug_image = self._should_save_runtime_debug_image(int(m_step7_rt.group(1)))
        if save_yolo_debug_image:
            cv2.imwrite(str(raw_path), frame_bgr)
        if result.obb is None or len(result.obb) == 0:
            if not save_yolo_debug_image:
                cv2.imwrite(str(raw_path), frame_bgr)
        ...
        raw_path_text = str(raw_path) if save_yolo_debug_image else ""
        overlay_path_text = str(overlay_path) if save_yolo_debug_image else ""
        meta_path_text = str(meta_path) if save_yolo_debug_image else ""
        if save_yolo_debug_image:
            try:
    ...
    def ensure_rule_ab_follower(self) -> ActualNanoBoundaryFollower:
        ...
        # B 标定已移除：没有 B 正点时不安装 ABC feature tracker，避免 init_B_template 卡住 A/C 预检。
    ...
    def prepare_runtime_for_midrun_recalibration_waiting(self) -> None:
        ...
        self.step9_current_c_mask = None
        self.step9_current_c_raw_mask = None
        self.step9_current_c_center = None
        self.step9_current_c_box = None
        self.step9_current_c_source = ""
        self.step9_current_c_update_reason = ""
        self.step9_c_mask_dirty = True
    ...
    @staticmethod
    def _to_axis_aligned_execution_polyline(
        points: List[Tuple[float, float]],
        mode: str = "hv",
        segment_px: float = 48.0,
    ) -> List[Tuple[float, float]]:
        """把闭合路径转换成分段水平/垂直执行路径，避免每个采样点都产生锯齿。"""
        if not points:
            return []
        ...
        for p in points:
            try:
                ...
                continue
            if not src or math.hypot(xy[0] - src[-1][0], xy[1] - src[-1][1]) >= 0.5:
                src.append(xy)
        if len(src) <= 1:
            return src

        use_vh = str(mode or "").lower().strip() == "vh"
        seg_px = max(4.0, float(segment_px or 48.0))

        vertices: List[Tuple[float, float]] = []
        n_src = len(src)
        for i, p in enumerate(src):
            prev_p = src[(i - 1) % n_src]
            next_p = src[(i + 1) % n_src]
            v1x, v1y = float(p[0] - prev_p[0]), float(p[1] - prev_p[1])
            v2x, v2y = float(next_p[0] - p[0]), float(next_p[1] - p[1])
            n1 = math.hypot(v1x, v1y)
            n2 = math.hypot(v2x, v2y)
            if n1 <= 1e-6 or n2 <= 1e-6:
                continue
            cross = abs(v1x * v2y - v1y * v2x)
            sin_angle = cross / max(n1 * n2, 1e-9)
            if sin_angle >= math.sin(math.radians(8.0)):
                vertices.append(p)
        ...
        if len(vertices) < 3:
            ...
            for i, p in enumerate(src):
                q = src[(i + 1) % len(src)]
                total_len += math.hypot(float(q[0] - p[0]), float(q[1] - p[1]))
            keep_step = max(1, int(round(seg_px / max(1.0, total_len / max(1, len(src))))))
            vertices = src[::keep_step]
            if len(vertices) < 3:
                vertices = src

        out: List[Tuple[float, float]] = []

        def _append(xy: Tuple[float, float]) -> None:
            if not out or math.hypot(float(xy[0]) - out[-1][0], float(xy[1]) - out[-1][1]) >= 0.5:
        ...
        for i, p0 in enumerate(vertices):
            ...
            dx = float(p1[0] - p0[0])
            dy = float(p1[1] - p0[1])
            chunks = max(1, int(math.ceil(max(abs(dx), abs(dy)) / seg_px)))
            prev_xy = p0
            for k in range(1, chunks + 1):
                t = float(k) / float(chunks)
                target = (float(p0[0]) + dx * t, float(p0[1]) + dy * t)
                if abs(target[0] - prev_xy[0]) > 0.5 and abs(target[1] - prev_xy[1]) > 0.5:
                    step_vh = use_vh if (k % 2 == 1) else (not use_vh)
                    corner = (float(prev_xy[0]), float(target[1])) if step_vh else (float(target[0]), float(prev_xy[1]))
                    _append(corner)
                _append(target)
                prev_xy = target
            _append(p1)

        if len(out) > 1 and math.hypot(out[-1][0] - out[0][0], out[-1][1] - out[0][1]) < 0.5:
            out.pop()
        return out

    @staticmethod
    def _load_c_edge_route_from_dir(c_dir: str) -> List[Tuple[float, float]]:
        """从 C 标定目录读取 static_c_edge_route.json 的实际执行目标线。"""
        if not c_dir:
            return []
        ...
        with route_path.open("r", encoding="utf-8") as f:
            data = json.load(f)
        pts = (
            data.get("execution_route_points_xy")
            or data.get("execution_target_route_points_xy")
            or data.get("route_points_xy")
            or data.get("target_route_points_xy")
            or data.get("points_xy")
            or []
        )
        out: List[Tuple[float, float]] = []
        for p in pts:
    ...
    def _load_c_edge_route_band_from_dir(c_dir: str) -> Dict[str, Any]:
        ...
        out: Dict[str, Any] = {
            ...
            "route_points_xy": [],
            "max_route_points_xy": [],
            "execution_base_route_points_xy": [],
            "execution_min_route_points_xy": [],
            "execution_route_points_xy": [],
            "execution_max_route_points_xy": [],
        }
        if not c_dir:
        ...
        execution_target = _pts("execution_route_points_xy") or _pts("execution_target_route_points_xy") or target
        execution_base = _pts("execution_base_route_points_xy") or base
        execution_min = _pts("execution_min_route_points_xy") or min_route
        execution_max = _pts("execution_max_route_points_xy") or max_route
        out.update({
            "ok": bool(execution_target),
            "source": data.get("source", ""),
            "route_geometry_mode": data.get("route_geometry_mode", ""),
            "execution_route_mode": data.get("execution_route_mode", ""),
            "safety_reference": data.get("safety_reference", ""),
            "circle_center_xy": data.get("circle_center_xy", None),
            "circle_base_radius_px": data.get("circle_base_radius_px", None),
            "circle_fit_debug": data.get("circle_fit_debug", {}),
            ...
            "execution_base_route_points_xy": execution_base,
            "execution_min_route_points_xy": execution_min,
            "execution_route_points_xy": execution_target,
            "execution_max_route_points_xy": execution_max,
        })
    ...
    def _load_or_build_step7_c_edge_route(self, follower: Any) -> List[Tuple[float, float]]:
        ...
        if route_source not in ("circle", "c_circle", "fit_circle", "sam2_outer_edge", "mask_outer_edge", "sam2_mask", "quad", "quadrilateral"):
            route_source = "circle"
        ...
        if route_path.exists():
            try:
                ...
                existing_route_geometry_mode = str(route_data.get("route_geometry_mode", "") or "")
                existing_execution_route_mode = str(route_data.get("execution_route_mode", "") or "")
                route = self._load_c_edge_route_from_dir(str(c_path))

                source_ok = False
                if route_source in ("circle", "c_circle", "fit_circle"):
                    source_ok = ("circle" in existing_source) and str(route_data.get("safety_reference", "") or "") == "circle_center_radius"
                elif route_source in ("quad", "quadrilateral"):
                ...
                band_param_ok = True
                if route_source in ("quad", "quadrilateral", "circle", "c_circle", "fit_circle"):
                    band_param_ok = (
                ...
                if route_source in ("quad", "quadrilateral", "circle", "c_circle", "fit_circle"):
                    band_ok = bool(
                        ...
                        and route_data.get("execution_base_route_points_xy")
                        and route_data.get("execution_min_route_points_xy")
                        and route_data.get("execution_route_points_xy")
                        and route_data.get("execution_max_route_points_xy")
                    )
                geometry_mode_ok = True
                if route_source in ("circle", "c_circle", "fit_circle"):
                    geometry_mode_ok = (
                        existing_route_geometry_mode == "circle"
                        and existing_execution_route_mode == "octagon_state_machine"
                        and int(route_data.get("octagon_vertex_count", 0) or 0) == 8
                    )
                elif route_source in ("quad", "quadrilateral"):
                    geometry_mode_ok = (
                        existing_route_geometry_mode == "mitered_rectangle"
                        and existing_execution_route_mode == "axis_aligned_hv_chunked_manhattan"
                    )

                if route and source_ok and clearance_ok and spacing_ok and follow_ok and band_ok and band_param_ok and geometry_mode_ok:
        ...
        if route_source in ("circle", "c_circle", "fit_circle"):
            route = self._build_and_save_circle_edge_route(c_path)
            if not route:
                self.log("[RuleAB-Route] C mask 拟合圆路线生成失败，最后回退到四边形路线。")
    ...
    @staticmethod
    def _generate_circle_route_points(
        ...
        """按圆心和半径生成闭合圆周采样点。follow_direction>=0 为视觉逆时针，<0 为视觉顺时针。"""
        cx, cy = float(center_xy[0]), float(center_xy[1])
        r = max(1.0, float(radius_px))
        spacing = max(1.0, float(spacing_px))
        n = max(16, int(math.ceil((2.0 * math.pi * r) / spacing)))
        # 图像坐标 y 向下；theta 递增在屏幕上表现为顺时针，所以 follow>=0 时取负向。
        ...
        for i in range(n):
            theta = sign * (2.0 * math.pi * float(i) / float(n))
    ...
    def _fit_quadrilateral_from_mask(mask_bool: np.ndarray) -> Tuple[np.ndarray, np.ndarray, str]:
        """
        将 C mask 近似为四边形，并生成四边形 mask。

        先尝试 approxPolyDP 得到 4 点；失败时退回凸包近似；仍失败则用最小外接旋转矩形。
        """
        c_bool = np.asarray(mask_bool).astype(bool)
        if c_bool.ndim != 2 or c_bool.size <= 0 or not np.any(c_bool):
            raise RuntimeError("C mask 为空，无法拟合四边形。")
        h, w = c_bool.shape[:2]
        ...
        quad_points = MeasurementWorkflow._order_quad_points_clockwise(quad_points)
        ...
        if int(np.count_nonzero(quad_mask)) <= 0:
    ...
    def _load_preferred_c_mask_for_route_shape(self, c_path: Path) -> Tuple[Optional[np.ndarray], Optional[Path]]:
    ...
    def _build_and_save_quad_edge_route(self, c_path: Path) -> List[Tuple[float, float]]:
        ...
        try:
            ...
            execution_segment_px = max(
                spacing * 2.0,
                float(getattr(self.cfg, "rule_ab_route_execution_segment_px", 48.0)),
            )
            execution_base_route = self._to_axis_aligned_execution_polyline(base_route, mode="hv", segment_px=execution_segment_px)
            execution_min_route = self._to_axis_aligned_execution_polyline(min_route, mode="hv", segment_px=execution_segment_px)
            execution_route = self._to_axis_aligned_execution_polyline(route, mode="hv", segment_px=execution_segment_px)
            execution_max_route = self._to_axis_aligned_execution_polyline(max_route, mode="hv", segment_px=execution_segment_px)
            ...
            with (c_path / "static_c_edge_route.json").open("w", encoding="utf-8") as f:
                json.dump({
                    ...
                    "route_geometry_mode": "mitered_rectangle",
                    "execution_route_mode": "axis_aligned_hv_chunked_manhattan",
                    "execution_segment_px": float(execution_segment_px),
                    ...
                    "target_route_points_xy": [[float(x), float(y)] for x, y in route],
                    "max_route_points_xy": [[float(x), float(y)] for x, y in max_route],
                    "execution_base_route_points_xy": [[float(x), float(y)] for x, y in execution_base_route],
                    "execution_min_route_points_xy": [[float(x), float(y)] for x, y in execution_min_route],
                    "execution_route_points_xy": [[float(x), float(y)] for x, y in execution_route],
                    "execution_target_route_points_xy": [[float(x), float(y)] for x, y in execution_route],
                    "execution_max_route_points_xy": [[float(x), float(y)] for x, y in execution_max_route],
                    "note": route_shape_note + " Step7 三线安全带：route_points_xy 是拟合矩形可视化路径；execution_*_points_xy 是控制器实际分段水平/垂直执行路径，避免每个采样点频繁转向。A 在 execution_min/execution_max 之间时只沿 execution_route 推进，越界才法向修正。",
                }, f, ensure_ascii=False, indent=2)

            # CSV 保存实际执行目标线，便于复现实验动作。
            try:
                with (c_path / "static_c_edge_route.csv").open("w", encoding="utf-8-sig", newline="") as f:
                    writer = csv.writer(f)
                    writer.writerow(["index", "x", "y"])
                    for idx0, (x, y) in enumerate(execution_route):
                        writer.writerow([idx0, float(x), float(y)])
            except Exception:
                pass
    ...
    def _generate_c_edge_route_points_from_quad(
        ...
        for i in range(4):
            edge = pts[(i + 1) % 4] - pts[i]
            length = float(np.linalg.norm(edge))
            if length <= 1e-6:
                return []
            unit_edge = edge.astype(np.float32) / length
            n1 = np.array([-unit_edge[1], unit_edge[0]], dtype=np.float32)
            n2 = -n1
            mid = 0.5 * (pts[i] + pts[(i + 1) % 4])
            normal = n1 if float(np.linalg.norm((mid + n1) - center)) >= float(np.linalg.norm((mid + n2) - center)) else n2
        ...
        def _line_intersection(p: np.ndarray, r: np.ndarray, q: np.ndarray, s: np.ndarray) -> Optional[np.ndarray]:
            denom = _cross2(r, s)
            if abs(denom) <= 1e-8:
                return None
        ...
        # 关键：先求四条外扩边的相邻交点，得到真正的矩形/四边形安全区；
        # 不再把四条单独外扩边直接相连，否则角上会出现斜切边，看起来像八边形。
        offset_pts: List[np.ndarray] = []
        for i in range(4):
            prev_i = (i - 1) % 4
            prev_p = pts[prev_i] + clearance * normals[prev_i]
            cur_p = pts[i] + clearance * normals[i]
            inter = _line_intersection(prev_p, edges[prev_i], cur_p, edges[i])
            if inter is None:
                avg = normals[prev_i] + normals[i]
                norm = float(np.linalg.norm(avg))
                avg = avg / norm if norm > 1e-6 else normals[i]
                inter = pts[i] + clearance * avg
            if image_shape is not None:
                h, w = int(image_shape[0]), int(image_shape[1])
                inter[0] = np.clip(inter[0], 0, max(0, w - 1))
                inter[1] = np.clip(inter[1], 0, max(0, h - 1))
            offset_pts.append(inter.astype(np.float32))
    ...
    def _save_step7_route_overlay_image(
        ...
        draw_route_points: Optional[List[Tuple[float, float]]] = None,
        draw_min_route_points: Optional[List[Tuple[float, float]]] = None,
        draw_max_route_points: Optional[List[Tuple[float, float]]] = None,
        draw_base_route_points: Optional[List[Tuple[float, float]]] = None,
        execution_route_mode: str = "",
        execution_preview_points: Optional[List[Tuple[float, float]]] = None,
        circle_segment_action_name: str = "",
        circle_segment_remaining: Optional[int] = None,
    ) -> str:
        """
        ...
        try:
            ...
            # 先画拟合矩形可视化路径，再画控制器实际水平/垂直执行路径。
            for _pts, _color, _thick in (
                (draw_base_route_points, (90, 90, 90), 1),
                (draw_min_route_points, (40, 150, 40), 1),
                (draw_max_route_points, (40, 40, 170), 1),
                (draw_route_points, (130, 40, 130), 1),
            ):
                if _pts:
            ...
            # 三线执行安全带可视化：base=灰色C边界，min=绿色内边界，target=紫色中线，max=红色外边界。
            for _pts, _color, _thick in (
            ...
            # circle_4way_segmented：绘制 4 方向实际执行路径预览（阶梯状折线）。
            if execution_preview_points:
                try:
                    arr = np.asarray(execution_preview_points, dtype=np.float32).reshape(-1, 2)
                    arr[:, 0] = np.clip(arr[:, 0], 0, max(0, w - 1))
                    arr[:, 1] = np.clip(arr[:, 1], 0, max(0, h - 1))
                    arr_i32 = np.round(arr).astype(np.int32).reshape((-1, 1, 2))
                    cv2.polylines(canvas, [arr_i32], isClosed=False, color=(0, 255, 255), thickness=3)
                    for j, pxy in enumerate(execution_preview_points[:8]):
                        cv2.circle(canvas, _pt(pxy), 4 if j == 0 else 3, (0, 255, 255), -1)
                    cv2.putText(canvas, "4way segmented preview", (_pt(execution_preview_points[0])[0] + 8, _pt(execution_preview_points[0])[1] + 34), cv2.FONT_HERSHEY_SIMPLEX, 0.48, (0, 255, 255), 2, cv2.LINE_AA)
                except Exception:
                    pass
            ...
            lines = [
                f"Step7 execution route: step={step_idx}, cur_idx={route_index}/{max(0, len(route_points)-1)}, target_idx={target_idx}, {loop_text}",
                ...
                f"route_signed_area={area_text}, mode={target_mode}{osc_text}, execution={execution_route_mode or 'legacy'}",
                f"route_distance_px={route_dist_text}, error_dist={err_text},{safe_text}",
                f"circle_segment_action={circle_segment_action_name or 'None'}, remaining={circle_segment_remaining if circle_segment_remaining is not None else 'None'}",
    ...
    @staticmethod
    def _action_id_to_unit_vec(action_id: int) -> Tuple[float, float]:
        """action_id -> 图像坐标单位方向。0 STAY, 1 UP, 2 DOWN, 3 LEFT, 4 RIGHT。"""
        aid = int(action_id)
        if aid == 1:
            return (0.0, -1.0)
        if aid == 2:
    ...
    @staticmethod
    def _quantize_vec_to_4way_action(vx: float, vy: float) -> int:
        """把任意二维向量量化成 4 个图像方向：RIGHT/LEFT/UP/DOWN。"""
        vx = float(vx)
        vy = float(vy)
        if abs(vx) <= 1e-9 and abs(vy) <= 1e-9:
            return 0
        if abs(vx) >= abs(vy):
            return 4 if vx > 0 else 3
        return 2 if vy > 0 else 1

    @staticmethod
    def _circle_tangent_vec_from_a_center(
        a_center: Tuple[float, float],
        circle_center_xy: Tuple[float, float],
        follow_direction: int,
    ) -> Tuple[float, float]:
        """
        根据 A center 相对 C 圆心的半径方向，计算圆周切向单位向量。
        图像坐标 y 向下：follow_direction>=0 表示视觉逆时针；<0 表示视觉顺时针。
        """
        ax, ay = float(a_center[0]), float(a_center[1])
        cx, cy = float(circle_center_xy[0]), float(circle_center_xy[1])
        rx, ry = ax - cx, ay - cy
        rn = math.hypot(rx, ry)
        if rn <= 1e-9:
            return (1.0, 0.0)
        ...
        if int(follow_direction) >= 0:
            ...
            tx, ty = -uy, ux
        tn = math.hypot(tx, ty)
        return (tx / max(tn, 1e-9), ty / max(tn, 1e-9))

    @classmethod
    def _generate_circle_4way_execution_preview(
        cls,
        ...
        """生成 4 方向量化切向分段路径预览：每段同一方向走 segment_steps 次。"""
        pts: List[Tuple[float, float]] = [(float(start_xy[0]), float(start_xy[1]))]
        cur = (float(start_xy[0]), float(start_xy[1]))
        seg_n = max(1, int(segment_steps))
        n_seg = max(1, int(preview_segments))
        step = max(1.0, float(step_px))
        for _ in range(n_seg):
            tvx, tvy = cls._circle_tangent_vec_from_a_center(cur, circle_center_xy, follow_direction)
            aid = cls._quantize_vec_to_4way_action(tvx, tvy)
            ux, uy = cls._action_id_to_unit_vec(aid)
            cur = (float(cur[0]) + ux * step * seg_n, float(cur[1]) + uy * step * seg_n)
            pts.append(cur)
        return pts
    ...
    def _route_safety_target_from_distance(
        ...
        a_center: Tuple[float, float],
        route_points: List[Tuple[float, float]],
        route_band: Optional[Dict[str, Any]] = None,
    ) -> Dict[str, Any]:
        """
        ...
        # 圆路线模式：安全距离定义为 C 圆心到 A center 的距离，而不是 A 到路线最近点的距离。
        rb = route_band if isinstance(route_band, dict) else {}
        if str(rb.get("safety_reference", "") or "") == "circle_center_radius" and rb.get("circle_center_xy") is not None:
            try:
                cc = rb.get("circle_center_xy")
                ...
                if dist < min_d:
                    mode = "too_close_to_circle_center_move_out"
                    should = True
                elif dist > max_d:
                    mode = "too_far_from_circle_center_move_in"
                    should = True
                else:
                    mode = "inside_circle_center_safe_radius_band"
                    should = False
                safe_target = (cx + ux * target_d, cy + uy * target_d)
                nearest_on_target_circle = (cx + ux * target_d, cy + uy * target_d)
                return {
                    "ok": True,
                    ...
                    "circle_center_xy": [float(cx), float(cy)],
                    "circle_base_radius_px": float(rb.get("circle_base_radius_px", 0.0) or 0.0),
                    "nearest_route_xy": [float(nearest_on_target_circle[0]), float(nearest_on_target_circle[1])],
    ...
    def _run_one_c_edge_route_cycle(
        ...
        draw_base_route_points = _band_pts("base_route_points_xy")
        draw_min_route_points = _band_pts("min_route_points_xy")
        draw_route_points = _band_pts("route_points_xy")
        draw_max_route_points = _band_pts("max_route_points_xy")
        base_route_points = _band_pts("execution_base_route_points_xy") or draw_base_route_points or route_points
        min_route_points = _band_pts("execution_min_route_points_xy") or draw_min_route_points
        max_route_points = _band_pts("execution_max_route_points_xy") or draw_max_route_points
        execution_route_mode = str(band_info.get("execution_route_mode", "") if isinstance(band_info, dict) else "")

        safety_info = self._route_safety_target_from_distance(a_center, base_route_points, route_band=band_info)
        ...
        is_circle_4way = bool(
            str(band_info.get("safety_reference", "") if isinstance(band_info, dict) else "") == "circle_center_radius"
            and str(execution_route_mode).lower().strip() in ("circle_4way_segmented", "circle_route_points", "")
            and isinstance(band_info, dict)
            and band_info.get("circle_center_xy") is not None
        )
        is_octagon_state_machine = bool(
            str(band_info.get("safety_reference", "") if isinstance(band_info, dict) else "") == "circle_center_radius"
            and str(execution_route_mode).lower().strip() == "octagon_state_machine"
            and isinstance(band_info, dict)
            and band_info.get("circle_center_xy") is not None
            and n_route == 8
        )
        circle_center_xy: Optional[Tuple[float, float]] = None
        if is_circle_4way or is_octagon_state_machine:
            try:
                cc0 = band_info.get("circle_center_xy")
                circle_center_xy = (float(cc0[0]), float(cc0[1]))
            except Exception:
                circle_center_xy = None
                is_circle_4way = False
                is_octagon_state_machine = False
        ...
        circle_segment_action_name = ""
        circle_segment_remaining: Optional[int] = None
        execution_preview_points: Optional[List[Tuple[float, float]]] = None
        forced_action_id: Optional[int] = None
        octagon_segment_type = ""
        octagon_segment_index: Optional[int] = None
        octagon_diag_axis = ""
        octagon_diag_remaining: Optional[int] = None

        if is_octagon_state_machine and circle_center_xy is not None:
            action_name_map_local = {0: "STAY", 1: "UP", 2: "DOWN", 3: "LEFT", 4: "RIGHT"}
            chunk_steps = max(1, int(getattr(self.cfg, "rule_ab_octagon_diagonal_chunk_steps", 3)))
            final_step_px = max(float(getattr(self.cfg, "rule_ab_c_edge_route_spacing_px", 12.0)), tol * 2.0, 10.0)
            execution_preview_points = list(route_points) + ([route_points[0]] if loop_route and route_points else [])

            def _segment_actions(idx0: int) -> Tuple[int, int, str]:
                idx1 = _advance_index(idx0, direction)
                p0 = route_points[idx0]
                p1 = route_points[idx1]
                sx = float(p1[0] - p0[0])
                sy = float(p1[1] - p0[1])
                x_aid = 4 if sx > 0 else 3
                y_aid = 2 if sy > 0 else 1
                if abs(sx) < max(1.0, tol * 0.25):
                    return 0, y_aid, "vertical"
                if abs(sy) < max(1.0, tol * 0.25):
                    return x_aid, 0, "horizontal"
                return x_aid, y_aid, "diagonal"

            # 到达当前段终点容差后切下一段；不要求精确到达顶点。
            if target_distance_px <= tol:
                current_route_index = _advance_index(current_route_index, direction)
                route_state["current_route_index"] = int(current_route_index)
                route_state["last_advanced_step"] = int(step_idx)
                route_state.pop("octagon_diag_axis", None)
                route_state.pop("octagon_diag_remaining", None)
                advanced_this_step = True
                target_idx = _advance_index(current_route_index, direction)
                current_target_xy = (float(route_points[target_idx][0]), float(route_points[target_idx][1]))
                target_distance_px = float(math.hypot(float(current_target_xy[0] - a_center[0]), float(current_target_xy[1] - a_center[1])))

            octagon_segment_index = int(current_route_index)
            x_aid, y_aid, octagon_segment_type = _segment_actions(current_route_index)
            if safety_state in ("too_close", "too_far"):
                forced_action_id = self._quantize_vec_to_4way_action(float(normal_correction_vec[0]), float(normal_correction_vec[1]))
                image_action_prefix = "SAFE_"
                target_mode = f"octagon_state_machine_safe_{safety_state}_radial"
                route_state.pop("octagon_diag_axis", None)
                route_state.pop("octagon_diag_remaining", None)
            elif octagon_segment_type == "horizontal":
                forced_action_id = int(x_aid)
                target_mode = "octagon_state_machine_horizontal"
                route_state.pop("octagon_diag_axis", None)
                route_state.pop("octagon_diag_remaining", None)
            elif octagon_segment_type == "vertical":
                forced_action_id = int(y_aid)
                target_mode = "octagon_state_machine_vertical"
                route_state.pop("octagon_diag_axis", None)
                route_state.pop("octagon_diag_remaining", None)
            else:
                prev_seg = route_state.get("octagon_segment_index")
                if prev_seg != int(current_route_index):
                    route_state["octagon_diag_axis"] = "x"
                    route_state["octagon_diag_remaining"] = int(chunk_steps)
                axis = str(route_state.get("octagon_diag_axis", "x") or "x")
                rem = int(route_state.get("octagon_diag_remaining", chunk_steps) or chunk_steps)
                forced_action_id = int(x_aid if axis == "x" else y_aid)
                rem -= 1
                if rem <= 0:
                    axis = "y" if axis == "x" else "x"
                    rem = int(chunk_steps)
                route_state["octagon_diag_axis"] = axis
                route_state["octagon_diag_remaining"] = int(rem)
                octagon_diag_axis = axis
                octagon_diag_remaining = int(rem)
                target_mode = f"octagon_state_machine_diagonal_chunk_{chunk_steps}"

            route_state["octagon_segment_index"] = int(current_route_index)
            ux_sm, uy_sm = self._action_id_to_unit_vec(int(forced_action_id or 0))
            final_move_target_xy = (float(a_center[0]) + ux_sm * final_step_px, float(a_center[1]) + uy_sm * final_step_px)
            circle_segment_action_name = action_name_map_local.get(int(forced_action_id or 0), "STAY")
            circle_segment_remaining = octagon_diag_remaining
            current_target_xy = (float(route_points[_advance_index(current_route_index, direction)][0]), float(route_points[_advance_index(current_route_index, direction)][1]))
            target_idx = _advance_index(current_route_index, direction)

        if (not is_octagon_state_machine) and is_circle_4way and circle_center_xy is not None:
            # 圆路线实际执行：安全带内不再追逐每个圆周采样点，而是把切向方向量化为
            # RIGHT/LEFT/UP/DOWN。一个方向连续执行 N 次；N 次后用新的 A center 重算下一段。
            seg_steps = max(1, int(getattr(self.cfg, "rule_ab_circle_tangent_segment_steps", 5)))
            final_step_px_for_preview = max(float(getattr(self.cfg, "rule_ab_c_edge_route_spacing_px", 12.0)), tol * 2.0, 10.0)
            execution_preview_points = self._generate_circle_4way_execution_preview(
                start_xy=a_center,
                circle_center_xy=circle_center_xy,
                follow_direction=follow_direction,
                step_px=final_step_px_for_preview,
                segment_steps=seg_steps,
                preview_segments=max(1, int(getattr(self.cfg, "rule_ab_circle_tangent_preview_segments", 12))),
            )

            if severe_safety and safe_target_xy is not None:
                # 严重越界时，仍然每帧优先径向回安全带，不沿用旧切向段。
                route_state.pop("circle_segment_action_id", None)
                route_state.pop("circle_segment_remaining", None)
                target_mode = f"circle_4way_hard_safe_{safety_state}_radial_only"
                mvx, mvy = normal_correction_vec
                image_action_prefix = "SAFE_"
                final_step_px = final_step_px_for_preview
                final_move_target_xy = (float(a_center[0]) + float(mvx) * final_step_px, float(a_center[1]) + float(mvy) * final_step_px)
            elif safety_state == "inside":
                # 安全带内：同一 4 方向切向动作连续执行 seg_steps 次，再重算下一段。
                action_name_map_local = {0: "STAY", 1: "UP", 2: "DOWN", 3: "LEFT", 4: "RIGHT"}
                rem = int(route_state.get("circle_segment_remaining", 0) or 0)
                aid = int(route_state.get("circle_segment_action_id", 0) or 0)
                if rem <= 0 or aid == 0:
                    tvx2, tvy2 = self._circle_tangent_vec_from_a_center(a_center, circle_center_xy, follow_direction)
                    aid = self._quantize_vec_to_4way_action(tvx2, tvy2)
                    rem = seg_steps
                    route_state["circle_segment_action_id"] = int(aid)
                    route_state["circle_segment_total_steps"] = int(seg_steps)
                    route_state["circle_segment_start_step"] = int(step_idx)
                ux2, uy2 = self._action_id_to_unit_vec(aid)
                route_state["circle_segment_remaining"] = int(max(0, rem - 1))
                circle_segment_remaining = int(route_state.get("circle_segment_remaining", 0) or 0)
                circle_segment_action_name = action_name_map_local.get(int(aid), f"ACTION_{aid}")
                target_mode = f"circle_4way_segmented_inside_{circle_segment_action_name}_repeat_{seg_steps}"
                image_action_prefix = ""
                final_step_px = final_step_px_for_preview
                final_move_target_xy = (float(a_center[0]) + float(ux2) * final_step_px, float(a_center[1]) + float(uy2) * final_step_px)
                # 让路线索引用当前 A 的角度位置同步推进；不再依赖 A 命中离散 route point。
                try:
                    route_state["current_route_index"] = int(_nearest_route_index_by_point(a_center))
                    current_route_index = int(route_state["current_route_index"])
                    target_idx = _advance_index(current_route_index, direction * lookahead)
                    current_target_xy = (float(route_points[target_idx][0]), float(route_points[target_idx][1]))
                except Exception:
                    pass
            else:
                # 轻微越界：本帧优先做径向修正，下一帧再判断是否回到安全带并继续切向段。
                route_state.pop("circle_segment_action_id", None)
                route_state.pop("circle_segment_remaining", None)
                target_mode = f"circle_4way_soft_safe_{safety_state}_radial"
                mvx, mvy = normal_correction_vec
                image_action_prefix = "SAFE_"
                final_step_px = final_step_px_for_preview
                final_move_target_xy = (float(a_center[0]) + float(mvx) * final_step_px, float(a_center[1]) + float(mvy) * final_step_px)
        ...
        action_id = int(forced_action_id) if forced_action_id is not None else self._route_action_from_error(dx, dy, tol)
        ...
        if (is_circle_4way or is_octagon_state_machine) and circle_segment_action_name and image_action_prefix == "":
            # 确保日志显示实际 4way 切向段动作，而不是由浮点目标误差二次推导出的名字。
            image_action = circle_segment_action_name
        ...
        save_step7_image = self._should_save_runtime_debug_image(step_idx, force_save=bool(action_id == 0))
        ...
        if save_step7_image:
            ...
            draw_route_points=draw_route_points,
            draw_min_route_points=draw_min_route_points,
            draw_max_route_points=draw_max_route_points,
            draw_base_route_points=draw_base_route_points,
            execution_route_mode=execution_route_mode,
            execution_preview_points=execution_preview_points,
            circle_segment_action_name=circle_segment_action_name,
            circle_segment_remaining=circle_segment_remaining,
        )
        ...
        info = {
            ...
            "execution_route_mode": str(execution_route_mode),
            "circle_4way_enabled": bool(is_circle_4way),
            "octagon_state_machine_enabled": bool(is_octagon_state_machine),
            "octagon_segment_index": int(octagon_segment_index) if octagon_segment_index is not None else None,
            "octagon_segment_type": str(octagon_segment_type),
            "octagon_diag_axis_next": str(octagon_diag_axis),
            "octagon_diag_remaining": int(octagon_diag_remaining) if octagon_diag_remaining is not None else None,
            "circle_segment_action_name": str(circle_segment_action_name),
            "circle_segment_remaining": int(circle_segment_remaining) if circle_segment_remaining is not None else None,
            "draw_route_len": int(len(draw_route_points)) if draw_route_points else 0,
            ...
            "route_band_source": "static_calibration_c_route",
            "route_distance_px": float(route_distance_px) if route_distance_px is not None else None,
            "safe_band": [float(min_d), float(max_d)],
        ...
        self.log(
            ...
            f"route_band_source={info['route_band_source']}, "
            ...
            f"image_action={image_action}, octagon_segment={octagon_segment_index}, "
            f"octagon_type={octagon_segment_type or 'None'}, octagon_diag_next={octagon_diag_axis or 'None'}, "
            f"circle_segment={circle_segment_action_name or 'None'}, "
            f"segment_remaining={circle_segment_remaining if circle_segment_remaining is not None else 'None'}, "
            f"stage_action={stage_action}, oscillation_detected={oscillation_detected}"
        )
        return True, info
    ...
    def _ensure_rule_ab_feature_tracker_installed(self, follower: Any, state: Optional[CalibrationState] = None) -> None:
        ...
        if not bool(getattr(self.cfg, "rule_ab_use_feature_tracker_after_sam2_init", True)):
            return
        if not self._state_has_b_calibration_points(state or self._get_loaded_calibration_state()):
            return
    ...
    def _state_has_b_calibration_points(self, state: Optional[CalibrationState] = None) -> bool:
        """当前版本允许不标定 B；此函数只用于决定是否启用旧 B 模板跟踪链路。"""
        if state is None:
            state = self._get_loaded_calibration_state()
        if state is None:
            return False
        return bool(
            self._normalize_points_xy(getattr(state, "rule_ab_b_positive_points", []))
            or self._normalize_points_xy(getattr(state, "angle_b_positive_points", []))
        )

    def _save_ac_feature_templates_without_b(self, follower: Any, state: Optional[CalibrationState] = None) -> Dict[str, str]:
        """
        B 标定禁用时仍保存 A/C 初始模板文件。

        这会生成 abc_feature_templates 下的：
            a_sam2_initial_mask_cleaned.png
            c_sam2_initial_mask_cleaned.png
        但不会安装依赖 B mask / init_B_template 的运行时 feature tracker。
        """
        seg = getattr(follower, "abc_segmenter", None) or getattr(follower, "segmenter", None)
        if seg is None:
            self.log("[A/C-only] 保存 A/C 模板失败：follower 中没有 abc_segmenter/segmenter。")
            return {}

        image_rgb = getattr(seg, "init_image_rgb", None)
        if image_rgb is None:
            image_rgb = getattr(seg, "last_image_rgb", None)
        if image_rgb is None:
            image_rgb = getattr(follower, "last_image_rgb", None)
        if image_rgb is None:
            self.log("[A/C-only] 保存 A/C 模板失败：没有可用首帧图像。")
            return {}
        image_rgb = np.asarray(image_rgb).copy()

        a_raw = getattr(seg, "init_a_mask", None)
        if a_raw is None:
            a_raw = getattr(seg, "last_a_mask", None)
        c_raw = getattr(seg, "static_c_mask", None)
        if c_raw is None:
            c_raw = getattr(seg, "last_c_mask", None)

        a_raw = self._mask_to_2d_bool(a_raw, target_shape=image_rgb.shape[:2])
        c_raw = self._mask_to_2d_bool(c_raw, target_shape=image_rgb.shape[:2])
        if a_raw is None or int(np.count_nonzero(a_raw)) <= 0:
            self.log("[A/C-only] 保存 A/C 模板失败：A raw 初始 mask 为空。")
            return {}
        if c_raw is None or int(np.count_nonzero(c_raw)) <= 0:
            self.log("[A/C-only] 保存 A/C 模板失败：C raw/static mask 为空。")
            return {}
        ...
        clean_debug = {
            "created_at": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
            "mode": "ac_only_no_b_template",
            "labels": {},
        }

        c_pts = positive_points.get("c", [])
        if c_pts:
            c_clean, c_info = self._select_initial_component_by_positive_points(c_raw, c_pts, "c", min_area)
            if not np.any(c_clean):
                self.log(f"[A/C-only] C 正点连通域清理为空，使用 C raw/static mask；debug={c_info}")
                c_clean = c_raw.astype(bool)
        else:
            c_clean = c_raw.astype(bool)
            c_info = {"mode": "no_c_positive_points_use_static_raw", "area_px": int(np.count_nonzero(c_clean))}
        clean_debug["labels"]["c"] = c_info

        exclude_radius = int(getattr(self.cfg, "rule_ab_a_exclude_b_dilate_px", 5))
        c_exclude = self._dilate_bool_mask(c_clean, exclude_radius) if exclude_radius > 0 else c_clean
        a_preclean = a_raw.astype(bool) & (~c_exclude)
        a_pts = positive_points.get("a", [])
        a_clean, a_info = self._select_initial_component_by_positive_points(a_preclean, a_pts, "a", min_area)
        a_info["raw_area_before_exclude_c_px"] = int(np.count_nonzero(a_raw))
        a_info["area_after_exclude_c_px"] = int(np.count_nonzero(a_preclean))
        a_info["exclude_c_dilate_px"] = int(exclude_radius)
        if not np.any(a_clean):
            self.log(f"[A/C-only] A cleaned mask 为空，退回 A raw 初始 mask；debug={a_info}")
            a_clean = a_raw.astype(bool)
            a_info["fallback_to_raw_a"] = True
        clean_debug["labels"]["a"] = a_info

        masks = {"a": a_clean.astype(bool), "c": c_clean.astype(bool)}
        raw_masks = {"a": a_raw.astype(bool), "c": c_raw.astype(bool)}
        profiles = {
            "a": self._extract_feature_profile_from_mask(image_rgb, masks["a"], "a"),
            "c": self._extract_feature_profile_from_mask(image_rgb, masks["c"], "c"),
        }
        for lab in ("a", "c"):
            profiles[lab]["initial_mask_source"] = "sam2_or_static_cleaned_ac_only_no_b"
        ...
        a_center = self._mask_center_xy(masks["a"])
        a_bbox = self._mask_bbox_xyxy(masks["a"], margin=0.0)
        if a_center is not None and a_bbox is not None:
            try:
                setattr(seg, "init_a_mask", masks["a"].copy())
                setattr(seg, "last_a_mask", masks["a"].copy())
                setattr(seg, "template_a_mask", masks["a"].copy())
                setattr(seg, "init_a_bbox", tuple(float(v) for v in a_bbox))
                setattr(seg, "last_a_bbox", tuple(float(v) for v in a_bbox))
                setattr(seg, "init_a_center", (float(a_center[0]), float(a_center[1])))
                setattr(seg, "last_a_center", (float(a_center[0]), float(a_center[1])))
                if hasattr(seg, "_crop_template_by_bbox"):
                    setattr(seg, "init_a_template_rgb", seg._crop_template_by_bbox(image_rgb, a_bbox, margin=8))
            except Exception as e:
                self.log(f"[A/C-only] 同步 cleaned A 到外部 segmenter 失败：{e}")

        out_dir = self._feature_tracker_output_dir()
        ...
        try:
            setattr(follower, "_ac_only_feature_profiles", copy.deepcopy(profiles))
            setattr(follower, "_ac_only_feature_masks", {k: v.copy() for k, v in masks.items()})
            setattr(follower, "_ac_only_feature_saved", dict(saved))
            self.context["abc_feature_tracker"] = {
                "enabled": True,
                "mode": "ac_only_feature_tracking_no_b",
                "profiles_json": saved.get("profiles_json", ""),
                "overlay_path": saved.get("overlay_path", ""),
                "out_dir": str(out_dir),
                "reason": "no_b_calibration",
            }
            ...
            pass
        self.log(
            "[A/C-only] 已保存 A/C 初始模板，不启用 B mask/template："
            f"A_clean_area={int(np.count_nonzero(masks['a']))}, C_clean_area={int(np.count_nonzero(masks['c']))}, "
            f"A_center={a_center}, dir={out_dir}"
        )
        return saved
    ...
    def _install_feature_tracker_on_rule_ab_follower(self, follower: Any, state: Optional[CalibrationState] = None):
        ...
        if not hasattr(follower, "capture_and_build_scene"):
            ...
            return
        if not self._state_has_b_calibration_points(state or self._get_loaded_calibration_state()):
            self.log("[A/C-only] 未检测到 B 标定点：保存 A/C 模板，并安装 A/C-only feature tracker。")
            self._save_ac_feature_templates_without_b(follower, state or self._get_loaded_calibration_state())
            original_capture_ac = follower.capture_and_build_scene

            def _ac_only_feature_capture_and_build_scene():
                image_rgb, scene = original_capture_ac()
                profiles = getattr(follower, "_ac_only_feature_profiles", {}) or {}
                if not isinstance(profiles, dict) or "a" not in profiles:
                    raise RuntimeError("[A/C-only] A feature profile 缺失，无法用 cleaned A 计算 A center。")
                masks, debug = self._track_abc_by_saved_features(image_rgb, profiles)
                a_mask = masks.get("a")
                if a_mask is None or int(np.count_nonzero(np.asarray(a_mask).astype(bool))) <= 0:
                    raise RuntimeError(f"[A/C-only] 当前帧 A feature mask 为空，不能计算 A center；debug={debug.get('a') if isinstance(debug, dict) else debug}")
                if "c" not in masks:
                    c_obj = getattr(scene, "c", None)
                    c_mask = self._extract_mask_from_scene_object(c_obj)
                    c_mask = self._mask_to_2d_bool(c_mask, target_shape=image_rgb.shape[:2])
                    if c_mask is not None:
                        masks["c"] = c_mask.astype(bool)
                ...
                scene = self._apply_feature_tracker_masks_to_scene(
                    ...
                    masks=masks,
                    debug=debug,
                    tracker_state=getattr(follower, "_ac_only_feature_tracker_state", None),
                    source="ac_only_feature_tracker_no_b",
                )
                try:
                    follower._ac_only_feature_profiles = profiles
                    follower._ac_only_feature_tracker_state = {
                        "last_scene": scene,
                        "last_debug": debug,
                        "last_image_rgb": np.asarray(image_rgb).copy(),
                    }
                except Exception:
                    pass
            ...
            try:
                setattr(follower, "_original_capture_and_build_scene_before_feature_tracker", original_capture_ac)
                follower.capture_and_build_scene = _ac_only_feature_capture_and_build_scene
                setattr(follower, "_abc_feature_tracker_installed", True)
                setattr(follower, "_abc_feature_tracker_skipped_no_b", False)
                setattr(follower, "_ac_only_feature_tracker_no_b", True)
            except Exception:
                pass
            return
    ...
    def _get_step9_sam2_predictor(self):
        ...
        避免每一帧重复加载模型。
        """
        # Step9 动态 C 重新检测需要复用 SAM2 ImagePredictor。
        # 这里不再因为 feature tracker 方案启用而直接报错；A/B 仍按原流程走 feature tracker，
        # 本 predictor 只用于 Step9 的 current/dynamic C，不会改写 Step7 static C。
        ...
        self.log(
            f"[Step9-颜色中心] 正在加载 SAM2 动态 C 分割：cfg={sam2_cfg}, "
    ...
    def _get_or_update_step9_current_c_mask(self, image_rgb: np.ndarray) -> Dict[str, Any]:
        """
        Step9 当前/动态 C 更新入口。

        分工：
            - Step7 继续使用 full_calibration_static_c/static_c_mask.png 和 static_c_edge_route.json；
            - Step9 使用 self.step9_current_c_mask 作为当前帧 C；
            - CH1/CH2 运动后 self.step9_c_mask_dirty=True，下一轮优先重新 SAM2 检测 C；
            - 动态 C 检测失败不终止测量，按 last_valid_dynamic_c -> fallback_static_c 兜底。
        """
        dirty_before = bool(getattr(self, "step9_c_mask_dirty", True))
        info: Dict[str, Any] = {
            "step9_c_dirty_before_update": bool(dirty_before),
            "step9_c_update_ok": False,
            "step9_c_update_reason": "",
            "step9_c_source": "",
            "step9_c_area_px": "",
            "step9_c_center_x": "",
        ...
        if image_rgb is None:
            info.update({"step9_c_source": "none", "step9_c_update_reason": "image_rgb_none"})
            return info

        def _fill_from(mask: Optional[np.ndarray], raw: Optional[np.ndarray], source: str, ok: bool, reason: str) -> Dict[str, Any]:
            if mask is None:
                info.update({
                    "step9_c_source": source,
                    "step9_c_update_ok": bool(ok),
                    "step9_c_update_reason": str(reason),
                    "c_mask": None,
                    "c_raw_mask": raw,
                })
                return info
            ...
            info.update({
                "step9_c_source": source,
                "step9_c_area_px": area,
                "step9_c_center_x": float(center[0]) if center is not None else "",
                "step9_c_center_y": float(center[1]) if center is not None else "",
                "step9_c_update_ok": bool(ok),
                "step9_c_update_reason": str(reason),
                "c_mask": m,
                "c_raw_mask": np.asarray(raw).astype(bool) if raw is not None else None,
            })
            return info

        # 未脏且已有当前 C，直接复用，不再重复 SAM2。
        if (not dirty_before) and self.step9_current_c_mask is not None:
            return _fill_from(
                self.step9_current_c_mask,
                self.step9_current_c_raw_mask,
                self.step9_current_c_source or "dynamic_sam2_cached",
                True,
                self.step9_current_c_update_reason or "cached_current_dynamic_c",
            )

        # 脏帧：优先重新检测 dynamic C。
        try:
            dyn = self._predict_step9_dynamic_c_mask_by_sam2(image_rgb)
            c_mask = np.asarray(dyn.get("c_mask_for_overlap")).astype(bool)
            raw_c = np.asarray(dyn.get("raw_c_mask")).astype(bool)
            if int(np.count_nonzero(c_mask)) <= 0:
                raise RuntimeError("dynamic_c_mask_empty")
            self.step9_current_c_mask = c_mask.copy()
            self.step9_current_c_raw_mask = raw_c.copy()
            self.step9_current_c_center = dyn.get("center") if dyn.get("center") is not None else self._mask_center_xy(c_mask)
            bbox = dyn.get("bbox")
            self.step9_current_c_box = tuple(float(v) for v in bbox) if bbox is not None else self._mask_bbox_xyxy(c_mask, margin=35.0)
            dyn_prompt = str(dyn.get("prompt_mode", ""))
            dyn_reason = str(dyn.get("reason", "ok"))
            source_name = "fallback_static_c" if ("fallback_static" in dyn_prompt or "fallback_static" in dyn_reason) else "dynamic_sam2"
            self.step9_current_c_source = source_name
            self.step9_current_c_update_reason = dyn_reason
            self.step9_c_mask_dirty = False if source_name == "dynamic_sam2" else True
            return _fill_from(c_mask, raw_c, source_name, True, dyn_reason)
        except Exception as e:
            # 检测失败时保持 dirty=True，下一轮仍会继续尝试重新检测 C。
            self.step9_c_mask_dirty = True
            if self.step9_current_c_mask is not None:
                self.log(f"[Step9-C] 当前帧 C 动态检测失败，使用上一帧有效 C mask：{e}")
                return _fill_from(
                    self.step9_current_c_mask,
                    self.step9_current_c_raw_mask,
                    "last_valid_dynamic_c",
                    False,
                    f"dynamic_failed_use_last:{e}",
                )
            try:
                static_c = self._load_step9_c_mask_from_config(image_shape=image_rgb.shape[:2])
                self.log(f"[Step9-C] 当前帧 C 动态检测失败，无历史 C，使用 static C 兜底：{e}")
                self.step9_current_c_mask = np.asarray(static_c).astype(bool).copy()
                self.step9_current_c_raw_mask = np.asarray(static_c).astype(bool).copy()
                self.step9_current_c_center = self._mask_center_xy(static_c)
                self.step9_current_c_box = self._mask_bbox_xyxy(static_c, margin=35.0)
                self.step9_current_c_source = "fallback_static_c"
                self.step9_current_c_update_reason = f"dynamic_failed_use_static:{e}"
                return _fill_from(static_c, static_c, "fallback_static_c", False, f"dynamic_failed_use_static:{e}")
            except Exception as ee:
                self.log(f"[Step9-C] 当前帧 C 动态检测失败，且 static C 兜底也失败：dynamic={e}; static={ee}")
                return _fill_from(None, None, "none", False, f"dynamic_failed:{e}; static_failed:{ee}")

    def _capture_step9_color_center_once(self) -> Dict[str, Any]:
        """
        Step9 当前帧检测：颜色检测区域中心对齐。

        当前版本不再计算 B∩C 重合区域，也不再每帧动态分割/更新 C。
        Step9 的位置对齐只使用 HSV 颜色区域中心；A/B mask 仅作为调试结果每帧保存，
        ...
        # 当前 Step9 只做 HSV 颜色区域中心检测，不再主动更新动态 C。
        # 保留这些空字段只是为了兼容旧 CSV/overlay 记录格式。
        c_info: Dict[str, Any] = {
            "step9_c_dirty_before_update": False,
            "step9_c_update_ok": "",
            "step9_c_update_reason": "disabled_hsv_color_center_only",
            "step9_c_source": "",
            "step9_c_area_px": "",
            "step9_c_center_x": "",
            "step9_c_center_y": "",
    ...
    def _save_step9_color_overlay(
        ...
        try:
            if not self._should_save_runtime_debug_image(cycle_idx, force_save=force_save):
                return saved
            ...
            c_dir = self._get_step9_output_dir("c_masks")
            ...
            if c_mask is not None:
                cm_static = self._mask_to_2d_bool(c_mask, target_shape=canvas.shape[:2])
                if cm_static is not None:
                    overlay = canvas.copy()
                    overlay[cm_static] = (255, 0, 0)
                    canvas = cv2.addWeighted(overlay, 0.18, canvas, 0.82, 0)
            if c_raw_mask is not None:
                cm_raw = self._mask_to_2d_bool(c_raw_mask, target_shape=canvas.shape[:2])
            ...
            if b_mask is not None:
                bm = self._mask_to_2d_bool(b_mask, target_shape=canvas.shape[:2])
                if bm is not None:
                    overlay = canvas.copy()
                    overlay[bm] = (0, 255, 0)
                    canvas = cv2.addWeighted(overlay, 0.22, canvas, 0.78, 0)
            if color_mask is not None:
                cm = self._mask_to_2d_bool(color_mask, target_shape=canvas.shape[:2])
                if cm is not None:
                    overlay = canvas.copy()
            ...
            if c_source:
                cv2.putText(canvas, f"C source: {str(c_source)}", (12, 24), cv2.FONT_HERSHEY_SIMPLEX, 0.62, (255, 255, 255), 3, cv2.LINE_AA)
                cv2.putText(canvas, f"C source: {str(c_source)}", (12, 24), cv2.FONT_HERSHEY_SIMPLEX, 0.62, (0, 0, 0), 1, cv2.LINE_AA)
...
class MeasurementWorkflowGUI:
    def __init__(self, root: tk.Tk):
        ...
        self._pending_gui_update = False
        self._pending_gui_workflow: Optional[MeasurementWorkflow] = None
        self._pending_plot_update = False
        self._pending_var_updates: Dict[str, Any] = {}
        self._last_var_values: Dict[str, str] = {}
        self._last_plot_signature: Optional[Tuple[Any, ...]] = None
        self._angle_fit_tree_rows: Dict[str, Tuple[Any, ...]] = {}
        self._log_queue: List[str] = []
        self._log_lock = threading.Lock()
        self._log_flush_after_id = None
        self._gui_flush_after_id = None

        self._build_ui()
        self._schedule_log_flush()
        self._schedule_gui_flush()
    ...
    def _build_ui(self):
        ...
        ttk.Button(button_grid, text="Step7标定预检", command=self.step7_ac_preflight_thread).grid(row=8, column=0, columnspan=2, padx=4, pady=4, sticky="ew")
    ...
    def log(self, msg: str):
        print(msg)
        if self._is_immediate_gui_text(msg):
            with self._log_lock:
                self._log_queue.append(str(msg))
            self._flush_log_queue_threadsafe()
            return
        with self._log_lock:
            self._log_queue.append(str(msg))
    ...
    def _is_immediate_gui_text(self, text: Any) -> bool:
        s = str(text)
        urgent_words = (
            "停止", "stop", "STOP",
            "完成", "结束", "finish", "FINISH",
            "失败", "错误", "异常", "error", "ERROR", "exception", "Exception",
            "Traceback",
        )
        return any(w in s for w in urgent_words)

    def _schedule_log_flush(self):
        try:
            self._log_flush_after_id = self.root.after(3000, self._flush_log_queue_periodic)
        except Exception:
            self._log_flush_after_id = None

    def _flush_log_queue_threadsafe(self):
        try:
            self.root.after(0, self._flush_log_queue_now)
        except Exception:
            pass

    def _flush_log_queue_periodic(self):
    ...
    def _schedule_gui_flush(self):
        try:
            self._gui_flush_after_id = self.root.after(500, self._flush_gui_updates_periodic)
        except Exception:
            self._gui_flush_after_id = None

    def _flush_gui_updates_periodic(self):
        self._flush_gui_updates_now()
        self._schedule_gui_flush()

    def _flush_pending_var_updates(self):
        with self._gui_update_lock:
            pending = dict(self._pending_var_updates)
            self._pending_var_updates.clear()
        for var, value in pending.items():
            key = str(value)
            vkey = str(var)
            if self._last_var_values.get(vkey) == key:
                continue
            try:
                var.set(value)
                self._last_var_values[vkey] = key
            except Exception:
                pass

    def _flush_gui_updates_now(self):
        wf = None
        do_refresh = False
        do_plot = False
        with self._gui_update_lock:
            if self._pending_gui_update:
                wf = self._pending_gui_workflow
                do_refresh = True
                self._pending_gui_update = False
                self._pending_gui_workflow = None
            if self._pending_plot_update:
                do_plot = True
                self._pending_plot_update = False

        self._flush_pending_var_updates()

        if do_refresh:
            self._refresh_result_labels_now(wf)
            with self._gui_update_lock:
                if self._pending_plot_update:
                    do_plot = True
                    self._pending_plot_update = False
        if do_plot:
            self.update_angle_fit_peak_plot(wf or self.workflow)

        self._flush_pending_var_updates()

    def _request_gui_update(self, workflow: Optional[MeasurementWorkflow] = None, *, plot: bool = False, immediate: bool = False):
        if immediate:
            try:
                self.root.after(0, lambda wf=workflow: self._refresh_result_labels_now(wf))
            except Exception:
                pass
            return
        with self._gui_update_lock:
            self._pending_gui_update = True
            if workflow is not None:
                self._pending_gui_workflow = workflow
            if plot:
                self._pending_plot_update = True

    def set_var(self, var: tk.Variable, value: Any):
        vkey = str(var)
        if self._is_immediate_gui_text(value):
            def _set_now():
                key = str(value)
                if self._last_var_values.get(vkey) != key:
                    var.set(value)
                    self._last_var_values[vkey] = key
            self.root.after(0, _set_now)
            return
        with self._gui_update_lock:
            self._pending_var_updates[vkey] = value
    ...
    def _generate_c_edge_route_points_from_quad(
        ...
        # 先求四条外扩边的相邻交点，保证 min/target/max 仍是矩形/四边形路径。
        offset_pts: List[np.ndarray] = []
        for i in range(4):
            prev_i = (i - 1) % 4
            prev_p = pts[prev_i] + clearance * normals[prev_i]
    ...
    def _save_static_c_map_result(
        ...
        circle_fit_for_route: Optional[Dict[str, Any]] = None
        circle_center_xy: Optional[Tuple[float, float]] = None
        circle_base_radius_px: float = 0.0
        if route_source in ("circle", "c_circle", "fit_circle"):
            circle_fit_for_route = MeasurementWorkflow._fit_circle_from_mask(c_sam2_bool)
            if not circle_fit_for_route:
                raise RuntimeError("C mask 拟合圆失败，无法生成 Step7 八边形路线。")
            circle_center_xy = (float(circle_fit_for_route["center_xy"][0]), float(circle_fit_for_route["center_xy"][1]))
            circle_base_radius_px = float(circle_fit_for_route["radius_px"])
            octagon_start_angle_deg = float(getattr(cfg_gui_for_route, "rule_ab_octagon_start_angle_deg", 22.5))
            base_route_points = MeasurementWorkflow._generate_octagon_route_points(
                circle_center_xy, circle_base_radius_px, route_follow_direction, image_shape=image_rgb.shape[:2], start_angle_deg=octagon_start_angle_deg
            )
            min_route_points = MeasurementWorkflow._generate_octagon_route_points(
                circle_center_xy, route_min_clearance_px, route_follow_direction, image_shape=image_rgb.shape[:2], start_angle_deg=octagon_start_angle_deg
            )
            route_points = MeasurementWorkflow._generate_octagon_route_points(
                circle_center_xy, route_safe_clearance_px, route_follow_direction, image_shape=image_rgb.shape[:2], start_angle_deg=octagon_start_angle_deg
            )
            max_route_points = MeasurementWorkflow._generate_octagon_route_points(
                circle_center_xy, route_max_clearance_px, route_follow_direction, image_shape=image_rgb.shape[:2], start_angle_deg=octagon_start_angle_deg
            )
            outer_edge_points = []
            route_payload_source = "circle_fit_center_radius_route"
        elif route_source in ("quad", "quadrilateral"):
            ...
            route_payload_source = "static_c_quad_points_three_line_safety_band"
        else:
        ...
        execution_segment_px = max(
            route_spacing_px * 2.0,
            float(getattr(cfg_gui_for_route, "rule_ab_route_execution_segment_px", 48.0)),
        )
        if route_source in ("circle", "c_circle", "fit_circle"):
            execution_base_route_points = list(base_route_points)
            execution_min_route_points = list(min_route_points)
            execution_route_points = list(route_points)
            execution_max_route_points = list(max_route_points)
        else:
            execution_base_route_points = MeasurementWorkflow._to_axis_aligned_execution_polyline(base_route_points, mode="hv", segment_px=execution_segment_px)
            execution_min_route_points = MeasurementWorkflow._to_axis_aligned_execution_polyline(min_route_points, mode="hv", segment_px=execution_segment_px)
            execution_route_points = MeasurementWorkflow._to_axis_aligned_execution_polyline(route_points, mode="hv", segment_px=execution_segment_px)
            execution_max_route_points = MeasurementWorkflow._to_axis_aligned_execution_polyline(max_route_points, mode="hv", segment_px=execution_segment_px)

        route_payload = {
            "created_at": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
            "source": route_payload_source,
            "source_mask": "static_c_sam2_mask.png" if route_payload_source in ("sam2_outer_edge_pixels", "circle_fit_center_radius_route") else "static_c_quad_points.json",
            "safety_reference": "circle_center_radius" if route_source in ("circle", "c_circle", "fit_circle") else "route_polyline_distance",
            "circle_center_xy": [float(circle_center_xy[0]), float(circle_center_xy[1])] if circle_center_xy is not None else None,
            "circle_base_radius_px": float(circle_base_radius_px),
            "circle_fit_debug": circle_fit_for_route or {},
            "c_mask_shape_for_route": "fit_circle" if route_source in ("circle", "c_circle", "fit_circle") else ("sam2_outer_edge" if route_payload_source == "sam2_outer_edge_pixels" else "quadrilateral"),
            "route_geometry_mode": "circle" if route_source in ("circle", "c_circle", "fit_circle") else ("mitered_rectangle" if route_source in ("quad", "quadrilateral") else "sam2_outer_edge_pixels"),
            "execution_route_mode": "octagon_state_machine" if route_source in ("circle", "c_circle", "fit_circle") else "axis_aligned_hv_chunked_manhattan",
            "execution_segment_px": float(execution_segment_px),
            "octagon_vertex_count": 8 if route_source in ("circle", "c_circle", "fit_circle") else 0,
            "octagon_start_angle_deg": float(octagon_start_angle_deg) if route_source in ("circle", "c_circle", "fit_circle") else None,
            "octagon_diagonal_chunk_steps": int(getattr(cfg_gui_for_route, "rule_ab_octagon_diagonal_chunk_steps", 3)) if route_source in ("circle", "c_circle", "fit_circle") else 0,
            ...
            "execution_base_route_points_xy": [[float(x), float(y)] for x, y in execution_base_route_points],
            "execution_min_route_points_xy": [[float(x), float(y)] for x, y in execution_min_route_points],
            "execution_route_points_xy": [[float(x), float(y)] for x, y in execution_route_points],
            "execution_target_route_points_xy": [[float(x), float(y)] for x, y in execution_route_points],
            "execution_max_route_points_xy": [[float(x), float(y)] for x, y in execution_max_route_points],
            "note": "Step7 八边形状态机路线：route_points_xy 为 8 个八边形关键点；水平段只左右、垂直段只上下、斜边按 X/Y 方向分块交替近似。安全距离判据为 distance(A_center, circle_center)。" if route_source in ("circle", "c_circle", "fit_circle") else "Step7 三线安全带：route_points_xy 是拟合矩形可视化路径；execution_*_points_xy 是控制器实际分段水平/垂直执行路径。",
        }
        ...
        with route_csv_path.open("w", encoding="utf-8-sig", newline="") as f:
            writer = csv.writer(f)
            writer.writerow(["index", "x", "y"])
            for idx, (x, y) in enumerate(execution_route_points):
                writer.writerow([idx, float(x), float(y)])
        ...
        if execution_route_points:
            exec_i32 = np.round(np.asarray(execution_route_points, dtype=np.float32)).astype(np.int32).reshape((-1, 1, 2))
            cv2.polylines(quad_overlay, [exec_i32], isClosed=True, color=(255, 255, 255), thickness=1)
        ...
        try:
            ...
            pass
        # route_overlay：淡色为拟合矩形路径；亮色为控制器实际水平/垂直执行路径。
        for _pts, _color, _thick in (
            (base_route_points if 'base_route_points' in locals() else [], (90, 90, 90), 1),
            (min_route_points if 'min_route_points' in locals() else [], (40, 150, 40), 1),
            (max_route_points if 'max_route_points' in locals() else [], (40, 40, 170), 1),
            (route_points if 'route_points' in locals() else [], (130, 40, 130), 1),
            (execution_base_route_points if 'execution_base_route_points' in locals() else [], (160, 160, 160), 1),
            (execution_min_route_points if 'execution_min_route_points' in locals() else [], (0, 255, 0), 2),
            (execution_max_route_points if 'execution_max_route_points' in locals() else [], (0, 0, 255), 2),
        ):
            if _pts:
                _i32 = np.round(np.asarray(_pts, dtype=np.float32)).astype(np.int32).reshape((-1, 1, 2))
                cv2.polylines(route_overlay, [_i32], isClosed=True, color=_color, thickness=_thick)
        if execution_route_points:
            route_i32 = np.round(np.asarray(execution_route_points, dtype=np.float32)).astype(np.int32).reshape((-1, 1, 2))
            ...
            sx, sy = execution_route_points[0]
            ex, ey = execution_route_points[-1]
            cv2.circle(route_overlay, (int(round(sx)), int(round(sy))), 7, (0, 255, 0), -1)
            cv2.putText(route_overlay, "EXEC START", (int(round(sx)) + 8, int(round(sy)) - 8), cv2.FONT_HERSHEY_SIMPLEX, 0.55, (0, 255, 0), 2, cv2.LINE_AA)
            ...
            sample_step = max(1, len(execution_route_points) // 24)
            for ridx, (rx, ry) in enumerate(execution_route_points[::sample_step]):
                cv2.circle(route_overlay, (int(round(rx)), int(round(ry))), 3, (255, 0, 255), -1)
            cv2.putText(
                route_overlay,
                f"EXEC chunked HV route: {len(execution_route_points)} pts; segment={execution_segment_px:.1f}px; draw route: {len(route_points)} pts; min/target/max={route_min_clearance_px:.1f}/{route_safe_clearance_px:.1f}/{route_max_clearance_px:.1f}px",
                (12, 28),
        ...
        meta = {
            ...
            "execution_route_mode": "axis_aligned_hv_chunked_manhattan",
            "execution_segment_px": float(execution_segment_px),
            "execution_route_points_count": int(len(execution_route_points)),
        ...
        self.log(
            ...
            f"method={quad_method}；已生成近似四边形A路线点数={len(route_points)}，实际横竖执行点数={len(execution_route_points)}；"
    ...
    def build_config_from_ui(self) -> MeasurementConfig:
        return MeasurementConfig(
            ...
            rule_ab_c_edge_route_source="circle",
    ...
    def schedule_plot_update(self, workflow: Optional[MeasurementWorkflow] = None):
        """
        Tkinter 只能在主线程更新控件；高频请求只标记 dirty，由 500 ms GUI flush 统一绘图。
        """
        with self._gui_update_lock:
            self._pending_plot_update = True
            if workflow is not None:
    ...
    def update_angle_fit_peak_plot(self, workflow: Optional[MeasurementWorkflow] = None):
        ...
        plot_signature = (
            len(list_points),
            list_points[-1] if list_points else None,
            len(raw_values),
            raw_values[-1] if raw_values else None,
            len(median_values),
            median_values[-1] if median_values else None,
        )
        if self._last_plot_signature == plot_signature:
            return
    ...
    def update_angle_fit_xy_list(self, valid_points: List[Tuple[int, Optional[float], Optional[float]]]):
        ...
        try:
            ...
            for cycle_index, angle, fit_peak in valid_points:
                ...
                live_iids.add(iid)
                if tree.exists(iid):
                    if self._angle_fit_tree_rows.get(iid) != values:
                        tree.item(iid, values=values)
                else:
                    tree.insert("", tk.END, iid=iid, values=values)
                self._angle_fit_tree_rows[iid] = values

            for item in list(tree.get_children()):
                item_s = str(item)
                if item_s.startswith("row_") and item_s not in live_iids:
                    tree.delete(item)
                    self._angle_fit_tree_rows.pop(item_s, None)
                elif item_s.startswith("padding_"):
                    tree.delete(item)
            ...
            for i in range(padding_rows):
                ...
                if not tree.exists(iid):
                    tree.insert("", tk.END, iid=iid, values=("", "", ""), tags=("padding",))
    ...
    def refresh_result_labels(self, workflow: Optional[MeasurementWorkflow] = None):
        self._request_gui_update(workflow, plot=True)

    def _refresh_result_labels_now(self, workflow: Optional[MeasurementWorkflow] = None):
        wf = workflow or self.workflow
        if wf is None:
            return
    ...
    def calibrate_global_c(self):
        ...
        try:
            ...
            self.log(f"[完整测量标定] C标定：开始截图，capture_area={cfg_gui.capture_area}, save_dir={save_dir}")
            image_rgb = self._capture_current_rule_ab_frame(output_dir=save_dir)
            preview_path = save_dir / "global_c_calibration_capture_preview.png"
            try:
                cv2.imwrite(str(preview_path), cv2.cvtColor(np.asarray(image_rgb), cv2.COLOR_RGB2BGR))
                self.log(f"[完整测量标定] C标定：截图完成，预览已保存：{preview_path}")
            except Exception as e:
                self.log(f"[完整测量标定] C标定：截图完成，但保存预览失败：{e}")
            self.log("[完整测量标定] C标定：即将弹出 C 点选窗口；左键正点，右键负点，Enter/N 完成。")
            pos, neg = self._select_c_points_interactively(
                image_rgb=image_rgb,
                window_name="Calibration Global C: left positive, right negative",
                scale=float(getattr(cfg_gui, "rule_ab_confirm_window_scale", 0.85)),
            )
            self.log(f"[完整测量标定] C标定：点选完成，C+={len(pos)}, C-={len(neg)}，开始 SAM2 分割。")
            c_mask = self._predict_c_mask_by_sam2_points(image_rgb=image_rgb, positive_points=pos, negative_points=neg)
    ...
    def step7_ac_preflight_thread(self):
        if self.is_busy:
            ...
            return
        self.worker_thread = self.run_in_thread(self.step7_ac_preflight)

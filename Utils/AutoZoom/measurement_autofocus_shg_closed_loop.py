...
FOCUS_SUMMARY_KEYS = [f"full_{name}" for name in FOCUS_METRIC_NAMES] + [f"roi_{name}" for name in FOCUS_METRIC_NAMES]
...
class MeasurementConfig:
    ...
    # 聚焦指标截图使用同一个 capture_area；focus_roi 是相对于该截图左上角的 ROI：x,y,w,h。
    focus_roi: Tuple[int, int, int, int] = tuple(_cfg("focus_roi", (0, 0, 300, 300)))  # type: ignore
    focus_fft_low_radius_ratio: float = float(_cfg("focus_fft_low_radius_ratio", 0.10))
    focus_edge_profile_half_width: int = int(_cfg("focus_edge_profile_half_width", 60))

    # -------------------- 自动补焦 / Z轴控制 --------------------
    autofocus_enabled: bool = bool(_cfg("autofocus_enabled", True))
    autofocus_focus_trigger_ratio: float = float(_cfg("autofocus_focus_trigger_ratio", 0.90))
    autofocus_focus_trigger_count: int = int(_cfg("autofocus_focus_trigger_count", 3))
    autofocus_shg_trigger_ratio: float = float(_cfg("autofocus_shg_trigger_ratio", 0.90))
    autofocus_shg_trigger_count: int = int(_cfg("autofocus_shg_trigger_count", 2))
    autofocus_shg_hard_ratio: float = float(_cfg("autofocus_shg_hard_ratio", 0.85))
    autofocus_stop_ratio: float = float(_cfg("autofocus_stop_ratio", 0.95))

    # FocusScore = weighted mean of current/reference ratios.
    focus_weight_highfreq: float = float(_cfg("focus_weight_highfreq", 0.35))
    focus_weight_tenengrad: float = float(_cfg("focus_weight_tenengrad", 0.25))
    focus_weight_brenner: float = float(_cfg("focus_weight_brenner", 0.25))
    focus_weight_red_blue: float = float(_cfg("focus_weight_red_blue", 0.15))

    # Newport 8742 / Picomotor Z轴。axis 通常为 1；正负方向由 signed steps 决定。
    z_enabled: bool = bool(_cfg("z_enabled", True))
    z_axis: int = int(_cfg("z_axis", 1))
    z_speed: int = int(_cfg("z_speed", 100))
    z_accel: int = int(_cfg("z_accel", 100))
    z_probe_steps: int = int(_cfg("z_probe_steps", 10))
    z_search_steps: int = int(_cfg("z_search_steps", 10))
    z_settle_time_s: float = float(_cfg("z_settle_time_s", 0.20))
    z_min_improve_ratio: float = float(_cfg("z_min_improve_ratio", 0.005))
    z_max_iter: int = int(_cfg("z_max_iter", 40))
    z_max_total_steps: int = int(_cfg("z_max_total_steps", 400))
    z_patience: int = int(_cfg("z_patience", 3))
    focus_reference_capture_count: int = int(_cfg("focus_reference_capture_count", 5))
    shg_reference_capture_count: int = int(_cfg("shg_reference_capture_count", 3))
...
class MeasurementWorkflow:
    def __init__(self, cfg: MeasurementConfig, on_log=None, on_update=None):
        ...
        self.z_controller = None

        # 每个样品自己的参考状态：
        # - focus_reference：由“建立参考”按钮采集 5 次 ROI 指标得到；
        # - shg_reference_values：正式测量前 3 次 fit_peak 自动累计得到 SHG_ref。
        ...
        self.context: Dict[str, Any] = {
            ...
            "focus_roi_image_path": None,
            "focus_roi": tuple(self.cfg.focus_roi),
            "focus_reference": None,
            "focus_reference_ready": False,
            "focus_score_ratio": None,
            "consecutive_focus_low_count": 0,
            "shg_ref": None,
            "shg_reference_values": [],
            "last_shg_ratio": None,
            "consecutive_shg_low_count": 0,
            "autofocus_last_result": None,
            "autofocus_last_reason": None,
    ...
    def connect_z_controller(self):
        """连接 Newport 8742 / Picomotor Z 轴控制器。"""
        if not bool(self.cfg.z_enabled):
            raise RuntimeError("Z轴补焦未启用：请在 GUI 勾选/启用 Z轴补焦")
        ...
        if Newport is None:
            raise ImportError("需要安装 pylablib 才能控制 Newport 8742：pip install pylablib")

        num_devices = Newport.get_usb_devices_number_picomotor()
        self.log(f"[Z轴] 检测到 Newport Picomotor 控制器数量：{num_devices}")
        if num_devices <= 0:
            raise RuntimeError("未检测到 Newport Picomotor 控制器")

        self.z_controller = Newport.Picomotor8742(conn=0)
        try:
            self.log(f"[Z轴] 设备ID：{self.z_controller.get_id()}")
        except Exception:
            pass

        self.setup_z_velocity()
        self.log("[Z轴] 连接成功")
        self.notify_update()
        return self.z_controller

    def setup_z_velocity(self):
        ...
        try:
            ...
            self.z_controller.setup_velocity(axis=axis, speed=speed, accel=accel)
            self.log(f"[Z轴] 已设置速度/加速度：axis={axis}, speed={speed}, accel={accel}")
        except Exception as e:
            self.log(f"[Z轴] 设置速度/加速度失败：{e}")

    def move_z_relative(self, signed_steps: int):
        """相对移动 Z 轴。signed_steps>0 表示 z_plus，<0 表示 z_minus。"""
        steps = int(round(signed_steps))
        if steps == 0:
            return
        if self.z_controller is None:
            self.connect_z_controller()
        if self.z_controller is None:
            raise RuntimeError("Z轴控制器未连接")

        axis = int(self.cfg.z_axis)
        self.log(f"[Z轴] move_by axis={axis}, signed_steps={steps}")
        try:
            self.z_controller.move_by(axis=axis, steps=steps)
        except TypeError:
            # 兼容少数 pylablib 版本的参数名差异。
            self.z_controller.move_by(axis, steps)
        except Exception as e:
            raise RuntimeError(f"Z轴移动失败：axis={axis}, signed_steps={steps}, error={e}") from e

        # 不强制等待硬件返回完成；开环控制以图像稳定等待为主。

    def _focus_component_weights(self) -> Dict[str, float]:
        weights = {
        ...
        if total <= 1e-12:
            return {"highfreq_ratio": 0.4, "tenengrad": 0.3, "brenner": 0.3, "red_blue_ratio": 0.0}
        return {k: max(0.0, float(v)) / total for k, v in weights.items()}

    def calculate_focus_score_ratio(self, roi_metrics: Optional[Dict[str, Any]]) -> Tuple[Optional[float], Dict[str, Optional[float]]]:
        """
        计算当前 ROI 聚焦分数相对本样品参考值的比例。
        ...
        if not isinstance(roi_metrics, dict):
            return None, {}
        ref = self.focus_reference.get("roi_metric_ref") if isinstance(self.focus_reference, dict) else None
        if not isinstance(ref, dict):
            return None, {}

        weights = self._focus_component_weights()
        ratios: Dict[str, Optional[float]] = {}
        vals = []
        wts = []
        for name, weight in weights.items():
            cur = self._safe_float(roi_metrics.get(name))
            base = self._safe_float(ref.get(name))
            if cur is None or base is None or abs(base) <= 1e-12 or weight <= 0:
                ratios[name] = None
                continue
            ratio = float(cur) / float(base)
            # 限幅，避免红蓝比例或极小指标异常把综合分数拉爆。
            ratio = max(0.0, min(2.5, ratio))
            ratios[name] = ratio
            vals.append(ratio)
            wts.append(weight)
        ...
        if not vals or sum(wts) <= 1e-12:
            return None, ratios
        score_ratio = float(np.average(vals, weights=wts))
        return score_ratio, ratios

    def update_focus_score_context(self, focus_metrics: Optional[Dict[str, Any]]) -> Optional[float]:
        roi_metrics = None
        if isinstance(focus_metrics, dict):
            roi_metrics = focus_metrics.get("roi_metrics")
        score_ratio, component_ratios = self.calculate_focus_score_ratio(roi_metrics)
        if isinstance(focus_metrics, dict):
            focus_metrics["focus_score_ratio"] = score_ratio
            focus_metrics["focus_component_ratios"] = component_ratios
        self.context["focus_score_ratio"] = score_ratio
        return score_ratio

    def build_focus_reference(self, capture_count: Optional[int] = None, sleep_s: float = 0.15) -> Dict[str, Any]:
        """采集多次 ROI 指标，建立当前样品自己的聚焦参考。"""
        n = int(capture_count or self.cfg.focus_reference_capture_count)
        n = max(1, n)
        self.log(f"========== 建立本样品 ROI 聚焦参考：采集 {n} 次 ==========")

        samples: List[Dict[str, Any]] = []
        ref_dir = self.output_root / "focus_reference" / datetime.now().strftime("%Y%m%d_%H%M%S")
        ref_dir.mkdir(parents=True, exist_ok=True)

        for i in range(1, n + 1):
            live = self.capture_focus_metrics_live()
            roi_metrics = live.get("roi_metrics") or {}
            samples.append(roi_metrics)
            try:
                if Image is not None:
                    Image.fromarray(live["full_rgb"]).save(ref_dir / f"reference_{i:02d}_full.png")
                    Image.fromarray(live["roi_rgb"]).save(ref_dir / f"reference_{i:02d}_roi.png")
            except Exception:
                pass
            self.log(
                f"[聚焦参考] {i}/{n}: "
        ...
        for name in FOCUS_METRIC_NAMES:
            vals = [self._safe_float(s.get(name)) for s in samples]
        ...
        self.context["focus_reference"] = self._json_safe(self.focus_reference)
        self.context["focus_reference_ready"] = True
        ...
        try:
            ...
            self.log(f"[聚焦参考] 参考指标已保存：{ref_csv}")
        except Exception as e:
            self.log(f"[聚焦参考] 保存参考CSV失败：{e}")
    ...
    def update_shg_reference_and_ratio(self):
        """
        在 Step 6 光谱采集后调用：
        - 前 3 次有效 fit_peak 自动建立 SHG_ref；
        - 之后每轮更新 SHG_ratio 和连续低 SHG 计数。"""
        fit_peak = self._safe_float(self.context.get("fit_peak"))
        if fit_peak is None:
            self.log("[SHG参考] 本轮 fit_peak 无效，无法更新 SHG_ratio")
            return None

        ref_n = max(1, int(self.cfg.shg_reference_capture_count))
        if len(self.shg_reference_values) < ref_n:
            ...
            self.log(
                f"[SHG参考] 已累计 {len(self.shg_reference_values)}/{ref_n} 次："
                f"values={self.shg_reference_values}, SHG_ref={self.shg_ref}"
            )
        elif self.shg_ref is None:
            self.shg_ref = float(np.mean(self.shg_reference_values[:ref_n]))

        self.context["shg_reference_values"] = list(self.shg_reference_values)
        self.context["shg_ref"] = self.shg_ref

        if self.shg_ref is not None and abs(self.shg_ref) > 1e-12:
            self.last_shg_ratio = float(fit_peak) / float(self.shg_ref)
            self.context["last_shg_ratio"] = self.last_shg_ratio

            if self.last_shg_ratio < float(self.cfg.autofocus_shg_trigger_ratio):
                self.consecutive_shg_low_count += 1
            else:
                self.consecutive_shg_low_count = 0
            self.context["consecutive_shg_low_count"] = self.consecutive_shg_low_count

            self.log(
                f"[SHG_ratio] fit_peak={fit_peak}, SHG_ref={self.shg_ref}, "
                f"SHG_ratio={self.last_shg_ratio:.4f}, "
                f"连续低SHG计数={self.consecutive_shg_low_count}"
            )
    ...
    def evaluate_autofocus_trigger(self, focus_score_ratio: Optional[float]) -> Tuple[bool, List[str]]:
        ...
        if self.last_shg_ratio is not None:
            if self.last_shg_ratio < float(self.cfg.autofocus_shg_hard_ratio):
                reasons.append(f"单轮 SHG_ratio={self.last_shg_ratio:.4f} < {self.cfg.autofocus_shg_hard_ratio}")
            elif self.consecutive_shg_low_count >= int(self.cfg.autofocus_shg_trigger_count):
                reasons.append(
    ...
    def step45_focus_check_and_autofocus(self, cycle_index: int, paths: Dict[str, str]) -> Dict[str, Any]:
        """
        Step 4.5：截图计算聚焦指标，并根据 FocusScore_ratio / SHG_ratio 判断是否补焦。
        如果触发补焦，则闭环移动 Z 轴直到 FocusScore_ratio >= autofocus_stop_ratio。
        """
        self.log("========== Step 4.5：截图、判断是否触发补焦 ==========")
        focus_metrics = self.capture_and_compute_focus_metrics(cycle_index=cycle_index, paths=paths)
        focus_score_ratio = self.update_focus_score_context(focus_metrics)

        if focus_score_ratio is None:
            self.log("[补焦判断] 尚未建立 ROI 聚焦参考，无法计算 FocusScore_ratio；本轮只保存聚焦指标，不执行自动补焦")
            self.context["autofocus_last_reason"] = "no_focus_reference"
            return focus_metrics

        need_autofocus, reasons = self.evaluate_autofocus_trigger(focus_score_ratio)
        self.log(
            f"[补焦判断] FocusScore_ratio={focus_score_ratio:.4f}, "
            f"last_SHG_ratio={self.last_shg_ratio}, "
            f"focus_low_count={self.consecutive_focus_low_count}, "
            f"shg_low_count={self.consecutive_shg_low_count}, "
            f"need_autofocus={need_autofocus}, reasons={reasons}"
        )

        if not need_autofocus:
            self.context["autofocus_last_reason"] = "not_triggered"
            return focus_metrics

        if not bool(self.cfg.autofocus_enabled):
            self.log("[补焦判断] 已触发补焦，但 autofocus_enabled=False，因此不移动 Z 轴")
            self.context["autofocus_last_reason"] = "triggered_but_disabled"
            return focus_metrics

        self.log("[补焦] 触发自动补焦：" + "；".join(reasons))
        autofocus_result = self.run_closed_loop_autofocus(initial_focus_metrics=focus_metrics)
        self.context["autofocus_last_result"] = self._json_safe(autofocus_result)
        self.context["autofocus_last_reason"] = "; ".join(reasons)

        # 补焦结束后重新截图保存，覆盖本轮 Step 4.5 的最终聚焦状态，确保 Step 9 保存的是补焦后的指标。
        self.log("[补焦] 补焦结束，重新截图并保存最终聚焦指标")
        final_metrics = self.capture_and_compute_focus_metrics(cycle_index=cycle_index, paths=paths)
        self.update_focus_score_context(final_metrics)
    ...
    def run_closed_loop_autofocus(self, initial_focus_metrics: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        """开环 Z 轴 + 图像反馈的闭环补焦。"""
        ...
        self.log(
            ...
            f"probe_steps={probe_steps}, search_steps={search_steps} =========="
        )

        self.connect_z_controller()

        if initial_focus_metrics is None:
            initial_live = self.capture_focus_metrics_live()
            initial_score, _ = self.calculate_focus_score_ratio(initial_live.get("roi_metrics"))
        else:
            initial_score = initial_focus_metrics.get("focus_score_ratio")
            if initial_score is None:
                initial_score = self.update_focus_score_context(initial_focus_metrics)

        if initial_score is None:
            return {"ok": False, "reason": "no_focus_score_reference"}

        best_score = float(initial_score)
        best_pos = 0
        current_pos = 0
        history = [{"iter": 0, "relative_z_steps": 0, "score": best_score, "phase": "start"}]

        if best_score >= target:
            self.log(f"[补焦] 初始 FocusScore_ratio={best_score:.4f} 已经 >= {target}，无需移动")
            return {"ok": True, "reason": "already_above_target", "best_score": best_score, "best_relative_z_steps": 0, "history": history}

        def measure_score(phase: str, iteration: int) -> Optional[float]:
            if settle_s > 0:
                time.sleep(settle_s)
            live = self.capture_focus_metrics_live()
            score, comp = self.calculate_focus_score_ratio(live.get("roi_metrics"))
            history.append({
                "iter": iteration,
            ...
            if score is not None:
                self.log(f"[补焦-{phase}] iter={iteration}, relative_z={current_pos}, score={score:.4f}, comp={comp}")
            else:
                self.log(f"[补焦-{phase}] iter={iteration}, relative_z={current_pos}, score=None")
            return score

        # 先试探 z_plus。
        direction = None
        self.move_z_relative(+probe_steps)
        ...
        if score_plus is not None and score_plus > best_score * (1.0 + min_improve):
            ...
            if score_minus is not None and score_minus > best_score * (1.0 + min_improve):
                ...
                # 两边都没有提升，回到原点附近。
                if current_pos != 0:
                    self.move_z_relative(-current_pos)
                    current_pos = 0
                    if settle_s > 0:
                        time.sleep(settle_s)
                self.log("[补焦] z_plus / z_minus 试探均无明显提升，停止补焦")
                return {
                    "ok": True,
        ...
        while iteration <= max_iter:
            if self.stop_requested:
                break
            if best_score >= target:
                self.log(f"[补焦] best_score={best_score:.4f} >= target={target}，停止搜索")
                break
            if abs(current_pos) >= max_total_steps:
                self.log(f"[补焦] 达到最大总步数 abs({current_pos}) >= {max_total_steps}，停止搜索")
                break

            step = int(direction * search_steps)
            self.move_z_relative(step)
            current_pos += step
            ...
            if no_improve_count >= patience:
                self.log(f"[补焦] 连续 {no_improve_count} 次无明显提升，停止搜索")
                break
            iteration += 1
        ...
        if delta_back != 0:
            self.log(f"[补焦] 回退到最佳相对位置：best_pos={best_pos}, current_pos={current_pos}, delta_back={delta_back}")
            self.move_z_relative(delta_back)
    ...
    @staticmethod
    def _clamp_roi(roi: Tuple[int, int, int, int], image_shape: Tuple[int, int, int]) -> Tuple[int, int, int, int]:
    ...
    @staticmethod
    def _profile_edge_and_halo_width(gray: np.ndarray, half_width: int = 60) -> Tuple[Optional[float], Optional[float]]:
        """
        ...
        这两个量不是物理厚度，而是工程图像指标，适合做长时间漂移趋势跟踪。
        """
        try:
    ...
    def compute_focus_metrics_for_image(self, image_rgb: np.ndarray) -> Dict[str, Optional[float]]:
        """计算一张 RGB 图像的 10 个聚焦/亮度/颜色指标。"""
        if cv2 is None:
            raise ImportError("需要安装 opencv-python 才能计算聚焦指标：pip install opencv-python")
        ...
        if gray.shape[0] > 2:
            brenner_terms.append((gray[2:, :] - gray[:-2, :]) ** 2)
        brenner = float(np.mean([np.mean(v) for v in brenner_terms])) if brenner_terms else None

        # 高频能量比例：去掉中心低频圆盘，统计剩余高频能量 / 总能量。
        try:
            ...
            low_radius = max(1.0, float(self.cfg.focus_fft_low_radius_ratio) * min(h, w))
        ...
        return {k: self._safe_float(v) for k, v in values.items()}

    def capture_focus_metrics_live(self) -> Dict[str, Any]:
        """截图并计算整图/ROI指标，但不保存图片，用于标定和闭环补焦中间检测。"""
        if pyautogui is None:
            raise ImportError("需要安装 pyautogui 才能截图：pip install pyautogui")
        left, top, width, height = [int(v) for v in self.cfg.capture_area]
        screenshot = pyautogui.screenshot(region=(left, top, width, height))
        image_rgb = np.asarray(screenshot.convert("RGB"))

        roi = self._clamp_roi(tuple(self.cfg.focus_roi), image_rgb.shape)
        x, y, rw, rh = roi
        roi_rgb = image_rgb[y:y + rh, x:x + rw].copy()

        full_metrics = self.compute_focus_metrics_for_image(image_rgb)
        roi_metrics = self.compute_focus_metrics_for_image(roi_rgb)
    ...
    def capture_and_compute_focus_metrics(self, cycle_index: int, paths: Dict[str, str]) -> Dict[str, Any]:
        """
        在 Step 4 和 Step 5 之间调用：
            1. 截取 cfg.capture_area 对应的整张图；
            2. 从整张图中裁剪 cfg.focus_roi；
            3. 分别计算整张图和 ROI 的聚焦指标；
            4. 保存整张截图和 ROI 截图；
            5. 把结果写入 self.context，供 Step 9 异步保存使用。
        """
        if pyautogui is None:
            raise ImportError("需要安装 pyautogui 才能截图：pip install pyautogui")
        if Image is None:
            raise ImportError("需要安装 pillow 才能保存截图：pip install pillow")

        left, top, width, height = [int(v) for v in self.cfg.capture_area]
        self.log(
            f"[聚焦指标] 截图区域 capture_area=(left={left}, top={top}, width={width}, height={height})"
        )
        ...
        full_path = Path(paths.get("focus_image_path") or "")
        roi_path = Path(paths.get("focus_roi_image_path") or "")
        ...
        score_ratio, component_ratios = self.calculate_focus_score_ratio(roi_metrics)
        result["focus_score_ratio"] = score_ratio
        result["focus_component_ratios"] = component_ratios

        self.context["focus_metrics"] = result
        self.context["focus_image_path"] = str(full_path)
        self.context["focus_roi_image_path"] = str(roi_path)
        self.context["focus_roi"] = [x, y, rw, rh]
        self.context["focus_score_ratio"] = score_ratio

        self.log(
            "[聚焦指标] 已完成："
            f"full_tenengrad={full_metrics.get('tenengrad')}, "
            f"roi_tenengrad={roi_metrics.get('tenengrad')}, "
            f"roi={roi}"
        )
        self.log(f"[聚焦指标] 整图截图：{full_path}")
        self.log(f"[聚焦指标] ROI截图：{roi_path}")
        self.notify_update()
        return result

    @staticmethod
    def _flatten_focus_metrics(focus_metrics: Optional[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    def build_save_path(self, cycle_index: int) -> Dict[str, str]:
        ...
        focus_image_filename = f"cycle_{cycle_index:04d}_{timestamp}_focus_full.png"
        focus_roi_image_filename = f"cycle_{cycle_index:04d}_{timestamp}_focus_roi.png"
        ...
        paths = {
            ...
            "focus_image_filename": focus_image_filename,
            "focus_image_path": str(self.run_session_dir / focus_image_filename),
            "focus_roi_image_filename": focus_roi_image_filename,
            "focus_roi_image_path": str(self.run_session_dir / focus_roi_image_filename),
            "focus_metrics_csv": str(self.run_session_dir / "focus_metrics_summary.csv"),
        }
    ...
    def _make_save_context_snapshot(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "focus_metrics": self._json_safe(ctx.get("focus_metrics")),
            "focus_image_path": ctx.get("focus_image_path"),
            "focus_roi_image_path": ctx.get("focus_roi_image_path"),
            "focus_roi": self._json_safe(ctx.get("focus_roi")),
            "focus_reference": self._json_safe(ctx.get("focus_reference")),
            "focus_score_ratio": ctx.get("focus_score_ratio"),
            "shg_ref": ctx.get("shg_ref"),
            "last_shg_ratio": ctx.get("last_shg_ratio"),
            "autofocus_last_result": self._json_safe(ctx.get("autofocus_last_result")),
            "autofocus_last_reason": ctx.get("autofocus_last_reason"),
        }
    ...
    def save_cycle_result(
        ...
        with self.save_io_lock:
            ...
            self._append_focus_metrics_csv(
                cycle_index=cycle_index,
                paths=paths,
    ...
    def _append_focus_metrics_csv(
        ...
        ctx = context_snapshot if context_snapshot is not None else self.context
        focus_metrics = ctx.get("focus_metrics")
        focus_csv = Path(paths.get("focus_metrics_csv") or Path(paths.get("run_session_dir", ".")) / "focus_metrics_summary.csv")
        focus_csv.parent.mkdir(parents=True, exist_ok=True)
        file_exists = focus_csv.exists()

        flat = self._flatten_focus_metrics(focus_metrics)
        roi = None
        if isinstance(focus_metrics, dict):
            roi = focus_metrics.get("roi")

        with focus_csv.open("a", encoding="utf-8-sig", newline="") as f:
            writer = csv.writer(f)
            if not file_exists:
                writer.writerow([
                    "cycle_index",
            ...
            roi_vals = list(roi) if isinstance(roi, (list, tuple)) and len(roi) == 4 else [None, None, None, None]
            writer.writerow([
                cycle_index,
                datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                ctx.get("angle_before"),
                ctx.get("focus_image_path"),
                ctx.get("focus_roi_image_path"),
                roi_vals[0],
                roi_vals[1],
                roi_vals[2],
                roi_vals[3],
            ] + [flat.get(k) for k in FOCUS_SUMMARY_KEYS])

        self.log(f"[保存] 聚焦指标CSV已追加：{focus_csv}")
    ...
    def _append_summary_csv(
        ...
        with self.summary_csv_path.open("a", encoding="utf-8-sig", newline="") as f:
            ...
            focus_metrics = ctx.get("focus_metrics")
            flat_focus = self._flatten_focus_metrics(focus_metrics)
            focus_roi = None
            if isinstance(focus_metrics, dict):
                focus_roi = focus_metrics.get("roi")
            ...
            writer.writerow([
                ...
                ctx.get("focus_roi_image_path"),
                focus_roi,
            ] + [flat_focus.get(k) for k in FOCUS_SUMMARY_KEYS])
    ...
    def _append_summary_xlsx(
        ...
        row = [
            ...
            ctx.get("focus_image_path"),
            ctx.get("focus_roi_image_path"),
            str(focus_roi) if focus_roi is not None else None,
        ] + [flat_focus.get(k) for k in FOCUS_SUMMARY_KEYS]
        ...
        for col_idx in range(1, len(expected_headers) + 1):
            ...
            elif col_idx in (2, 3, 4, 6) or col_idx >= 10:
    ...
    def run_one_cycle(self, cycle_index: int) -> bool:
        """
        单轮循环测量流程（改为每轮只检测一次角度）：

        Step 1：信号发生器 CH1 ON，持续 signal_on_time_ms
        Step 2：信号发生器 CH1 OFF
        Step 3：角度检测一次，得到本轮角度 current_angle
        Step 4：生成保存路径
        Step 4.5：截图，计算并保存整张图片与指定 ROI 的聚焦指标
        Step 5：照明光 OFF，并按 stable_wait_ms 等待稳定
        Step 6：LabVIEW 光谱采集
        Step 7：照明光 ON
        Step 8：用“本轮角度”和“上一轮角度”的差值调节下一轮 signal_on_time_ms
        Step 9：保存数据

        说明：
            - 跨轮 angle_delta = abs(current_angle - previous_cycle_angle)，不再使用同一轮 before/after 差值；
            - 第一轮 previous_cycle_angle=None，不调节信号发生器打开时间；
            - 旧字段 angle_before 保留为“本轮角度”，angle_after 置为 None，便于兼容保存和显示逻辑。
        """
        ...
        try:
            used_ms = float(self.cfg.signal_on_time_ms)
            self.context["signal_on_time_used_ms"] = used_ms
            self.context["signal_on_time_next_ms"] = used_ms
            self.context["signal_time_adjust_action"] = None

            self.log("========== Step 1：信号发生器 CH1 ON ==========")
            self.signal_ch1_on()
            self.log(f"[信号发生器] CH1 ON 持续时间：{used_ms:.3f} ms")
            time.sleep(used_ms / 1000.0)

            if self.stop_requested:
                return False

            self.log("========== Step 2：信号发生器 CH1 OFF ==========")
            self.signal_ch1_off()

            if self.stop_requested:
                return False

            self.log("========== Step 3：角度检测 current_angle ==========")
            ...
            self.log("========== Step 4：生成保存路径 ==========")
            paths = self.build_save_path(cycle_index)

            self.step45_focus_check_and_autofocus(cycle_index=cycle_index, paths=paths)

            self.log("========== Step 5：照明光 OFF，等待稳定 ==========")
            self.light_off()
            ...
            self.log("========== Step 6：LabVIEW 光谱采集 ==========")
            labview_result = self.request_labview_spectrum(cycle_index)
            self.update_shg_reference_and_ratio()

            if self.stop_requested:
                return False

            self.log("========== Step 7：照明光 ON，等待稳定 ==========")
            self.light_on()
            wait_s = float(self.cfg.stable_wait_ms) / 1000.0
            self.log(f"[照明光] ON 后等待：{wait_s:.3f} s")
            time.sleep(wait_s)

            previous_angle = self.previous_cycle_angle
            self.context["previous_cycle_angle"] = previous_angle

            if previous_angle is not None and current_angle is not None:
                angle_delta = self.angle_diff_deg(current_angle, previous_angle)
                self.log(
                    f"[跨轮角度差-Step8使用] previous={previous_angle:.6g}, "
                    f"current={current_angle:.6g}, delta={angle_delta:.6g} deg"
                )
            else:
                angle_delta = None
                if previous_angle is None:
                    self.log("[跨轮角度差-Step8使用] 第一轮没有上一轮角度，本轮不调节信号发生器 ON 时间")
                else:
                    self.log("[跨轮角度差-Step8使用] 本轮角度无效，本轮不调节信号发生器 ON 时间")

            self.context["angle_delta"] = angle_delta
            self.notify_update()

            self.log("========== Step 8：判断下一轮信号发生器 ON 时间 ==========")
            if angle_delta is not None:
                self.adjust_signal_time_by_angle_delta(angle_delta)
            else:
                self.context["signal_on_time_next_ms"] = float(self.cfg.signal_on_time_ms)
                self.context["signal_time_adjust_action"] = "keep_no_previous_or_invalid_angle"
                self.log(
                    f"[信号时间调节] 无有效跨轮 angle_delta，本轮不调节，"
                    f"下一轮保持 {self.cfg.signal_on_time_ms:.3f} ms"
                )
                self.notify_update()

            if current_angle is not None:
                self.previous_cycle_angle = float(current_angle)
                self.context["previous_cycle_angle"] = float(current_angle)

            self.log("========== Step 9：保存数据（异步线程） ==========")
    ...
    def close_all(self):
        ...
        try:
            if self.z_controller is not None:
                if hasattr(self.z_controller, "close"):
                    self.z_controller.close()
                self.z_controller = None
                self.log("[Z轴] 已关闭连接")
        except Exception as e:
            self.z_controller = None
            self.log(f"[Z轴] 关闭失败：{e}")
...
class MeasurementWorkflowGUI:
    def __init__(self, root: tk.Tk):
        ...
        self.current_focus_var = tk.StringVar(value="聚焦指标：None")
        self.current_autofocus_var = tk.StringVar(value="补焦状态：未建立参考")
    ...
    def _build_ui(self):
        ...
        result_labels = [
            ...
            self.current_focus_var,
            self.current_autofocus_var,
        ...
        self.focus_roi_var = tk.StringVar(value=",".join(str(v) for v in cfg0.focus_roi))
        ...
        ttk.Label(angle_frame, text="聚焦ROI").grid(row=2, column=0, padx=4, pady=4, sticky="w")
        ttk.Entry(angle_frame, textvariable=self.focus_roi_var).grid(row=2, column=1, padx=4, pady=4, sticky="ew")
        ttk.Button(angle_frame, text="选择ROI", command=self.select_focus_roi).grid(row=2, column=2, padx=4, pady=4, sticky="ew")
        ...
        # 左侧 6：Z轴闭环补焦
        # =====================================================
        z_frame = ttk.LabelFrame(left_inner, text="6. Z轴闭环补焦", padding=10, style="Panel.TLabelframe")
        z_frame.pack(fill=tk.X, padx=4, pady=8)

        self.autofocus_enabled_var = tk.BooleanVar(value=cfg0.autofocus_enabled)
        self.autofocus_focus_trigger_ratio_var = tk.DoubleVar(value=cfg0.autofocus_focus_trigger_ratio)
        self.autofocus_shg_trigger_ratio_var = tk.DoubleVar(value=cfg0.autofocus_shg_trigger_ratio)
        self.autofocus_shg_hard_ratio_var = tk.DoubleVar(value=cfg0.autofocus_shg_hard_ratio)
        self.autofocus_stop_ratio_var = tk.DoubleVar(value=cfg0.autofocus_stop_ratio)
        self.z_axis_var = tk.IntVar(value=cfg0.z_axis)
        self.z_speed_var = tk.IntVar(value=cfg0.z_speed)
        self.z_accel_var = tk.IntVar(value=cfg0.z_accel)
        self.z_probe_steps_var = tk.IntVar(value=cfg0.z_probe_steps)
        self.z_search_steps_var = tk.IntVar(value=cfg0.z_search_steps)
        self.z_settle_time_s_var = tk.DoubleVar(value=cfg0.z_settle_time_s)
        self.z_max_total_steps_var = tk.IntVar(value=cfg0.z_max_total_steps)
        self.z_status_var = tk.StringVar(value="Z轴补焦：未连接；未建立参考")

        for i in range(4):
            z_frame.columnconfigure(i, weight=1)

        ttk.Checkbutton(z_frame, text="启用自动补焦", variable=self.autofocus_enabled_var).grid(row=0, column=0, columnspan=2, padx=4, pady=4, sticky="w")
        ttk.Label(z_frame, text="停止阈值").grid(row=0, column=2, padx=4, pady=4, sticky="w")
        ttk.Entry(z_frame, textvariable=self.autofocus_stop_ratio_var, width=8).grid(row=0, column=3, padx=4, pady=4, sticky="ew")

        ttk.Label(z_frame, text="Focus触发").grid(row=1, column=0, padx=4, pady=4, sticky="w")
        ttk.Entry(z_frame, textvariable=self.autofocus_focus_trigger_ratio_var, width=8).grid(row=1, column=1, padx=4, pady=4, sticky="ew")
        ttk.Label(z_frame, text="SHG触发").grid(row=1, column=2, padx=4, pady=4, sticky="w")
        ttk.Entry(z_frame, textvariable=self.autofocus_shg_trigger_ratio_var, width=8).grid(row=1, column=3, padx=4, pady=4, sticky="ew")

        ttk.Label(z_frame, text="SHG硬阈值").grid(row=2, column=0, padx=4, pady=4, sticky="w")
        ttk.Entry(z_frame, textvariable=self.autofocus_shg_hard_ratio_var, width=8).grid(row=2, column=1, padx=4, pady=4, sticky="ew")
        ttk.Label(z_frame, text="Z轴号").grid(row=2, column=2, padx=4, pady=4, sticky="w")
        ttk.Entry(z_frame, textvariable=self.z_axis_var, width=8).grid(row=2, column=3, padx=4, pady=4, sticky="ew")

        ttk.Label(z_frame, text="速度").grid(row=3, column=0, padx=4, pady=4, sticky="w")
        ttk.Entry(z_frame, textvariable=self.z_speed_var, width=8).grid(row=3, column=1, padx=4, pady=4, sticky="ew")
        ttk.Label(z_frame, text="加速度").grid(row=3, column=2, padx=4, pady=4, sticky="w")
        ttk.Entry(z_frame, textvariable=self.z_accel_var, width=8).grid(row=3, column=3, padx=4, pady=4, sticky="ew")

        ttk.Label(z_frame, text="试探steps").grid(row=4, column=0, padx=4, pady=4, sticky="w")
        ttk.Entry(z_frame, textvariable=self.z_probe_steps_var, width=8).grid(row=4, column=1, padx=4, pady=4, sticky="ew")
        ttk.Label(z_frame, text="搜索steps").grid(row=4, column=2, padx=4, pady=4, sticky="w")
        ttk.Entry(z_frame, textvariable=self.z_search_steps_var, width=8).grid(row=4, column=3, padx=4, pady=4, sticky="ew")

        ttk.Label(z_frame, text="稳定/s").grid(row=5, column=0, padx=4, pady=4, sticky="w")
        ttk.Entry(z_frame, textvariable=self.z_settle_time_s_var, width=8).grid(row=5, column=1, padx=4, pady=4, sticky="ew")
        ttk.Label(z_frame, text="最大总steps").grid(row=5, column=2, padx=4, pady=4, sticky="w")
        ttk.Entry(z_frame, textvariable=self.z_max_total_steps_var, width=8).grid(row=5, column=3, padx=4, pady=4, sticky="ew")

        z_buttons = ttk.Frame(z_frame)
        z_buttons.grid(row=6, column=0, columnspan=4, padx=2, pady=4, sticky="ew")
        for i in range(3):
            z_buttons.columnconfigure(i, weight=1)
        ttk.Button(z_buttons, text="连接Z轴", command=self.connect_z_thread).grid(row=0, column=0, padx=4, pady=3, sticky="ew")
        ttk.Button(z_buttons, text="建立参考(5帧ROI)", command=self.build_focus_reference_thread).grid(row=0, column=1, padx=4, pady=3, sticky="ew")
        ttk.Button(z_buttons, text="单次调焦", command=self.single_autofocus_thread).grid(row=0, column=2, padx=4, pady=3, sticky="ew")

        ttk.Label(z_frame, textvariable=self.z_status_var, wraplength=450).grid(row=7, column=0, columnspan=4, padx=4, pady=(4, 0), sticky="w")
    ...
    def parse_focus_roi(self) -> Tuple[int, int, int, int]:
        try:
            vals = [int(x.strip()) for x in self.focus_roi_var.get().split(",")]
            if len(vals) != 4:
                raise ValueError
            x, y, w, h = vals
            if w <= 0 or h <= 0:
            ...
            raise ValueError("聚焦ROI格式错误，应为 x,y,width,height，例如 0,0,300,300；坐标相对于截图区域左上角")

    def select_focus_roi(self):
        """运行完整流程之前使用：截取当前 capture_area，让用户拖拽选择聚焦 ROI。"""
        try:
            if pyautogui is None:
                raise ImportError("需要安装 pyautogui 才能截图选择ROI：pip install pyautogui")
            if Image is None or ImageTk is None:
                raise ImportError("需要安装 pillow 才能显示截图选择ROI：pip install pillow")

            left, top, width, height = self.parse_capture_area()
            screenshot = pyautogui.screenshot(region=(left, top, width, height)).convert("RGB")

            win = tk.Toplevel(self.root)
            win.title("选择聚焦 ROI：拖拽矩形后点击确认")
            win.transient(self.root)
            win.grab_set()

            max_w, max_h = 900, 700
            scale = min(max_w / max(1, width), max_h / max(1, height), 1.0)
            show_w = max(1, int(width * scale))
            show_h = max(1, int(height * scale))
            display_img = screenshot.resize((show_w, show_h), Image.LANCZOS)
            tk_img = ImageTk.PhotoImage(display_img)

            info_var = tk.StringVar(value="在图像上按住鼠标左键拖拽选择 ROI；坐标相对于截图区域左上角。")
            canvas = tk.Canvas(win, width=show_w, height=show_h, cursor="crosshair")
            canvas.pack(padx=10, pady=(10, 4))
            canvas.create_image(0, 0, anchor="nw", image=tk_img)
            canvas.image = tk_img
            ttk.Label(win, textvariable=info_var).pack(fill=tk.X, padx=10, pady=(0, 6))
            ...
            def on_drag(event):
                if state["start"] is None or state["rect"] is None:
                    return
                x0, y0 = state["start"]
                x1 = max(0, min(event.x, show_w - 1))
                y1 = max(0, min(event.y, show_h - 1))
                canvas.coords(state["rect"], x0, y0, x1, y1)

                rx0, ry0 = _to_real(min(x0, x1), min(y0, y1))
                rx1, ry1 = _to_real(max(x0, x1), max(y0, y1))
                rw = max(1, rx1 - rx0 + 1)
                rh = max(1, ry1 - ry0 + 1)
                state["roi"] = (rx0, ry0, rw, rh)
                info_var.set(f"当前ROI：x={rx0}, y={ry0}, w={rw}, h={rh}")
            ...
            def confirm():
                roi = state.get("roi")
                if roi is None:
                    messagebox.showwarning("未选择ROI", "请先用鼠标拖拽选择一个 ROI。", parent=win)
                    return
                self.focus_roi_var.set(",".join(str(v) for v in roi))
                self.log(f"[聚焦ROI] 已选择 ROI：{roi}，坐标相对于 capture_area 左上角")
                win.destroy()
            ...
            ttk.Button(btn_frame, text="确认使用该 ROI", command=confirm).pack(side=tk.LEFT, fill=tk.X, expand=True, padx=(0, 5))
            ttk.Button(btn_frame, text="取消", command=win.destroy).pack(side=tk.LEFT, fill=tk.X, expand=True, padx=(5, 0))
        except Exception as e:
            self.log(f"[聚焦ROI] 选择失败：{e}")
            self.root.after(0, lambda: messagebox.showerror("选择聚焦ROI失败", str(e)))
    ...
    def build_config_from_ui(self) -> MeasurementConfig:
        return MeasurementConfig(
            ...
            focus_roi=self.parse_focus_roi(),
            autofocus_enabled=bool(self.autofocus_enabled_var.get()),
            autofocus_focus_trigger_ratio=float(self.autofocus_focus_trigger_ratio_var.get()),
            autofocus_shg_trigger_ratio=float(self.autofocus_shg_trigger_ratio_var.get()),
            autofocus_shg_hard_ratio=float(self.autofocus_shg_hard_ratio_var.get()),
            autofocus_stop_ratio=float(self.autofocus_stop_ratio_var.get()),
            z_enabled=bool(self.autofocus_enabled_var.get()),
            z_axis=int(self.z_axis_var.get()),
            z_speed=int(self.z_speed_var.get()),
            z_accel=int(self.z_accel_var.get()),
            z_probe_steps=int(self.z_probe_steps_var.get()),
            z_search_steps=int(self.z_search_steps_var.get()),
            z_settle_time_s=float(self.z_settle_time_s_var.get()),
            z_max_total_steps=int(self.z_max_total_steps_var.get()),
    ...
    def refresh_result_labels(self, workflow: Optional[MeasurementWorkflow] = None):
        ...
        focus_metrics = ctx.get("focus_metrics")
        if isinstance(focus_metrics, dict):
            full_m = focus_metrics.get("full") or {}
            roi_m = focus_metrics.get("roi_metrics") or {}
            self.set_var(
                self.current_focus_var,
                "聚焦指标："
                f"ROI={focus_metrics.get('roi')}；"
                f"full_Tenengrad={full_m.get('tenengrad')}；"
                f"roi_Tenengrad={roi_m.get('tenengrad')}；"
                f"roi_R/B={roi_m.get('red_blue_ratio')}"
            )
        else:
            self.set_var(self.current_focus_var, "聚焦指标：None")
        ...
        try:
            ...
            while True:
                ...
                self.log(
                    ...
                    f"focus_roi={wf.cfg.focus_roi}, "
                    f"autofocus_stop={wf.cfg.autofocus_stop_ratio}, "
                    f"z_steps={wf.cfg.z_search_steps}, "
    ...
    def connect_z_thread(self):
        self.run_in_thread(self.connect_z)

    def connect_z(self):
        try:
            wf = self.ensure_workflow()
            wf.connect_z_controller()
            self.set_var(self.z_status_var, f"Z轴补焦：已连接；axis={wf.cfg.z_axis}, speed={wf.cfg.z_speed}, accel={wf.cfg.z_accel}")
            self.refresh_result_labels(wf)
        except Exception as e:
            self.set_var(self.z_status_var, "Z轴补焦：连接失败")
            self.log(f"[Z轴] 连接失败：{e}")
            self.log(traceback.format_exc())
            self.root.after(0, lambda: messagebox.showerror("Z轴连接失败", str(e)))

    def build_focus_reference_thread(self):
        self.run_in_thread(self.build_focus_reference)

    def build_focus_reference(self):
        try:
            wf = self.ensure_workflow()
            wf.build_focus_reference(capture_count=int(wf.cfg.focus_reference_capture_count))
            self.set_var(self.z_status_var, "Z轴补焦：ROI聚焦参考已建立")
            self.refresh_result_labels(wf)
        except Exception as e:
            self.set_var(self.z_status_var, "Z轴补焦：建立参考失败")
            self.log(f"[聚焦参考] 建立失败：{e}")
            self.log(traceback.format_exc())
            self.root.after(0, lambda: messagebox.showerror("建立聚焦参考失败", str(e)))

    def single_autofocus_thread(self):
        self.run_in_thread(self.single_autofocus)

    def single_autofocus(self):
        try:
            wf = self.ensure_workflow()
            if not wf.context.get("focus_reference_ready", False):
                raise RuntimeError("尚未建立 ROI 聚焦参考。请先点击“建立参考(5帧ROI)”。")
            result = wf.run_closed_loop_autofocus()
            wf.context["autofocus_last_result"] = wf._json_safe(result)
            self.set_var(self.z_status_var, f"Z轴补焦：单次调焦完成，result={result.get('reason')}, best={result.get('best_score')}")
            self.refresh_result_labels(wf)
        except Exception as e:
            self.set_var(self.z_status_var, "Z轴补焦：单次调焦失败")
            self.log(f"[补焦] 单次调焦失败：{e}")
            self.log(traceback.format_exc())
            self.root.after(0, lambda: messagebox.showerror("单次调焦失败", str(e)))

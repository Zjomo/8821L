...
from control.stage34_fixed import Stage34
...
class RuleACConfig:
    ...
    stage_device_id: str = "97101208"
    stage_axis: str = "horizontal"  # horizontal / vertical
    stage_direction: int = 1        # 1 or -1
    stage_step_size: int = 100
    stage_velocity: float = 500
    stage_acceleration: float = 500
    stage_max_voltage: float = 100
...
class RuleACOverlapController:
    ...
    def __init__(self, cfg: RuleACConfig):
        ...
        if cfg.enable_stage:
            self.stage = Stage34(
                conn=cfg.stage_device_id,
                x_channel=1,
                y_channel=2,
                default_velocity=cfg.stage_velocity,
                default_acceleration=cfg.stage_acceleration,
                default_max_voltage=cfg.stage_max_voltage,
                auto_enable=True,
            )
            self._setup_stage_drive()
            logger.info("Stage12 已连接：device_id=%s", cfg.stage_device_id)
        else:
...
def main() -> None:
    cfg = RuleACConfig(
        ...
        # Stage12：调试时保持 False；确认方向后再改 True。控制器序列号默认 97101208。
        enable_stage=False,
        stage_device_id="97101208",

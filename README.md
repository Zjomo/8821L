...
1、现在的量程变量主要是："像素"、"步长"，但是我想基于实际距离（微米or纳米等）	✔

```
探测器的屏幕尺寸 与 光斑移动的距离，理论上是由光斑到探测器的距离决定的 
		-- 尝试是否能进行调整
		-- 可通过显微镜的量程来进行映射
		-- 完成显微镜的画面通信
		-- 确定 像素尺寸
【优化：暂时可以先不考虑具体长度的问题，后续如果要继续优化，可以继续补充】

```

2、光斑范围精度，量化量程：收敛后的误差在5px之内		✔

【正常的探测器就是有波动的，所以开启自动准直之后，其对应的图像，应该就不再会波动了】
...
2、点击“启动UCC预览”，进入4轴闭环，
3、设定目标点（记录目标点坐标）
4、点击标定映射 -- 完成步长(电机)与像素(光镜)的映射关系
5、开始稳定闭环，等待收敛完成后，解锁“收敛误差记录”，
6、点击“收敛误差记录”，设定记录时长(单位为h，支持小数点后一位的精度)、导出文件路径（.xlsx格式）
7、记录内容包括：
    收敛坐标质心，
    目标点坐标，
    每1min，两者的位置距离差：x坐标误差，y坐标误差，总体误差（单位：px）
8、收敛误差可视化：针对当前开始记录的"收敛误差"数据，进行适合的可视化分析，其中主要分析的内容为 -- 总误差的分布情况
9、解决光斑突然消失，出现了电机故障问题【补充逻辑：当探测器检测的光斑突然消失超过n秒时，自动关闭"稳定闭环"】
...
补充稳定精度面板【官网量化精度的本质 也是：收敛后的光斑质点 与 目标点的距离】	

```
采样数: N 		  -- 采集N帧，在收敛后每一帧"光斑质心"到"目标点"的距离Dn，单位：px像素
RMS: 2.1 px		-- RMS = sqrt(mean(r^2))
P95: 3.6 px		-- P95：第95%的r，用以衡量收敛的"误差"稳定性（95%的时间内，光斑的稳定情况）
Max: 8.4 px		-- 表示100%时间内，最大的 r
X_std: 1.2 px	-- X_std = sqrt(mean(dx^2))
Y_std: 1.7 px	-- y_std = sqrt(mean(dy^2))
线性区域（光斑必须在探测器线性区域内，才能量化到具体距离）：
    -- 仅绿色和黄色 LED 发亮，设备处于线性性能区域，测试信号与激光位置之间有线性的直接关系；		
    -- 若红色 LED 发亮，线性关系不存在，得换其他策略量化误差
位置稳定性/角度稳定性： 
    --	后续"双探测器"的误差，分别可以量化位置稳定性 和 角度稳定性
    -- “位置稳定性”：光斑位置 - 目标点位置
    -- “指向稳定性”：（光斑位置 - 目标点位置）/（探测器1位置 - 探测器2位置）
带宽：
    -- 表示系统可抑制 多高频率以内的扰动
